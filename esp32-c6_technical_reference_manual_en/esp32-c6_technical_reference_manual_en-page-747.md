...
*   `UART_RXFIFO_TOUT_INT`: Triggered when the receiver has received at least one byte, and the bus remains idle for `UART_RX_TOUT_THRD`.
...
*   `UART_FRM_ERR_ERR_INT`: Triggered when the receiver detects a data frame error.
