...
Before hash operation begins for any secure hash algorithms, the initial Hash value H(O) must be set based on different algorithms. However, the SHA accelerator uses the initial Hash values (constant C) stored in the hardware for hash tasks.
