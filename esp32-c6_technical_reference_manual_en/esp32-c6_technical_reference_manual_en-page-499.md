...
| 0 <= tc - tt < 2^51 (tc < 2^51 and tt < 2^51, or tc >= 2^51 and tt >= 2^51) | An alarm is triggered immediately. |
| tc - tt >= 2^51 - 1           | tc overflows after counting to its maximum value 52'hfffffffffffff, and then starts counting up from 0. When its value reaches tt, an alarm is triggered. |

## 13.4.3 Event Task Matrix

The system timer on ESP32-C6 supports the Event Task Matrix (ETM) function, which allows the system timer's ETM tasks to be triggered by any peripherals' ETM events, or system timer's ETM events to trigger any peripherals' ETM tasks. This section introduces the ETM tasks and events related to the system timer. For more information, please refer to Chapter 11 Event Task Matrix (SOC_ETM).
