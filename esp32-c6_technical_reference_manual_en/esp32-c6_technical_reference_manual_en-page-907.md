...
5. Write the address of `I2C_slave` and data to be sent to TX RAM of `I2C_master` in either FIFO mode or non-FIFO mode according to Section 29.4.10.
6. Write the address of `I2C_slave` to `I2C_SLAVE_ADDR (slave)` register, and write it to `I2C_SLAVE_ADDR_REG (slave)`.
...
11. If data to be sent (N) is larger than TX FIFO depth, TX RAM of `I2C_master` may wrap around in FIFO mode. For details, please refer to Section 29.4.10.
12. If data to be received (N) is larger than RX FIFO depth, RX RAM of `I2C_slave` may wrap around in FIFO mode. For details, please refer to Section 29.4.10.

    If data to be received (N) is larger than RX FIFO depth, the other way is to enable clock stretching by setting the `I2C_SLAVE_SCL_STRETCH_EN (slave)`, and clearing `I2C_RX_FULL_ACK_LEVEL`. When RX RAM is full, an `I2C_SLAVE_STRETCH_INT (slave)` interrupt is generated. In this way, `I2C_slave` can hold SCL low, in exchange for more time to read data. After software has finished reading, you can set `I2C_SLAVE_STRETCH_INT_CLR (slave)` to 1 to clear interrupt, and set `I2C_SLAVE_SCL_STRETCH_CLR (slave)` to release the SCL line.
