...
```math
T_{hold0} = RTC_WDT_STG0_HOLD << (EFUSE_WDT_DELAY_SEL + 1)
```

where `<<` is a left-shift operator. For example, if RTC_WDT_STGO_HOLD is configured as 100 and EFUSE_WDT_DELAY_SEL is 1, the *Thold0* will be 400 cycles.
...
| System reset   | Reset the main system, power management unit and RTC peripherals (see details in Chapter 12 Low-Power Management). This action is only available in RWDT |
