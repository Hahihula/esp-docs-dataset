...
The SHA accelerator is activated by setting the PCR_SHA_CLK_EN bit and clearing the PCR_SHA_RST_EN bit in the PCR_SHA_CONF_REG register. Additionally, users also need to clear PCR_DS_RST_EN and PCR_HMAC_RST_EN bits to reset Digital Signature (DS) and HMAC Accelerator (HMAC).
