...
```markdown
7. Set bits `SPI_DMA_RX_ENA` and `SPI_DMA_TX_ENA`. Clear the bit `SPI_RX_EOF_EN`. Configure GDMA TX/RX link and start GDMA TX/RX engine, as shown in Section 29.5.7 and Section 29.5.8.
8. Set `SPI_DMA_SLV_SEG_TRANS_EN` in `SPI_DMA_CONF_REG` to enable slave segmented transfer.
9. Set `SPI_DMA_SEG_TRANS_DONE_INT_ENA` in `SPI_DMA_INT_ENA_REG` and wait for the interrupt `SPI_DMA_SEG_TRANS_DONE_INT`, which means that the segmented transfer has finished and data has been put into the related memory. Other interrupts described in Section 29.9 are optional.
...
### 29.5.10.6 Configuration of Slave Segmented Transfer in Full-Duplex

GDMA must be used in this mode. In such transfer, the data is transferred from and to the GDMA buffer. The interrupt `GDMA_IN_SUC_EOF_CHn` is triggered when the transfer ends. The configuration procedure is as follows:
...
5. Set `SPI_DMA_TX_ENA/SPI_DMA_RX_ENA`. Configure GDMA TX/RX link and start GDMA TX/RX engine, as shown in Section 29.5.7 and Section 29.5.8.
...
## 29.6 CS Setup Time and Hold Time Control
...
When operating as slave, the CS setup time and hold time should be longer than 0.5 × `T_SPI_CLK`, otherwise the SPI transfer may be incorrect. `T_SPI_CLK` is one cycle of `SPI_CLK`.
...
* If `SPI_CS_SETUP` is cleared, the SPI CS setup time is 0.5 × `T_SPI_CLK`.
* If `SPI_CS_SETUP` is set, the SPI CS setup time is (`SPI_CS_SETUP_TIME` + 1.5) × `T_SPI_CLK`.
```