...
```markdown
## 28.4.9.1 Hardware Flow Control

Figure 28.4-8 shows the hardware flow control of a UART controller. Hardware flow control uses output signal `rtsn_out` and input signal `dsrn_in`. Figure 28.4-9 illustrates how these signals are connected between UART on ESP32-H2 (hereinafter referred to as IUO) and the external UART (hereinafter referred to as EUO).
...
Figure 28.4-8. Hardware Flow Control Diagram
```