

```markdown
Chapter 15 Permission Control (PMS)

Register 15.14. HP_APM_M2_STATUS_REG (0x00E8)

HP_APM_M2_EXCEPTION_STATUS Represents exception status.
bit0: 1 represents permission restrictions
bit1: 1 represents address out of bounds
(RO)

Register 15.15. HP_APM_M2_STATUS_CLR_REG (0x00EC)

HP_APM_M2_REGION_STATUS_CLR Configures to clear exception status. (WT)
```