...
```markdown
DE can be controlled by either software or hardware. To reduce the cost of software, in our design DE is controlled by hardware. As shown in Figure 28.4-5, DE is connected to dtrn_out of UART (please refer to Section 28.4.9.1 for more details).

![Figure 28.4-5. Driver Control Diagram in RS485 Mode](image)

## 28.4.6.2 Turnaround Delay
...
## 28.4.6.3 Bus Snooping

In a two-wire multidrop network, UART controllers support bus snooping if RE of the external RS485 transceiver is 0. By default, a UART controller is not allowed to transmit and receive data simultaneously. If `UART_RS485TX_RX_EN` is set and the external RS485 transceiver is configured as in Figure 28.4-5, a UART controller may receive data in transmitter mode and snoop the bus. If `UART_RS485RXBY_TX_EN` is set, a UART controller may transmit data in receiver mode.
