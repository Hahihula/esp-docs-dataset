...
Since there is a one-to-one correspondence between the reading data registers and the programming data registers (see table 5.3-4 for details), users can find out where the data to be programmed is located in programming registers by checking the parameter description and the parameter location in the corresponding reading data registers.
