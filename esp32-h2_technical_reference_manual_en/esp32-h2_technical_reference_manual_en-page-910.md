...
```markdown
## 31.9.2.2 PDM TX Mode

In PDM TX mode, I2S supports both PDM raw data transmission and PCM-to-PDM data format conversion.

In PDM TX mode, fetching data through GDMA is controlled by `I2S_TX_MONO` and `I2S_TX_MONO_FST_VLD`. See Table 31.9-3. Configure the two bits according to the data stored in memory, be it the single-channel or dual-channel data.

Table 31.9-3. Data-Fetching Control in PDM Mode
...
When I2S is in PDM TX master mode, the default level of WS signal is controlled by `I2S_TX_WS_IDLE_POL`, and the WS signal frequency is half of the BCK signal frequency. The configuration of WS signal is similar to that of BCK signal. Please refer to Section 31.6 and Figure 31.6.
...
Table 31.9-4. I2S Channel Control for Raw PDM Data
