...
### 34.2.2.2 Error and Overload Frames
...
When a node detects a bus error, it will transmit an error frame starting from the next bit. However, when a node detects a CRC error, the error frame will start at the bit following the ACK Delim (see Section 34.2.3 for more details). The following Figure 34.2-2 shows different fields of an error frame:
```