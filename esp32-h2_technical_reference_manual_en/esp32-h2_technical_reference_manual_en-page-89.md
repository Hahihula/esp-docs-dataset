...
Synchronization is accomplished by sending a full-value instruction address. When the synchronization counter value reaches the value of the `TRACE_RESYNC_PROLONGED` field of the `TRACE_RESYNC_PROLONGED_REG` register, the encoder will send a synchronization packet (format 3 subformat 0, see Section 2.6.3.1).
