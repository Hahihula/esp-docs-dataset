...
To distinguish between these two groups of registers easily, all registers that implement the clock domain crossing design have the `_SYNC` suffix, and are put together in Section 28.6. Those without the `_SYNC` suffix in Section 28.6 are configuration registers that require no clock domain crossing.

### 28.5.2 Detailed Steps

Figure 28.5-1 illustrates the process to program UART controllers, namely initialize UART, configure registers, enable the UART transmitter or receiver, and finish data transmission.
```