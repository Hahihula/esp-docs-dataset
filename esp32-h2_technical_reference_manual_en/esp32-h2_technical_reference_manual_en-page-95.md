...
Once the encoder is enabled, it will keep tracing the CPU’s instruction trace interface and writing packets to  
the trace memory.
...
- Find the first address to decode  
  - Read the `TRACE_MEM_FULL_INTR_RAW` bit of the `TRACE_INTR_RAW_REG` register to know if the  
    ...
    * if read 1, and the loop mode is enabled, then the old trace packets are overwritten. In this case,  
