...
```markdown
## 29.5.10.1 Communication Formats
...
1.  CMD:
    ...
    *   Only the values in Table 29.5-13 and Table 29.5-14 are valid;
...
```
```markdown
When a half-duplex transaction is complete, the transferred CMD and ADDR values are latched into `SPI_SLV_LAST_COMMAND` and `SPI_SLV_LAST_ADDR` respectively. The `SPI_SLV_CMD_ERR_INT_RAW` will be
```