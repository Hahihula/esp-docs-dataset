---
original_file_path: api-guides/kconfig/configuration_structure.rst
---

# Configuration Files Structure and Relationships {#configuration-structure}
...
This article primarily focuses on the structure of the files. For more information about project configuration, please refer to the `Project Configuration Guide <project-configuration-guide>`{.interpreted-text role="ref"}. For the component configuration, please refer to the `Component Configuration Guide <component-configuration-guide>`{.interpreted-text role="ref"}.
::::
...
For more information about the configuration system in ESP-IDF, please refer to the `Configuration Overview <configuration-overview>`{.interpreted-text role="ref"}.

## Kconfig and Kconfig.projbuild Files {#kconfig-files}
...
### sdkconfig and sdkconfig.old {#sdkconfig-file}
...
### sdkconfig.defaults and sdkconfig.defaults.\<chip\> {#sdkconfig-defaults-file}
...
`sdkconfig.defaults` .. code-block:: kconfig

> CONFIG_SUBLIGHT_SPEED=42

`sdkconfig` .. code-block:: kconfig

> \# user changed the value e.g. in menuconfig -\> value from sdkconfig.defaults will be ignored CONFIG_SUBLIGHT_SPEED=10
::::

It is also possible to override the name of this file by setting an environment variable. For information on how to set the custom file name and in which order the files are processed if multiple files with default values are present, please visit `Custom Sdkconfig Defaults <custom-sdkconfig-defaults>`{.interpreted-text role="ref"} section of Build System documentation.
