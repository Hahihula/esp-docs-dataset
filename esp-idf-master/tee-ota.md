---
original_file_path: security/tee/tee-ota.rst
---
...
The OTA update mechanism allows a device to update itself based on data received while the normal firmware is running (for example, over Wi-Fi or Bluetooth). While OTA updates for application images are extensively documented `here <../../api-reference/system/ota>`{.interpreted-text role="doc"}, this section specifically details how OTA updates for the TEE image are securely managed.
...
> <img src="../../../_static/esp_tee/esp_tee_ota_flash_partitions.png" alt="ESP TEE OTA Flash Partition" />
> <figcaption>Sample flash partition layout with TEE and application OTA support</figcaption>
> </figure>
...
> <figure class="align-center">
> <img src="../../../_static/esp_tee/tee_ota_data_entry.png" alt="ESP TEE OTA Data Entry" />
> <figcaption>ESP-TEE: OTA Data partition entry</figcaption>
> </figure>
...
> - If `Secure Boot <secure_boot-guide>`{.interpreted-text role="ref"} is enabled, the bootloader will verify the TEE and the non-secure app.
> ::::

<figure class="align-center">
<img src="../../../_static/esp_tee/esp_tee_ota_flow.png" alt="ESP TEE OTA Flow" />
<figcaption>TEE OTA process flow</figcaption>
</figure>

## Application Example

The `tee_secure_ota <security/tee/tee_secure_ota>`{.interpreted-text role="example"} example demonstrates how to perform an Over-the-Air updates with HTTP/S for the TEE.
...
To use the TEE OTA APIs in your project, ensure that the `tee_ota_ops <esp_tee/subproject/components/tee_ota_ops>`{.interpreted-text role="component"} component is listed as a local dependency in the component manager manifest file [idf_component.yml](https://docs.espressif.com/projects/idf-component-manager/en/latest/reference/manifest_file.html). Refer to the `tee_secure_ota <security/tee/tee_secure_ota>`{.interpreted-text role="example"} example for guidance.
::::

::: include-build-file
inc/esp_tee_ota_ops.inc
:::
