---
original_file_path: api-guides/tools/idf-tools.rst
---
...
Installing the tools using an OS-specific package manager (e.g., apt, yum, brew, etc.) is the preferred method, when the required version of the tool is available. This recommendation is reflected in the `Get Started <../../get-started/index>`{.interpreted-text role="doc"}. For example, on Linux and macOS, it is recommended to install CMake using an OS package manager.
...
If you are looking for instructions on how to install the tools, see `Get Started <../../get-started/index>`{.interpreted-text role="doc"}.
::::

## Tools Metadata File

The list of tools and tool versions required for each platform is located in `tools/tools.json`{.interpreted-text role="idf_file"}. The schema of this file is defined by `tools/tools_schema.json`{.interpreted-text role="idf_file"}.

This file is used by the `tools/idf_tools.py`{.interpreted-text role="idf_file"} script when installing the tools or setting up the environment variables.

## Tools Installation Directory {#idf-tools-path}
...
## `idf_tools.py` Script {#idf-tools-py}

The `tools/idf_tools.py`{.interpreted-text role="idf_file"} script bundled with ESP-IDF performs several functions:
...
## Install Scripts {#idf-tools-install}
...
It is also possible to install tools for specific chip targets. For example, `install.sh esp32` installs tools only for ESP32. See `Step 3. Set up the Tools <get-started-set-up-tools-legacy>`{.interpreted-text role="ref"} for more examples.

`install.sh --enable-XY` enables feature `XY` (by running `idf_tools.py install-python-env --features=core,XY`).

## Export Scripts {#idf-tools-export}
...
- `ESP-IDF Installation Manager <get-started-how-to-get-esp-idf>`{.interpreted-text role="ref"} can download and install the tools. Internally the installer uses `idf_tools.py`.
...
## Uninstall ESP-IDF {#idf-tools-uninstall}
...
- Users using the `ESP-IDF Installation Manager <get-started-how-to-get-esp-idf>`{.interpreted-text role="ref"} can remove ESP-IDF in the graphical user interface (GUI) or command line interface (CLI).
- To remove an installation performed by running the supported `install scripts <idf-tools-install>`{.interpreted-text role="ref"}, simply delete the `tools installation directory <idf-tools-path>`{.interpreted-text role="ref"} including the downloaded and installed tools. Any environment variables set by the `export scripts <idf-tools-export>`{.interpreted-text role="ref"} are not permanent and will not be present after opening a new environment.
...
## List of ESP-IDF Tools {#idf-tools-list}
