---
original_file_path: api-reference/api-conventions.rst
---
...
- Build system functions, predefined variables, and options. These are documented in the `ESP-IDF CMake Build System API <cmake_buildsystem_api>`{.interpreted-text role="ref"}.
- `Kconfig options <configs-in-C-Cmake>`{.interpreted-text role="ref"} can be used in code and in the build system (`CMakeLists.txt`) files.
- `Host tools <../api-guides/tools/index>`{.interpreted-text role="doc"} and their command line parameters are also part of the ESP-IDF interfaces.
...
Most ESP-IDF APIs return error codes defined with the `esp_err_t`{.interpreted-text role="cpp:type"} type. See `Error Handling <../api-guides/error-handling>`{.interpreted-text role="doc"} section for more information about error handling approaches. `Error Codes Reference <error-codes>`{.interpreted-text role="doc"} contains the list of error codes returned by ESP-IDF components.

## Configuration Structures {#api_reference_config_structures}
...
## Private APIs {#api_reference_private_apis}

Certain header files in ESP-IDF contain APIs intended to be used only in ESP-IDF source code rather than by the applications. Such header files often contain `private` or `esp_private` in their name or path. Certain components, such as `hal <../api-guides/hardware-abstraction>`{.interpreted-text role="doc"} only contain private APIs.

Private APIs may be removed or changed in an incompatible way between minor or patch releases.

## Components in Example Projects {#api_reference_example_components}

ESP-IDF examples contain a variety of projects demonstrating the usage of ESP-IDF APIs. In order to reduce code duplication in the examples, a few common helpers are defined inside components that are used by multiple examples. This includes components located in `common_components`{.interpreted-text role="example"} directory, as well as some of the components located in the examples themselves. These components are not considered to be part of the ESP-IDF API.
...
ESP-IDF uses [Semantic Versioning](https://semver.org/) as explained in the `Versioning Scheme <versioning-scheme>`{.interpreted-text role="ref"}.
...
- Renaming Kconfig options. Kconfig system\'s `backward compatibility <configuration-options-compatibility>`{.interpreted-text role="ref"} ensures that the original Kconfig option names can still be used by the application in `sdkconfig` file, CMake files, and source code.
...
- Adding new structure members or changing the order of members. See `api_reference_config_structures`{.interpreted-text role="ref"} for tips that help ensure compatibility.
...
- `api_reference_private_apis`{.interpreted-text role="ref"}.
- `api_reference_example_components`{.interpreted-text role="ref"}.
...
- Location of `Kconfig <project-configuration-guide>`{.interpreted-text role="ref"} options in menuconfig.
