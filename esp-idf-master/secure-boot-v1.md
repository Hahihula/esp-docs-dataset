---
original_file_path: security/secure-boot-v1.rst
---
...
All references in this document are related to Secure Boot V1, i.e., the AES-based Secure Boot Scheme. For ESP32 v3.0 onwards, the preferred secure boot scheme is `secure-boot-v2`{.interpreted-text role="doc"}.

Please refer to `secure-boot-v2`{.interpreted-text role="doc"} for ESP32 v3.0 or ESP32-S2.
::::
...
Secure boot is separate from the `flash-encryption`{.interpreted-text role="doc"} feature, and you can use secure boot without encrypting the flash contents. However, for a secure environment, both should be used simultaneously. See `secure-boot-and-flash-encr`{.interpreted-text role="ref"} for more details.
...
- eFuses are used to store the secure boot information permanently. The secure boot key is stored in eFuse BLOCK2, and also, a single eFuse bit ABS_DONE_0 is burned, or written to 1, to permanently enable secure boot on the chip. For more details on eFuses, see *{IDF_TARGET_NAME} Technical Reference Manual* \> *eFuse Controller (EFUSE)* \[[PDF]({IDF_TARGET_TRM_EN_URL}#efuse)\].
- To understand the secure boot process, please familiarize yourself with the standard `../api-guides/startup`{.interpreted-text role="doc"}.
...
This is a high-level overview of the secure boot process. Step-by-step instructions are supplied under `secure-boot-how-to`{.interpreted-text role="ref"}. Further in-depth details are supplied under `secure-boot-technical-details`{.interpreted-text role="ref"}:

1.  The options to enable secure boot are provided in the `project-configuration-menu`{.interpreted-text role="ref"}, under `Secure Boot Configuration`.
...
- The secure bootloader key is a 256-bit AES key that is stored in eFuse block 2. The bootloader can generate this key itself from the internal hardware random number generator, and you do not need to supply it. It is optionally possible to supply this key, see `secure-boot-reflashable`{.interpreted-text role="ref"}. The eFuse holding this key is read and write protected before the secure boot is enabled to prevent further software access of the key.
  ...
  For more details, see *{IDF_TARGET_NAME} Technical Reference Manual* \> *eFuse Controller (EFUSE)* \> *System Parameter coding_scheme* \[[PDF]({IDF_TARGET_TRM_EN_URL}#efuse)\].
  :::

  The algorithm operates on a 256-bit key in all cases. 192-bit keys are extended by repeating some bits, see `secure-bootloader-digest-algorithm`{.interpreted-text role="ref"}.

- `secure boot signing key` is a standard ECDSA public/private key pair in PEM format, see `secure-boot-image-signing-algorithm`{.interpreted-text role="ref"}.
...
## How to Enable Secure Boot {#secure-boot-how-to}

1.  Open the `project-configuration-menu`{.interpreted-text role="ref"}, navigate to `Secure Boot Configuration` and select the option `One-time Flash`. To understand the alternative `Reflashable` option, see `secure-boot-reflashable`{.interpreted-text role="ref"}.
...
A signing key generated this way will use the best random number source available to the OS and its Python installation, i.e., [/dev/urandom]{.title-ref} on OSX/Linux and [CryptGenRandom()]{.title-ref} on Windows. If this random number source is weak, then the private key will be weak.
::::
...
For production environments, we recommend generating the key pair using OpenSSL or another industry-standard encryption program. See `secure-boot-generate-key`{.interpreted-text role="ref"} for more details.
::::
...
::: {#secure-boot-resume-normal-flashing}
...
## Reflashable Second Stage Bootloader {#secure-boot-reflashable}
...
However, an alternative mode `CONFIG_SECURE_BOOTLOADER_MODE`{.interpreted-text role="ref"} is also available. This mode allows you to supply a binary key file that is used for the secure bootloader key. As you have the key file, you can generate new bootloader images and secure boot digests for them.

In the ESP-IDF build process, this 256-bit key file is derived from the ECDSA app signing key generated by the user, see the `secure-boot-generate-key`{.interpreted-text role="ref"} step below. This private key\'s SHA-256 digest is used as the secure bootloader key in eFuse, as-is for Coding Scheme `None`, or truncated to 192 bytes for `3/4 Encoding`. This is a convenience so you only need to generate or protect a single private key.
...
1.  In the `project-configuration-menu`{.interpreted-text role="ref"}, select `Bootloader Config` \> `CONFIG_SECURE_BOOT`{.interpreted-text role="ref"} \> `CONFIG_SECURE_BOOT_V1_ENABLED` \> `CONFIG_SECURE_BOOTLOADER_MODE`{.interpreted-text role="ref"} \> `Reflashable`.
2.  If necessary, set the `CONFIG_SECURE_BOOTLOADER_KEY_ENCODING`{.interpreted-text role="ref"} based on the coding scheme used by the device. The coding scheme is shown in the `Features` line when `esptool` connects to the chip, or in the `idf.py efuse-summary` output.
3.  Please follow the steps shown in `secure-boot-generate-key`{.interpreted-text role="ref"} to generate the signing key. The path of the generated key file must be specified in the `Secure Boot Configuration` menu.
...
5.  Resume from `Step 6 of the one-time flashing process <secure-boot-resume-normal-flashing>`{.interpreted-text role="ref"}, to flash the bootloader and enable secure boot. Watch the console log output closely to ensure there were no errors in the secure boot configuration.

## Generating Secure Boot Signing Key {#secure-boot-generate-key}
...
## Remote Signing of Images {#remote-sign-image}
...
## Technical Details {#secure-boot-technical-details}
...
For a Python version of this algorithm, see the `espsecure` tool in the `/esptool_py`{.interpreted-text role="component"} directory. Specifically, the `digest-secure-bootloader` command.

Items marked with (\^) are to fulfill hardware restrictions, as opposed to cryptographic restrictions.

1.  Read the AES key from eFuse block 2, in reversed byte order. If the Coding Scheme is set to `3/4 Encoding`, extend the 192-bit key to 256 bits using the same algorithm described in `flash-encryption-algorithm`{.interpreted-text role="ref"}.
...
### Image Signing Algorithm {#secure-boot-image-signing-algorithm}
...
## Secure Boot & Flash Encryption {#secure-boot-and-flash-encr}
...
## Signed App Verification Without Hardware Secure Boot {#signed-app-verify}

The integrity of apps can be checked even without enabling the hardware secure boot option. This option uses the same app signature scheme as hardware secure boot, but unlike hardware secure boot, it does not prevent the bootloader from being physically updated. This means that the device can be secured against remote network access, but not physical access. Compared to using hardware secure boot, this option is much simpler to implement. See `signed-app-verify-how-to`{.interpreted-text role="ref"} for step-by-step instructions.
...
### How To Enable Signed App Verification {#signed-app-verify-how-to}

1.  Open `project-configuration-menu`{.interpreted-text role="ref"} \> `Security features` \> enable `CONFIG_SECURE_SIGNED_APPS_NO_SECURE_BOOT`{.interpreted-text role="ref"}
...
4.  If you disable the `Sign binaries during build` option then you will have to enter the path of a public key file used to verify signed images in the `Secure boot public signature verification key`.
    ...
    In this case, the private signing key should be generated by following instructions in `secure-boot-generate-key`{.interpreted-text role="ref"}; the public verification key and signed image should be generated by following instructions in `remote-sign-image`{.interpreted-text role="ref"}.
