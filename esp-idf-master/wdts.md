---
original_file_path: api-reference/system/wdts.rst
---
...
\- RTC/LP Watchdog Timer (RTC_WDT/LP_WDT) :SOC_XT_WDT_SUPPORTED: - XTAL32K Watchdog Timer (Crystal 32K Watchdog Timer, i.e., XTWDT)
:::
...
The various watchdog timers can be enabled by `project-configuration-menu`{.interpreted-text role="ref"}. However, the TWDT can also be enabled during runtime.

## Hardware Watchdog Timers {#app-hardware-watchdog-timers}
...
Refer to the `bootloader-watchdog`{.interpreted-text role="ref"} section to understand how watchdogs are utilized in the bootloader.

The app\'s behaviour can be adjusted so the RTC Watchdog remains enabled after app startup. The Watchdog would need to be explicitly reset (i.e., fed) or disabled by the app to avoid the chip reset. To do this, set the `CONFIG_BOOTLOADER_WDT_DISABLE_IN_USER_CODE`{.interpreted-text role="ref"} option, modify the app as needed, and then recompile the app. In this case, the following APIs should be used:

::: list
- `wdt_hal_disable`{.interpreted-text role="cpp:func"}: see `hw-abstraction-hal-layer-disable-rtc-wdt`{.interpreted-text role="ref"}

\- `wdt_hal_feed`{.interpreted-text role="cpp:func"}: see `hw-abstraction-hal-layer-feed-rtc-wdt`{.interpreted-text role="ref"} :esp32 or esp32s2: - `rtc_wdt_feed`{.interpreted-text role="cpp:func"} :esp32 or esp32s2: - `rtc_wdt_disable`{.interpreted-text role="cpp:func"}
:::

If RTC_WDT is not reset/disabled in time, the chip will be automatically reset. See `RTC-Watchdog-Timeout`{.interpreted-text role="ref"} for more information.
...
- The IWDT is enabled by default via the `CONFIG_ESP_INT_WDT`{.interpreted-text role="ref"} option.

- The IWDT\'s timeout is configured by setting the `CONFIG_ESP_INT_WDT_TIMEOUT_MS`{.interpreted-text role="ref"} option.
  ...
  > - The configured timeout duration for IWDT should always be at least twice longer than the period between two FreeRTOS ticks, e.g., if two FreeRTOS ticks occur 10 ms apart, then IWDT timeout duration should at least be more than 20 ms (see `CONFIG_FREERTOS_HZ`{.interpreted-text role="ref"}).
...
Neither critical sections or interrupt handlers should ever block waiting for another event to occur. If changing the code to reduce the processing time is not possible or desirable, it is possible to increase the `CONFIG_ESP_INT_WDT_TIMEOUT_MS`{.interpreted-text role="ref"} setting instead.

## Task Watchdog Timer (TWDT) {#task-watchdog-timer}
...
- `esp_task_wdt_init`{.interpreted-text role="cpp:func"} to initialize the TWDT and subscribe the idle tasks.
- `esp_task_wdt_add`{.interpreted-text role="cpp:func"} subscribes other tasks to the TWDT.
- Once subscribed, `esp_task_wdt_reset`{.interpreted-text role="cpp:func"} should be called from the task to feed the TWDT.
- `esp_task_wdt_delete()`{.interpreted-text role="cpp:func"} unsubscribes a previously subscribed task.
- `esp_task_wdt_deinit()`{.interpreted-text role="cpp:func"} unsubscribes the idle tasks and deinitializes the TWDT.
...
- `esp_task_wdt_add_user`{.interpreted-text role="cpp:func"} to subscribe an arbitrary user of the TWDT. This function returns a user handle to the added user.
- `esp_task_wdt_reset_user`{.interpreted-text role="cpp:func"} must be called using the user handle in order to prevent a TWDT timeout.
- `esp_task_wdt_delete_user`{.interpreted-text role="cpp:func"} unsubscribes an arbitrary user of the TWDT.

### Configuration

The default timeout period for the TWDT is set using config item `CONFIG_ESP_TASK_WDT_TIMEOUT_S`{.interpreted-text role="ref"}. This should be set to at least as long as you expect any single task needs to monopolize the CPU (for example, if you expect the app will do a long intensive calculation and should not yield to other tasks). It is also possible to change this timeout at runtime by calling `esp_task_wdt_init`{.interpreted-text role="cpp:func"}.
...
- Increase `CONFIG_ESP_TASK_WDT_TIMEOUT_S`{.interpreted-text role="ref"} in menuconfig for a larger watchdog timeout period.
- You can also call `esp_task_wdt_init`{.interpreted-text role="cpp:func"} again to increase the watchdog timeout period before erasing a large flash area.

For more information, you can refer to `../peripherals/spi_flash/index`{.interpreted-text role="doc"}.
::::
...
::: list
- `CONFIG_ESP_TASK_WDT_EN`{.interpreted-text role="ref"} - enables TWDT feature. If this option is disabled, TWDT cannot be used, even if initialized at runtime.
- `CONFIG_ESP_TASK_WDT_INIT`{.interpreted-text role="ref"} - initializes the TWDT automatically during startup. If this option is disabled, it is still possible to initialize the Task WDT at runtime by calling `esp_task_wdt_init`{.interpreted-text role="cpp:func"}.

\- `CONFIG_ESP_TASK_WDT_CHECK_IDLE_TASK_CPU0`{.interpreted-text role="ref"} - subscribes {IDF_TARGET_IDLE_TASK} to the TWDT during startup. If this option is disabled, it is still possible to subscribe the idle task by calling `esp_task_wdt_init`{.interpreted-text role="cpp:func"} again, or by using `esp_task_wdt_add`{.interpreted-text role="cpp:func"} and passing the idle task handle obtained via `xTaskGetIdleTaskHandleForCore`{.interpreted-text role="cpp:func"}. :SOC_HP_CPU_HAS_MULTIPLE_CORES: - `CONFIG_ESP_TASK_WDT_CHECK_IDLE_TASK_CPU1`{.interpreted-text role="ref"} - Subscribes CPU1 Idle task to the TWDT during startup.
:::
...
On a TWDT timeout the default behaviour is to simply print a warning and a backtrace before continuing running the app. If you want a timeout to cause a panic and a system reset then this can be configured through `CONFIG_ESP_TASK_WDT_PANIC`{.interpreted-text role="ref"}.
::::
...
- When the external 32 KHz crystal or oscillator is selected (`CONFIG_RTC_CLK_SRC`{.interpreted-text role="ref"}) the XTWDT can be enabled via the `CONFIG_ESP_XT_WDT`{.interpreted-text role="ref"} configuration option.
- The timeout is configured by setting the `CONFIG_ESP_XT_WDT_TIMEOUT`{.interpreted-text role="ref"} option.
...
Stages can be configured via `wdt_hal_config_stage`{.interpreted-text role="cpp:func"} (or equivalent APIs), selecting actions per stage to match application behavior.
...
- `Task watchdog got triggered. The following tasks/users did not reset the watchdog in time: - IDLE0 (CPU 0), Tasks currently running: CPU 0: main, CPU 1: IDLE1`: Indicates that the Task Watchdog Timer (TWDT) has detected that one or more tasks have not yielded within the configured timeout period, and hence the IDLE task couldn\'t feed the TWDT in time. This can be fixed by ensuring that tasks yield appropriately, reducing the duration of long-running tasks, or by increasing the TWDT timeout period. The user can also use `esp_task_wdt_add`{.interpreted-text role="cpp:func"}, `esp_task_wdt_add_user`{.interpreted-text role="cpp:func"} and `esp_task_wdt_reset_user`{.interpreted-text role="cpp:func"} APIs to find out which task and which code path inside that task is taking the longest time and causing the TWDT timeout.
- `CONFIG_BOOTLOADER_WDT_DISABLE_IN_USER_CODE`{.interpreted-text role="ref"} is enabled and causes WDT timeout: Make sure RTC WDT is fed frequently enough from user code.
...
- `system/task_watchdog`{.interpreted-text role="example"} demonstrates how to initialize, subscribe and unsubscribe tasks and users to the task watchdog, and how tasks and users can reset (feed) the task watchdog.
