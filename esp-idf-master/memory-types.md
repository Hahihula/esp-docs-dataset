---
original_file_path: api-guides/memory-types.rst
---
...
By applying the `EXT_RAM_BSS_ATTR` macro, zero-initialized data can also be placed into external RAM. To use this macro, the `CONFIG_SPIRAM_ALLOW_BSS_SEG_EXTERNAL_MEMORY`{.interpreted-text role="ref"} needs to be enabled. See `external_ram_config_bss`{.interpreted-text role="ref"}.
:::
...
The maximum statically allocated DRAM size is reduced by the `iram`{.interpreted-text role="ref"} size of the compiled application. The available heap memory at runtime is reduced by the total static IRAM and DRAM usage of the application.
::::
:::::

Constant data may also be placed into DRAM, for example if it is used in an non-flash-safe ISR (see explanation under `how-to-place-code-in-iram`{.interpreted-text role="ref"}).
...
By applying the `EXT_RAM_NOINIT_ATTR` macro, non-initialized value could also be placed in external RAM. To do this, the `CONFIG_SPIRAM_ALLOW_NOINIT_SEG_EXTERNAL_MEMORY`{.interpreted-text role="ref"} needs to be enabled. See `external_ram_config_noinit`{.interpreted-text role="ref"}. If the `CONFIG_SPIRAM_ALLOW_NOINIT_SEG_EXTERNAL_MEMORY`{.interpreted-text role="ref"} is not enabled, `EXT_RAM_NOINIT_ATTR` will behave just as `__NOINIT_ATTR`, it will make data to be placed into `.noinit` segment in internal RAM.
:::
...
Any internal SRAM which is not used for Instruction RAM will be made available as `dram`{.interpreted-text role="ref"} for static data and dynamic allocation (heap).
::::
:::::
...
- Interrupt handlers must be placed into IRAM if `ESP_INTR_FLAG_IRAM` is used when registering the interrupt handler. For more information, see `iram-safe-interrupt-handlers`{.interpreted-text role="ref"}.
...
If some specific application code needs to be placed into IRAM, it can be done by using the `linker-script-generation`{.interpreted-text role="doc"} feature and adding a linker script fragment file to your component that targets at the entire source files or functions with the `noflash` placement. See the `linker-script-generation`{.interpreted-text role="doc"} docs for more information.
...
Jump table optimizations can be re-enabled for individual source files that do not need to be placed in IRAM. For instructions on how to add the `-fno-jump-tables -fno-tree-switch-conversion` options when compiling individual source files, see `component_build_control`{.interpreted-text role="ref"}.

## IROM (Code Executed from flash) {#irom}

If a function is not explicitly placed into `iram`{.interpreted-text role="ref"} or RTC memory, it is placed into flash. As IRAM is limited, most of an application\'s binary code must be placed into IROM instead.

::: only
esp32

The mechanism by which Flash MMU is used to allow code execution from flash is described in *{IDF_TARGET_NAME} Technical Reference Manual* \> *Memory Management and Protection Units (MMU, MPU)* \[[PDF]({IDF_TARGET_TRM_EN_URL}#mpummu)\].
:::

During `startup`{.interpreted-text role="doc"}, the bootloader (which runs from IRAM) configures the MMU flash cache to map the app\'s instruction code region to the instruction space. Flash accessed via the MMU is cached using some internal SRAM and accessing cached flash data is as fast as accessing other types of internal memory.

## DROM (Data Stored in flash) {#drom}

By default, constant data is placed by the linker into a region mapped to the MMU flash cache. This is the same as the `irom`{.interpreted-text role="ref"} section, but is for read-only data not executable code.
...
The `DRAM_ATTR` attribute can be used to force constants from DROM into the `dram`{.interpreted-text role="ref"} section (see above).
...
Global and static variables used by code which runs from RTC memory must be placed into RTC Slow memory. For example `deep sleep <deep-sleep-stub>`{.interpreted-text role="doc"} variables can be placed here instead of RTC FAST memory, or code and variables accessed by the `/api-reference/system/ulp`{.interpreted-text role="doc"}.
:::
...
The same region of RTC FAST memory can be accessed as both instruction and data memory. Code which has to run after wake-up from deep sleep mode has to be placed into RTC memory. Please check detailed description in `deep sleep <deep-sleep-stub>`{.interpreted-text role="doc"} documentation.
:::

::: only
esp32

In single core mode (`CONFIG_FREERTOS_UNICORE`{.interpreted-text role="ref"}), remaining RTC FAST memory is added to the heap, unless the option `CONFIG_ESP_SYSTEM_ALLOW_RTC_FAST_MEM_AS_HEAP`{.interpreted-text role="ref"} is disabled. This memory can be used interchangeably with `DRAM`{.interpreted-text role="ref"}, but is slightly slower to access and not DMA-capable.
...
Remaining RTC FAST memory is added to the heap unless the option `CONFIG_ESP_SYSTEM_ALLOW_RTC_FAST_MEM_AS_HEAP`{.interpreted-text role="ref"} is disabled. This memory can be used interchangeably with `DRAM`{.interpreted-text role="ref"}, but is slightly slower to access.
:::
:::::::::
...
It is also possible to allocate DMA-capable memory buffers dynamically by using the `MALLOC_CAP_DMA <dma-capable-memory>`{.interpreted-text role="ref"} capabilities flag.
...
:   - Placing DRAM buffers on the stack is not recommended if the stack may be in PSRAM. If the stack of a task is placed in the PSRAM, several steps have to be taken as described in `external-ram`{.interpreted-text role="doc"}.
