---
original_file_path: api-guides/esp-ble-mesh/ble-mesh-faq.rst
---
...
- `ble-mesh-faq-provisioner-development`{.interpreted-text role="ref"}
- `ble-mesh-faq-node-development`{.interpreted-text role="ref"}
- `ble-mesh-faq-ble-mesh-and-wi-fi-coexistence`{.interpreted-text role="ref"}
- `ble-mesh-faq-fast-provisioning`{.interpreted-text role="ref"}
- `ble-mesh-faq-log-help`{.interpreted-text role="ref"}
- `ble-mesh-faq-example-help`{.interpreted-text role="ref"}
- `ble-mesh-faq-others`{.interpreted-text role="ref"}
...
## 1. Provisioner Development {#ble-mesh-faq-provisioner-development}
...
> - The API `esp_ble_mesh_provisioner_add_local_net_key`{.interpreted-text role="cpp:func"} can be used to add a NetKey with a fixed or random value.
> - The API `esp_ble_mesh_provisioner_add_local_app_key`{.interpreted-text role="cpp:func"} can be used to add an AppKey with a fixed or random value.

### 1.5 Is the Unicast Address of Provisioner Fixed?

> The value of `prov_unicast_addr` in `esp_ble_mesh_prov_t`{.interpreted-text role="cpp:type"} is used to set the unicast address of Provisioner, it can be set only once during initialization and cannot be changed afterwards.
...
> The value of `prov_attention` in `esp_ble_mesh_prov_t`{.interpreted-text role="cpp:type"} is used by Provisioner set to unprovisioned device during provisioning. It can be set only once during initialization and cannot be changed afterwards. When the unprovisioned device is joining the mesh network, it can display in a specific way like flashing light to notify Provisioner that it is being provisioned.

### 1.10 How Many Ways to Authenticate the Devices During Provisioning? Which Way Was Used in the `provided examples <bluetooth/esp_ble_mesh>`{.interpreted-text role="example"}?
...
### 1.14 How Can Provisioner Get and Parse the `Composition Data <ble-mesh-terminology-composition>`{.interpreted-text role="ref"} of Nodes Through Configuration Client Model?

> - Provisioner can get the Composition Data of nodes using the `Configuration Client Model <ble-mesh-terminology-foundation-models>`{.interpreted-text role="ref"} API `esp_ble_mesh_config_client_set_state`{.interpreted-text role="cpp:func"} with `comp_data_get` in the parameter `esp_ble_mesh_cfg_client_get_state_t`{.interpreted-text role="cpp:type"} set properly.
...
> Provisioner do the following configuration by calling the `Configuration Client Model <ble-mesh-terminology-foundation-models>`{.interpreted-text role="ref"} API `esp_ble_mesh_config_client_set_state`{.interpreted-text role="cpp:func"}.
>
> - Add AppKey to nodes with `app_key_add` in the parameter `esp_ble_mesh_cfg_client_set_state_t`{.interpreted-text role="cpp:type"} set properly.
> - Add subscription address to the models of nodes with `model_sub_add` in the parameter `esp_ble_mesh_cfg_client_set_state_t`{.interpreted-text role="cpp:type"} set properly.
> - Set publication information to the models of nodes with `model_pub_set` in the parameter `esp_ble_mesh_cfg_client_set_state_t`{.interpreted-text role="cpp:type"} set properly.
...
> - Secondly, Provisioner performs the `Key Refresh procedure <ble-mesh-terminology-network-management>`{.interpreted-text role="ref"}.
...
> - Provisioner updates the NetKey of nodes using the `Configuration Client Model <ble-mesh-terminology-foundation-models>`{.interpreted-text role="ref"} API `esp_ble_mesh_config_client_set_state`{.interpreted-text role="cpp:func"} with `net_key_update` in the parameter `esp_ble_mesh_cfg_client_set_state_t`{.interpreted-text role="cpp:type"} set properly.
> - Provisioner updates the AppKey of nodes using the `Configuration Client Model <ble-mesh-terminology-foundation-models>`{.interpreted-text role="ref"} API `esp_ble_mesh_config_client_set_state`{.interpreted-text role="cpp:func"} with `app_key_update` in the parameter `esp_ble_mesh_cfg_client_set_state_t`{.interpreted-text role="cpp:type"} set properly.

### 1.22 How Does Provisioner Manage Nodes in the Mesh Network?

> ESP-BLE-MESH implements several functions related to basic node management in the example, such as `esp_ble_mesh_store_node_info`{.interpreted-text role="cpp:func"}. And ESP-BLE-MESH also provides the API `esp_ble_mesh_provisioner_set_node_name`{.interpreted-text role="cpp:func"} which can be used to set the node\'s local name and the API `esp_ble_mesh_provisioner_get_node_name`{.interpreted-text role="cpp:func"} which can be used to get the node\'s local name.
...
>
> - Provisioner add NetKey by calling the API `esp_ble_mesh_provisioner_add_local_net_key`{.interpreted-text role="cpp:func"}.
> - Provisioner add AppKey by calling the API `esp_ble_mesh_provisioner_add_local_app_key`{.interpreted-text role="cpp:func"}.
>
> Provisioner shall configure its own client model.
>
> - Provisioner bind AppKey to its own client model by calling the API `esp_ble_mesh_provisioner_bind_app_key_to_local_model`{.interpreted-text role="cpp:func"}.
...
>
>   > - The API `esp_ble_mesh_config_client_get_state`{.interpreted-text role="cpp:func"} can be used to get the `esp_ble_mesh_cfg_client_get_state_t`{.interpreted-text role="cpp:type"} values of Configuration Server Model.
>   > - The API `esp_ble_mesh_config_client_set_state`{.interpreted-text role="cpp:func"} can be used to set the `esp_ble_mesh_cfg_client_set_state_t`{.interpreted-text role="cpp:type"} values of Configuration Server Model.
>
> - Health Client Model
>
>   > - The API `esp_ble_mesh_health_client_get_state`{.interpreted-text role="cpp:func"} can be used to get the `esp_ble_mesh_health_client_get_state_t`{.interpreted-text role="cpp:type"} values of Health Server Model.
>   > - The API `esp_ble_mesh_health_client_set_state`{.interpreted-text role="cpp:func"} can be used to set the `esp_ble_mesh_health_client_set_state_t`{.interpreted-text role="cpp:type"} values of Health Server Model.
>
> - Generic Client Models
>
>   > - The API `esp_ble_mesh_generic_client_get_state`{.interpreted-text role="cpp:func"} can be used to get the `esp_ble_mesh_generic_client_get_state_t`{.interpreted-text role="cpp:type"} values of Generic Server Models.
>   > - The API `esp_ble_mesh_generic_client_set_state`{.interpreted-text role="cpp:func"} can be used to set the `esp_ble_mesh_generic_client_set_state_t`{.interpreted-text role="cpp:type"} values of Generic Server Models.
>
> - Lighting Client Models
>
>   > - The API `esp_ble_mesh_light_client_get_state`{.interpreted-text role="cpp:func"} can be used to get the `esp_ble_mesh_light_client_get_state_t`{.interpreted-text role="cpp:type"} values of Lighting Server Models.
>   > - The API `esp_ble_mesh_light_client_set_state`{.interpreted-text role="cpp:func"} can be used to set the `esp_ble_mesh_light_client_set_state_t`{.interpreted-text role="cpp:type"} values of Lighting Server Models.
>
> - Sensor Client Models
>
>   > - The API `esp_ble_mesh_sensor_client_get_state`{.interpreted-text role="cpp:func"} can be used to get the `esp_ble_mesh_sensor_client_get_state_t`{.interpreted-text role="cpp:type"} values of Sensor Server Model.
>   > - The API `esp_ble_mesh_sensor_client_set_state`{.interpreted-text role="cpp:func"} can be used to set the `esp_ble_mesh_sensor_client_set_state_t`{.interpreted-text role="cpp:type"} values of Sensor Server Model.
>
> - 
>
>   Time and Scenes Client Models
>
>   :   - The API `esp_ble_mesh_time_scene_client_get_state`{.interpreted-text role="cpp:func"} can be used to get the `esp_ble_mesh_time_scene_client_get_state_t`{.interpreted-text role="cpp:type"} values of Time and Scenes Server Models.
>       - The API `esp_ble_mesh_time_scene_client_set_state`{.interpreted-text role="cpp:func"} can be used to set the `esp_ble_mesh_time_scene_client_set_state_t`{.interpreted-text role="cpp:type"} values of Time and Scenes Server Models.

## 2. Node Development {#ble-mesh-faq-node-development}
...
> - For client models, users can use the API `esp_ble_mesh_client_model_send_msg`{.interpreted-text role="cpp:func"} to send messages.
> - For server models, users can use the API `esp_ble_mesh_server_model_send_msg`{.interpreted-text role="cpp:func"} to send messages.
> - For publication, users call the API `esp_ble_mesh_model_publish`{.interpreted-text role="cpp:func"} to publish messages.

### 2.4 How to Achieve the Transmission of Messages Without Packet Loss?

> Acknowledged message is needed if users want to transmit messages without packet loss. The default time to wait for corresponding response is set in `CONFIG_BLE_MESH_CLIENT_MSG_TIMEOUT`{.interpreted-text role="ref"}. If the sender waits for the response until the timer expires, the corresponding timeout event would be triggered.
...
Response timeout can be set in the API `esp_ble_mesh_client_model_send_msg`{.interpreted-text role="cpp:func"}. The default value (4 seconds) would be applied if the parameter `msg_timeout` is set to **0**.
::::

### 2.5 How to Send Unacknowledged Messages?

> For client models, users can use the API `esp_ble_mesh_client_model_send_msg`{.interpreted-text role="cpp:func"} with the parameter `need_rsp` set to `false` to send unacknowledged messages.
>
> For server models, the messages sent by using the API `esp_ble_mesh_server_model_send_msg`{.interpreted-text role="cpp:func"} are always unacknowledged messages.
...
> Messages sent by calling the API `esp_ble_mesh_client_model_send_msg`{.interpreted-text role="cpp:func"} or `esp_ble_mesh_server_model_send_msg`{.interpreted-text role="cpp:func"} will be sent in the duration determined by the Network Transmit state.
>
> Messages published by calling the API `esp_ble_mesh_model_publish`{.interpreted-text role="cpp:func"} will be published determined by the Model Publication state. And the publication of messages is generally periodic or with a fixed number of counts. The publication period and publication count are controlled by the Model Publication state, and can be configured through Configuration Client Model.
...
### 2.9 When Should the `Relay <ble-mesh-terminology-Features>`{.interpreted-text role="ref"} Feature of Nodes Be Enabled?
...
### 2.10 When Should the `Proxy <ble-mesh-terminology-Features>`{.interpreted-text role="ref"} Feature of Node Be Enabled?
...
### 2.14 What Is the Principle of Reducing Power Consumption Using `Low Power <ble-mesh-terminology-Features>`{.interpreted-text role="ref"} Feature?
...
> Enable the configuration `Store ESP-BLE-MESH Node configuration persistently` in [menuconfig]{.title-ref}.
...
## 3. ESP-BLE-MESH and Wi-Fi Coexistence {#ble-mesh-faq-ble-mesh-and-wi-fi-coexistence}
...
## 4. Fast Provisioning {#ble-mesh-faq-fast-provisioning}
...
### 4.5 When will Configuration Client Model of the node running `fast_prov_server <bluetooth/esp_ble_mesh/fast_provisioning/fast_prov_server>`{.interpreted-text role="example"} example start to work?
...
## 5. Log Help {#ble-mesh-faq-log-help}
...
> When the node has no space for receiving new segmented message, this warning will appear. Users can make the space larger through the configuration `CONFIG_BLE_MESH_RX_SEG_MSG_COUNT`{.interpreted-text role="ref"}.
...
## 6. Example Help {#ble-mesh-faq-example-help}

### 6.1 How Are the ESP-BLE-MESH Callback Functions Classified?

> - The API `esp_ble_mesh_register_prov_callback`{.interpreted-text role="cpp:func"} is used to register callback function used to handle provisioning and networking related events.
> - The API `esp_ble_mesh_register_config_client_callback`{.interpreted-text role="cpp:func"} is used to register callback function used to handle Configuration Client Model related events.
> - The API `esp_ble_mesh_register_config_server_callback`{.interpreted-text role="cpp:func"} is used to register callback function used to handle Configuration Server Model related events.
> - The API `esp_ble_mesh_register_health_client_callback`{.interpreted-text role="cpp:func"} is used to register callback function used to handle Health Client Model related events.
> - The API `esp_ble_mesh_register_health_server_callback`{.interpreted-text role="cpp:func"} is used to register callback function used to handle Health Server Model related events.
> - The API `esp_ble_mesh_register_generic_client_callback`{.interpreted-text role="cpp:func"} is used to register callback function used to handle Generic Client Models related events.
> - The API `esp_ble_mesh_register_light_client_callback`{.interpreted-text role="cpp:func"} is used to register callback function used to handle Lighting Client Models related events.
> - The API `esp_ble_mesh_register_sensor_client_callback`{.interpreted-text role="cpp:func"} is used to register callback function used to handle Sensor Client Model related events.
> - The API `esp_ble_mesh_register_time_scene_client_callback`{.interpreted-text role="cpp:func"} is used to register callback function used to handle Time and Scenes Client Models related events.
> - The API `esp_ble_mesh_register_custom_model_callback`{.interpreted-text role="cpp:func"} is used to register callback function used to handle vendor model and unrealized server models related events.

## 7. Others {#ble-mesh-faq-others}

### 7.1 How to Print the Message Context?

> The examples use `ESP_LOG_BUFFER_HEX`{.interpreted-text role="cpp:func"} to print the message context while the ESP-BLE-MESH protocol stack uses `bt_hex`{.interpreted-text role="cpp:func"}.

### 7.2 Which API Can Be Used to Restart {IDF_TARGET_NAME}?

> The API `esp_restart`{.interpreted-text role="cpp:func"}.

### 7.3 How to Monitor the Remaining Space of the Stack of a Task?

> The API `vTaskList`{.interpreted-text role="cpp:func"} can be used to print the remaining space of the task stack periodically.

### 7.4 How to Change the Level of Log Without Changing the Menuconfig Output Level?

> The API `esp_log_level_set`{.interpreted-text role="cpp:func"} can be used to change the log output level rather than using menuconfig to change it.
