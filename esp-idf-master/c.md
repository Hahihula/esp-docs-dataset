---
original_file_path: api-guides/c.rst
---
...
- [Picolibc](https://keithp.com/picolibc/) (enabled with `CONFIG_LIBC_PICOLIBC<CONFIG_LIBC_PICOLIBC>`{.interpreted-text role="ref"} Kconfig option)

The Newlib version is specified in `esp_libc/sbom.yml`{.interpreted-text role="component_file"}.

In general, all C features supported by the compiler (currently GCC) can be used in ESP-IDF, unless otherwise noted in `unsupported_c_features`{.interpreted-text role="ref"} below.
...
## Unsupported C Features {#unsupported_c_features}
