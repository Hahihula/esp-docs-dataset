---
original_file_path: get-started/linux-macos-setup-legacy.rst
---
...
- `get-started-prerequisites-legacy`{.interpreted-text role="ref"}
- `get-started-get-esp-idf-legacy`{.interpreted-text role="ref"}
- `get-started-set-up-tools-legacy`{.interpreted-text role="ref"}
- `get-started-set-up-env-legacy`{.interpreted-text role="ref"}

## Step 1. Install Prerequisites {#get-started-prerequisites-legacy}
...
## Step 2. Get ESP-IDF {#get-started-get-esp-idf-legacy}
...
Consult `/versions`{.interpreted-text role="doc"} for information about which ESP-IDF version to use in a given situation.

## Step 3. Set up the Tools {#get-started-set-up-tools-legacy}
...
## Step 4. Set up the Environment Variables {#get-started-set-up-env-legacy}
...
## Updating ESP-IDF and Python Packages in the ESP-IDF Environment {#get-started-update-esp-idf-legacy}
...
The simplest way to do the update is to delete the existing `esp-idf` folder and clone it again, as if performing the initial installation described in `get-started-get-esp-idf-legacy`{.interpreted-text role="ref"}.

Another solution is to update only what has changed. For specific instructions, please visit `Updating ESP-IDF <updating-master>`{.interpreted-text role="ref"} page.

After updating ESP-IDF, execute the install script again (`./install.sh` in your `$IDF_PATH`), in case the new ESP-IDF version requires different versions of tools. See instructions at `get-started-set-up-tools-legacy`{.interpreted-text role="ref"}.

Once all the new tools are installed, enter the ESP-IDF environment using the export script as described in `get-started-set-up-env-legacy`{.interpreted-text role="ref"}.
...
If you are an advanced user and want more control over the update process, you can also check `idf-tools-py`{.interpreted-text role="ref"} and its `install-python-env` command, which is used by the install script and handles the creation or update of the ESP-IDF environment.
::::
