---
original_file_path: api-reference/system/chip_revision.rst
---
...
{IDF_TARGET_NAME} may have different revisions. These revisions mainly fix some issues, and sometimes also bring new features to the chip. `versioning scheme`{.interpreted-text role="ref"} describes the versioning of these chip revisions, and the APIs to read the versions at runtime.
...
`revision_limitation`{.interpreted-text role="ref"} describes how the application can specify its chip revision requirements, and the way ESP-IDF checks the compatibility. After that, there is troubleshooting information for this mechanism.
...
- Ignore maximum wafer revision (`DISABLE_WAFER_VERSION_MAJOR` eFuse). See `revision_limitation`{.interpreted-text role="ref"} on how this is used.
...
- Ignore maximum efuse block revision (`DISABLE_BLK_VERSION_MAJOR` eFuse). See `revision_limitation`{.interpreted-text role="ref"} on how this is used.

### Chip Revision APIs

These APIs helps to get chip revision from eFuses:

- `efuse_hal_chip_revision`{.interpreted-text role="cpp:func"}. It returns revision in the `major * 100 + minor` format.
- `efuse_hal_get_major_chip_version`{.interpreted-text role="cpp:func"}. It returns Major revision of wafer.
- `efuse_hal_get_minor_chip_version`{.interpreted-text role="cpp:func"}. It returns Minor revision of wafer.
...
- `efuse_hal_blk_version`{.interpreted-text role="cpp:func"}. It returns revision in the `major * 100 + minor` format.
- `efuse_ll_get_blk_version_major`{.interpreted-text role="cpp:func"}. It returns Major revision of eFuse block.
- `efuse_ll_get_blk_version_minor`{.interpreted-text role="cpp:func"}. It returns Minor revision of eFuse block.
...
## Compatibility Checks of ESP-IDF {#revision_limitation}
...
The minimum chip revision can be configured via the `CONFIG_{IDF_TARGET_CFG_PREFIX}_REV_MIN`{.interpreted-text role="ref"} option. Specifying the minimum chip revision will limit the software to only run on a chip revisions that are high enough to support some features or bugfixes.
...
- Lower the `CONFIG_{IDF_TARGET_CFG_PREFIX}_REV_MIN`{.interpreted-text role="ref"} value and rebuild the image so that it is compatible with the chip revision being used.
...
For the chip revision, the second stage bootloader and the application binary images contain the `esp_image_header_t`{.interpreted-text role="cpp:type"} header, which stores information specifying the chip revisions that the image is permitted to run on. This header has 3 fields related to the chip revisions:

- `min_chip_rev` - Minimum chip MAJOR revision required by image (but for ESP32-C3 it is MINOR revision). Its value is determined by `CONFIG_{IDF_TARGET_CFG_PREFIX}_REV_MIN`{.interpreted-text role="ref"}.
- `min_chip_rev_full` - Minimum chip MINOR revision required by image in format: `major * 100 + minor`. Its value is determined by `CONFIG_{IDF_TARGET_CFG_PREFIX}_REV_MIN`{.interpreted-text role="ref"}.
...
1.  The first stage (ROM) bootloader does not check minimum and maximum revision fields from `esp_image_header_t`{.interpreted-text role="cpp:type"} before running the 2nd stage bootloader.
...
::: only
esp32 or esp32c3 or esp32s2 or esp32s3
...
- `/versions`{.interpreted-text role="doc"}
