---
original_file_path: api-guides/kconfig/project-configuration-guide.rst
---
...
3.  
    ...
    If present, `sdkconfig` file is loaded. What happens next depends on whether the `sdkconfig` value is default or not:

    :   a.  

            If the `sdkconfig` value is default and:
            ...
            :   - `sdkconfig.defaults` has already set given configuration option, the `sdkconfig.defaults` value is used (values from `sdkconfig.defaults` are considered user-set). See the note below for more information.
                - Otherwise, it is checked whether the `sdkconfig` default value is the same as the `Kconfig` default value. If not, an info statement is printed. In both cases, the `sdkconfig` default value is used by default. See the section `default-values-in-kconfig`{.interpreted-text role="ref"} for more information.
...
### Default Values Handling {#default-values-in-kconfig}
...
## How to Use Configuration Variables in C Code and CMake {#configs-in-C-Cmake}
...
Detailed information about the configuration report can be found in the [esp-idf-kconfig Documentation](https://docs.espressif.com/projects/esp-idf-kconfig/en/latest/kconfiglib/configuration-report.html).
::::
