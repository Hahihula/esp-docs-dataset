---
original_file_path: api-reference/template.rst
---
...
1.  Use this file (`docs/en/api-reference/template.rst`{.interpreted-text role="idf_file"}) as a template to document API.
...
3.  To distinguish between sections, use the following [heading levels](https://www.sphinx-doc.org/en/master/usage/restructuredtext/basics.html#sections):
...
1.  ESP-IDF repository provides automatic update of API reference documentation using `code markup retrieved by Doxygen from header files <../contribute/documenting-code>`{.interpreted-text role="doc"}.

2.  Update is done on each documentation build by invoking Sphinx extension `esp_extensions/run_doxygen.py` for all header files listed in the `INPUT` statement of `docs/doxygen/Doxyfile`{.interpreted-text role="idf_file"}.
...
6.  To show contents of `*.inc` file in documentation, include it as follows:
    ...
    For example see `docs/en/api-reference/network/esp_wifi.rst`{.interpreted-text role="idf_file"}

7.  Optionally, rather that using `*.inc` files, you may want to describe API in you own way. See `docs/en/api-reference/storage/fatfs.rst`{.interpreted-text role="idf_file"} for example.
    ...
    To provide a link to header file, use the [link custom role]{.title-ref} directive as follows:

    ``` none
    * :component_file:`path_to/header_file.h`
    ```

8.  In any case, to generate API reference, the file `docs/doxygen/Doxyfile`{.interpreted-text role="idf_file"} should be updated with paths to `*.h` headers that are being documented.

9.  When changes are committed and documentation is built, check how this section has been rendered. `Correct annotations <../contribute/documenting-code>`{.interpreted-text role="doc"} in respective header files, if required.
::::
