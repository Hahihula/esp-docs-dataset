---
original_file_path: api-guides/cplusplus.rst
---
...
- `cplusplus_exceptions`{.interpreted-text role="ref"}
- `cplusplus_multithreading`{.interpreted-text role="ref"}
- `cplusplus_rtti`{.interpreted-text role="ref"}
- `thread-local-storage`{.interpreted-text role="doc"} (`thread_local` keyword)
- `cplusplus_filesystem`{.interpreted-text role="ref"}
- All C++ features implemented by GCC, except for some `cplusplus_limitations`{.interpreted-text role="ref"}. See [GCC documentation](https://gcc.gnu.org/projects/cxx-status.html) for details on features implemented by GCC.
...
## Multithreading {#cplusplus_multithreading}
...
See `cxx/pthread`{.interpreted-text role="example"} for an example of creating threads in C++. Specifically, this example demonstrates how to use the ESP-pthread component to modify the stack sizes, priorities, names, and core affinities of C++ threads.
...
The destructor of [std::jthread](https://en.cppreference.com/w/cpp/thread/jthread) can only safely be called from a task that has been created by `posix_thread_api`{.interpreted-text role="ref"} or by the [C++ threading library API](https://en.cppreference.com/w/cpp/thread).
::::

## Exception Handling {#cplusplus_exceptions}

Support for C++ Exceptions in ESP-IDF is disabled by default, but can be enabled using the `CONFIG_COMPILER_CXX_EXCEPTIONS`{.interpreted-text role="ref"} option.

If an exception is thrown, but there is no `catch` block, the program is terminated by the `abort` function, and the backtrace is printed. See `fatal-errors`{.interpreted-text role="doc"} for more information about backtraces.
...
See `cxx/exceptions`{.interpreted-text role="example"} for an example of C++ exception handling. Specifically, this example demonstrates how to enable and use C++ exceptions in {IDF_TARGET_NAME}, with a class that throws an exception from the constructor if the provided argument is equal to 0.
...
The amount of memory in the emergency pool can be set using the `CONFIG_COMPILER_CXX_EXCEPTIONS_EMG_POOL_SIZE`{.interpreted-text role="ref"} variable.
...
## Runtime Type Information (RTTI) {#cplusplus_rtti}

Support for RTTI in ESP-IDF is disabled by default, but can be enabled using `CONFIG_COMPILER_CXX_RTTI`{.interpreted-text role="ref"} option.
...
### Designated Initializers {#cplusplus_designated_initializers}

Many of the ESP-IDF components use `api_reference_config_structures`{.interpreted-text role="ref"} as arguments to the initialization functions. ESP-IDF examples written in C routinely use [designated initializers](https://en.cppreference.com/w/c/language/struct_initialization) to fill these structures in a readable and a maintainable way.
...
2.  By default, ESP-IDF uses a simple non-blocking implementation of the standard input stream (`stdin`). To get the usual behavior of `std::cin`, the application has to initialize the UART driver and enable the blocking mode as shown in `common_components/protocol_examples_common/stdin_out.c`{.interpreted-text role="example_file"}.

## Limitations {#cplusplus_limitations}

- Linker script generator does not support function level placements for functions with C++ linkage.
- Vtables are placed into Flash and are not accessible when the flash cache is disabled. Therefore, virtual function calls should be avoided in `iram-safe-interrupt-handlers`{.interpreted-text role="ref"}. Placement of Vtables cannot be adjusted using the linker script generator, yet.
