---
original_file_path: security/secure-boot-v2.rst
---
...
For ESP32 before chip revision v3.0, refer to `secure-boot-v1`{.interpreted-text role="doc"}. It is recommended to use Secure Boot v2 if you have a chip revision that supports it. Secure Boot v2 is safer and more flexible than Secure Boot V1.
:::

Secure Boot v2 uses {IDF_TARGET_SBV2_SCHEME} based app and `second-stage-bootloader`{.interpreted-text role="ref"} verification. This document can also be used as a reference for signing apps using the {IDF_TARGET_SBV2_SCHEME} scheme without signing the bootloader.
:::::

::: only
esp32

`Secure Boot v2` and `RSA App Signing Scheme` options are available for ESP32 from chip revision v3.0 onwards. To use these options in menuconfig, set `CONFIG_ESP32_REV_MIN`{.interpreted-text role="ref"} greater than or equal to [v3.0]{.title-ref}.
:::

::: only
esp32c3

`Secure Boot v2` is available for ESP32-C3 from chip revision v0.3 (ECO3) onwards. To use these options in menuconfig, set `CONFIG_ESP32C3_REV_MIN`{.interpreted-text role="ref"} greater than or equal to [v0.3 (ECO3)]{.title-ref}.
:::
...
This is an overview of the Secure Boot v2 Process. Instructions on how to enable Secure Boot are supplied in section `secure-boot-v2-howto`{.interpreted-text role="ref"}.
...
Each signature block contains a signature of the preceding image as well as the corresponding {IDF_TARGET_SBV2_KEY} public key. For more details about the format, refer to `signature-block-format`{.interpreted-text role="ref"}. A digest of the {IDF_TARGET_SBV2_KEY} public key is stored in the eFuse.
...
2.  The ROM code verifies the bootloader\'s signature block, see `verify_signature-block`{.interpreted-text role="ref"}. If this fails, the boot process will be aborted.
3.  The ROM code verifies the bootloader image using the raw image data, its corresponding signature block(s), and the eFuse, see `verify_image`{.interpreted-text role="ref"}. If this fails, the boot process will be aborted.
4.  The ROM code executes the bootloader.
5.  The bootloader verifies the application image\'s signature block, see `verify_signature-block`{.interpreted-text role="ref"}. If this fails, the boot process will be aborted.
6.  The bootloader verifies the application image using the raw image data, its corresponding signature blocks, and the eFuse, see `verify_image`{.interpreted-text role="ref"}. If this fails, the boot process will be aborted. If the verification fails but another application image is found, the bootloader will then try to verify that other image using steps 5 to 7. This repeats until a valid image is found or no other images are found.
...
The signature block starts on a 4 KB aligned boundary and has a flash sector of its own. The signature is calculated over all bytes in the image including the padding bytes, see `secure_padding`{.interpreted-text role="ref"}.
...
::: list
\- Default flash MMU page size is 64 KB :SOC_MMU_PAGE_SIZE_CONFIGURABLE: - {IDF_TARGET_NAME} supports configurable flash MMU page size, and `CONFIG_MMU_PAGE_SIZE` gets set based on the `CONFIG_ESPTOOLPY_FLASHSIZE`{.interpreted-text role="ref"} - Secure padding is applied through the option `--secure-pad-v2` in the `elf2image` conversion using `esptool`
:::
...
Enabling Secure Boot and/or flash encryption will increase the size of the bootloader, which might require updating the partition table offset. See `bootloader-size`{.interpreted-text role="ref"}.

When `CONFIG_SECURE_BOOT_BUILD_SIGNED_BINARIES`{.interpreted-text role="ref"} is disabled, the bootloader will use the `--pad-to-size` option in `elf2image` command of `esptool` for sector padding, with a size of 4 KB per sector.
...
- BLK2 - Stores the SHA-256 digest of the public key. SHA-256 hash of public key modulus, exponent, pre-calculated R & M\' values is written to an eFuse key block. This digest is represented as 776 bytes, with offsets of 36 to 812, as per the `signature-block-format`{.interpreted-text role="ref"}. The write-protection bit must be set, but the read-protection bit must not.
:::
...
To ensure no trusted keys can be added later by an attacker, each unused key digest slot should be revoked with KEY_REVOKEX. It will be checked during app startup in `esp_secure_boot_init_checks`{.interpreted-text role="cpp:func"} and fixed unless `CONFIG_SECURE_BOOT_ALLOW_UNUSED_DIGEST_SLOTS`{.interpreted-text role="ref"} is enabled.
:::
...
## How To Enable Secure Boot v2 {#secure-boot-v2-howto}

1.  Open the `project-configuration-menu`{.interpreted-text role="ref"}, in `Security features` set `Enable hardware Secure Boot in bootloader` to enable Secure Boot.
...
A signing key generated this way will use the best random number source available to the OS and its Python installation, which is [/dev/urandom]{.title-ref} on OSX/Linux and [CryptGenRandom()]{.title-ref} on Windows. If this random number source is weak, then the private key will be weak.
::::
...
For production environments, we recommend generating the key pair using OpenSSL or another industry-standard encryption program. See `secure-boot-v2-generate-key`{.interpreted-text role="ref"} for more details.
::::
...
When Secure Boot is enabled, it shall disable the ability to read-protect further eFuses by default. If you want keep the ability to read-protect an eFuse later in the application (e.g, a key mentioned in the above list of read-protected keys), then you need to enable the config `CONFIG_SECURE_BOOT_V2_ALLOW_EFUSE_RD_DIS`{.interpreted-text role="ref"} at the same time when you enable Secure Boot.

Ideally, it is strongly recommended that all such keys must been burned before enabling secure boot. However, if you need to enable `CONFIG_SECURE_BOOT_V2_ALLOW_EFUSE_RD_DIS`{.interpreted-text role="ref"}, make sure that you burn the eFuse {IDF_TARGET_EFUSE_WR_DIS_RD_DIS}, using `esp_efuse_write_field_bit`{.interpreted-text role="cpp:func"}, once all the read-protected eFuse keys have been programmed.
...
If `/security/flash-encryption`{.interpreted-text role="doc"} is enabled by the second stage bootloader at the time of enabling Secure Boot, it ensures that the flash encryption key generated on the first boot shall already be read-protected.
::::

## Generating Secure Boot Signing Key {#secure-boot-v2-generate-key}
...
## Remote Signing of Images {#remote-sign-v2-image}
...
To use remote signing, disable the option `CONFIG_SECURE_BOOT_BUILD_SIGNED_BINARIES`{.interpreted-text role="ref"} and build the firmware. The private signing key does not need to be present on the build system.
...
The above command appends the image signature to the existing binary. You can use the [\--output]{.title-ref} argument to write the signed binary to a separate file:
...
In such cases, the firmware image should be built by disabling the option `CONFIG_SECURE_BOOT_BUILD_SIGNED_BINARIES`{.interpreted-text role="ref"}. This image will be secure-padded and to generate a signed binary use the following command:

``` 
idf.py secure-sign-data --pub-key PUBLIC_SIGNING_KEY --signature SIGNATURE_FILE --output SIGNED_BINARY_FILE BINARY_FILE
```

The above command verifies the signature, generates a signature block (refer to `signature-block-format`{.interpreted-text role="ref"}), and appends it to the binary file.
...
In such cases, disable the option `CONFIG_SECURE_BOOT_BUILD_SIGNED_BINARIES`{.interpreted-text role="ref"} and build the firmware. This secure-padded image then can be used to supply the external HSM for generating a signature. Refer to [Signing using an External HSM](https://docs.espressif.com/projects/esptool/en/latest/{IDF_TARGET_PATH_NAME}/espsecure/index.html#remote-signing-using-an-external-hsm) to generate a signed image.
...
- Use Secure Boot in combination with `flash-encryption`{.interpreted-text role="doc"} to prevent local readout of the flash contents.
...
- Apps should only be signed with a single private key, with the others being stored securely elsewhere. However, they may be signed with multiple private keys if some are being revoked, see `secure-boot-v2-key-revocation`{.interpreted-text role="ref"} below.
...
## Key Revocation {#secure-boot-v2-key-revocation}
...
Note that enabling the config `CONFIG_SECURE_BOOT_ALLOW_UNUSED_DIGEST_SLOTS`{.interpreted-text role="ref"} only makes sure that the **app** does not revoke the unused digest slots. But if you plan to enable secure boot during the fist boot up, the bootloader will intentionally revoke the unused digest slots while enabling secure boot, even if the above config is enabled. Because keeping the unused key slots unrevoked would be a security hazard. In case for any development workflow if you need to avoid this revocation, you should `enable-secure-boot-v2-externally`{.interpreted-text role="ref"}, rather than enabling it during the boot up, so that the bootloader would not need to enable secure boot, and thus you could avoid its revocation strategy.
::::
...
7.  The API `esp_ota_revoke_secure_boot_public_key`{.interpreted-text role="cpp:func"} can be used to revoke the key #N-1.
...
It may be necessary to revoke a key that isn\'t currently being used. For example, if the active application is signed with key #0, but key #1 becomes compromised, you should revoke key #1 by using the above approach. The new OTA update should continue to be signed with key #0, and the API [esp_ota_revoke_secure_boot_public_key (SECURE_BOOT_PUBLIC_KEY_INDEX\_\[N\])]{.title-ref} can be used to revoke the key #N (N would be 1 in this case). After revoking, all remaining unrevoked keys can still be used to sign future applications.
::::

#### Aggressive Approach {#secure-boot-v2-aggressive-key-revocation}

ROM code has an additional feature of revoking a public key digest if the signature verification fails.

To enable this feature, you need to burn `SECURE_BOOT_AGGRESSIVE_REVOKE` eFuse or enable `CONFIG_SECURE_BOOT_ENABLE_AGGRESSIVE_KEY_REVOKE`{.interpreted-text role="ref"}.

Key revocation is not applicable unless Secure Boot is successfully enabled. Also, a key is not revoked in case of an invalid signature block or invalid image digest, it is only revoked in case the signature verification fails, i.e., revoke key only if failure in step 3 of `verify_image`{.interpreted-text role="ref"}.
...
## Technical Details {#secure-boot-v2-technical-details}

The following sections contain low-level reference descriptions of various Secure Boot elements.

Secure Boot is integrated into the ESP-IDF build system, so `idf.py build` will sign an app image, and `idf.py bootloader` will produce a signed bootloader if `CONFIG_SECURE_BOOT_BUILD_SIGNED_BINARIES`{.interpreted-text role="ref"} is enabled.
...
## Secure Boot & Flash Encryption {#secure-boot-v2-and-flash-encr}

If Secure Boot is used without `flash-encryption`{.interpreted-text role="doc"}, it is possible to launch a `time-of-check to time-of-use` attack, where flash contents are swapped after the image is verified and running. Therefore, it is recommended to use both features together.
...
## Signed App Verification Without Hardware Secure Boot {#signed-app-verify-v2}
...
#### How To Enable Signed App Verification {#signed-app-verify-v2-howto}

1.  Open `project-configuration-menu`{.interpreted-text role="ref"} \> `Security features`.

::: only
esp32

2.  Ensure `App Signing Scheme` is `RSA`. For the ESP32 chip revision v3.0 chip, select `CONFIG_ESP32_REV_MIN`{.interpreted-text role="ref"} to `v3.0` to get the `RSA` option available.
:::
...
3.  Enable `CONFIG_SECURE_SIGNED_APPS_NO_SECURE_BOOT`{.interpreted-text role="ref"}.
...
5.  If you disable the `Sign binaries during build` option then all app binaries must be manually signed by following instructions in `remote-sign-v2-image`{.interpreted-text role="ref"}.
...
See `jtag-debugging-security-features`{.interpreted-text role="ref"} for more information about using JTAG Debugging with either Secure Boot or signed app verification enabled.
