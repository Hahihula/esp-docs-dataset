---
original_file_path: api-guides/esp-ble-mesh/ble-mesh-architecture.rst
---
...
<img src="../../../_static/esp-ble-mesh-architecture.png" alt="Figure 1.1 ESP-BLE-MESH Architecture Diagram" />
<figcaption aria-hidden="true">Figure 1.1 ESP-BLE-MESH Architecture Diagram</figcaption>
</figure>
...
- For more features, please see `ble-mesh-feature-list`{.interpreted-text role="doc"}.
...
<figure class="align-center">
<img src="../../../_static/esp-ble-mesh-interface.png" alt="Figure 2.1 ESP-BLE-MESH Architecture Implementation Diagram" />
<figcaption aria-hidden="true">Figure 2.1 ESP-BLE-MESH Architecture Implementation Diagram</figcaption>
</figure>
...
The list of files and the functions implemented in each file in `Mesh Networking` are shown in Table 2.1:
  ...
  File                                                                                        Functionality
  ------------------------------------------------------------------------------------------- ----------------------------------------------------------------------------------------------------------------------------------------------------
  `access.c <bt/esp_ble_mesh/core/access.c>`{.interpreted-text role="component_file"}         ESP-BLE-MESH Access Layer

  `transport.c <bt/esp_ble_mesh/core/transport.c>`{.interpreted-text role="component_file"}   ESP-BLE-MESH Lower/Upper Transport Layer

  `net.c <bt/esp_ble_mesh/core/net.c>`{.interpreted-text role="component_file"}               ESP-BLE-MESH Network Layer

  `adv.c <bt/esp_ble_mesh/core/adv.c>`{.interpreted-text role="component_file"}               A task used to send ESP-BLE-MESH advertising packets, a callback used to handle received advertising packets and APIs used to allocate adv buffers
  ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Specific files that provide implementation of provisioning of Node are shown in Table 2.2:
  ...
  File                                                                                              Functionality
  ------------------------------------------------------------------------------------------------- --------------------------------------------------------
  `prov_node.c <bt/esp_ble_mesh/core/prov_node.c>`{.interpreted-text role="component_file"}         ESP-BLE-MESH Node provisioning (PB-ADV & PB-GATT)

  `proxy_server.c <bt/esp_ble_mesh/core/proxy_server.c>`{.interpreted-text role="component_file"}   ESP-BLE-MESH Proxy Server related functionalities

  `beacon.c <bt/esp_ble_mesh/core/beacon.c>`{.interpreted-text role="component_file"}               APIs used to handle ESP-BLE-MESH Beacons
  ----------------------------------------------------------------------------------------------------------------------------------------------------------
...
Specific files that implement functions of Provisioner are shown in Table 2.3:
  ...
  File                                                                                              Functionality
  ------------------------------------------------------------------------------------------------- -------------------------------------------------------------
  `prov_pvnr.c <bt/esp_ble_mesh/core/prov_pvnr.c>`{.interpreted-text role="component_file"}         ESP-BLE-MESH Provisioner provisioning (PB-ADV & PB-GATT)

  `proxy_client.c <bt/esp_ble_mesh/core/proxy_client.c>`{.interpreted-text role="component_file"}   ESP-BLE-MESH Proxy Client related functionalities

  `pvnr_mgmt.c <bt/esp_ble_mesh/core/pvnr_mgmt.c>`{.interpreted-text role="component_file"}         ESP-BLE-MESH Provisioner networking related functionalities
  ---------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Mesh Models are used to implement the specific functions of model in nodes. Server model is used to maintain node status. Client model is used to obtain and modify node state.
  ...
  File                                                                                                                 Functionality
  -------------------------------------------------------------------------------------------------------------------- ------------------------------------------------------------------------------------------
  `cfg_cli.c <bt/esp_ble_mesh/core/cfg_cli.c>`{.interpreted-text role="component_file"}                                Send Configuration Client messages and receive corresponding response messages

  `cfg_srv.c <bt/esp_ble_mesh/core/cfg_srv.c>`{.interpreted-text role="component_file"}                                Receive Configuration Client messages and send proper response messages

  `health_cli.c <bt/esp_ble_mesh/core/health_cli.c>`{.interpreted-text role="component_file"}                          Send Health Client messages and receive corresponding response messages

  `health_srv.c <bt/esp_ble_mesh/core/health_srv.c>`{.interpreted-text role="component_file"}                          Receive Health Client messages and send proper response messages

  `client_common.c <bt/esp_ble_mesh/models/client/client_common.c>`{.interpreted-text role="component_file"}           ESP-BLE-MESH model related operations

  `generic_client.c <bt/esp_ble_mesh/models/client/generic_client.c>`{.interpreted-text role="component_file"}         Send ESP-BLE-MESH Generic Client messages and receive corresponding response messages

  `lighting_client.c <bt/esp_ble_mesh/models/client/lighting_client.c>`{.interpreted-text role="component_file"}       Send ESP-BLE-MESH Lighting Client messages and receive corresponding response messages

  `sensor_client.c <bt/esp_ble_mesh/models/client/sensor_client.c>`{.interpreted-text role="component_file"}           Send ESP-BLE-MESH Sensor Client messages and receive corresponding response messages

  `time_scene_client.c <bt/esp_ble_mesh/models/client/time_scene_client.c>`{.interpreted-text role="component_file"}   Send ESP-BLE-MESH Time Scene Client messages and receive corresponding response messages

  `generic_server.c <bt/esp_ble_mesh/models/server/generic_server.c>`{.interpreted-text role="component_file"}         Receive ESP-BLE-MESH Generic Client messages and send corresponding response messages

  `lighting_server.c <bt/esp_ble_mesh/models/server/lighting_server.c>`{.interpreted-text role="component_file"}       Receive ESP-BLE-MESH Lighting Client messages and send corresponding response messages

  `sensor_server.c <bt/esp_ble_mesh/models/server/sensor_server.c>`{.interpreted-text role="component_file"}           Receive ESP-BLE-MESH Sensor Client messages and send corresponding response messages

  `time_scene_server.c <bt/esp_ble_mesh/models/server/time_scene_server.c>`{.interpreted-text role="component_file"}   Receive ESP-BLE-MESH Time Scene Client messages and send corresponding response messages
  ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Portability is fully considered in the implementation of Mesh Bearers. When the ESP-BLE-MESH protocol stack is being ported to other platforms, users only need to modify `adapter.c <bt/esp_ble_mesh/core/bluedroid_host/adapter.c>`{.interpreted-text role="component_file"} (example of `NimBLE version <bt/esp_ble_mesh/core/nimble_host/adapter.c>`{.interpreted-text role="component_file"}).

  -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  File                                                                                                   Functionality
  ------------------------------------------------------------------------------------------------------ ----------------------------------------------------------------------------------------------------------------------------------------
  `adapter.c <bt/esp_ble_mesh/core/bluedroid_host/adapter.c>`{.interpreted-text role="component_file"}   ESP-BLE-MESH Bearer Layer adapter，This file provides the interfaces used to receive and send ESP-BLE-MESH ADV & GATT related packets.
...
`adapter.c <bt/esp_ble_mesh/core/bluedroid_host/adapter.c>`{.interpreted-text role="component_file"} is the implementation of `Advertising Bearer` and `GATT Bearer` in Mesh Networking framework.
::::

### 2.3 Mesh Applications Implementation

We have provided a series of application examples for customer development, and users can develop products based on `esp-ble-mesh-examples`{.interpreted-text role="ref"}.

## 3. Auxiliary Routine

Auxiliary routine refers to optional functions in the ESP-BLE-MESH protocol stack. The design of the auxiliary routine generally implement the truncation of code through `CONFIG_BLE_MESH`{.interpreted-text role="ref"}.
...
- The functions in the module will be used repeatedly, so it is reasonable to design it into a module. Independent module is shown in Table 3.1:
  ...
  File                                                                                              Functionality
  ------------------------------------------------------------------------------------------------- -----------------------------------------------------------------------------------------------------------------------------------------------
  `lpn.c <bt/esp_ble_mesh/core/lpn.c>`{.interpreted-text role="component_file"}                     ESP-BLE-MESH Low Power functionality

  `friend.c <bt/esp_ble_mesh/core/friend.c>`{.interpreted-text role="component_file"}               ESP-BLE-MESH Friend functionality

  `net.c <bt/esp_ble_mesh/core/net.c>`{.interpreted-text role="component_file"}                     ESP-BLE-MESH Relay feature, network creation, IV Update procedure, IV Index recovery procedure, Key Refresh procedure related functionalities

  `proxy_server.c <bt/esp_ble_mesh/core/proxy_server.c>`{.interpreted-text role="component_file"}   ESP-BLE-MESH Proxy Server related functionalities

  `proxy_client.c <bt/esp_ble_mesh/core/proxy_client.c>`{.interpreted-text role="component_file"}   ESP-BLE-MESH Proxy Client related functionalities

  `settings.c <bt/esp_ble_mesh/core/storage/settings.c>`{.interpreted-text role="component_file"}   ESP-BLE-MESH NVS storage functionality

  `main.c <bt/esp_ble_mesh/core/main.c>`{.interpreted-text role="component_file"}                   ESP-BLE-MESH stack initialize, stack enable, node removal related functionalities
  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
