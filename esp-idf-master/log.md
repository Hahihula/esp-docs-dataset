---
original_file_path: api-reference/system/log.rst
---
...
ESP-IDF provides a flexible logging system with two configurable versions, **Log V1** and **Log V2**, selectable via `CONFIG_LOG_VERSION`{.interpreted-text role="ref"}. This document outlines their features, configurations, usage guidelines, and performance comparisons.
...
- Centralized formatting via a single function, `esp_log`{.interpreted-text role="cpp:func"}.
...
- Change the **maximum log level** for a source file (do not add it in header files, as it may not work due to the single-inclusion approach used by header files). Define `LOG_LOCAL_LEVEL` with one of the values from `esp_log_level_t`{.interpreted-text role="cpp:type"} before including `esp_log.h`. This allows you to control which log messages are included in the binary for that specific source file.
...
- Change the **maximum log level** for an entire component by defining `LOG_LOCAL_LEVEL` in the component\'s [CMakeLists.txt]{.title-ref}. This ensures that the specified log level is applied across all source files within the component, controlling which log messages are included in the binary:
...
By default, all log levels up to the **log level** are enabled at the startup. The function `esp_log_level_set`{.interpreted-text role="cpp:func"} can be used to set the **log level** globally or on a per-module basis. Modules are identified by their tags, which are human-readable ASCII zero-terminated strings. This functionality depends on `CONFIG_LOG_DYNAMIC_LEVEL_CONTROL`{.interpreted-text role="ref"}, which is enabled by default. If this feature is not required, you can disable it to reduce code size and improve performance.
...
Adjusting log output per module (tag) depends on `CONFIG_LOG_TAG_LEVEL_IMPL`{.interpreted-text role="ref"}, which is enabled by default. If this feature is not required, you can disable it to reduce code size and improve performance:
...
- Verbose: `ESP_LOGV`{.interpreted-text role="c:macro"}, `ESP_EARLY_LOGV`{.interpreted-text role="c:macro"}, `ESP_DRAM_LOGV`{.interpreted-text role="c:macro"}.
- Debug: `ESP_LOGD`{.interpreted-text role="c:macro"}, `ESP_EARLY_LOGD`{.interpreted-text role="c:macro"}, `ESP_DRAM_LOGD`{.interpreted-text role="c:macro"}.
- Info: `ESP_LOGI`{.interpreted-text role="c:macro"}, `ESP_EARLY_LOGI`{.interpreted-text role="c:macro"}, `ESP_DRAM_LOGI`{.interpreted-text role="c:macro"}.
- Warning: `ESP_LOGW`{.interpreted-text role="c:macro"}, `ESP_EARLY_LOGW`{.interpreted-text role="c:macro"}, `ESP_DRAM_LOGW`{.interpreted-text role="c:macro"}.
- Error: `ESP_LOGE`{.interpreted-text role="c:macro"}, `ESP_EARLY_LOGE`{.interpreted-text role="c:macro"}, `ESP_DRAM_LOGE`{.interpreted-text role="c:macro"}.

There are three groups of macros available:

- **ESP_LOGx**: Standard logging macros suitable for most use cases during normal operation. Use these in your application code for logging in non-constrained environments, avoiding use in ISRs, early startup, or when the flash cache is disabled. A key characteristic of these macros is that they use the [vprintf]{.title-ref} function from the Newlib library for formatting and outputting logs.
- **ESP_EARLY_LOGx**: Designed for use in constrained environments during early startup, before the heap allocator or syscalls are initialized. These macros are commonly used in critical startup code or in critical sections where interrupts are disabled. A key characteristic of these macros is that they use the ROM [printf]{.title-ref} function, always output timestamps in microseconds, and do not support per-module log verbosity settings.
- **ESP_DRAM_LOGx**: Designed for use in constrained environments where logging occurs with interrupts disabled or when the flash cache is inaccessible. These macros should be used sparingly, as they can impact performance. They are suitable for critical sections or interrupt routines where other logging macros may not work reliably. A key characteristic of these macros is that they use the ROM [printf]{.title-ref} function, do not output timestamps, allocate the format argument in DRAM to ensure accessibility when the cache is disabled, and do not support per-module log verbosity settings.
...
- **Color**: Adds color codes to enhance log visibility globally. Controlled by `CONFIG_LOG_COLORS`{.interpreted-text role="ref"}, which is disabled by default because the ESP-IDF monitor tool ([idf.py monitor]{.title-ref}) can detect the log level by its **level name** and apply the standard IDF color scheme.

  - For **Log V2**, the `CONFIG_LOG_COLORS_SUPPORT`{.interpreted-text role="ref"} option enables runtime support for adding color output to specific logs, files, or components, even if global color is disabled. To enable color for a specific context use `ESP_LOG_COLOR_DISABLED`.
  ...
  Please note that IDF Monitor requires the log message format described above in order to apply color highlighting automatically. The minimum required format is a log level name followed by a timestamp, and each log message must end with a newline character. For example, `I (56): Log message\n`. If this format is not followed, such as when the timestamp is disabled, automatic log coloring will not work. In these cases, it is recommended to enable `CONFIG_LOG_COLORS`{.interpreted-text role="ref"} in the menuconfig. Another limitation is that for multi-line log messages, only the first line will be colored correctly.
  ::::
...
- **Timestamp**: Adds a timestamp to log messages globally. Controlled by `CONFIG_LOG_TIMESTAMP_SOURCE`{.interpreted-text role="ref"}.
  ...
  - **Milliseconds since boot** [(18532)]{.title-ref} (default): Derived from the RTOS tick count multiplied by the tick period.
  ...
  - For **Log V2**, the `CONFIG_LOG_TIMESTAMP_SUPPORT`{.interpreted-text role="ref"} option enables runtime support for adding timestamp output to specific logs, files, or components, even if global timestamp is disabled. To enable the **Milliseconds since boot** timestamp for a specific context, use `ESP_LOG_TIMESTAMP_DISABLED`.
...
- **ESP_LOG_CONSTRAINED_ENV**:
  - Define as `1` to force the log handler `esp_log`{.interpreted-text role="cpp:func"} to use a safe printf function suitable for the specified scope.
...
- **ESP_LOG_COLOR_DISABLED**: Requires `CONFIG_LOG_COLORS_SUPPORT`{.interpreted-text role="ref"} to be enabled.
  - If global color (`CONFIG_LOG_COLORS`{.interpreted-text role="ref"}) is disabled, define as `0` to enable color output for the specified scope.
  - If global color (`CONFIG_LOG_COLORS`{.interpreted-text role="ref"}) is enabled, define as `1` to disable color output for the specified scope.
- **ESP_LOG_TIMESTAMP_DISABLED**: Requires `CONFIG_LOG_TIMESTAMP_SUPPORT`{.interpreted-text role="ref"} to be enabled.
  - If global timestamping (`CONFIG_LOG_TIMESTAMP_SOURCE`{.interpreted-text role="ref"}) is disabled, define as `0` to enable tick timestamp output for the specified scope.
  - If global timestamping (`CONFIG_LOG_TIMESTAMP_SOURCE`{.interpreted-text role="ref"}) is enabled, define as `1` to disable tick timestamp output for the specified scope.
...
The logging library allows adjusting log output per module (tag) at runtime using the function `esp_log_level_set`{.interpreted-text role="cpp:func"}. This feature applies only to non-constrained environments (**ESP_LOGx** macros). Constrained environments (e.g., **ESP_EARLY_LOGx** or **ESP_DRAM_LOGx**) do not support dynamic log levels due to the absence of locks and lightweight requirements in their log handlers.
...
- **Dynamic Log Level Control** (`CONFIG_LOG_DYNAMIC_LEVEL_CONTROL`{.interpreted-text role="ref"}, enabled by default): Enables runtime log level changes via `esp_log_level_set`{.interpreted-text role="cpp:func"}. This feature increases flexibility but adds memory and performance overhead. If binary size is a concern and dynamic log level changes are unnecessary, consider disabling this option, especially when `CONFIG_LOG_TAG_LEVEL_IMPL`{.interpreted-text role="ref"} is set to **None**, to minimize program size.
...
- **Tag-Level Checks** (`CONFIG_LOG_TAG_LEVEL_IMPL`{.interpreted-text role="ref"}, default **Cache + Linked List**): Determines how per-tag log level checks are performed, affecting memory usage and lookup speed:
  ...
  - **Cache + Linked List** (Default): It is a hybrid mode that combines caching with a **linked list** for log tag level checks. This hybrid approach offers a balance between speed and memory usage. The cache stores recently accessed log tags and their corresponding log levels, providing faster lookups for frequently used tags. The cache approach compares the tag pointers, which is faster than performing full string comparisons. For less frequently used tags, the **linked list** is utilized to search for the log level. This option may not work properly when dynamic tag definitions are used, as it relies on tag pointer comparisons in the cache, which are not suitable for dynamically defined tags. This hybrid approach improves the efficiency of log level retrieval by leveraging the speed of caching for common tags and the memory efficiency of a linked list for less frequently used tags. Selecting this option automatically enables **Dynamic Log Level Control**.
    ...
    There are some cache configurations to balance memory usage and lookup performance. These settings determine how log tag levels are stored and accessed: `CONFIG_LOG_TAG_LEVEL_CACHE_IMPL`{.interpreted-text role="ref"}.

    - **Array**: A simple implementation without reordering, suitable for low-memory applications that prioritize simplicity.
    - **Binary Min-Heap** (default): An optimized implementation for fast lookups with automatic reordering. Ideal for high-performance applications with sufficient memory. The **Cache Size** (`CONFIG_LOG_TAG_LEVEL_IMPL_CACHE_SIZE`{.interpreted-text role="ref"}) defines the capacity, which defaults to 31 entries.
...
- **Master Log Level** (`CONFIG_LOG_MASTER_LEVEL`{.interpreted-text role="ref"}, disabled by default): It is an optional setting designed for specific debugging scenarios. It enables a global \"master\" log level check that occurs before timestamps and tag cache lookups. This is useful for compiling numerous logs that can be selectively enabled or disabled at runtime while minimizing performance impact when log output is unnecessary.
  ...
  If enabled, the master log level defaults to `CONFIG_LOG_DEFAULT_LEVEL`{.interpreted-text role="ref"} and can be adjusted at runtime using `esp_log_set_level_master`{.interpreted-text role="cpp:func"}. This global check takes precedence over `esp_log_get_default_level`.

  The snippet below shows how it works. Setting the **Master log level** to `ESP_LOG_NONE` disables all logging globally. `esp_log_level_set`{.interpreted-text role="cpp:func"} does not currently affect logging. However, after the **Master log level** is adjusted to a higher level, logs will be printed as configured by `esp_log_level_set`{.interpreted-text role="cpp:func"}:
...
- `ESP_LOG_BUFFER_HEX`{.interpreted-text role="c:macro"} and `ESP_LOG_BUFFER_HEX_LEVEL`{.interpreted-text role="c:macro"}: Logs a buffer of hexadecimal bytes. The data is split into lines with 16 bytes per line. `ESP_LOG_BUFFER_HEX`{.interpreted-text role="c:macro"} is only for the `Info` log level.
...
- `ESP_LOG_BUFFER_CHAR`{.interpreted-text role="c:macro"} and `ESP_LOG_BUFFER_CHAR_LEVEL`{.interpreted-text role="c:macro"}: Logs a buffer of printable characters. Each line contains up to 16 characters. `ESP_LOG_BUFFER_CHAR`{.interpreted-text role="c:macro"} is only for the `Info` log level.
...
- `ESP_LOG_BUFFER_HEXDUMP`{.interpreted-text role="c:macro"}: Dumps a buffer in a formatted hex dump style, displaying both the memory address and corresponding ASCII values. This is especially useful for debugging raw memory content.
...
This feature introduces the `ESP_LOG_ATTR_STR`{.interpreted-text role="c:macro"} macro, which relocates format strings to a `.noload` section, effectively removing them from the final binary image. You can also use this mechanism for assertions or user-defined logging messages to further minimize flash usage.
...
1.  `Chip Side <chip-side>`{.interpreted-text role="ref"}: Encodes and transmits log data.
    - Encoding process
    - Argument type encoding
    - Runtime argument type encoding
2.  `Host Side <host-side>`{.interpreted-text role="ref"}: Receives and decodes data using the [esp-idf-monitor tool](https://github.com/espressif/esp-idf-monitor). The `idf.py monitor` command automatically decodes binary logs.
...
Since the format string is removed from the final binary, the chip must still identify argument types to correctly transmit them to the host. This is achieved using the `ESP_LOG_ARGS_TYPE`{.interpreted-text role="c:macro"} macro, which leverages the [\_Generic]{.title-ref} feature to classify user arguments at compile time into three categories: **32-bit**, **64-bit**, and **pointers**. This macro generates an **argument type array** at runtime and passes it to `esp_log` before the user arguments, ensuring that:
...
- `ESP_LOG_BUFFER_HEX_LEVEL`{.interpreted-text role="c:macro"}
- `ESP_LOG_BUFFER_CHAR_LEVEL`{.interpreted-text role="c:macro"}
- `ESP_LOG_BUFFER_HEXDUMP`{.interpreted-text role="c:macro"}
...
### Host Side (Monitor Tool) {#host-side}
...
By default, the logging library uses the vprintf-like function to write formatted output to the dedicated UART. By calling a simple API, all log output may be routed to JTAG instead, making logging several times faster. For details, please refer to Section `app_trace-logging-to-host`{.interpreted-text role="ref"}.
...
The logging library is commonly used by most ESP-IDF components and examples. For demonstration of log functionality, check ESP-IDF\'s `examples`{.interpreted-text role="idf"} directory. The most relevant examples that deal with logging are the following:

- `system/ota`{.interpreted-text role="example"}
- `storage/sd_card`{.interpreted-text role="example"}
- `protocols/https_request`{.interpreted-text role="example"}
