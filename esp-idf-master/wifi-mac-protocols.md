---
original_file_path: api-guides/wifi-driver/wifi-mac-protocols.rst
---
...
{IDF_TARGET_NAME} supports Wi-Fi bandwidth HT20 or HT40 and does not support HT20/40 coexist. `esp_wifi_set_bandwidth()`{.interpreted-text role="cpp:func"} can be used to change the default bandwidth of station or AP. The default bandwidth for {IDF_TARGET_NAME} station and AP is HT40.
...
When operating in the 2.4 GHz + 5 GHz band mode (`WIFI_BAND_MODE_AUTO`), can use the function `esp_wifi_set_bandwidths()`{.interpreted-text role="cpp:func"} to configure the bandwidth for the 2.4 GHz and 5 GHz bands separately.
::::
:::::
...
{IDF_TARGET_NAME} supports receiving and transmitting AMSDU. AMSDU TX is disabled by default, since enable AMSDU TX need more memory. Select `CONFIG_ESP_WIFI_AMSDU_TX_ENABLED`{.interpreted-text role="ref"} to enable AMSDU Tx feature, it depends on `CONFIG_SPIRAM`{.interpreted-text role="ref"}.
:::
