---
original_file_path: api-guides/tools/idf-py.rst
---
...
`Configure Your Project for Windows, <get-started-configure>`{.interpreted-text role="ref"} `Linux, or macOS <get-started-configure-linux-macos>`{.interpreted-text role="ref"} contains a brief introduction on how to set up `idf.py` to configure, build, and flash projects.
...
This command creates a new component, which will have a minimum set of files necessary for building. The `-C` option can be used to specify the directory the component will be created in. For more information about components see the `Component CMakeLists Files <component-directories>`{.interpreted-text role="ref"}.

### Select the Target Chip: `set-target` {#selecting-idf-target}
...
> - Run the main build tool ([Ninja](https://ninja-build.org) or [GNU Make]{.title-ref}). By default, the build tool is automatically detected but it can be explicitly set by passing the `-G` option to `idf.py`.
...
### Merge binaries: `merge-bin` {#merging-binaries}
...
The uf2 binary can also be generated by `idf.py uf2 <generate-uf2-binary>`{.interpreted-text role="ref"}. The `idf.py uf2` is functionally equivalent to `idf.py merge-bin -f uf2`. However, the `idf.py merge-bin` command provides more flexibility and options for merging binaries into various formats described above.
...
To enable autocompletion for `idf.py`, use the `export` command (`setting up the environment for Windows <get-started-set-up-env>`{.interpreted-text role="ref"}, `Linux or macOS <get-started-set-up-env-linux-macos>`{.interpreted-text role="ref"}). Autocompletion is initiated by pressing the TAB key. Type `idf.py -` and press the TAB key to autocomplete options.
...
### Generate a UF2 Binary: `uf2` {#generate-uf2-binary}
...
The `idf.py uf2` command is functionally equivalent to `idf.py merge-bin -f uf2` described `above <merging-binaries>`{.interpreted-text role="ref"}. However, the `idf.py merge-bin` command provides more flexibility and options for merging binaries into various formats, not only uf2.
...
This command prints the contents of the `otadata` partition which stores the information about the currently selected OTA app slot. Refer to `/api-reference/system/ota`{.interpreted-text role="doc"} for more about the `otadata` partition.
...
The `version` field represents the JSON schema version for CMake Presets. In this example it is set to `3` to match the schema supported by ESP-IDF\'s minimal supported CMake version. If you are using a newer CMake version, you can increase the `version` field accordingly---see [CMake Presets](https://cmake.org/cmake/help/latest/manual/cmake-presets.7.html).
::::
...
For a complete example, see the `Multiple Build Configurations Example <build_system/cmake/multi_config/README.md>`{.interpreted-text role="example_file"}.
...
- `global_action_callbacks`: A list of functions called once before any task execution. Each global action callback function accepts three arguments:
  ...
  > - `tasks` --- The list of tasks to be executed. Task refer to the action / sub-command used with [idf.py]{.title-ref}
