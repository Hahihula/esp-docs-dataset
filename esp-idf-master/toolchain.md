---
original_file_path: migration-guides/release-6.x/6.0/toolchain.rst
---
...
To suprress all new warnings, enable `CONFIG_COMPILER_DISABLE_GCC15_WARNINGS`{.interpreted-text role="ref"} config option.
...
Warns when a value is moved to itself with [std::move]{.title-ref}. Such a [std::move]{.title-ref} typically has no effect.
...
When building with `CONFIG_LIBC_PICOLIBC<CONFIG_LIBC_PICOLIBC>`{.interpreted-text role="ref"} enabled, the following adaptation is required.
