---
original_file_path: api-guides/partition-tables.rst
---
...
A single {IDF_TARGET_NAME}\'s flash can contain multiple apps, as well as many different kinds of data (calibration data, filesystems, parameter storage, etc). For this reason a partition table is flashed to (`default offset <CONFIG_PARTITION_TABLE_OFFSET>`{.interpreted-text role="ref"}) 0x8000 in the flash.

The partition table length is 0xC00 bytes, as we allow a maximum of 95 entries. An MD5 checksum, used for checking the integrity of the partition table at runtime, is appended after the table data. Thus, the partition table occupies an entire flash sector, which size is 0x1000 (4 KB). As a result, any partition following it must be at least located at (`default offset <CONFIG_PARTITION_TABLE_OFFSET>`{.interpreted-text role="ref"}) + 0x1000.

Each entry in the partition table has a name (label), type (app, data, or something else), subtype and the offset in flash where the partition is loaded.

The simplest way to use the partition table is to open the project configuration menu (`idf.py menuconfig`) and choose one of the simple predefined partition tables under `CONFIG_PARTITION_TABLE_TYPE`{.interpreted-text role="ref"}:
...
The `gen_esp32part.py` tool will replace each `N/A` with appropriate values based on the selected Kconfig options: {IDF_TARGET_CONFIG_BOOTLOADER_OFFSET_IN_FLASH} for the bootloader offset and `CONFIG_PARTITION_TABLE_OFFSET`{.interpreted-text role="ref"} for the partition table offset.
...
See `esp_partition_type_t`{.interpreted-text role="cpp:type"} for the enum definitions for `app` and `data` partitions.

If writing in C++ then specifying a application-defined partition type requires casting an integer to `esp_partition_type_t`{.interpreted-text role="cpp:type"} in order to use it with the `partition API<api-reference-partition-table>`{.interpreted-text role="ref"}. For example:
...
{IDF_TARGET_ESP_PHY_REF:default = \"`CONFIG_ESP_PHY_INIT_DATA_IN_PARTITION`{.interpreted-text role="ref"}\", esp32p4, esp32c5, esp32c61=\"NOT UPDATED YET\"}
...
See enum `esp_partition_subtype_t`{.interpreted-text role="cpp:type"} for the full list of subtypes defined by ESP-IDF, including the following:
...
- When type is `app`, the SubType field can be specified as `factory` (0x00), `ota_0` (0x10) through `ota_15` (0x1F), or `test` (0x20). Additionally, if `ESP-TEE <../security/tee/tee>`{.interpreted-text role="doc"} functionality is enabled, two TEE-specific subtypes become available: `tee_0` (0x30) and `tee_1` (0x31).
...
- When type is `bootloader`, the SubType field can be specified as:
  ...
  >
  > The size of the bootloader type is calculated by the `gen_esp32part.py` tool based on the specified `--offset` (the partition table offset) and `--primary-partition-offset` arguments. Specifically, the bootloader size is defined as (`CONFIG_PARTITION_TABLE_OFFSET`{.interpreted-text role="ref"} - {IDF_TARGET_CONFIG_BOOTLOADER_OFFSET_IN_FLASH}). This calculated size applies to all subtypes of the bootloader.

- When type is `partition_table`, the SubType field can be specified as:

  > - `primary` (0x00). This is the primary partition table, located at the `CONFIG_PARTITION_TABLE_OFFSET`{.interpreted-text role="ref"} address in flash memory. The tool automatically determines the appropriate size and offset for this subtype, so any size or offset specified for this subtype will be ignored. You can either leave these fields blank or use `N/A` as a placeholder.
...
- When type is `data`, the subtype field can be specified as `ota` (0x00), `phy` (0x01), `nvs` (0x02), nvs_keys (0x04), or a range of other component-specific subtypes (see `subtype enum <esp_partition_subtype_t>`{.interpreted-text role="cpp:type"}).

  > - `ota` (0) is the `OTA data partition <ota_data_partition>`{.interpreted-text role="ref"} which stores information about the currently selected OTA app slot. This partition should be 0x2000 bytes in size. Refer to the `OTA documentation <ota_data_partition>`{.interpreted-text role="ref"} for more details.
  >
  ...
  >
  > - `nvs` (2) is for the `Non-Volatile Storage (NVS) API <../api-reference/storage/nvs_flash>`{.interpreted-text role="doc"}.
  >
  ...
  >   > SOC_WIFI_SUPPORTED
  >   >
  >   > - NVS is used to store Wi-Fi data if the `esp_wifi_set_storage(WIFI_STORAGE_FLASH) <../api-reference/network/esp_wifi>`{.interpreted-text role="doc"} initialization function is used.
  >   > :::
  >   >
  >   > - The NVS API can also be used for other application data.
  >   > - It is strongly recommended that you include an NVS partition of at least 0x3000 bytes in your project.
  >   > - If using NVS API to store a lot of data, increase the NVS partition size from the default 0x6000 bytes.
  >   > - When NVS is used to store factory settings, it is recommended to keep these settings in a separate read-only NVS partition. The minimal size of a read-only NVS partition is 0x1000 bytes. See `read-only-nvs`{.interpreted-text role="ref"} for more details. ESP-IDF provides `NVS Partition Generator Utility </api-reference/storage/nvs_partition_gen>`{.interpreted-text role="doc"} to generate NVS partitions with factory settings and to flash them along with the application.
  >
  > - `nvs_keys` (4) is for the NVS key partition. See `Non-Volatile Storage (NVS) API <../api-reference/storage/nvs_flash>`{.interpreted-text role="doc"} for more details.
  >
  >   > - It is used to store NVS encryption keys when [NVS Encryption]{.title-ref} feature is enabled.
  >   > - The size of this partition should be 4096 bytes (minimum partition size).
  >
  > ::: only
  > esp32c6
  >
  > - `tee_ota` (0x90) is the `TEE OTA data partition <tee-ota-data-partition>`{.interpreted-text role="ref"} which stores information about the currently selected TEE OTA app slot. This partition should be 0x2000 bytes in size. Refer to the `TEE OTA documentation <../security/tee/tee-ota>`{.interpreted-text role="doc"} for more details.
  > :::
  >
  > - There are other predefined data subtypes for data storage supported by ESP-IDF. These include:
  >
  >   > - `coredump` (0x03) is for storing core dumps while using a custom partition table CSV file. See `/api-guides/core_dump`{.interpreted-text role="doc"} for more details.
  >   > - `efuse` (0x05) is for emulating eFuse bits using `virtual-efuses`{.interpreted-text role="ref"}.
  ...
  >   > - `fat` (0x81) is for `/api-reference/storage/fatfs`{.interpreted-text role="doc"}.
  >   > - `spiffs` (0x82) is for `/api-reference/storage/spiffs`{.interpreted-text role="doc"}.
  >   > - `littlefs` (0x83) is for [LittleFS filesystem](https://github.com/littlefs-project/littlefs). See `storage/littlefs`{.interpreted-text role="example"} example for more details.
...
- If the partition type is any application-defined value (range 0x40-0xFE), then `subtype` field can be any value chosen by the application (range 0x00-0xFE).
  ...
  > Note that when writing in C++, an application-defined subtype value requires casting to type `esp_partition_subtype_t`{.interpreted-text role="cpp:type"} in order to use it with the `partition API <api-reference-partition-table>`{.interpreted-text role="ref"}.

### Extra Partition SubTypes

A component can define a new partition subtype by setting the `EXTRA_PARTITION_SUBTYPES` property. This property is a CMake list, each entry of which is a comma separated string with `<type>, <subtype>, <value>` format. The build system uses this property to add extra subtypes and creates fields named `ESP_PARTITION_SUBTYPE_<type>_<subtype>` in `esp_partition_subtype_t`{.interpreted-text role="cpp:type"}. The project can use this subtype to define partitions in the partitions table CSV file and use the new fields in `esp_partition_subtype_t`{.interpreted-text role="cpp:type"}.

### Offset & Size {#partition-offset-and-size}
...
If you want the partitions in the partition table to work relative to any placement (`CONFIG_PARTITION_TABLE_OFFSET`{.interpreted-text role="ref"}) of the table itself, leave the offset field (in CSV file) for all partitions blank. Similarly, if changing the partition table offset, then be aware that all blank partition offsets may change to match, and that any fixed offsets may now collide with the partition table (causing an error).
::::
...
> - If `encrypted` flag is set, the partition will be encrypted if `/security/flash-encryption`{.interpreted-text role="doc"} is enabled.
>
> ::::: note
> ::: title
> Note
...
The partition table which is flashed to the {IDF_TARGET_NAME} is in a binary format, not CSV. The tool `partition_table/gen_esp32part.py`{.interpreted-text role="component_file"} is used to convert between CSV and binary formats.
...
- Bootloader binary must fit in space before partition table (see `bootloader-size`{.interpreted-text role="ref"}).
...
The MD5 checksum generation can be disabled by the `--disable-md5sum` option of `gen_esp32part.py` or by the `CONFIG_PARTITION_TABLE_MD5`{.interpreted-text role="ref"} option. This is useful for example when one `uses a bootloader from ESP-IDF before v3.1 <CONFIG_APP_COMPATIBLE_PRE_V3_1_BOOTLOADERS>`{.interpreted-text role="ref"} which cannot process MD5 checksums and the boot fails with the error message `invalid magic number 0xebeb`.
:::

::: only
not esp32

The MD5 checksum generation can be disabled by the `--disable-md5sum` option of `gen_esp32part.py` or by the `CONFIG_PARTITION_TABLE_MD5`{.interpreted-text role="ref"} option.
:::
...
The component [partition_table]{.title-ref} provides a tool `parttool.py <partition_table/parttool.py>`{.interpreted-text role="component_file"} for performing partition-related operations on a target device. The following operations can be performed using the tool:
...
Before anything else, make sure that the [parttool]{.title-ref} module is imported.
...
The starting point for using the tool\'s Python API to do is create a [ParttoolTarget]{.title-ref} object:
...
The partition to operate on is specified using [PartitionName]{.title-ref} or [PartitionType]{.title-ref} or PARTITION_BOOT_DEFAULT. As the name implies, these can be used to refer to partitions of a particular name, type-subtype combination, or the default boot partition.

More information on the Python API is available in the docstrings for the tool.

### Command-line Interface

The command-line interface of [parttool.py]{.title-ref} has the following structure:
...
More information can be obtained by specifying [\--help]{.title-ref} as argument:
