---
original_file_path: api-guides/thread-local-storage.rst
---
...
- `freertos-native`{.interpreted-text role="ref"}: ESP-IDF FreeRTOS native APIs.
- `pthread-api`{.interpreted-text role="ref"}: ESP-IDF pthread APIs.
- `c11-std`{.interpreted-text role="ref"}: C11 standard introduces special keywords to declare variables as thread local.

## FreeRTOS Native APIs {#freertos-native}

The ESP-IDF FreeRTOS provides the following APIs to manage thread local variables:

- `vTaskSetThreadLocalStoragePointer`{.interpreted-text role="cpp:func"}
- `pvTaskGetThreadLocalStoragePointer`{.interpreted-text role="cpp:func"}
- `vTaskSetThreadLocalStoragePointerAndDelCallback`{.interpreted-text role="cpp:func"}

In this case, the maximum number of variables that can be allocated is limited by `CONFIG_FREERTOS_THREAD_LOCAL_STORAGE_POINTERS`{.interpreted-text role="ref"}. Variables are kept in the task control block (TCB) and accessed by their index. Note that index 0 is reserved for ESP-IDF internal uses.
...
## Pthread APIs {#pthread-api}

The ESP-IDF provides the following `/api-reference/system/pthread`{.interpreted-text role="doc"} to manage thread local variables:

- `pthread_key_create`{.interpreted-text role="cpp:func"}
- `pthread_key_delete`{.interpreted-text role="cpp:func"}
- `pthread_getspecific`{.interpreted-text role="cpp:func"}
- `pthread_setspecific`{.interpreted-text role="cpp:func"}
