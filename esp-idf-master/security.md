---
original_file_path: security/security.rst
---
...
It is possible to try out the security features for {IDF_TARGET_NAME} target SoC under `../api-guides/tools/qemu`{.interpreted-text role="doc"} virtually. Once the security workflow is established, you can then proceed to the real hardware.
::::
:::::
...
### Secure Boot {#secure_boot-guide}

The Secure Boot feature ensures that only authenticated software can execute on the device. The Secure Boot process forms a chain of trust by verifying all **mutable** software entities involved in the `../api-guides/startup`{.interpreted-text role="doc"}. Signature verification happens during both boot-up as well as in OTA updates.

Please refer to `secure-boot-v2`{.interpreted-text role="doc"} for detailed documentation about this feature.

::: only
esp32

For ESP32 before ECO3, please refer to `secure-boot-v1`{.interpreted-text role="doc"}.
:::
...
### Flash Encryption {#flash_enc-guide}
...
Please refer to `flash-encryption`{.interpreted-text role="doc"} for detailed information about this feature.
...
- It is recommended to use `flash-enc-release-mode`{.interpreted-text role="ref"} for the production use-cases.
- It is recommended to have a unique flash encryption key per device.
- Enable `secure_boot-guide`{.interpreted-text role="ref"} as an extra layer of protection, and to prevent an attacker from selectively corrupting any part of the flash before boot.
...
Please refer to the `../api-reference/peripherals/ds`{.interpreted-text role="doc"} for detailed documentation.
:::

::: only
SOC_ECDSA_SUPPORTED

Please refer to the `../api-reference/peripherals/ecdsa`{.interpreted-text role="doc"} and `../api-reference/peripherals/ds`{.interpreted-text role="doc"} guides for detailed documentation.
:::
::::::
...
This feature depends on the config option `CONFIG_ESP_SYSTEM_MEMPROT`{.interpreted-text role="ref"} and it is kept enabled by default. Please note that the API for this feature is **private** and used exclusively by ESP-IDF code only.
...
{IDF_TARGET_NAME} has support for protection mechanisms against the Differential Power Analysis related security attacks. DPA protection dynamically adjusts the clock frequency of the crypto peripherals, thereby blurring the power consumption trajectory during its operation. Based on the configured DPA security level, the clock variation range changes. Please refer to the *{IDF_TARGET_NAME} Technical Reference Manual* \[[PDF]({IDF_TARGET_TRM_EN_URL})\]. for more details on this topic.

`CONFIG_ESP_CRYPTO_DPA_PROTECTION_LEVEL`{.interpreted-text role="ref"} can help to select the DPA level. Higher level means better security, but it can also have an associated performance impact. By default, the lowest DPA level is kept enabled but it can be modified based on the security requirement.
...
Please note that hardware `RNG <../api-reference/system/random>`{.interpreted-text role="doc"} must be enabled for DPA protection to work correctly.
::::
:::::
...
`CONFIG_MBEDTLS_AES_USE_PSEUDO_ROUND_FUNC_STRENGTH`{.interpreted-text role="ref"} can be used to select the strength of the pseudo-round function. Increasing the strength improves the security provided, but would slow down the encrryption/decryption operations.
...
::: list
- JTAG interface stays disabled if any of the security features are enabled. Please refer to `jtag-debugging-security-features`{.interpreted-text role="ref"} for more information.

\- JTAG interface can also be disabled in the absence of any other security features using `efuse_API`{.interpreted-text role="ref"}. :SOC_HMAC_SUPPORTED: - {IDF_TARGET_NAME} supports soft disabling the JTAG interface and it can be re-enabled by programming a secret key through HMAC. (`hmac_for_enabling_jtag`{.interpreted-text role="ref"})
:::
...
For ESP32 ECO3 case, UART Download mode stays disabled if any of the security features are enabled in their release configuration. Alternatively, it can also be disabled by calling `esp_efuse_disable_rom_download_mode`{.interpreted-text role="cpp:func"} at runtime.
...
- Secure UART Download mode can also be enabled by calling `esp_efuse_enable_rom_secure_download_mode`{.interpreted-text role="cpp:func"}.
...
- To disable Download Mode entirely, select the `CONFIG_SECURE_UART_ROM_DL_MODE`{.interpreted-text role="ref"} to the recommended option `Permanently disable ROM Download Mode` or call `esp_efuse_disable_rom_download_mode`{.interpreted-text role="cpp:func"} at runtime.
...
In addition to the traditional security methods (WEP/WPA-TKIP/WPA2-CCMP), Wi-Fi driver in ESP-IDF also supports additional state-of-the-art security protocols. Please refer to the `../api-guides/wifi-security`{.interpreted-text role="doc"} for detailed documentation.

### TLS (Transport Layer Security)

It is recommended to use TLS (Transport Layer Security) in all external communications (e.g., cloud communication, OTA updates) from the ESP device. ESP-IDF supports `../api-reference/protocols/mbedtls`{.interpreted-text role="doc"} as the official TLS stack.

TLS is default integrated in `../api-reference/protocols/esp_http_client`{.interpreted-text role="doc"}, `../api-reference/protocols/esp_https_server`{.interpreted-text role="doc"} and several other components that ship with ESP-IDF.
...
ESP-IDF provides an abstraction layer for the most-used TLS functionalities. Hence, it is recommended that an application uses the API exposed by `../api-reference/protocols/esp_tls`{.interpreted-text role="doc"}.

`esp_tls_server_verification`{.interpreted-text role="ref"} section highlights diverse ways in which the identity of server could be established on the device side.

#### ESP Certificate Bundle

The `../api-reference/protocols/esp_crt_bundle`{.interpreted-text role="doc"} API provides an easy way to include a bundle of custom x509 root certificates for TLS server verification. The certificate bundle is the easiest way to verify the identity of almost all standard TLS servers.
...
Root Certificates embedded inside the application must be managed carefully. Any update to the root certificate list or the `../api-reference/protocols/esp_crt_bundle`{.interpreted-text role="doc"} can have an impact on the TLS connection with the remote endpoint. This includes a connection to the OTA update server. In some cases, the problem shall be visible on the next OTA update and it may leave device unable to perform OTA updates forever.
...
- Please consider enabling `OTA rollback <ota_rollback>`{.interpreted-text role="ref"} and then keep the successful connection to the OTA update server as the checkpoint to cancel the rollback process. This ensures that the newly updated firmware can successfully reach till the OTA update server, otherwise rollback process will go back to the previous firmware on the device.
- If you plan to enable the `CONFIG_MBEDTLS_HAVE_TIME_DATE`{.interpreted-text role="ref"} option, then please consider to have the time sync mechanism (SNTP) and sufficient number of trusted certificates in place.
:::::::
...
ESP-IDF provides various security schemes to establish a secure session between ESP and the provisioning entity, they are highlighted at `provisioning_security_schemes`{.interpreted-text role="ref"}.
...
- ESP-IDF provides a simplified abstraction layer `../api-reference/system/esp_https_ota`{.interpreted-text role="doc"} for this.
- If `secure_boot-guide`{.interpreted-text role="ref"} is enabled, then the server should host the signed application image.
- If `flash_enc-guide`{.interpreted-text role="ref"} is enabled, then no additional steps are required on the server side, encryption shall be taken care on the device itself during flash write.
- OTA update `ota_rollback`{.interpreted-text role="ref"} can help to switch the application as `active` only after its functionality has been verified.
...
Please see more information to enable this feature in the `anti-rollback`{.interpreted-text role="ref"} guide.
...
Please see working example for this documented in `ota_updates_pre-encrypted-firmware`{.interpreted-text role="ref"} section.
...
ESP-IDF provides the **NVS (Non-volatile Storage)** management component which allows encrypted data partitions. This feature is tied with the platform `flash_enc-guide`{.interpreted-text role="ref"} feature described earlier.

Please refer to the `nvs_encryption`{.interpreted-text role="ref"} for detailed documentation on the working and instructions to enable this feature.
...
Please refer to the `../api-reference/protocols/esp_local_ctrl`{.interpreted-text role="doc"} for detailed documentation about this feature.
...
[^1]: The above performance numbers have been calculated using the AES performance test of the mbedtls test application `test_aes_perf.c <mbedtls/test_apps/main/test_aes_perf.c>`{.interpreted-text role="component_file"}.
