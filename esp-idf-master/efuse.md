---
original_file_path: api-reference/system/efuse.rst
---
...
For more details, see *{IDF_TARGET_NAME} Technical Reference Manual* \> *eFuse Controller (eFuse)* \[[PDF]({IDF_TARGET_TRM_EN_URL}#efuse)\].
...
- `field_name`
  ...
  > - Using `.` will define a child eFuse field. See `structured-efuse-fields`{.interpreted-text role="ref"} for more details.
...
- `bit_count`
  ...
  >   > - Depending on the coding scheme selected via `CONFIG_EFUSE_CODE_SCHEME_SELECTOR`{.interpreted-text role="ref"}, `MAX_BLK_LEN` could be 256 (\"None\"), 192 (\"3/4\"), or 128 (\"REPEAT\").
  >   > :::
...
The `efuse_table_gen.py` tool is designed to generate C source files containing C structures (of type `esp_efuse_desc_t`{.interpreted-text role="cpp:type"}) representing the eFuse fields defined in CSV files. Moreover, the tool also runs some checks on the provided CSV files before generation to ensure that:
...
When using a coding scheme, the length of the payload that can be written is limited. For more details, Please Refer to *{IDF_TARGET_NAME} Technical Reference Manual* \> *Chapter 20 eFuse Controller* \[[PDF]({IDF_TARGET_TRM_EN_URL}#efuse)\] \> *Section 20.3.1.3 System Parameter coding_scheme*.
...
- calling the function `esp_efuse_get_coding_scheme`{.interpreted-text role="cpp:func"} in the application for the EFUSE_BLK3 block.

The eFuse fields specified in the CSV files must always comply with the eFuse coding scheme used by the chip. The `CONFIG_EFUSE_CODE_SCHEME_SELECTOR`{.interpreted-text role="ref"} option selects which coding scheme is used by the CSV files. When generating source files, if the records in the CSV files do not adhere to the coding scheme, an error message will be displayed. In that case, you must adjust the `bit_start` and `bit_count` of the records to comply with the limitations of the selected coding scheme.
...
1.  Enable batch writing mode by calling `esp_efuse_batch_write_begin`{.interpreted-text role="cpp:func"}
2.  Write to the eFuse fields as usual using various `esp_efuse_write_...` functions.
3.  Once all writes are complete, call `esp_efuse_batch_write_commit`{.interpreted-text role="cpp:func"} which burns prepared data to the eFuse blocks.
...
- `esp_efuse_read_field_blob`{.interpreted-text role="cpp:func"} - returns an array of read eFuse bits.
- `esp_efuse_read_field_cnt`{.interpreted-text role="cpp:func"} - returns the number of bits programmed as \"1\".
- `esp_efuse_write_field_blob`{.interpreted-text role="cpp:func"} - writes an array.
- `esp_efuse_write_field_cnt`{.interpreted-text role="cpp:func"} - writes a required count of bits as \"1\".
- `esp_efuse_get_field_size`{.interpreted-text role="cpp:func"} - returns the number of bits by the field name.
- `esp_efuse_read_reg`{.interpreted-text role="cpp:func"} - returns value of eFuse register.
- `esp_efuse_write_reg`{.interpreted-text role="cpp:func"} - writes value to eFuse register.
- `esp_efuse_get_coding_scheme`{.interpreted-text role="cpp:func"} - returns eFuse coding scheme for blocks.
- `esp_efuse_read_block`{.interpreted-text role="cpp:func"} - reads a key from an eFuse block starting at the offset with required size.
- `esp_efuse_write_block`{.interpreted-text role="cpp:func"} - writes a key to an eFuse block starting at the offset with required size.
- `esp_efuse_batch_write_begin`{.interpreted-text role="cpp:func"} - set the batch mode of writing fields.
- `esp_efuse_batch_write_commit`{.interpreted-text role="cpp:func"} - writes all prepared data for batch writing mode and reset the batch writing mode.
- `esp_efuse_batch_write_cancel`{.interpreted-text role="cpp:func"} - reset the batch writing mode and prepared data.
- `esp_efuse_get_key_dis_read`{.interpreted-text role="cpp:func"} - Returns a read protection for the key block.
- `esp_efuse_set_key_dis_read`{.interpreted-text role="cpp:func"} - Sets a read protection for the key block.
- `esp_efuse_get_key_dis_write`{.interpreted-text role="cpp:func"} - Returns a write protection for the key block.
- `esp_efuse_set_key_dis_write`{.interpreted-text role="cpp:func"} - Sets a write protection for the key block.
- `esp_efuse_get_key_purpose`{.interpreted-text role="cpp:func"} - Returns the current purpose set for an eFuse key block.
- `esp_efuse_write_key`{.interpreted-text role="cpp:func"} - Programs a block of key data to an eFuse block.
- `esp_efuse_write_keys`{.interpreted-text role="cpp:func"} - Programs keys to unused eFuse blocks.
- `esp_efuse_find_purpose`{.interpreted-text role="cpp:func"} - Finds a key block with the particular purpose set.
- `esp_efuse_get_keypurpose_dis_write`{.interpreted-text role="cpp:func"} - Returns a write protection of the key purpose field for an eFuse key block (for esp32 always true).
- `esp_efuse_key_block_unused`{.interpreted-text role="cpp:func"} - Returns true if the key block is unused, false otherwise.
- `esp_efuse_destroy_block`{.interpreted-text role="cpp:func"} - Destroys the data in this eFuse block. There are two things to do: (1) if write protection is not set, then the remaining unset bits are burned, (2) set read protection for this block if it is not locked.

For frequently used fields, special functions are made, like this `esp_efuse_get_pkg_ver`{.interpreted-text role="cpp:func"}.
...
EFUSE_BLK_KEY0 - EFUSE_BLK_KEY5 are intended to keep up to 6 keys with a length of 256-bits. Each key has an `ESP_EFUSE_KEY_PURPOSE_x` field which defines the purpose of these keys. The purpose field is described in `esp_efuse_purpose_t`{.interpreted-text role="cpp:type"}.
...
- `esp_efuse_get_purpose_field`{.interpreted-text role="cpp:func"} - Returns a pointer to a key purpose for an eFuse key block.
- `esp_efuse_get_key`{.interpreted-text role="cpp:func"} - Returns a pointer to a key block.
- `esp_efuse_set_key_purpose`{.interpreted-text role="cpp:func"} - Sets a key purpose for an eFuse key block.
- `esp_efuse_set_keypurpose_dis_write`{.interpreted-text role="cpp:func"} - Sets a write protection of the key purpose field for an eFuse key block.
- `esp_efuse_find_unused_key_block`{.interpreted-text role="cpp:func"} - Search for an unused key block and return the first one found.
- `esp_efuse_count_unused_key_blocks`{.interpreted-text role="cpp:func"} - Returns the number of unused eFuse key blocks in the range EFUSE_BLK_KEY0 to EFUSE_BLK_KEY_MAX
:::

::: only
SOC_SUPPORT_SECURE_BOOT_REVOKE_KEY

- `esp_efuse_get_digest_revoke`{.interpreted-text role="cpp:func"} - Returns the status of the Secure Boot public key digest revocation bit.
- `esp_efuse_set_digest_revoke`{.interpreted-text role="cpp:func"} - Sets the Secure Boot public key digest revocation bit.
- `esp_efuse_get_write_protect_of_digest_revoke`{.interpreted-text role="cpp:func"} - Returns a write protection of the Secure Boot public key digest revocation bit.
- `esp_efuse_set_write_protect_of_digest_revoke`{.interpreted-text role="cpp:func"} - Sets a write protection of the Secure Boot public key digest revocation bit.
:::
:::::
...
To add child fields to an existing field, `structured-efuse-fields`{.interpreted-text role="ref"} can be used. The following example demonstrates adding of the the fields `SERIAL_NUMBER`, `MODEL_NUMBER` and `HARDWARE_REV` to an existing `USER_DATA` field by using the `.` operator:
...
You may get errors such as `intersects with` or `out of range`. Please see how to solve them in the `structured-efuse-fields`{.interpreted-text role="ref"} article.
...
These functions can be used from a top-level project `CMakeLists.txt` (`system/efuse/CMakeLists.txt`{.interpreted-text role="example_file"}):
...
There is an example test `system/efuse/CMakeLists.txt`{.interpreted-text role="example_file"} which adds a custom target `efuse-filter`. This allows you to run the `idf.py efuse-filter` command to read the required eFuses (specified in the `efuse_names` list) at any time, not just during the project build.

## Debug eFuse & Unit Tests

### Virtual eFuses

The Kconfig option `CONFIG_EFUSE_VIRTUAL`{.interpreted-text role="ref"} virtualizes eFuse values inside the eFuse Manager, so writes are emulated and no eFuse values are permanently changed. This can be useful for debugging and unit testing.
...
In addition to the `CONFIG_EFUSE_VIRTUAL`{.interpreted-text role="ref"} option, there is the `CONFIG_EFUSE_VIRTUAL_KEEP_IN_FLASH`{.interpreted-text role="ref"} option that adds a feature to keep eFuses in flash memory. To use this mode, the partition_table should have include an `efuse` partition in `partition.csv`:
...
Flash encryption is a hardware feature that requires the physical burning of eFuses `key` and `FLASH_CRYPT_CNT`. If flash encryption is not actually enabled, then enabling the `CONFIG_EFUSE_VIRTUAL_KEEP_IN_FLASH`{.interpreted-text role="ref"} option just provides testing possibilities and does not encrypt anything in the flash, even though the logs indicates that encryption happens.

The `bootloader_flash_write`{.interpreted-text role="cpp:func"} is adapted for this purpose. But if flash encryption is already enabled on the chip when the application is run, or if the bootloader is created with the `CONFIG_EFUSE_VIRTUAL_KEEP_IN_FLASH`{.interpreted-text role="ref"} option, then the flash encryption/decryption operations will work properly. This means that data are encrypted as it is written into an encrypted flash partition and decrypted when they are read from an encrypted partition.

### `espefuse`

esptool includes a useful tool for reading/writing {IDF_TARGET_NAME} eFuse bits - [espefuse](https://docs.espressif.com/projects/esptool/en/latest/{IDF_TARGET_PATH_NAME}/espefuse/index.html).
...
- `system/efuse`{.interpreted-text role="example"} demonstrates how to use the eFuse API on {IDF_TARGET_NAME}, showing read and write operations with fields from the common and custom eFuse tables, and explaining the use of virtual eFuses for debugging purposes.
