---
original_file_path: api-guides/deep-sleep-stub.rst
---
...
The wake stub in esp-idf is realized by the function `esp_wake_deep_sleep()`{.interpreted-text role="cpp:func"}. This function is executed whenever the SoC wakes from Deep-sleep. As this function is weakly-linked to the default function `esp_default_wake_deep_sleep()`{.interpreted-text role="cpp:func"}, if your application contains a function with the name `esp_wake_deep_sleep()`, the default version `esp_default_wake_deep_sleep()`{.interpreted-text role="cpp:func"} in esp-idf will be overridden.

Please note that implementing the function `esp_wake_deep_sleep()`{.interpreted-text role="cpp:func"} in your application is not mandatory for utilizing the Deep-sleep functionality. It becomes necessary only if you want to introduce certain behavior immediately upon the SoC\'s wake-up.

When you develop a customized wake stub, the first step it should do is to call the default function `esp_default_wake_deep_sleep()`{.interpreted-text role="cpp:func"}.

In addition, you can switch between different wake stubs by calling the function `esp_set_deep_sleep_wake_stub()`{.interpreted-text role="cpp:func"} during runtime.
...
- Employ the attribute `RTC_IRAM_ATTR` to place `esp_wake_deep_sleep()`{.interpreted-text role="cpp:func"} into RTC fast memory:
...
- Place the function `esp_wake_deep_sleep()`{.interpreted-text role="cpp:func"} into any source file with name starting with `rtc_wake_stub`. For files with such names `rtc_wake_stub*`, their contents can be automatically put into RTC fast memory by the linker.
...
The RTC memory area where the data will be placed can be configured via the menuconfig option `CONFIG_{IDF_TARGET_CFG_PREFIX}_RTCDATA_IN_FAST_MEM`{.interpreted-text role="ref"}. This option allows keeping slow memory area for ULP programs. Once it is enabled, the data marked with `RTC_DATA_ATTR` and `RTC_RODATA_ATTR` are placed in the RTC fast memory segment; otherwise, it goes to RTC slow memory (the default option). This option depends on the `CONFIG_FREERTOS_UNICORE`{.interpreted-text role="ref"} option because RTC fast memory can be accessed only by PRO_CPU.

::: only
esp32

This option depends on the `CONFIG_FREERTOS_UNICORE`{.interpreted-text role="ref"} because RTC fast memory can be accessed only by PRO_CPU.
...
- Place the data into any source file with name starting with `rtc_wake_stub`, as demonstrated in the example source file `system/deep_sleep_wake_stub/main/rtc_wake_stub_example.c`{.interpreted-text role="example_file"}.
...
You can enable the Kconfig option `CONFIG_BOOTLOADER_SKIP_VALIDATE_IN_DEEP_SLEEP`{.interpreted-text role="ref"} to reduce wake-up time. See more information in `Fast boot from Deep-sleep <bootloader>`{.interpreted-text role="doc"}.

All of the above functions are declared in `esp_hw_support/include/esp_sleep.h`{.interpreted-text role="component_file"}.
...
- `system/deep_sleep_wake_stub`{.interpreted-text role="example"} demonstrates how to use the Deep-sleep wake stub on {IDF_TARGET_NAME} to quickly perform some tasks (the wake stub code) immediately after wake-up before going back to sleep.
:::
...
This method uses the CPU\'s internal cycle counter to estimate the wake-up time. At the beginning of the stub (with the function type of [esp_deep_sleep_wake_stub_fn_t]{.title-ref}), the current CPU cycle count is read and converted into time based on the running CPU frequency.

Reference example: `system/deep_sleep_wake_stub`{.interpreted-text role="example"}.
...
<img src="../../_static/deep-sleep-stub-logic-analyzer-result.png" style="width:100.0%" alt="Time from Wake-up to Stub Execution" />
<figcaption aria-hidden="true">Time from Wake-up to Stub Execution</figcaption>
</figure>
