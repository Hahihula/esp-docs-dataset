---
original_file_path: api-reference/peripherals/sdio_slave.rst
---
...
Please check if CMD and DATA lines DAT0-DAT3 of the card are properly pulled up by 10 KOhm - 90 KOhm resistors, which should be ensured even in 1-bit mode or SPI mode. Most official modules do not offer these pullups internally. If you are using official development boards, check `compatibility_overview_espressif_hw_sdio`{.interpreted-text role="ref"} to see whether your development boards have such pullups.
::::
...
Most official modules have conflicts on strapping pins with the SDIO slave function. If you are using an ESP32 module with 3.3 V flash inside, when you are developing on the module for the first time, you will need to perform an eFuse burn-in prior to development. This will adjust the pin configuration of the module to make the module compatible with SDIO functionality. See `compatibility_overview_espressif_hw_sdio`{.interpreted-text role="ref"} for details on how to configure this.

> Here is a list of modules/kits with 3.3 V flash:
>
> - Modules: All modules except ESP32-WROVER, ESP32-WROVER-I, ESP32-S3-WROOM-2, and please check [Modules Overview](https://www.espressif.com/en/products/modules) for module list
...
Refer to `sd_pullup_requirements`{.interpreted-text role="doc"} for more technical details of the pullups.
...
- Interrupts: The {IDF_TARGET_NAME} SDIO slave supports interrupts in two directions: from host to slave (referred to as slave interrupts) and from slave to host (referred to as host interrupts). For more details, refer to `interrupts`{.interpreted-text role="ref"}.
...
There is also a component [ESSL](https://components.espressif.com/components/espressif/esp_serial_slave_link) designed for {IDF_TARGET_NAME} master to communicate with {IDF_TARGET_NAME} SDIO slave. See example `peripherals/sdio`{.interpreted-text role="example"} when programming your host.
...
>
>     For more about this, see `peripherals/sdio`{.interpreted-text role="example"}.
...
- `peripherals/sdio/host`{.interpreted-text role="example"} and `peripherals/sdio/slave`{.interpreted-text role="example"} demonstrate how to use a host to communicate with an ESP SDIO slave device.
