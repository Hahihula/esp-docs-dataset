---
original_file_path: api-reference/peripherals/sdspi_share.rst
---
...
When using an SD card with other SPI devices on the same SPI bus, due to the restrictions of the SD card startup flow, the following initialization sequence should be followed. Refer to `storage/sd_card`{.interpreted-text role="example"} for further details.

1.  Initialize the SPI bus properly by `spi_bus_initialize`{.interpreted-text role="cpp:func"}.
...
2.  Tie the CS lines of all other devices than the SD card to idle state (by default it\'s high). This is to avoid conflicts with the SD card in the following step.
    ...
    1.  Attach devices to the SPI bus by calling `spi_bus_add_device`{.interpreted-text role="cpp:func"}. This function will by default initialize the GPIO that is used as CS to the idle level: high.
...
3.  Mount the card to the filesystem by calling `esp_vfs_fat_sdspi_mount`{.interpreted-text role="cpp:func"}.
