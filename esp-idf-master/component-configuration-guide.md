---
original_file_path: api-guides/kconfig/component-configuration-guide.rst
---
...
More detailed information about the configuration system in ESP-IDF can be found in the `configuration-overview`{.interpreted-text role="ref"}.
::::
...
When the configuration is saved (for more information about configuration editing, see e.g. `project-configuration-guide`{.interpreted-text role="ref"}), the **values** in the `sdkconfig` file are updated, as well as those in `sdkconfig.h`, `sdkconfig.cmake` and `sdkconfig.json`.
...
If you plan to write `Kconfig` configuration files for your component, but you are not familiar with `Kconfig language` it is recommended to refer to the [esp-idf-kconfig Documentation](https://docs.espressif.com/projects/esp-idf-kconfig/en/latest/index.html), where an in-depth guide is provided.
::::
...
For more information about the visibility and dependencies, please refer to the [Kconfig Documentation](https://docs.espressif.com/projects/esp-idf-kconfig/en/latest/kconfiglib/language.html).
::::

## How to Ensure Backward Compatibility {#configuration-options-compatibility}
...
For more information about the `sdkconfig.rename` file, please refer to the `sdkconfig.rename <sdkconfig-rename-file>`{.interpreted-text role="ref"} section of Configuration Structure.
...
## Suppressing Info/Warning Messages in Kconfig {#supressing-kconfig-messages}
...
For more information about report areas or the configuration report, please refer to the `configuration-report`{.interpreted-text role="ref"} section of the Project Configuration Guide.
::::
