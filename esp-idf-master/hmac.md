---
original_file_path: api-reference/peripherals/hmac.rst
---
...
For more detailed information on the application workflow and the HMAC calculation process, see **{IDF_TARGET_NAME} Technical Reference Manual** \> **HMAC Accelerator (HMAC)** \[[PDF]({IDF_TARGET_TRM_EN_URL}#hmac)\].
...
Six physical eFuse blocks can be used as keys for the HMAC module: block 4 \~ block 9. The enum `hmac_key_id_t`{.interpreted-text role="cpp:enum"} in the API maps them to `HMAC_KEY0` \~ `HMAC_KEY5`.
...
To calculate an HMAC, the software has to provide the ID of the key block containing the secret key as well as the **key purpose** (see **{IDF_TARGET_NAME} Technical Reference Manual** \> **eFuse Controller (eFuse)** \[[PDF]({IDF_TARGET_TRM_EN_URL}#efuse)\]).
...
The API to calculate the HMAC is `esp_hmac_calculate`{.interpreted-text role="cpp:func"}. The input arguments for the function are the message, message length, and the eFuse key block ID which contains the secret and has the efuse key purpose set to Upstream mode.
...
### HMAC for Enabling JTAG {#hmac_for_enabling_jtag}
...
2.  Pass this key value when calling the `esp_hmac_jtag_enable`{.interpreted-text role="cpp:func"} function from the firmware.
3.  To re-disable JTAG in the firmware, reset the system or call `esp_hmac_jtag_disable`{.interpreted-text role="cpp:func"}.

For a complete workflow of soft-disabling and re-enabling JTAG, refer to `security/hmac_soft_jtag`{.interpreted-text role="example"}. This example demonstrates how to use HMAC to re-enable a soft-disabled JTAG interface, covering steps like generating an HMAC key, burning it to eFuse, and creating token data from the key.

For more details, see **{IDF_TARGET_NAME} Technical Reference Manual** \> **HMAC Accelerator (HMAC)** \[[PDF]({IDF_TARGET_TRM_EN_URL}#hmac)\].
