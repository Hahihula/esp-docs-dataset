---
original_file_path: api-guides/build-system-v2.rst
---
...
Finally, the `idf_project_default`{.interpreted-text role="cmakev2:ref"} function creates the default project executable based on the `main` component and its transitive dependencies. It also generates binary images and adds related targets such as `flash` and `menuconfig`.

## Creating a New Component

This section explains how to create a new component. The preferred and generally sufficient method is described in `cmakev2-creating-compatible-component`{.interpreted-text role="ref"}. However, if you need more control or prefer a native CMake approach without concern for v1 compatibility, see `cmakev2-creating-v2-component`{.interpreted-text role="ref"}.
...
### Creating a Component Compatible with v1 and v2 {#cmakev2-creating-compatible-component}

The recommended method for creating a new component is to use the `idf_component_register`{.interpreted-text role="cmakev2:ref"} function. This function is provided in v2 to maintain backward compatibility with v1 and remains the preferred approach for creating components that work with both versions. The only difference between the v1 and v2 versions of this function is that the v2 version does not support the `KCONFIG` and `KCONFIG_PROJBUILD` options when using Kconfig filenames that do not follow the `cmakev2-standardized-kconfigs`{.interpreted-text role="ref"}. For components that require functionality beyond what `idf_component_register`{.interpreted-text role="cmakev2:ref"} provides, please refer to `cmakev2-breaking-changes`{.interpreted-text role="ref"}.

``` {.cmake caption="Minimal Component CMakeLists.txt"}
idf_component_register(SRCS "foo.c" "bar.c"
                       INCLUDE_DIRS "include"
                       REQUIRES mbedtls)
```

### Creating a Component for v2 Only {#cmakev2-creating-v2-component}
...
This section describes how to create a component for v2, which is not compatible or usable with v1. To create a component that works with both versions, it is recommended to follow `cmakev2-creating-compatible-component`{.interpreted-text role="ref"}.
::::
...
``` {.c caption="include/esp_target_info.h"}
...
``` {.c caption="srcs/esp_target_info.c"}
#include <stdio.h>
#include <inttypes.h>
...
``` {.bash caption="esp_target_info.ld"}
esp_target_chip_info = esp_chip_info;
```

The `linker.lf` is a linker fragment that determines the placement of `print_esp_target_info` in IRAM instead of flash. This is merely an example for demonstration purposes. For more details on linker fragments, please refer to `/api-guides/linker-script-generation`{.interpreted-text role="doc"}.

``` {.bash caption="linker.lf"}
...
``` {.cmake caption="CMakeLists.txt" linenos="" emphasize-lines="1-7"}
...
The `esp_target_info` component uses functionality from other components, namely `esp_hw_support`, `esp_system`, and `spi_flash`, to collect basic information about the target. Therefore, it must declare its dependencies on these components. The component acquires the required component interface targets using the `idf_component_include`{.interpreted-text role="cmakev2:ref"} function.

Each component is evaluated exactly once by the `idf_component_include`{.interpreted-text role="cmakev2:ref"} function. This function essentially invokes CMake's `add_subdirectory` for the specified component directory and links the `COMPONENT_TARGET` created by that component with its interface target.
...
``` {.cmake caption="CMakeLists.txt" linenos="" emphasize-lines="19-23"}
add_library(${COMPONENT_TARGET} STATIC
    "srcs/esp_target_info.c"
)
...
:   Include files from this property to be processed with the `/api-guides/linker-script-generation`{.interpreted-text role="doc"}.
...
## Breaking Changes for v1 Components {#cmakev2-breaking-changes}
