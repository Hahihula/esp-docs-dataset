---
original_file_path: api-guides/low-power-mode/low-power-mode-ble.rst
---

# Introduction to Low Power Mode in Bluetooth^®^ Low Energy Scenarios
...
`CONFIG_BTDM_CTRL_LOW_POWER_CLOCK`{.interpreted-text role="ref"} = Main crystal ([CONFIG_BTDM_CTRL_LPCLK_SEL_MAIN_XTAL]{.title-ref})
:::

::: only
esp32c3 or esp32s3

`CONFIG_BT_CTRL_LOW_POWER_CLOCK`{.interpreted-text role="ref"} = Main crystal ([CONFIG_BT_CTRL_LPCLK_SEL_MAIN_XTAL]{.title-ref})
:::

::: only
esp32c2 or esp32c6 or esp32h2 or esp32c5 or esp32c61

`CONFIG_BT_LE_LP_CLK_SRC`{.interpreted-text role="ref"} = Use main XTAL as RTC clock source ([CONFIG_BT_LE_LP_CLK_SRC_MAIN_XTAL]{.title-ref})
:::

When this is selected, the main XTAL remains powered on during light-sleep, resulting in higher current consumption. Please refer to `Power Save README <bluetooth/nimble/power_save/README.md>`{.interpreted-text role="example_file"} for the typical current consumption in light-sleep using XTAL versus a 32 kHz external crystal.
...
`CONFIG_BTDM_CTRL_LOW_POWER_CLOCK`{.interpreted-text role="ref"} = External 32 kHz crystal/oscillator ([CONFIG_BTDM_CTRL_LPCLK_SEL_EXT_32K_XTAL]{.title-ref})
:::

::: only
esp32c3 or esp32s3

`CONFIG_BT_CTRL_LOW_POWER_CLOCK`{.interpreted-text role="ref"} = External 32 kHz crystal/oscillator ([CONFIG_BT_CTRL_LPCLK_SEL_EXT_32K_XTAL]{.title-ref})
:::

::: only
esp32c2 or esp32c6 or esp32h2 or esp32c5 or esp32c61

`CONFIG_BT_LE_LP_CLK_SRC`{.interpreted-text role="ref"} = Use system RTC slow clock source ([CONFIG_BT_LE_LP_CLK_SRC_DEFAULT]{.title-ref})
:::

**Configuration Path 2:**

`CONFIG_RTC_CLK_SRC`{.interpreted-text role="ref"} = External 32 kHz crystal ([CONFIG_RTC_CLK_SRC_EXT_CRYS]{.title-ref})
...
`CONFIG_BT_CTRL_LOW_POWER_CLOCK`{.interpreted-text role="ref"} = Internal 136kHz RC oscillator ([CONFIG_BT_CTRL_LPCLK_SEL_RTC_SLOW]{.title-ref})
...
`CONFIG_RTC_CLK_SRC`{.interpreted-text role="ref"} = Internal 136 kHz RC oscillator ([CONFIG_RTC_CLK_SRC_INT_RC]{.title-ref})
:::
...
> - `CONFIG_BT_LE_LL_PEER_SCA_SET_ENABLE`{.interpreted-text role="ref"} = y
> - `CONFIG_BT_LE_LL_PEER_SCA`{.interpreted-text role="ref"} = 3000
