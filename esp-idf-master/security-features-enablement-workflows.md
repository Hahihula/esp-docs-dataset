---
original_file_path: security/security-features-enablement-workflows.rst
---
...
4.  Eliminate dependency on `second-stage-bootloader`{.interpreted-text role="ref"}.
...
- `enable-flash-encryption-and-secure-boot-v2-externally`{.interpreted-text role="ref"}
- `enable-flash-encryption-externally`{.interpreted-text role="ref"}
- `enable-secure-boot-v2-externally`{.interpreted-text role="ref"}
- `enable-nvs-encryption-externally`{.interpreted-text role="ref"}
...
1.  Enable the Flash Encryption feature by following the steps listed in `enable-flash-encryption-externally`{.interpreted-text role="ref"}.
2.  Enable the Secure Boot v2 feature by following the steps listed in `enable-secure-boot-v2-externally`{.interpreted-text role="ref"}.
...
In this case all the eFuses related to Flash Encryption are written with help of the espefuse tool. More details about Flash Encryption can process can be found in `/security/flash-encryption`{.interpreted-text role="doc"}.
...
2.  Generate a Flash Encryption key
    ...
    >
    > If `Size of generated AES-XTS key <CONFIG_SECURE_FLASH_ENCRYPTION_KEYSIZE>`{.interpreted-text role="ref"} is AES-128 (256-bit key):
    >
    > ``` bash
    > espsecure generate-flash-encryption-key my_flash_encryption_key.bin
    > ```
    >
    > else if `Size of generated AES-XTS key <CONFIG_SECURE_FLASH_ENCRYPTION_KEYSIZE>`{.interpreted-text role="ref"} is AES-256 (512-bit key):
    >
    > ``` bash
    ...
    > ```
    >
    > else if `Size of generated AES-XTS key <CONFIG_SECURE_FLASH_ENCRYPTION_KEYSIZE>`{.interpreted-text role="ref"} is AES-128 key derived from 128 bits (SHA256(128 bits)):
    >
    > ``` bash
...
7.  Build, Encrypt and Flash the binaries
    ...
    >
    > When the application contains the following partition: `otadata` and `nvs_encryption_keys`, they need to be encrypted as well. Please refer to `encrypted-partitions`{.interpreted-text role="ref"} for more details about encrypted partitions.
    >
    > ::::: note
    > ::: title
    > Note
    ...
    > esp32
    >
    > If your ESP32 uses non-default `FLASH_CRYPT_CONFIG value in eFuse <setting-flash-crypt-config>`{.interpreted-text role="ref"} then you will need to pass the `--flash-crypt-conf` argument to `espsecure` to set the matching value. This will not happen when the Flash Encryption is enabled by the second stage bootloader but may happen when burning eFuses manually to enable Flash Encryption.
    > :::
    > :::::
    >
...
- See `flash-encryption-limitations`{.interpreted-text role="ref"} for more details.

### Enable Secure Boot v2 Externally

In this workflow we shall use `espsecure` tool to generate signing keys and use the `espefuse` tool to burn the relevant eFuses. The details about the Secure Boot v2 process can be found at `/security/secure-boot-v2`{.interpreted-text role="doc"}.
...
6.  Configure the project
    ...
    > By default, the first stage (ROM) bootloader would only verify the `second-stage-bootloader`{.interpreted-text role="ref"}. The second stage bootloader would verify the app partition only when the `CONFIG_SECURE_BOOT`{.interpreted-text role="ref"} option is enabled (and `CONFIG_SECURE_BOOT_VERSION`{.interpreted-text role="ref"} is set to `SECURE_BOOT_V2_ENABLED`) while building the bootloader.
    >
    > A)  Open the `project-configuration-menu`{.interpreted-text role="ref"}, in `Security features` set `Enable hardware Secure Boot in bootloader` to enable Secure Boot.
    >
    > ::: only
    > esp32
    >
    ...
    > :::
    >
    > B)  Disable the option `CONFIG_SECURE_BOOT_BUILD_SIGNED_BINARIES`{.interpreted-text role="ref"} for the project in the `project-configuration-menu`{.interpreted-text role="ref"}. This shall make sure that all the generated binaries are secure padded and unsigned. This step is done to avoid generating signed binaries as we are going to manually sign the binaries using `espsecure` tool.
...
- It is recommended to store the Secure Boot key in a highly secure place. A physical or a cloud HSM may be used for secure storage of the Secure Boot private key. Please take a look at `remote-sign-v2-image`{.interpreted-text role="ref"} for more details.
...
The details about NVS encryption and related schemes can be found at `NVS Encryption </api-reference/storage/nvs_encryption>`{.interpreted-text role="doc"}.
...
1.  Generate the HMAC key and NVS encryption key
    ...
    >
    > The above keys can be generated with the `nvs_flash/nvs_partition_generator/nvs_partition_gen.py`{.interpreted-text role="component_file"} script with help of the following command:
    >
    > ``` bash
...
3.  Generate the encrypted NVS partition
    ...
    > We shall generate the actual encrypted NVS partition on the host. More details about generating the encryption NVS partition can be found at `generate-encrypted-nvs-partition`{.interpreted-text role="ref"}.For this purpose, the contents of the NVS file shall be available in a CSV file. Please check out `nvs-csv-file-format`{.interpreted-text role="ref"} for more details.
    >
...
4.  Configure the project
    ...
    > - Enable [NVS Encryption]{.title-ref} by enabling `CONFIG_NVS_ENCRYPTION`{.interpreted-text role="ref"}.
    > - Enable the HMAC based NVS encryption by setting `CONFIG_NVS_SEC_KEY_PROTECTION_SCHEME`{.interpreted-text role="ref"} to `CONFIG_NVS_SEC_KEY_PROTECT_USING_HMAC`.
    > - Set the HMAC efuse key ID at `CONFIG_NVS_SEC_HMAC_EFUSE_KEY_ID`{.interpreted-text role="ref"} to the one in which the eFuse key was burned in Step 2.
...
#### Enable NVS Encryption Based on Flash Encryption {#enable-flash-enc-based-nvs-encryption}

In this case we generate NVS Encryption keys on a host. This key is then flashed on the chip and protected with the help of `Flash Encryption </security/flash-encryption>`{.interpreted-text role="doc"} features.

1.  Generate the NVS encryption key

    > For generation of respective keys, we shall use `NVS partition generator utility </api-reference/storage/nvs_partition_gen>`{.interpreted-text role="doc"}. We shall generate the encryption key on host and this key shall be stored on the flash of {IDF_TARGET_NAME} in encrypted state.
    >
    > The key can be generated with the `nvs_flash/nvs_partition_generator/nvs_partition_gen.py`{.interpreted-text role="component_file"} script with the help of the following command:
    >
    > ``` bash
...
2.  Generate the encrypted NVS partition
    ...
    > We shall generate the actual encrypted NVS partition on host. More details about generating the encrypted NVS partition can be found at `generate-encrypted-nvs-partition`{.interpreted-text role="ref"}.For this, the contents of the NVS file shall be available in a CSV file. Please refer to `nvs-csv-file-format`{.interpreted-text role="ref"} for more details.
    >
    > The encrypted NVS partition can be generated with following command:
    >
    > ``` bash
    ...
    > Some command arguments are explained below:
    >
    > - CSV file name - In this case [sample_singlepage_blob.csv]{.title-ref} is the CSV file which contains the NVS data. Please replace it with the file you wish to choose.
    > - NVS partition size - This is the size of the NVS partition in bytes. Please update the sample value of `0x3000` in the above-provided command to the correct size of your NVS partition.

3.  Configure the project

    > - Enable [NVS Encryption]{.title-ref} by enabling `CONFIG_NVS_ENCRYPTION`{.interpreted-text role="ref"}.
    > - Set NVS to use Flash Encryption based scheme by setting `CONFIG_NVS_SEC_KEY_PROTECTION_SCHEME`{.interpreted-text role="ref"} to `CONFIG_NVS_SEC_KEY_PROTECT_USING_FLASH_ENC`.
