---
original_file_path: api-reference/peripherals/i3c_master.rst
---
...
::: {.blockdiag scale="100%" caption="General usage flow of I3C driver (click image to view full size)" align="center"}

blockdiag {

:   default_fontsize = 14; node_width = 250; node_height = 80; class emphasis \[color = pink, style = dashed\];

    create_bus \[label=\"Create I3C Busn(i3c_new_master_bus)\"\]; add_device \[label=\"Add Devicen(add_i2c_device / nadd_i3c_static_device / nscan_devices_by_entdaa)\"\]; transfer \[label=\"Data Transfern(transmit / receive / ntransmit_receive)\", class=\"emphasis\"\]; cleanup \[label=\"Remove Device and Busn(rm_device / del_master_bus)\"\];
...
The I3C master bus is represented by `i3c_master_bus_handle_t`{.interpreted-text role="cpp:type"} in the driver. The driver internally maintains a resource pool that can manage multiple buses and allocates free bus ports when requested.

<figure class="align-center">
<img src="../../../_static/diagrams/i3c/i3c_bus_structure.svg" alt="I3C Bus Structure" />
<figcaption aria-hidden="true">I3C Bus Structure</figcaption>
</figure>

When creating an I3C bus instance, we need to configure GPIO pins, clock source, frequency, and other parameters through `i3c_master_bus_config_t`{.interpreted-text role="cpp:type"}. These parameters will determine how the bus operates. The following code shows how to create a basic I3C bus:
...
<figure class="align-center">
<img src="../../../_static/diagrams/i3c/i3c_i2c_write.svg" alt="Write to legacy I2C device" />
<figcaption aria-hidden="true">Write to legacy I2C device</figcaption>
</figure>

<figure class="align-center">
<img src="../../../_static/diagrams/i3c/i3c_i2c_read.svg" alt="Read from legacy I2C device" />
<figcaption aria-hidden="true">Read from legacy I2C device</figcaption>
</figure>
...
1.  Created an I3C bus instance through `i3c_new_master_bus`{.interpreted-text role="cpp:func"}
2.  Added an I2C device through `i3c_master_bus_add_i2c_device`{.interpreted-text role="cpp:func"}, which requires specifying the device\'s static address and clock frequency
3.  Used `i3c_master_i2c_device_transmit`{.interpreted-text role="cpp:func"} to write data. By default, it works in blocking mode. For non-blocking mode, please refer to `dma-support`{.interpreted-text role="ref"}. The same applies to other transfer functions.
4.  Used `i3c_master_i2c_device_receive`{.interpreted-text role="cpp:func"} to read data
5.  Used `i3c_master_i2c_device_transmit_receive`{.interpreted-text role="cpp:func"} to execute write-read combined transactions (commonly used to write register address first, then read data, with no STOP bit in between)
...
<figure class="align-center">
<img src="../../../_static/diagrams/i3c/i3c_icon.svg" alt="I3C Transfer Legend" />
<figcaption aria-hidden="true">I3C Transfer Legend</figcaption>
</figure>

If you know the static address of an I3C device, you can add the device using the SETDASA method:

<figure class="align-center">
<img src="../../../_static/diagrams/i3c/i3c_setdasa.svg" alt="I3C Directed Dynamic Address Assignment" />
<figcaption aria-hidden="true">I3C Directed Dynamic Address Assignment</figcaption>
</figure>
...
1.  We use `i3c_master_bus_add_i3c_static_device`{.interpreted-text role="cpp:func"} to add an I3C device
...
4.  After that, we can use the dynamic address for data transfers through `i3c_master_i3c_device_transmit`{.interpreted-text role="cpp:func"} or `i3c_master_i3c_device_receive`{.interpreted-text role="cpp:func"} or `i3c_master_i3c_device_transmit_receive`{.interpreted-text role="cpp:func"}. By default, it works in blocking mode. For non-blocking mode, please refer to `dma-support`{.interpreted-text role="ref"}. The same applies to other transfer functions.
...
<figure class="align-center">
<img src="../../../_static/diagrams/i3c/i3c_entdaa.svg" alt="I3C Automatic Dynamic Address Assignment" />
<figcaption aria-hidden="true">I3C Automatic Dynamic Address Assignment</figcaption>
</figure>
...
1.  When creating the bus, we need to set [entdaa_device_num]{.title-ref}, which represents the expected number of devices to be discovered
2.  Use `i3c_master_scan_devices_by_entdaa`{.interpreted-text role="cpp:func"} to scan all I3C devices on the bus
3.  The system automatically assigns dynamic addresses to each device
4.  We can get the device count through `i3c_master_get_device_count`{.interpreted-text role="cpp:func"}
5.  Get each device\'s handle through `i3c_master_get_device_handle`{.interpreted-text role="cpp:func"}
6.  Use `i3c_master_i3c_device_get_info`{.interpreted-text role="cpp:func"} to get detailed device information (dynamic address, BCR, DCR, PID)
7.  Perform data transfers through `i3c_master_i3c_device_transmit`{.interpreted-text role="cpp:func"} or `i3c_master_i3c_device_receive`{.interpreted-text role="cpp:func"} based on the obtained device information
...
`i3c_master_scan_devices_by_entdaa`{.interpreted-text role="cpp:func"} is thread-safe, and there will not be two threads addressing simultaneously. According to the protocol, when a slave is addressed and discovered by `i3c_master_scan_devices_by_entdaa`{.interpreted-text role="cpp:func"}, it no longer has the ability to respond to a second addressing. Therefore, there will be no address changes due to addressing in different threads. This interface supports adding new devices after initialization. To rescan, use the CCC mechanism to reset addresses on the I3C bus, or clear address information on the bus by power cycling.
::::

## Common Command Code (CCC) Transfer

The I3C protocol uses Common Command Code (CCC) for bus management and device configuration. You can use the `i3c_master_transfer_ccc`{.interpreted-text role="cpp:func"} function to send CCC commands.

CCC transfers can be broadcast (sent to all devices) or direct (sent to a specific device):

<figure class="align-center">
<img src="../../../_static/diagrams/i3c/i3c_broadcast_ccc.svg" alt="I3C Broadcast Command" />
<figcaption aria-hidden="true">I3C Broadcast Command</figcaption>
</figure>

<figure class="align-center">
<img src="../../../_static/diagrams/i3c/i3c_direct_ccc.svg" alt="I3C Direct Command" />
<figcaption aria-hidden="true">I3C Direct Command</figcaption>
</figure>
...
`i3c_master_transfer_ccc`{.interpreted-text role="cpp:func"} is always blocking and is not affected by DMA and asynchronous configuration. Users need to query the I3C protocol to know the specific format of CCC commands, and fill `i3c_master_ccc_transfer_config_t::direction`{.interpreted-text role="cpp:member"} as `I3C_MASTER_TRANSFER_DIRECTION_READ` or `I3C_MASTER_TRANSFER_DIRECTION_WRITE` and fill `i3c_master_ccc_transfer_config_t::data`{.interpreted-text role="cpp:member"} and `i3c_master_ccc_transfer_config_t::data_size`{.interpreted-text role="cpp:member"} according to the format of sending commands or obtaining values.
::::

## Resource Cleanup

When the previously installed I3C bus or device is no longer needed, call `i3c_master_bus_rm_i3c_device`{.interpreted-text role="cpp:func"} or `i3c_master_bus_rm_i2c_device`{.interpreted-text role="cpp:func"} to remove the device, then call `i3c_del_master_bus`{.interpreted-text role="cpp:func"} to reclaim resources and release the underlying hardware.
...
The clock source of the I3C bus can be selected through `i3c_master_bus_config_t::clock_source`{.interpreted-text role="cpp:member"}.
...
When the I3C push-pull output frequency is greater than 3 MHz, please set the clock source to `i3c_master_clock_source_t::I3C_MASTER_CLK_SRC_PLL_F120M`{.interpreted-text role="cpp:enumerator"} or `i3c_master_clock_source_t::I3C_MASTER_CLK_SRC_PLL_F160M`{.interpreted-text role="cpp:enumerator"}.
::::
...
When the I3C controller generates events such as send or receive completion, it notifies the CPU through interrupts. If you need to call a function when a specific event occurs, you can register event callbacks with the I3C driver\'s interrupt service routine (ISR) by calling `i3c_master_i3c_device_register_event_callbacks`{.interpreted-text role="cpp:func"} and `i3c_master_i2c_device_register_event_callbacks`{.interpreted-text role="cpp:func"} for I3C and I2C slaves respectively. Since these callback functions are called in the ISR, they should not involve blocking operations. You can check the suffix of the called API to ensure that only FreeRTOS APIs with the ISR suffix are called in the function. The callback function has a boolean return value indicating whether the callback unblocked a higher priority task.

For event callbacks of I2C slaves, please refer to i2c_master_i2c_event_callbacks_t.

> - `i3c_master_i2c_event_callbacks_t::on_trans_done`{.interpreted-text role="cpp:member"} can be set to a callback function for the master \"transfer done\" event. The function prototype is declared in `i3c_master_i2c_callback_t`{.interpreted-text role="cpp:type"}. Note that this callback function can only be used when the I2C slave device has DMA enabled and uses asynchronous transfer. For details, please refer to `dma-support`{.interpreted-text role="ref"}.

For event callbacks of I3C slaves, please refer to i3c_master_i3c_event_callbacks_t.

> - `i3c_master_i3c_event_callbacks_t::on_trans_done`{.interpreted-text role="cpp:member"} can be set to a callback function for the master \"transfer done\" event. The function prototype is declared in `i3c_master_i3c_callback_t`{.interpreted-text role="cpp:type"}. Note that this callback function can only be used when the I3C slave device has DMA enabled and uses asynchronous transfer. For details, please refer to `dma-support`{.interpreted-text role="ref"}.
> - `i3c_master_i3c_event_callbacks_t::on_ibi`{.interpreted-text role="cpp:member"} can be set to a callback function for IBI events. The function prototype is declared in `i3c_master_ibi_callback_t`{.interpreted-text role="cpp:type"}. For detailed information about IBI events, please refer to `in-band-interrupt`{.interpreted-text role="ref"}
...
## In-Band Interrupt (IBI) {#in-band-interrupt}
...
The I3C bus configuration structure `i3c_master_bus_config_t`{.interpreted-text role="cpp:type"} contains IBI-related global configuration items:

- `i3c_master_bus_config_t::ibi_rstart_trans_en`{.interpreted-text role="cpp:member"} enables restart transaction on IBI. The I3C controller continues to execute the command that was interrupted by IBI after IBI completion. If IBI occurs during bus idle and the I3C transfer task is not empty, the I3C controller will continue to execute that task. If IBI conflicts with I3C controller transfer and wins arbitration, the interrupted task will continue to execute after IBI processing is complete.
- `i3c_master_bus_config_t::ibi_silent_sir_rejected`{.interpreted-text role="cpp:member"} when written as 0, does not notify the application layer when a slave interrupt request (SIR) is rejected. When written as 1, the IBI status is still written to the IBI FIFO and the application layer is notified.
- `i3c_master_bus_config_t::ibi_no_auto_disable`{.interpreted-text role="cpp:member"} if set, does not automatically disable IBI after the controller NACKs an In-Band interrupt, keeping in-band interrupt enabled.

You can use the `i3c_master_i3c_device_ibi_config`{.interpreted-text role="cpp:func"} function to configure IBI for a specific device:
...
Detailed information about IBI events will be provided from the callback through `i3c_master_ibi_info_t`{.interpreted-text role="cpp:type"}:

`i3c_master_ibi_info_t::ibi_id`{.interpreted-text role="cpp:member"} is the raw identifier of the IBI, usually encoded from the slave device\'s dynamic address; it is the raw value, i.e., dynamic address + read/write bit. `i3c_master_ibi_info_t::ibi_sts`{.interpreted-text role="cpp:member"} is the IBI status field reported by the controller. `i3c_master_ibi_info_t::data_length`{.interpreted-text role="cpp:member"} is the number of valid bytes in the payload buffer `i3c_master_ibi_info_t::ibi_data`{.interpreted-text role="cpp:member"}. `i3c_master_ibi_info_t::ibi_data`{.interpreted-text role="cpp:member"} is the optional payload bytes associated with the IBI. Only the first `i3c_master_ibi_info_t::data_length`{.interpreted-text role="cpp:member"} bytes are valid.
...
## DMA and Asynchronous Transfer {#dma-support}
...
You can configure DMA for the bus through the `i3c_master_bus_decorate_dma`{.interpreted-text role="cpp:func"} function:
...
At this point, the I3C master transfer functions will return immediately after being called. When each transfer is complete, the `i3c_master_i3c_event_callbacks_t::on_trans_done`{.interpreted-text role="cpp:member"} callback function will be called to indicate the completion of a transfer. If you need to wait for the transfer to complete, you can call the `i3c_master_bus_wait_all_done`{.interpreted-text role="cpp:func"} function to wait for all transfers to complete:
...
Factory functions: - `i3c_new_master_bus`{.interpreted-text role="cpp:func"} - `i3c_del_master_bus`{.interpreted-text role="cpp:func"}

I3C master operation functions (thread safety guaranteed through bus operation signals): - `i3c_master_bus_add_i3c_static_device`{.interpreted-text role="cpp:func"} - `i3c_master_bus_rm_i3c_device`{.interpreted-text role="cpp:func"} - `i3c_master_i3c_device_transmit`{.interpreted-text role="cpp:func"} - `i3c_master_i3c_device_receive`{.interpreted-text role="cpp:func"} - `i3c_master_i3c_device_transmit_receive`{.interpreted-text role="cpp:func"} - `i3c_master_i2c_device_transmit`{.interpreted-text role="cpp:func"} - `i3c_master_i2c_device_receive`{.interpreted-text role="cpp:func"} - `i3c_master_i2c_device_transmit_receive`{.interpreted-text role="cpp:func"} - `i3c_master_transfer_ccc`{.interpreted-text role="cpp:func"}
...
When power management `CONFIG_PM_ENABLE`{.interpreted-text role="ref"} is enabled, the system may adjust or disable clock sources before entering sleep mode, which can cause I3C transfer errors.
...
- `CONFIG_I3C_MASTER_ISR_CACHE_SAFE`{.interpreted-text role="ref"}: Ensure I3C interrupts work properly when cache is disabled (e.g., during SPI Flash write)
- `CONFIG_I3C_MASTER_ISR_HANDLER_IN_IRAM`{.interpreted-text role="ref"}: Place I3C master ISR handler in IRAM to improve performance and reduce cache misses
- `CONFIG_I3C_MASTER_ENABLE_DEBUG_LOG`{.interpreted-text role="ref"}: Enable I3C debug logging

## About Resource Consumption

You can use the `/api-guides/tools/idf-size`{.interpreted-text role="doc"} tool to view the code and data consumption of the I3C driver. The following are the test prerequisites (using ESP32-P4 as an example):
...
- 
  ...
  :   - `CONFIG_I3C_MASTER_ISR_HANDLER_IN_IRAM`{.interpreted-text role="ref"} - ISR handler is not placed in IRAM.
      - `CONFIG_I3C_MASTER_ISR_CACHE_SAFE`{.interpreted-text role="ref"} - Cache safety option is not enabled.
...
- `peripherals/i3c/i3c_i2c_basic`{.interpreted-text role="example"} demonstrates the basic steps of initializing the I3C master driver and reading data from the ICM42688 sensor using the I2C interface.
- `peripherals/i3c/i3c_lsm6dscx`{.interpreted-text role="example"} demonstrates how to read and write data from a connected LSM6DSOX sensor using I3C master mode, and supports in-band interrupt (IBI) event handling.
...
::: include-build-file
inc/i3c_master_i2c.inc
:::

## I3C Driver Types

::: include-build-file
inc/components/esp_hal_i3c/include/hal/i3c_master_types.inc
:::

## I3C HAL Types

::: include-build-file
inc/components/esp_driver_i3c/include/driver/i3c_master_types.inc
:::
