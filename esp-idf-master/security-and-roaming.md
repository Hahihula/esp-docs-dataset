---
original_file_path: api-guides/wifi-driver/security-and-roaming.rst
---
...
{IDF_TARGET_NAME} supports the following Wi-Fi security modes. For detailed configuration and usage, refer to `Wi-Fi Security <../wifi-security>`{.interpreted-text role="doc"}.
...
Wi-Fi Easy Connect^TM^ (or Device Provisioning Protocol) is a secure and standardized provisioning protocol for configuring Wi-Fi devices. More information can be found in `esp_dpp <../../api-reference/network/esp_dpp>`{.interpreted-text role="doc"}.

## WPS Enrollee

{IDF_TARGET_NAME} supports WPS enrollee feature in Wi-Fi mode `WIFI_MODE_STA`{.interpreted-text role="cpp:enumerator"} or `WIFI_MODE_APSTA`{.interpreted-text role="cpp:enumerator"}. Currently, {IDF_TARGET_NAME} supports WPS enrollee type PBC and PIN.
...
Refer ESP-IDF example `examples/wifi/roaming/README.md`{.interpreted-text role="idf_file"} to set up and use these APIs. Example code only demonstrates how these APIs can be used, and the application should define its own algorithm and cases as required.
...
A config option `CONFIG_ESP_WIFI_11R_SUPPORT`{.interpreted-text role="ref"} and configuration parameter `ft_enabled`{.interpreted-text role="cpp:type"} in `wifi_sta_config_t`{.interpreted-text role="cpp:type"} is provided to enable 802.11r support for station. Refer ESP-IDF example `examples/wifi/roaming/README.md`{.interpreted-text role="idf_file"} for further details.
...
FTM is used to measure Wi-Fi Round Trip Time (Wi-Fi RTT) which is the time a Wi-Fi signal takes to travel from a device to another device and back again. Using Wi-Fi RTT, the distance between the devices can be calculated with a simple formula of [RTT \* c / 2]{.title-ref}, where c is the speed of light.
...
Refer to ESP-IDF example `examples/wifi/ftm/README.md`{.interpreted-text role="idf_file"} for steps on how to set up and perform FTM.
::::
