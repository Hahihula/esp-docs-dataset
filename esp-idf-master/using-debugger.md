---
original_file_path: api-guides/jtag-debugging/using-debugger.rst
---
...
- `jtag-debugging-using-debugger-eclipse`{.interpreted-text role="ref"}
- `jtag-debugging-using-debugger-command-line`{.interpreted-text role="ref"}
- `jtag-debugging-with-idf-py`{.interpreted-text role="ref"}
...
## Eclipse {#jtag-debugging-using-debugger-eclipse}
...
It is recommended to first check if debugger works using `jtag-debugging-with-idf-py`{.interpreted-text role="ref"} or from `jtag-debugging-using-debugger-command-line`{.interpreted-text role="ref"} and then move to using Eclipse.
::::
...
This approach is recommended only if you are unable to debug using [ESP-IDF GDB OpenOCD Debugging](https://docs.espressif.com/projects/espressif-ide/en/latest/openocddebugging.html) for some reason.
::::

To install the `GDB Hardware Debugging` plugin, open Eclipse and select `Help` \> `Install` New Software.

After installation is complete, follow these steps to configure the debugging session. Please note that some configuration parameters are generic, while others are project-specific. This will be shown below by configuring debugging for \"blink\" example project. If not done already, add this project to Eclipse workspace following [Eclipse Plugin](https://github.com/espressif/idf-eclipse-plugin/blob/master/README.md). The source of `get-started/blink`{.interpreted-text role="example"} application is available in `examples`{.interpreted-text role="idf"} directory of ESP-IDF repository.
...
5.  Finally, under `Build (if required) before launching` click `Disable auto build`.
    ...
    <img src="../../../_static/hw-debugging-main-tab.jpg" alt="Configuration of GDB Hardware Debugging - Main tab" />
    <figcaption aria-hidden="true">Configuration of GDB Hardware Debugging - Main tab</figcaption>
    </figure>
...
7.  Change the default configuration of the `Remote host` by entering `3333` under the `Port number`.
    ...
    <figure class="align-center">
    <img src="../../../_static/hw-debugging-debugger-tab.jpg" alt="Configuration of GDB Hardware Debugging - Debugger tab" />
    <figcaption aria-hidden="true">Configuration of GDB Hardware Debugging - Debugger tab</figcaption>
    </figure>
...
8.  The last tab that requires changing the default configuration is `Startup`. Under `Initialization Commands` uncheck `Reset and Delay (seconds)` and `Halt`. Then, in the entry field below, enter the following lines:
    ...
    For description of `program_esp` command, see `jtag-upload-app-debug`{.interpreted-text role="ref"}.
...
11. Checkout `Resume` option. This will make the program to resume after `mon reset halt` is invoked per point 8. The program will then stop at breakpoint inserted at `app_main`.
    ...
    <figure class="align-center">
    <img src="../../../_static/hw-debugging-startup-tab.jpg" alt="Configuration of GDB Hardware Debugging - Startup tab" />
    <figcaption aria-hidden="true">Configuration of GDB Hardware Debugging - Startup tab</figcaption>
    </figure>

    If the `Startup` sequence looks convoluted and respective `Initialization Commands` are unclear, check `jtag-debugging-tip-debugger-startup-commands`{.interpreted-text role="ref"} for additional explanation.

12. If you have completed the `jtag-debugging-configuring-target`{.interpreted-text role="ref"} steps described above, so the target is running and ready to talk to debugger, go right to debugging by pressing `Debug` button. Otherwise press `Apply` to save changes, go back to `jtag-debugging-configuring-target`{.interpreted-text role="ref"} and return here to start debugging.
    ...
    If you are not quite sure how to use GDB, check `jtag-debugging-examples-eclipse`{.interpreted-text role="ref"} example debugging session in section `jtag-debugging-examples`{.interpreted-text role="ref"}.

## Command Line {#jtag-debugging-using-debugger-command-line}

1.  Begin by completing the steps described under `jtag-debugging-configuring-target`{.interpreted-text role="ref"}. This is prerequisite to start a debugging session.
...
3.  When launching a debugger, you will need to provide a couple of configuration parameters and commands. The build system generates several `.gdbinit` files to facilitate efficient debugging. Paths to these files can be found in the `build/project_description.json`, under the `gdbinit_files` section. The paths to these files are defined as follows:
    ...
    - `prefix_map` - configures the prefix map to modify source paths in GDB. For more details, see `reproducible-builds-and-debugging`{.interpreted-text role="ref"}.
...
5.  If the previous steps have been done correctly, you will see a similar log concluded with the `(gdb)` prompt:
    ...
    If you are not sure how to use GDB, check `jtag-debugging-examples-command-line`{.interpreted-text role="ref"} example debugging session in section `jtag-debugging-examples`{.interpreted-text role="ref"}.

## Idf.py Debug Targets {#jtag-debugging-with-idf-py}
...
1.  `idf.py openocd`
    ...
    To configure the JTAG configuration for the current board, please use the environmental variable `OPENOCD_COMMANDS` or `--openocd-commands` command line argument. If none of the above is defined, OpenOCD is started with [\|idf-py-def-cfg\|](##SUBST##|idf-py-def-cfg|) board definition.

2.  `idf.py gdb`

    Starts GDB in the same way as the `jtag-debugging-using-debugger-command-line`{.interpreted-text role="ref"}, using generated GDB scripts referring to the current project\'s ELF file. For more details, see `jtag-debugging-using-debugger-command-line`{.interpreted-text role="ref"}.

3.  `idf.py gdbtui`

    > The same as [2]{.title-ref}, but starts the gdb with `tui` argument, allowing for a simple source code view.
