---
original_file_path: api-guides/openthread.rst
---
...
::: {.blockdiag caption="OpenThread device modes" align="center"}
...
The OpenThread `openthread/ot_cli`{.interpreted-text role="example"} example is a good place to start at. It demonstrates basic OpenThread initialization and simple socket-based server and client.

### Before OpenThread Initialization

- s1.1: The main task calls `esp_vfs_eventfd_register`{.interpreted-text role="cpp:func"} to initialize the eventfd virtual file system. The eventfd file system is used for task notification in the OpenThread driver.
- s1.2: The main task calls `nvs_flash_init`{.interpreted-text role="cpp:func"} to initialize the NVS where the Thread network data is stored.
- s1.3: **Optional**. The main task calls `esp_netif_init`{.interpreted-text role="cpp:func"} only when it wants to create the network interface for Thread.
- s1.4: The main task calls `esp_event_loop_create`{.interpreted-text role="cpp:func"} to create the system Event task and initialize an application event\'s callback function.

### OpenThread Stack Initialization

- s2.1: Call `esp_openthread_init`{.interpreted-text role="cpp:func"} to initialize the OpenThread stack.
...
- s3.1: Call `esp_netif_new`{.interpreted-text role="cpp:func"} with `ESP_NETIF_DEFAULT_OPENTHREAD` to create the interface.
- s3.2: Call `esp_openthread_netif_glue_init`{.interpreted-text role="cpp:func"} to create the OpenThread interface handlers.
- s3.3: Call `esp_netif_attach`{.interpreted-text role="cpp:func"} to attach the handlers to the interface.

### The OpenThread Main Loop

- s4.3: Call `esp_openthread_launch_mainloop`{.interpreted-text role="cpp:func"} to launch the OpenThread main loop. Note that this is a busy loop and does not return until the OpenThread stack is terminated.

### Calling OpenThread APIs

The OpenThread APIs are not thread-safe. When calling OpenThread APIs from other tasks, make sure to hold the lock with `esp_openthread_lock_acquire`{.interpreted-text role="cpp:func"} and release the lock with `esp_openthread_lock_release`{.interpreted-text role="cpp:func"} afterwards.

### Deinitialization

The following steps are required to deinitialize the OpenThread stack:

- Call `esp_netif_destroy`{.interpreted-text role="cpp:func"} and `esp_openthread_netif_glue_deinit`{.interpreted-text role="cpp:func"} to deinitialize the OpenThread network interface if you have created one.
- Call `esp_openthread_deinit`{.interpreted-text role="cpp:func"} to deinitialize the OpenThread stack.
...
Calling `esp_openthread_border_router_init`{.interpreted-text role="cpp:func"} during the initialization launches all the border routing functionalities.

You may refer to the `openthread/ot_br`{.interpreted-text role="example"} example and the README for further border router details.
