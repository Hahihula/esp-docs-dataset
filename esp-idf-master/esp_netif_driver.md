---
original_file_path: api-reference/network/esp_netif_driver.rst
---
...
As shown in the `esp-netif structure`{.interpreted-text role="ref"} diagram, ESP-NETIF is in fact an intermediary between the I/O driver and the TCP/IP stack. This manual describes customization of these two sides, that is if you need to implement `esp_netif_custom_driver`{.interpreted-text role="ref"} or if you need to employ `esp_netif_tcpip_stack`{.interpreted-text role="ref"}.

## ESP-NETIF Custom I/O Driver {#esp_netif_custom_driver}
...
According to the diagram shown in the `esp-netif structure`{.interpreted-text role="ref"} part, the following three API functions for the packet data path must be defined for connecting with ESP-NETIF:

- `esp_netif_transmit()`{.interpreted-text role="cpp:func"}
- `esp_netif_free_rx_buffer()`{.interpreted-text role="cpp:func"}
- `esp_netif_receive()`{.interpreted-text role="cpp:func"}
...
The receiving function on the other hand gets called from the I/O driver, so that the driver\'s code simply calls `esp_netif_receive()`{.interpreted-text role="cpp:func"} on a new data received event.
...
So when the `esp_netif_attach()`{.interpreted-text role="cpp:func"} gets called from the initialization code, the post-attach callback from I/O driver\'s code gets executed to mutually register callbacks between ESP-NETIF and I/O driver instances. Typically the driver is started as well in the post-attach callback. An example of a simple post-attach callback is outlined below:
...
In this case, please choose `ESP_NETIF_PROVIDE_CUSTOM_IMPLEMENTATION` in the ESP-NETIF component configuration menu. This option will disable lwIP implementation of the ESP-NETIF functions and provide only header files with declarations of types and API. You will have to supply the necessary implementation in your custom component. You can refer to the `esp_netif/loopback/esp_netif_loopback.c`{.interpreted-text role="component_file"} for example of dummy implementations of these functions.

It is also possible to build ESP-IDF without lwIP, please refer to `components/esp_netif_stack/README.md`{.interpreted-text role="idf_file"}.
