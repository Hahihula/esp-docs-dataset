---
original_file_path: api-guides/low-power-mode/low-power-mode-soc.rst
---
...
The frequency adjustment mechanism of DFS operates based on the maximum frequency demand dictated by held power locks. Additionally, the values of `CONFIG_FREERTOS_HZ`{.interpreted-text role="ref"} also influence the frequency adjustments of DFS. Higher values lead to a higher frequency of task scheduling, then the system can also more quickly re-adjust the clock frequencies according to the system requirements. For further details regarding the frequency adjustment mechanism, please refer to `Power Management <../../api-reference/system/power_management>`{.interpreted-text role="doc"}.
...
## Light-sleep Mode {#Light-sleep Mode}

Light-sleep mode is a low power mode preset in the {IDF_TARGET_NAME}. Users can switch to Light-sleep mode by calling `esp_light_sleep_start`{.interpreted-text role="cpp:func"} interface. Upon entering sleep, the chip will shut down unnecessary power domains and apply clock gating to modules not in use, based on the current operational states of peripherals. The {IDF_TARGET_NAME} supports various wake-up sources. Please refer to `Sleep Modes <../../api-reference/system/sleep_modes>`{.interpreted-text role="doc"} for more information. When the chip wakes up from Light-sleep mode, the CPU continues running from the context it was in before entering sleep, and the operational states of peripherals remain unaffected. To effectively reduce chip power consumption under Light-sleep mode, it is highly recommended that users utilize Auto Light-sleep mode described below.

Auto Light-sleep mode is a low power mode provided by ESP-IDF `Power Management <../../api-reference/system/power_management>`{.interpreted-text role="doc"} component that leverages FreeRTOS\'s Tickless IDLE feature. When the application releases all power locks and all FreeRTOS tasks are in a blocked or suspended state, the system automatically calculates the next time point when an event will wake the operating system. If this calculated time point exceeds a set duration (`CONFIG_FREERTOS_IDLE_TIME_BEFORE_SLEEP`{.interpreted-text role="ref"}), the `esp_pm` component automatically configures the timer wake-up source and enters light sleep to reduce power consumption. To enable this mode, users need to set the `light_sleep_enable` field to true in `esp_pm_config_t`{.interpreted-text role="cpp:type"} when configuring DFS. For more details, please refer to [DFS Configuration](#dfs-configuration).
...
Devices can wake up periodically from deep sleep to measure and upload data, and then return to deep sleep. Alternatively, it can store multiple data sets in RTC memory and transmit them all at once upon the next wake-up. This feature can be implemented using the deep-sleep-stub functionality. For details, please refer to `Deep-sleep Wake Stubs <../../api-guides/deep-sleep-stub>`{.interpreted-text role="doc"}.
:::
...
- 
  ...
  single/dual core operation mode (`CONFIG_FREERTOS_UNICORE`{.interpreted-text role="ref"})

  :   For multi-core chips, the single core operation mode can be selected.

- 

  RTOS Tick rate (Hz) (`CONFIG_FREERTOS_HZ`{.interpreted-text role="ref"})
...
1.  Enable `CONFIG_PM_ENABLE`{.interpreted-text role="ref"}
...
### Recommended Configuration
  ...
  -------------------------------------------------------------------------------------- ----------------------------------------------------------
  Enable power management component (`CONFIG_PM_ENABLE`{.interpreted-text role="ref"})   ON

  RTOS Tick rate (Hz) (`CONFIG_FREERTOS_HZ`{.interpreted-text role="ref"})               1000
...
- Minimum IDLE Tick count before entering sleep state (`CONFIG_FREERTOS_IDLE_TIME_BEFORE_SLEEP`{.interpreted-text role="ref"})

- Put light sleep related codes in IRAM (`CONFIG_PM_SLP_IRAM_OPT`{.interpreted-text role="ref"})

- Put RTOS IDLE related codes in IRAM (`CONFIG_PM_RTOS_IDLE_OPT`{.interpreted-text role="ref"})

- RTC slow clock source (`CONFIG_RTC_CLK_SRC`{.interpreted-text role="ref"})
...
- Disable all GPIO when chip at sleep (`CONFIG_PM_SLP_DISABLE_GPIO`{.interpreted-text role="ref"})

::: only
SOC_PM_SUPPORT_MAC_BB_PD

- Power down MAC and baseband (`CONFIG_ESP_PHY_MAC_BB_PD`{.interpreted-text role="ref"})
:::

:::: only
SOC_PM_SUPPORT_CPU_PD

- Power down CPU (`CONFIG_PM_POWER_DOWN_CPU_IN_LIGHT_SLEEP`{.interpreted-text role="ref"})
...
- Power down I/D-cache tag memory (`CONFIG_PM_RESTORE_CACHE_TAGMEM_AFTER_LIGHT_SLEEP`{.interpreted-text role="ref"})
:::

::: only
SOC_PM_SUPPORT_VDDSDIO_PD

- Power down flash in light sleep (`CONFIG_ESP_SLEEP_POWER_DOWN_FLASH`{.interpreted-text role="ref"})
...
- Power down Digital Peripheral modules and all child power domains under the TOP power domain in light sleep (`CONFIG_ESP_SLEEP_POWER_DOWN_FLASH`{.interpreted-text role="ref"})
:::

::: only
SOC_PM_SUPPORT_RTC_PERIPH_PD

- Power down the RTC digital peripheral module. Automatically powered down the RTC digital peripherals during system sleep is the default system power management policy. Users can call [esp_sleep_pd_config(ESP_PD_DOMAIN_RTC_PERIPH, ESP_PD_OPTION_ON)]{.title-ref} to enable the RTC digital peripheral module while in sleep mode.
:::
...
- To reduce the on-chip RAM usage of the Power Management module (:ref:CONFIG_PM_SLEEP_FUNC_IN_IRAM), the options in the table below are used to control whether individual components of the Power Management module enable or disable on-chip RAM optimization when :ref:CONFIG_PM_SLEEP_FUNC_IN_IRAM is disabled.
  ...
  --------------------------------------------------------------------- ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  `CONFIG_PM_SLP_IRAM_OPT`{.interpreted-text role="ref"}                When this option is enabled, the software flow for entering and exiting light/deep sleep is linked to on-chip RAM, which shortens the sleep transition time but consumes more on-chip RAM resources. When this option is disabled, the light/deep sleep software flow is compiled into flash memory instead, resulting in longer sleep transition time but saving on-chip RAM resources.

  `CONFIG_PM_RTOS_IDLE_OPT`{.interpreted-text role="ref"}               In auto light sleep mode, when this option is enabled, the FreeRTOS system tick, IDLE task hook functions, and the FreeRTOS tickless idle software context are linked to on-chip RAM. Otherwise, they will be linked to flash memory.

  `CONFIG_ESP_PERIPH_CTRL_FUNC_IN_IRAM`{.interpreted-text role="ref"}   When this option is enabled, the software implementation related to peripheral clock and reset control will be linked to on-chip RAM. Otherwise, it will be linked to flash memory.

  `CONFIG_ESP_REGI2C_CTRL_FUNC_IN_IRAM`{.interpreted-text role="ref"}   When this option is enabled, the software implementation related to Analog I2C read/write access will be linked to on-chip RAM. Otherwise, it will be linked to flash memory.
  ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
:::

Configuration Steps:

1.  Configure wake-up sources (refer to `Sleep Modes <../../api-reference/system/sleep_modes>`{.interpreted-text role="doc"} for details)
2.  Enable `CONFIG_PM_ENABLE`{.interpreted-text role="ref"}
3.  Enable `CONFIG_FREERTOS_USE_TICKLESS_IDLE`{.interpreted-text role="ref"}
...
esp32c6 or esp32c5
  ...
  Enable RTOS Tickless IDLE mode (`CONFIG_FREERTOS_USE_TICKLESS_IDLE`{.interpreted-text role="ref"})                            ON

  RTOS Tick rate (Hz) (`CONFIG_FREERTOS_HZ`{.interpreted-text role="ref"})                                                      1000

  Minimum IDLE Tick count before entering sleep mode (`CONFIG_FREERTOS_IDLE_TIME_BEFORE_SLEEP`{.interpreted-text role="ref"})   3

  Put light sleep related codes in IRAM (`CONFIG_PM_SLP_IRAM_OPT`{.interpreted-text role="ref"})                                OFF

  Put RTOS IDLE related codes in IRAM (`CONFIG_PM_RTOS_IDLE_OPT`{.interpreted-text role="ref"})                                 OFF

  RTC slow clock source (`CONFIG_RTC_CLK_SRC`{.interpreted-text role="ref"})                                                    Internal 150 kHz OSC

  Disable all GPIO when chip at sleep (`CONFIG_PM_SLP_DISABLE_GPIO`{.interpreted-text role="ref"})                              ON

  Power down MAC and baseband (`CONFIG_ESP_PHY_MAC_BB_PD`{.interpreted-text role="ref"})                                        ON

  Power down CPU (`CONFIG_PM_POWER_DOWN_CPU_IN_LIGHT_SLEEP`{.interpreted-text role="ref"})                                      ON

  Power down flash in light sleep (`CONFIG_ESP_SLEEP_POWER_DOWN_FLASH`{.interpreted-text role="ref"})                           OFF

  Set flash into deep power-down mode in light sleep (`CONFIG_ESP_SLEEP_SET_FLASH_DPD`{.interpreted-text role="ref"})           OFF
...
esp32s3
  ...
  Power down I/D-cache tag memory (`CONFIG_PM_RESTORE_CACHE_TAGMEM_AFTER_LIGHT_SLEEP`{.interpreted-text role="ref"})            ON
