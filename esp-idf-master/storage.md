---
original_file_path: migration-guides/release-6.x/6.0/storage.rst
---
...
- TERMIOS support is now disabled by default. Re-enable `CONFIG_VFS_SUPPORT_TERMIOS`{.interpreted-text role="ref"} in menuconfig if your application calls POSIX `termios` APIs, such as `tcsetattr`/`tcgetattr` for UART configuration.
...
- Dynamic buffers (`CONFIG_FATFS_USE_DYN_BUFFERS`{.interpreted-text role="ref"}) now default to enabled to trim static memory usage when multiple volumes are mounted; turn this off in menuconfig if you prefer static allocation.
- Long filename support now defaults to heap-based buffers (`CONFIG_FATFS_LFN_HEAP=y`) so filenames longer than 8.3 work out of the box; you can disable it in menuconfig (`CONFIG_FATFS_LONG_FILENAMES`{.interpreted-text role="ref"}) if you need to conserve heap.
