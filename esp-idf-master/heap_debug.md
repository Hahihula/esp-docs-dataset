---
original_file_path: api-reference/system/heap_debug.rst
---
...
ESP-IDF integrates tools for requesting `heap information <heap-information>`{.interpreted-text role="ref"}, `heap corruption detection <heap-corruption>`{.interpreted-text role="ref"}, and `heap tracing <heap-tracing>`{.interpreted-text role="ref"}. These can help track down memory-related bugs.

For general information about the heap memory allocator, see `Heap Memory Allocation </api-reference/system/mem_alloc>`{.interpreted-text role="doc"}.

## Heap Information

To obtain information about the state of the heap, call the following functions:

- `heap_caps_get_free_size`{.interpreted-text role="cpp:func"} can be used to return the current free memory for different memory capabilities.
- `heap_caps_get_largest_free_block`{.interpreted-text role="cpp:func"} can be used to return the largest free block in the heap, which is also the largest single allocation currently possible. Tracking this value and comparing it to the total free heap allows you to detect heap fragmentation.
- `heap_caps_get_minimum_free_size`{.interpreted-text role="cpp:func"} can be used to track the heap \"low watermark\" since boot.
- `heap_caps_get_info`{.interpreted-text role="cpp:func"} returns a `multi_heap_info_t`{.interpreted-text role="cpp:class"} structure, which contains the information from the above functions, plus some additional heap-specific data (number of allocations, etc.).
- `heap_caps_print_heap_info`{.interpreted-text role="cpp:func"} prints a summary of the information returned by `heap_caps_get_info`{.interpreted-text role="cpp:func"} to stdout.
- `heap_caps_dump`{.interpreted-text role="cpp:func"} and `heap_caps_dump_all`{.interpreted-text role="cpp:func"} output detailed information about the structure of each block in the heap. Note that this can be a large amount of output.
...
- Providing a definition of `esp_heap_trace_alloc_hook`{.interpreted-text role="cpp:func"} allows you to be notified of every successful memory allocation operation.
- Providing a definition of `esp_heap_trace_free_hook`{.interpreted-text role="cpp:func"} allows you to be notified of every successful memory-free operations.

This feature can be enabled by setting the `CONFIG_HEAP_USE_HOOKS`{.interpreted-text role="ref"} option. `esp_heap_trace_alloc_hook`{.interpreted-text role="cpp:func"} and `esp_heap_trace_free_hook`{.interpreted-text role="cpp:func"} have weak declarations (e.g., `__attribute__((weak))`), thus it is not necessary to provide declarations for both hooks. Given that it is technically possible to allocate and free memory from an ISR (**though strongly discouraged from doing so**), the `esp_heap_trace_alloc_hook`{.interpreted-text role="cpp:func"} and `esp_heap_trace_free_hook`{.interpreted-text role="cpp:func"} can potentially be called from an ISR.
...
## Memory Allocation Failed Hook {#heap-allocation-failed}

Users can use `heap_caps_register_failed_alloc_callback`{.interpreted-text role="cpp:func"} to register a callback that is invoked every time an allocation operation fails.

Additionally, users can enable the `CONFIG_HEAP_ABORT_WHEN_ALLOCATION_FAILS`{.interpreted-text role="ref"}, which will automatically trigger a system abort if any allocation operation fails.
...
The heap implementation (`heap/multi_heap.c`{.interpreted-text role="component_file"}, etc.) includes numerous assertions that will fail if the heap memory is corrupted. To detect heap corruption most effectively, ensure that assertions are enabled in the project configuration via the `CONFIG_COMPILER_OPTIMIZATION_ASSERTION_LEVEL`{.interpreted-text role="ref"} option.
...
- Adding regular calls to `heap_caps_check_integrity_all`{.interpreted-text role="cpp:func"} or `heap_caps_check_integrity_addr`{.interpreted-text role="cpp:func"} in your code helps you pin down the exact time that the corruption happened. You can move these checks around to \"close in on\" the section of code that corrupted the heap.
- Based on the memory address that has been corrupted, you can use `JTAG debugging <jtag-debugging-introduction>`{.interpreted-text role="ref"} to set a watchpoint on this address and have the CPU halt when it is written to.
- If you do not have JTAG, but you do know roughly when the corruption happens, set a watchpoint in software just beforehand via `esp_cpu_set_watchpoint`{.interpreted-text role="cpp:func"}. A fatal exception will occur when the watchpoint triggers. The following is an example of how to use the function - `esp_cpu_set_watchpoint(0, (void *)addr, 4, ESP_WATCHPOINT_STORE)`. Note that watchpoints are per-CPU and are set on the current running CPU only. So if you do not know which CPU is corrupting memory, call this function on both CPUs.
...
- Calling `heap_caps_dump`{.interpreted-text role="cpp:func"} or `heap_caps_dump_all`{.interpreted-text role="cpp:func"} can give an indication of what heap blocks are surrounding the corrupted region and may have overflowed or underflowed, etc.
...
In the project configuration menu, under `Component config`, there is a menu `Heap memory debugging`. The option `CONFIG_HEAP_CORRUPTION_DETECTION`{.interpreted-text role="ref"} can be set to one of the following three levels:
...
Calling `heap_caps_check_integrity`{.interpreted-text role="cpp:func"} in Basic mode checks the integrity of all heap structures, and print errors if any appear to be corrupted.
...
Each time `heap_caps_free`{.interpreted-text role="cpp:func"} is called in Light Impact mode, the head and tail canary bytes of the buffer being freed are checked against the expected values.

When `heap_caps_check_integrity`{.interpreted-text role="cpp:func"} or `heap_caps_check_integrity_all`{.interpreted-text role="cpp:func"} is called, all allocated blocks of heap memory have their canary bytes checked against the expected values.
...
Enabling Comprehensive mode has a substantial impact on runtime performance, as all memory needs to be set to the allocation patterns each time a `heap_caps_malloc`{.interpreted-text role="cpp:func"} or `heap_caps_free`{.interpreted-text role="cpp:func"} completes, and the memory also needs to be checked each time. However, this mode allows easier detection of memory corruptions which are much more subtle to find otherwise. It is recommended to only enable this mode when debugging, not in production.

The checks for allocated and free patterns (`0xCE` and `0xFE`, respectively) are also done when calling `heap_caps_check_integrity`{.interpreted-text role="cpp:func"} or `heap_caps_check_integrity_all`{.interpreted-text role="cpp:func"}.

##### Crashes in Comprehensive Mode

If an application crashes when reading or writing an address related to `0xCECECECE` in Comprehensive mode, it indicates that it has read uninitialized memory. The application should be changed to either use `heap_caps_calloc`{.interpreted-text role="cpp:func"} (which zeroes memory), or initialize the memory before using it. The value `0xCECECECE` may also be seen in stack-allocated automatic variables, because, in ESP-IDF, most task stacks are originally allocated from the heap, and in C, stack memory is uninitialized by default.
...
If a call to `heap_caps_malloc`{.interpreted-text role="cpp:func"} or `heap_caps_realloc`{.interpreted-text role="cpp:func"} causes a crash because it was expected to find the pattern `0xFEFEFEFE` in free memory and a different pattern was found, it indicates that the app has a use-after-free bug where it is writing to memory that has already been freed.

##### Manual Heap Checks in Comprehensive Mode

Calls to `heap_caps_check_integrity`{.interpreted-text role="cpp:func"} or `heap_caps_check_integrity_all`{.interpreted-text role="cpp:func"} may print errors relating to `0xFEFEFEFE`, `0xABBA1234`, or `0xBAAD5678`. In each case the checker is expected to find a given pattern, and will error out if not found:
...
The Heap Task Tracking can be enabled via the menuconfig: `Component config` \> `Heap memory debugging` \> `Enable heap task tracking` (see `CONFIG_HEAP_TASK_TRACKING`{.interpreted-text role="ref"}).
...
An additional configuration can be enabled by the user via the menuconfig: `Component config` \> `Heap memory debugging` \> `Keep information about the memory usage of deleted tasks` (see `CONFIG_HEAP_TRACK_DELETED_TASKS`{.interpreted-text role="ref"}) to keep the statistics collected for a given task even after it is deleted.
...
The `heap_caps_print_single_task_stat_overview`{.interpreted-text role="cpp:func"} API prints an overview of heap usage for a specific task to the provided output stream.
...
`heap_caps_print_all_task_stat_overview`{.interpreted-text role="cpp:func"} prints an overview of heap usage for all tasks (including the deleted tasks if `CONFIG_HEAP_TRACK_DELETED_TASKS`{.interpreted-text role="ref"} is enabled).
...
Use `heap_caps_print_single_task_stat`{.interpreted-text role="cpp:func"} to dump the complete set of statistics for a specific task, or `heap_caps_print_all_task_stat`{.interpreted-text role="cpp:func"} to dump statistics for all tasks:
...
The dump shown above has been truncated (see \"\[\...\]\") for readability reasons and only displays the statistics and information of the **main** task and the **Pre-scheduler**. The goal here is only to demonstrate the information displayed when calling the `heap_caps_print_all_task_stat`{.interpreted-text role="cpp:func"} (resp. `heap_caps_print_single_task_stat`{.interpreted-text role="cpp:func"}) API functions.
::::
...
Detailed use of the API functions described in this section can be found in `system/heap_task_tracking/basic`{.interpreted-text role="example"}.
::::

### Getting the Statistics And Information

`heap_caps_get_single_task_stat`{.interpreted-text role="cpp:func"} allows the user to access information of a specific task. The information retrieved by calling this API is identical to the one dumped using `heap_caps_print_single_task_stat`{.interpreted-text role="cpp:func"}.

`heap_caps_get_all_task_stat`{.interpreted-text role="cpp:func"} allows the user to access an overview of the information of all tasks (including the deleted tasks if `CONFIG_HEAP_TRACK_DELETED_TASKS`{.interpreted-text role="ref"} is enabled). The information retrieved by calling this API is identical to the one dumped using `heap_caps_print_all_task_stat`{.interpreted-text role="cpp:func"}.
...
The size of the arrays used to store information is difficult to estimate. Examples include the number of allocations per task, the number of heaps used by each task, and the number of tasks created since startup. Therefore, the heap task tracking also provides `heap_caps_alloc_single_task_stat_arrays`{.interpreted-text role="cpp:func"} (resp. `heap_caps_alloc_all_task_stat_arrays`{.interpreted-text role="cpp:func"}) to dynamically allocate the required amount of memory for those arrays.

Similarly, the heap task tracking also provides `heap_caps_free_single_task_stat_arrays`{.interpreted-text role="cpp:func"} (resp. `heap_caps_free_all_task_stat_arrays`{.interpreted-text role="cpp:func"}) to free the memory dynamically allocated when calling `heap_caps_alloc_single_task_stat_arrays`{.interpreted-text role="cpp:func"} (resp. `heap_caps_alloc_all_task_stat_arrays`{.interpreted-text role="cpp:func"}).
...
Detailed use of the API functions described in this section can be found in `system/heap_task_tracking/advanced`{.interpreted-text role="example"}.
::::
...
If you suspect a memory leak, the first step is to figure out which part of the program is leaking memory. Use the `heap_caps_get_free_size`{.interpreted-text role="cpp:func"} or related functions in `heap information <heap-information>`{.interpreted-text role="ref"} to track memory use over the life of the application. Try to narrow the leak down to a single function or sequence of functions where free memory always decreases and never recovers.

### Standalone Mode

Once you have identified the code which you think is leaking:

- Enable the `CONFIG_HEAP_TRACING_DEST`{.interpreted-text role="ref"} option.
- Call the function `heap_trace_init_standalone`{.interpreted-text role="cpp:func"} early in the program, to register a buffer that can be used to record the memory trace.
- Call the function `heap_trace_start`{.interpreted-text role="cpp:func"} to begin recording all mallocs or frees in the system. Call this immediately before the piece of code which you suspect is leaking memory.
- Call the function `heap_trace_stop`{.interpreted-text role="cpp:func"} to stop the trace once the suspect piece of code has finished executing. This state will stop the tracing of both allocations and frees.
- Call the function `heap_trace_alloc_pause`{.interpreted-text role="cpp:func"} to pause the tracing of new allocations while continuing to trace the frees. Call this immediately after the piece of code which you suspect is leaking memory to prevent any new allocations to be recorded.
- Call the function `heap_trace_dump`{.interpreted-text role="cpp:func"} to dump the results of the heap trace.
...
The above example output uses `IDF Monitor </api-guides/tools/idf-monitor>`{.interpreted-text role="doc"} to automatically decode PC addresses to their source files and line numbers.
...
`(NB: New entries were traced while dumping, so trace dump may have duplicate entries.)` will be logged in the summary if new entries are traced while calling `heap_trace_dump`{.interpreted-text role="cpp:func"} or `heap_trace_dump_caps`{.interpreted-text role="cpp:func"}.
::::
...
- `@ 0x...` is the heap address returned from `heap_caps_malloc`{.interpreted-text role="cpp:func"} or `heap_caps_calloc`{.interpreted-text role="cpp:func"} .
...
\- `ccount 0x...` is the CCOUNT (CPU cycle count) register value the allocation was made. The value is different for CPU 0 vs CPU 1. :CONFIG_IDF_TARGET_ARCH_XTENSA: - `caller 0x...` gives the call stack of the call to `heap_caps_malloc`{.interpreted-text role="cpp:func"}, as a list of PC addresses. These can be decoded to source files and line numbers, as shown above.
:::
...
:   - when memory is freed, the associated record field `freed by` is filled with the call stack of the call to `heap_caps_free`{.interpreted-text role="cpp:func"}, as a list of PC addresses. These can be decoded to source files and line numbers, as shown above.
...
For this reason, the option to use a hashmap mechanism to store records is available by enabling `Component config` \> `Heap Memory Debugging` \> `CONFIG_HEAP_TRACE_HASH_MAP`{.interpreted-text role="ref"} in the project configuration menu, allowing users to track significant amounts of records without suffering from drastic performance loss.

Each hashmap entry is a singly linked list of records sharing the same hash ID.

Each record hash ID is calculated based on the pointer to the memory they track. The hash function used is based on the Fowler-Noll-Vo hash function modified to ensure an even spread of all records in the range \[0, hashmap size\[ where hashmap size can be defined by setting `Component config` \> `Heap Memory Debugging` \> `CONFIG_HEAP_TRACE_HASH_MAP_SIZE`{.interpreted-text role="ref"} in the project configuration menu.
...
::: list
- The option `CONFIG_HEAP_TRACE_HASH_MAP_SIZE`{.interpreted-text role="ref"} defines the number of entries in the hashmap. The total number of records that can be stored is still defined by the user when calling `heap_trace_init_standalone`{.interpreted-text role="cpp:func"}. If `N` is the maximum number of records and `H` the number of entries in the hashmap, Then each entry will contain at max `N / H` records.

\- The hashmap complements the doubly-linked list and does not replace it. This means that the hashmap usage can create a significant memory overhead. :SOC_SPIRAM_SUPPORTED: - The memory used to store the hashmap is dynamically allocated (in internal memory by default) but by setting `Component config` \> `Heap Memory Debugging` \> `CONFIG_HEAP_TRACE_HASH_MAP_IN_EXT_RAM`{.interpreted-text role="ref"}, the user can force the hashmap in external memory (this option is available under the condition that `CONFIG_SPIRAM`{.interpreted-text role="ref"} is enabled).
:::
:::::
...
- In the project configuration menu, navigate to `Component config` \> `Heap Memory Debugging` \> `CONFIG_HEAP_TRACING_DEST`{.interpreted-text role="ref"} and select `Host-Based`.

- In the project configuration menu, navigate to `Component config` \> `ESP Trace Configuration` \> `Application Level Tracing` \> `Data Destination` `CONFIG_APPTRACE_DESTINATION`{.interpreted-text role="ref"} and select `JTAG`.
...
- Call the function `heap_trace_init_tohost`{.interpreted-text role="cpp:func"} early in the program, to initialize the JTAG heap tracing module.

- Call the function `heap_trace_start`{.interpreted-text role="cpp:func"} to begin recording all memory allocation and free calls in the system. Call this immediately before the piece of code which you suspect is leaking memory.
...
- Call the function `heap_trace_stop`{.interpreted-text role="cpp:func"} to stop the trace once the suspect piece of code has finished executing.
...
1.  Build the program and download it to the target as described in `Step 5. First Steps on ESP-IDF <get-started-build>`{.interpreted-text role="ref"}.
2.  Run OpenOCD (see `JTAG Debugging </api-guides/jtag-debugging/index>`{.interpreted-text role="doc"}).
...
Using this file GDB can connect to the target, reset it, and start tracing when the program hits breakpoint at `heap_trace_start`{.interpreted-text role="cpp:func"}. Tracing will be stopped when the program hits breakpoint at `heap_trace_stop`{.interpreted-text role="cpp:func"}. Traced data will be saved to `/tmp/heap_log.svdat`.

4.  Run GDB using `{IDF_TARGET_TOOLCHAIN_PREFIX}-gdb -x gdbinit </path/to/program/elf>`.
5.  Quit GDB when the program stops at `heap_trace_stop`{.interpreted-text role="cpp:func"}. Traced data are saved in `/tmp/heap.svdat`.
...
To mitigate the performance loss when the heap tracing is enabled and active, enable `CONFIG_HEAP_TRACE_HASH_MAP`{.interpreted-text role="ref"}. With this configuration enabled, a hash map mechanism will be used to handle the heap trace records, thus considerably decreasing the heap allocation or free execution time. The size of the hash map can be modified by setting the value of `CONFIG_HEAP_TRACE_HASH_MAP_SIZE`{.interpreted-text role="ref"}.

::: only
SOC_SPIRAM_SUPPORTED

By default, the hash map is placed into internal RAM. It can also be placed into external RAM if `CONFIG_HEAP_TRACE_HASH_MAP_IN_EXT_RAM`{.interpreted-text role="ref"} is enabled. In order to enable this configuration, make sure to enable `CONFIG_SPIRAM`{.interpreted-text role="ref"} and `CONFIG_SPIRAM_ALLOW_BSS_SEG_EXTERNAL_MEMORY`{.interpreted-text role="ref"}.
:::

### False-Positive Memory Leaks

Not everything printed by `heap_trace_dump`{.interpreted-text role="cpp:func"} is necessarily a memory leak. The following cases may also be printed:

- Any memory that is allocated after `heap_trace_start`{.interpreted-text role="cpp:func"} but freed after `heap_trace_stop`{.interpreted-text role="cpp:func"} appears in the leaked dump.
- Allocations may be made by other tasks in the system. Depending on the timing of these tasks, it is quite possible that this memory is freed after `heap_trace_stop`{.interpreted-text role="cpp:func"} is called.
- The first time a task uses stdio - e.g., when it calls `heap_caps_printf`{.interpreted-text role="cpp:func"} - a lock, i.e., RTOS mutex semaphore, is allocated by the libc. This allocation lasts until the task is deleted.
- Certain uses of `heap_caps_printf`{.interpreted-text role="cpp:func"}, such as printing floating point numbers and allocating some memory from the heap on demand. These allocations last until the task is deleted.
...
- `system/heap_task_tracking/basic`{.interpreted-text role="example"} demonstrates the use of the overview feature of the heap task tracking, dumping per-task summary statistics on heap memory usage.
- `system/heap_task_tracking/advanced`{.interpreted-text role="example"} demonstrates the use of the statistics getter functions of the heap task tracking, accessing per-task complete statistic on the heap memory usage.
