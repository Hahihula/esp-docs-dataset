---
original_file_path: api-reference/peripherals/spi_flash/spi_flash_concurrency.rst
---

# Concurrency Constraints for Flash on SPI1 {#concurrency-constraints-flash}
...
On {IDF_TARGET_NAME}, the config option `CONFIG_SPI_FLASH_AUTO_SUSPEND`{.interpreted-text role="ref"} allows the cache to read flash concurrently with SPI1 operations. This is an optional feature that depends on special SPI Flash models, hence disabled by default. See `auto-suspend`{.interpreted-text role="ref"} for more details.

If this option is disabled, the caches must be disabled while reading/writing/erasing operations. There are some constraints using driver on the SPI1 bus, see `impact_disabled_cache`{.interpreted-text role="ref"}. These constraints will cause more IRAM/DRAM usages.
:::

::: only
SOC_SPIRAM_XIP_SUPPORTED

On {IDF_TARGET_NAME}, the config options `CONFIG_SPIRAM_XIP_FROM_PSRAM`{.interpreted-text role="ref"} (disabled by default) allows the cache to read/write PSRAM concurrently with SPI1 operations. See `xip_from_psram`{.interpreted-text role="ref"} for more details.

If these options are disabled, the caches must be disabled while reading/writing/erasing operations. There are some constraints using driver on the SPI1 bus, see `impact_disabled_cache`{.interpreted-text role="ref"}. These constraints will cause more IRAM/DRAM usages.
:::

## When the Caches Are Disabled {#impact_disabled_cache}
...
When `CONFIG_SPI_FLASH_AUTO_SUSPEND`{.interpreted-text role="ref"} is enabled, these APIs will not disable the caches. The hardware will handle the arbitration between them.
::::
:::::
...
When `CONFIG_SPIRAM_XIP_FROM_PSRAM`{.interpreted-text role="ref"} is enabled, these APIs will not disable the caches.
::::
:::::
...
See also `esp_flash_os_func`{.interpreted-text role="ref"} and `spi_bus_lock`{.interpreted-text role="ref"}.

There are no such constraints and impacts for flash chips on other SPI buses than SPI0/1.

For differences between internal RAM (e.g., IRAM, DRAM) and flash cache, please refer to the `application memory layout <memory-layout>`{.interpreted-text role="ref"} documentation.

### IRAM-Safe Interrupt Handlers

For interrupt handlers which need to execute when the cache is disabled (e.g., for low latency operations), set the `ESP_INTR_FLAG_IRAM` flag when the `interrupt handler is registered </api-reference/system/intr_alloc>`{.interpreted-text role="doc"}.

You must ensure that all data and functions accessed by these interrupt handlers, including the ones that handlers call, are located in IRAM or DRAM. See `how-to-place-code-in-iram`{.interpreted-text role="ref"}.
...
When working with strings in ISRs, it is not advised to use `printf` and other output functions. For debugging purposes, use `ESP_DRAM_LOGE`{.interpreted-text role="cpp:func"} and similar macros when logging from ISRs. Make sure that both `TAG` and format string are placed into `DRAM` in that case.
::::
...
## Flash Auto Suspend Feature {#auto-suspend}
...
When `CONFIG_SPI_FLASH_AUTO_SUSPEND`{.interpreted-text role="ref"} is enabled, the caches will be kept enabled. They would be disabled if `CONFIG_SPI_FLASH_AUTO_SUSPEND`{.interpreted-text role="ref"} is disabled. The hardware handles the arbitration between SPI0 and SPI1. If the SPI1 operation is short, such as a reading operation, the CPU and the cache will wait until the SPI1 operation is completed. However, during processes like erasing, page programming, or status register writing (e.g., `SE`, `PP`, and `WRSR`), an auto suspend will happen, interrupting the ongoing flash operation. This allows the CPU to access data from the cache and flash within limited time.
...
Regarding the flash suspend feature usage and corresponding response time delay, please also see the `system/flash_suspend`{.interpreted-text role="example"} example.
...
::: wavedrom
/../\_static/diagrams/spi_flash/flash_auto_suspend_timing.json
:::
...
The `tsus` time mentioned in point 2 can be found by looking through the flash datasheets, usually in the AC CHARACTERISTICS section. Users needs to make sure that the `tsus` value obtained from the datasheets is not greater than the `CONFIG_SPI_FLASH_SUSPEND_TSUS_VAL_US`{.interpreted-text role="ref"} value in Kconfig.
...
## XIP from PSRAM Feature {#xip_from_psram}

If `CONFIG_SPIRAM_XIP_FROM_PSRAM`{.interpreted-text role="ref"} is enabled, the flash `.text` sections (used for instructions) and the flash `.rodata` sections (used for read only data) will be placed in PSRAM.
