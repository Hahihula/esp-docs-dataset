---
original_file_path: api-guides/coexist.rst
---
...
::: {.blockdiag scale="100%" caption="Coexistence Mechanism" align="center"}
blockdiag {
...
### Coexistence Policy {#coexist_policy}
...
<img src="../../_static/coexist_wifi_connected_and_ble_connected_time_slice.png" alt="Time Slice Under the Status of Wi-Fi CONNECTED and BLE CONNECTED" />
<figcaption aria-hidden="true">Time Slice Under the Status of Wi-Fi CONNECTED and BLE CONNECTED</figcaption>
</figure>
::::::
...
To some extent, some combinations of connectionless power-saving parameters [Window]{.title-ref} and [Interval]{.title-ref} would lead to extra Wi-Fi priority request out of Wi-Fi time slice. It\`s for obtaining RF resources at coexistence for customized parameters, while leading to impact on Bluetooth performance.
...
Please refer to `connectionless module power save <connectionless-module-power-save>`{.interpreted-text role="ref"} to get more detail.
:::
...
::: list
\- After writing the coexistence program, you must check `CONFIG_ESP_COEX_SW_COEXIST_ENABLE`{.interpreted-text role="ref"} option through menuconfig to open coexistence configuration on software, otherwise the coexistence function mentioned above cannot be used. :esp32: - To ensure better communication performance of Wi-Fi and Bluetooth in the case of coexistence, run the task of the Wi-Fi protocol stack, the task of the Bluetooth Controller and Host protocol stack on different CPUs. You can use `CONFIG_BTDM_CTRL_PINNED_TO_CORE_CHOICE`{.interpreted-text role="ref"} and `CONFIG_BT_BLUEDROID_PINNED_TO_CORE_CHOICE`{.interpreted-text role="ref"} (or `CONFIG_BT_NIMBLE_PINNED_TO_CORE_CHOICE`{.interpreted-text role="ref"}) to put the tasks of the Bluetooth controller and the host protocol stack on the same CPU, and then use `CONFIG_ESP_WIFI_TASK_CORE_ID`{.interpreted-text role="ref"} to place the task of the Wi-Fi protocol stack on another CPU. :esp32s3: - To ensure better communication performance of Wi-Fi and Bluetooth in the case of coexistence, run the task of the Wi-Fi protocol stack, the task of the Bluetooth Controller and Host protocol stack on different CPUs. You can use `CONFIG_BT_CTRL_PINNED_TO_CORE_CHOICE`{.interpreted-text role="ref"} and `CONFIG_BT_BLUEDROID_PINNED_TO_CORE_CHOICE`{.interpreted-text role="ref"} (or `CONFIG_BT_NIMBLE_PINNED_TO_CORE_CHOICE`{.interpreted-text role="ref"} ）to put the tasks of the Bluetooth controller and the host protocol stack on the same CPU, and then use `CONFIG_ESP_WIFI_TASK_CORE_ID`{.interpreted-text role="ref"} to place the task of the Wi-Fi protocol stack on another CPU. :esp32: - In the case of coexistence, BLE SCAN may be interrupted by Wi-Fi and Wi-Fi releases RF resources before the end of the current BLE scan window. In order to make BLE acquire RF resources again within the current scan window, you can check the FULL SCAN configuration option through `CONFIG_BTDM_CTRL_FULL_SCAN_SUPPORTED`{.interpreted-text role="ref"}. :esp32c3 or esp32s3: - When using LE Coded PHY during a BLE connection, to avoid affecting Wi-Fi performance due to the long duration of Bluetooth packets, you can select [BT_CTRL_COEX_PHY_CODED_TX_RX_TLIM_EN]{.title-ref} in the sub-options of `CONFIG_BT_CTRL_COEX_PHY_CODED_TX_RX_TLIM`{.interpreted-text role="ref"} to limit the maximum time of TX/RX. :esp32c2 or esp32c6: - When using LE Coded PHY during a BLE connection, to avoid affecting Wi-Fi performance due to the long duration of Bluetooth packets, you can select [BT_LE_COEX_PHY_CODED_TX_RX_TLIM_EN]{.title-ref} in the sub-options of `CONFIG_BT_LE_COEX_PHY_CODED_TX_RX_TLIM`{.interpreted-text role="ref"} to limit the maximum time of TX/RX. :SOC_BT_SUPPORTED or SOC_WIFI_SUPPORTED: - You can reduce the memory consumption by configuring the following options on menuconfig.

> ::: only
> SOC_BT_SUPPORTED
>
> - `CONFIG_BT_BLE_DYNAMIC_ENV_MEMORY`{.interpreted-text role="ref"}: enable the configuration of dynamic memory for Bluetooth protocol stack.
> :::
>
> ::: only
> SOC_WIFI_SUPPORTED
>
> - `CONFIG_ESP_WIFI_STATIC_RX_BUFFER_NUM`{.interpreted-text role="ref"}: reduce the number of Wi-Fi static RX buffers.
> - `CONFIG_ESP_WIFI_DYNAMIC_RX_BUFFER_NUM`{.interpreted-text role="ref"}: reduce the number of Wi-Fi dynamic RX buffers.
> - `CONFIG_ESP_WIFI_TX_BUFFER`{.interpreted-text role="ref"}: enable the configuration of dynamic allocation TX buffers.
> - `CONFIG_ESP_WIFI_DYNAMIC_TX_BUFFER_NUM`{.interpreted-text role="ref"}: reduce the number of Wi-Fi dynamic TX buffers.
> - `CONFIG_ESP_WIFI_TX_BA_WIN`{.interpreted-text role="ref"}: reduce the number of Wi-Fi Block Ack TX windows.
> - `CONFIG_ESP_WIFI_RX_BA_WIN`{.interpreted-text role="ref"}: reduce the number of Wi-Fi Block Ack RX windows.
> - `CONFIG_ESP_WIFI_MGMT_SBUF_NUM`{.interpreted-text role="ref"}: reduce the number of Wi-Fi Management Short Buffer.
> - `CONFIG_ESP_WIFI_RX_IRAM_OPT`{.interpreted-text role="ref"}: turning off this configuration option will reduce the IRAM memory by approximately 17 KB.
> - `CONFIG_LWIP_TCP_SND_BUF_DEFAULT`{.interpreted-text role="ref"}: reduce the default TX buffer size for TCP sockets.
> - `CONFIG_LWIP_TCP_WND_DEFAULT`{.interpreted-text role="ref"}: reduce the default size of the RX window for TCP sockets.
> - `CONFIG_LWIP_TCP_RECVMBOX_SIZE`{.interpreted-text role="ref"}: reduce the size of the TCP receive mailbox. Receive mailbox buffers data within active connections and handles data flow during connections.
> - `CONFIG_LWIP_TCP_ACCEPTMBOX_SIZE`{.interpreted-text role="ref"}: reduce the size of the TCP accept mailbox. Accept mailbox queues incoming connection requests and manages the initiation of new connections.
> - `CONFIG_LWIP_UDP_RECVMBOX_SIZE`{.interpreted-text role="ref"}: reduce the size of the UDP receive mailbox.
> - `CONFIG_LWIP_TCPIP_RECVMBOX_SIZE`{.interpreted-text role="ref"}: reduce the size of TCPIP task receive mailbox.
> :::
:::
