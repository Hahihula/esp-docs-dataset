---
original_file_path: api-reference/peripherals/usb_device.rst
---
...
{IDF_TARGET_USB_DP_GPIO_NUM:default=\"20\", esp32h4=\"22\"} {IDF_TARGET_USB_DM_GPIO_NUM:default=\"19\", esp32h4=\"21\"} {IDF_TARGET_USB_EP_NUM: default=\"6\", esp32p4=\"15\"} {IDF_TARGET_USB_EP_NUM_INOUT:default=\"5\", esp32p4=\"8\"} {IDF_TARGET_USB_EP_NUM_IN:default=\"1\", esp32p4=\"7\"}
...
The Device Stack is built around the TinyUSB stack, but extends TinyUSB with some minor features and modifications for better integration with ESP-IDF. The Device stack is distributed as a managed component via the [ESP Component Registry](https://components.espressif.com/components/espressif/esp_tinyusb).
...
<img src="../../../_static/usb-board-connection.png" alt="Connection of an USB GPIOs directly to a USB standard-A plug" />
</figure>
...
Self-powered devices must also connect VBUS through a voltage divider or comparator. For more details, please refer to `self-powered-device`{.interpreted-text role="ref"}.
::::
...
An external PHY is not the only way to enable debugging alongside USB Host or Device functionality. It is also possible to switch the debugging interface from USB-Serial-JTAG to plain JTAG by burning the appropriate eFuses. For details, refer to document `JTAG Debugging <../../api-guides/jtag-debugging/index>`{.interpreted-text role="doc"} in ESP-IDF Programming Guide for your target.
::::
...
<figure class="align-center">
<img src="../../../_static/usb_device/usb_fs_phy_sp5301.png" alt="usb_fs_phy_sp5301" />
<figcaption>A typical circuit diagram for an external PHY</figcaption>
</figure>
...
<figure class="align-center">
<img src="../../../_static/usb_device/ext_phy_schematic_stusb03e.png" alt="External PHY with Analog Switch Schematic (Device mode)" />
<figcaption>Example connection using STUSB03E and analog switch (Device mode)</figcaption>
</figure>
...
The `usb_phy_ext_io_conf_t::suspend_n_io_num`{.interpreted-text role="cpp:member"} pin is **currently not supported** and does not need to be connected. The `usb_phy_ext_io_conf_t::fs_edge_sel_io_num`{.interpreted-text role="cpp:member"} pin is optional and only required if switching between low-speed and full-speed modes is needed.
::::

Starting from version 2.0, the ESP TinyUSB Device Stack supports external PHY usage. To use an external PHY in device mode:

1.  Configure the GPIO mapping and PHY using `usb_phy_config_t`{.interpreted-text role="cpp:type"}.
2.  Create the PHY using `usb_new_phy()`{.interpreted-text role="cpp:func"}.
3.  Use `TINYUSB_DEFAULT_CONFIG()`{.interpreted-text role="cpp:func"} to initialize `tinyusb_config_t`{.interpreted-text role="cpp:type"}.
4.  Set the [phy.skip_setup]{.title-ref} field of `tinyusb_config_t`{.interpreted-text role="cpp:type"} to `true` to bypass PHY reinitialization and use the externally configured PHY.
...
The Device Stack is distributed via the [ESP Component Registry](https://components.espressif.com/components/espressif/esp_tinyusb). Thus, to use it, please add the Device Stack component as dependency using the following command:
...
### Descriptor Configuration {#descriptors-configuration}

The `tinyusb_config_t`{.interpreted-text role="cpp:type"} structure provides USB descriptor related fields that should be initialized.

The following descriptors should be initialized for both full-speed and high-speed devices:

- `device_descriptor`{.interpreted-text role="cpp:member"}
- `string_descriptor`{.interpreted-text role="cpp:member"}

Full-speed devices should initialize the following field to provide their configuration descriptor:

- `configuration_descriptor`{.interpreted-text role="cpp:member"}
...
- `fs_configuration_descriptor`{.interpreted-text role="cpp:member"}
- `hs_configuration_descriptor`{.interpreted-text role="cpp:member"}
- `qualifier_descriptor`{.interpreted-text role="cpp:member"}
...
Both `fs_configuration_descriptor`{.interpreted-text role="cpp:member"} and `hs_configuration_descriptor`{.interpreted-text role="cpp:member"} must be present to comply with USB 2.0 specification.
::::
:::::

The Device Stack will instantiate a USB device based on the descriptors provided in the fields described above when `tinyusb_driver_install`{.interpreted-text role="cpp:func"} is called.

The Device Stack also provides default descriptors that can be installed by setting the corresponding field in `tinyusb_driver_install`{.interpreted-text role="cpp:func"} to `NULL`. Default descriptors include:

- Default device descriptor: Enabled by setting `device_descriptor`{.interpreted-text role="cpp:member"} to `NULL`. Default device descriptor will use the values set by the corresponding menuconfig options (e.g., PID, VID, bcdDevice etc).

- Default string descriptor: Enabled by setting `string_descriptor`{.interpreted-text role="cpp:member"} to `NULL`. Default string descriptors will use the value set by corresponding menuconfig options (e.g., manufacturer, product, and serial string descriptor options).
...
- Default configuration descriptor. Some classes that rarely require custom configuration (such as CDC and MSC) will provide default configuration descriptors. These can be enabled by setting associated configuration descriptor field to `NULL`:
  ...
  > - `configuration_descriptor`{.interpreted-text role="cpp:member"}: full-speed descriptor for full-speed devices only
  > - `fs_configuration_descriptor`{.interpreted-text role="cpp:member"}: full-speed descriptor for high-speed devices
  > - `hs_configuration_descriptor`{.interpreted-text role="cpp:member"}: high-speed descriptor for high-speed devices
...
For backward compatibility, when Device Stack supports high-speed, the field `configuration_descriptor`{.interpreted-text role="cpp:member"} could be used instead of `fs_configuration_descriptor`{.interpreted-text role="cpp:member"} for full-speed configuration descriptor.
::::

## Installation

To install the Device Stack, please call `tinyusb_driver_install`{.interpreted-text role="cpp:func"}. The Device Stack\'s configuration is specified in a `tinyusb_config_t`{.interpreted-text role="cpp:type"} structure that is passed as an argument to `tinyusb_driver_install`{.interpreted-text role="cpp:func"}.
...
The `tinyusb_config_t`{.interpreted-text role="cpp:type"} structure can be zero-initialized (e.g., `const tinyusb_config_t tusb_cfg = { 0 };`) or partially (as shown below). For any member that is initialized to `0` or `NULL`, the stack uses its default configuration values for that member, see example below.
::::
...
<figure class="align-center">
<img src="../../../_static/diagrams/usb/usb_vbus_voltage_monitor.png" alt="Simple voltage divider for VBUS monitoring" />
<figcaption aria-hidden="true">Simple voltage divider for VBUS monitoring</figcaption>
</figure>

To use this feature, in `tinyusb_config_t`{.interpreted-text role="cpp:type"}, you must set `self_powered`{.interpreted-text role="cpp:member"} to `true` and `vbus_monitor_io`{.interpreted-text role="cpp:member"} to GPIO number that is used for VBUS monitoring.

## USB Serial Device (CDC-ACM)

If the CDC option is enabled in Menuconfig, the USB Serial Device can be initialized with `tusb_cdc_acm_init`{.interpreted-text role="cpp:func"} according to the settings from `tinyusb_config_cdcacm_t`{.interpreted-text role="cpp:type"}, see example below.
...
To specify callbacks, you can either set the pointer to your `tusb_cdcacm_callback_t`{.interpreted-text role="cpp:type"} function in the configuration structure or call `tinyusb_cdcacm_register_callback`{.interpreted-text role="cpp:func"} after initialization.
...
Users should call `esp_tusb_init_console`{.interpreted-text role="cpp:func"} to switch the standard input/output streams to USB, and `esp_tusb_deinit_console`{.interpreted-text role="cpp:func"} to switch them back to UART.
...
The examples can be found in the directory `peripherals/usb/device`{.interpreted-text role="example"}.

- `peripherals/usb/device/tusb_console`{.interpreted-text role="example"} demonstrates how to set up {IDF_TARGET_NAME} to get log output via a Serial Device connection using the TinyUSB component, applicable for any Espressif boards that support USB-OTG.
- `peripherals/usb/device/tusb_serial_device`{.interpreted-text role="example"} demonstrates how to set up {IDF_TARGET_NAME} to function as a USB Serial Device using the TinyUSB component, with the ability to be configured as a double serial device.
- `peripherals/usb/device/tusb_midi`{.interpreted-text role="example"} demonstrates how to set up {IDF_TARGET_NAME} to function as a USB MIDI Device, outputting a MIDI note sequence via the native USB port using the TinyUSB component.
- `peripherals/usb/device/tusb_hid`{.interpreted-text role="example"} demonstrates how to implement a USB keyboard and mouse using the TinyUSB component, which sends \'key a/A pressed & released\' events and moves the mouse in a square trajectory upon connection to a USB host.
- `peripherals/usb/device/tusb_msc`{.interpreted-text role="example"} demonstrates how to use the USB capabilities to create a Mass Storage Device that can be recognized by USB-hosts, allowing access to its internal data storage, with support for SPI Flash and SD MMC Card storage media.
- `peripherals/usb/device/tusb_composite_msc_serialdevice`{.interpreted-text role="example"} demonstrates how to set up {IDF_TARGET_NAME} to function simultaneously as both a USB Serial Device and an MSC device (SPI-Flash as the storage media) using the TinyUSB component.

::: only
not esp32p4 and not esp32h4

- `peripherals/usb/device/tusb_ncm`{.interpreted-text role="example"} demonstrates how to transmit Wi-Fi data to a Linux or Windows host via USB using the Network Control Model (NCM), a sub-class of Communication Device Class (CDC) USB Device for Ethernet-over-USB applications, with the help of a TinyUSB component.
:::
