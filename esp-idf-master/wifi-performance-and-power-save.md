---
original_file_path: api-guides/wifi-driver/wifi-performance-and-power-save.rst
---
...
## Wi-Fi Buffer Usage {#wifi-buffer-usage}
...
lwIP also allocates buffers at the TCP/IP layer, and this buffer allocation is also dynamic. See `lwIP documentation section about memory use and performance <lwip-performance>`{.interpreted-text role="ref"}.

### Peak Wi-Fi Dynamic Buffer

The Wi-Fi driver supports several types of buffer (refer to `wifi-buffer-configure`{.interpreted-text role="ref"}). However, this section is about the usage of the dynamic Wi-Fi buffer only. The peak heap memory that Wi-Fi consumes is the **theoretically-maximum memory** that the Wi-Fi driver consumes. Generally, the peak memory depends on:
...
When the throughput is tested by iperf example, the sdkconfig is `examples/wifi/iperf/sdkconfig.defaults.esp32`{.interpreted-text role="idf_file"}.
:::
...
When the throughput is tested by iperf example, the sdkconfig is `examples/wifi/iperf/sdkconfig.defaults.esp32s2`{.interpreted-text role="idf_file"}.
:::
...
When the throughput is tested by iperf example, the sdkconfig is `examples/wifi/iperf/sdkconfig.defaults.esp32c3`{.interpreted-text role="idf_file"}.
:::
...
When the throughput is tested by iperf example, the sdkconfig is `examples/wifi/iperf/sdkconfig.defaults.esp32c5`{.interpreted-text role="idf_file"}.
:::
...
When the throughput is tested by iperf example, the sdkconfig is `examples/wifi/iperf/sdkconfig.defaults.esp32c6`{.interpreted-text role="idf_file"}.
:::
...
When the throughput is tested by iperf example, the sdkconfig is `examples/wifi/iperf/sdkconfig.defaults.esp32c61`{.interpreted-text role="idf_file"}.
:::
...
When the throughput is tested by iperf example, the sdkconfig is `examples/wifi/iperf/sdkconfig.defaults.esp32s3`{.interpreted-text role="idf_file"}.
:::

## How to Improve Wi-Fi Performance {#How-to-improve-Wi-Fi-performance}
...
<img src="../../../_static/api-guides-WiFi-driver-how-to-improve-WiFi-performance.png" alt="{IDF_TARGET_NAME} datapath" />
<figcaption aria-hidden="true">{IDF_TARGET_NAME} datapath</figcaption>
</figure>
...
> - 
>
>   `CONFIG_ESP_WIFI_STATIC_RX_BUFFER_NUM`{.interpreted-text role="ref"}
>
...
> - 
>
>   `CONFIG_ESP_WIFI_DYNAMIC_RX_BUFFER_NUM`{.interpreted-text role="ref"}
>
...
> - 
>
>   `CONFIG_ESP_WIFI_RX_BA_WIN`{.interpreted-text role="ref"}
>
>   :   This parameter indicates the size of the AMPDU BA Window at the receiving end. This parameter should be configured to the smaller value between twice of `CONFIG_ESP_WIFI_STATIC_RX_BUFFER_NUM`{.interpreted-text role="ref"} and `CONFIG_ESP_WIFI_DYNAMIC_RX_BUFFER_NUM`{.interpreted-text role="ref"}.
>
> - 
>
>   `CONFIG_LWIP_TCP_WND_DEFAULT`{.interpreted-text role="ref"}
>
...
> - 
>
>   `CONFIG_ESP_WIFI_TX_BUFFER`{.interpreted-text role="ref"}
>
...
> - 
>
>   `CONFIG_ESP_WIFI_DYNAMIC_TX_BUFFER_NUM`{.interpreted-text role="ref"}
>
...
> - 
>
>   `CONFIG_LWIP_TCP_SND_BUF_DEFAULT`{.interpreted-text role="ref"}
>
...
\- `CONFIG_LWIP_IRAM_OPTIMIZATION`{.interpreted-text role="ref"} If this option is enabled, some LwIP functions are moved to IRAM, improving throughput. This increases IRAM usage by 13 kB.
:::
...
- 
  ...
  `CONFIG_LWIP_IRAM_OPTIMIZATION`{.interpreted-text role="ref"}
...
> - 
>
>   `CONFIG_ESP32S2_INSTRUCTION_CACHE_LINE_SIZE`{.interpreted-text role="ref"}
>
...
> - 
>
>   `CONFIG_ESP32S3_INSTRUCTION_CACHE_LINE_SIZE`{.interpreted-text role="ref"}
>
>   :   Configure the size of the instruction Cache bus.
>
> - 
>
>   `CONFIG_ESP32S3_ICACHE_ASSOCIATED_WAYS`{.interpreted-text role="ref"}
>
...
> - 
>
>   `CONFIG_ESP32S3_DATA_CACHE_LINE_SIZE`{.interpreted-text role="ref"}
>
>   :   Configure the line size of the Data Cache.
>
> - 
>
>   `CONFIG_ESP32S3_DCACHE_ASSOCIATED_WAYS`{.interpreted-text role="ref"}
>
...
The test was performed with a single stream in a shielded box using an ASUS RT-N66U router. {IDF_TARGET_NAME}\'s CPU is dual core with 240 MHz. {IDF_TARGET_NAME}\'s flash is in QIO mode with 80 MHz.
::::
:::::
...
The test was performed with a single stream in a shielded box using an ASUS RT-N66U router. {IDF_TARGET_NAME}\'s CPU is single core with 240 MHz. {IDF_TARGET_NAME}\'s flash is in QIO mode with 80 MHz.
::::
:::::
...
The test was performed with a single stream in a shielded box using an ASUS RT-N66U router. {IDF_TARGET_NAME}\'s CPU is single core with 160 MHz. {IDF_TARGET_NAME}\'s flash is in QIO mode with 80 MHz.
::::
:::::
...
The test was performed with a single stream in a shielded box using an XIAOMI AX-6000 router. {IDF_TARGET_NAME}\'s CPU is single core with 160 MHz. {IDF_TARGET_NAME}\'s flash is in QIO mode with 80 MHz.
::::
:::::
...
The test was performed with a single stream in a shielded box using an Redmi RM2100 router. {IDF_TARGET_NAME}\'s CPU is single core with 120 MHz. {IDF_TARGET_NAME}\'s flash is in QIO mode with 60 MHz.
::::
:::::
...
PSRAM is generally used when the application takes up a lot of memory. In this mode, the `CONFIG_ESP_WIFI_TX_BUFFER`{.interpreted-text role="ref"} is forced to be static. `CONFIG_ESP_WIFI_STATIC_TX_BUFFER_NUM`{.interpreted-text role="ref"} indicates the number of DMA buffers at the hardware layer, and increasing this parameter can improve performance. The following are the recommended ranks for using PSRAM:
...
This subsection will briefly introduce the concepts and usage related to Wi-Fi Power Saving Mode, for a more detailed introduction please refer to the `Low Power Mode User Guide <../../api-guides/low-power-mode/index>`{.interpreted-text role="doc"}.
...
Note that in coexist mode, Wi-Fi will remain active only during Wi-Fi time slice, and sleep during non Wi-Fi time slice even if `esp_wifi_set_ps(WIFI_PS_NONE)` is called. Please refer to `coexist policy <coexist_policy>`{.interpreted-text role="ref"}.
:::
...
Disconnected state is the duration without Wi-Fi connection between `esp_wifi_start`{.interpreted-text role="cpp:func"} to `esp_wifi_stop`{.interpreted-text role="cpp:func"}.

Currently, {IDF_TARGET_NAME} Wi-Fi supports sleep mode in disconnected state if running at station mode. This feature could be configured by Menuconfig choice `CONFIG_ESP_WIFI_STA_DISCONNECTED_PM_ENABLE`{.interpreted-text role="ref"}.

If `CONFIG_ESP_WIFI_STA_DISCONNECTED_PM_ENABLE`{.interpreted-text role="ref"} is enabled, RF, PHY and BB would be turned off in disconnected state when IDLE. The current would be same with current at modem-sleep.

The choice `CONFIG_ESP_WIFI_STA_DISCONNECTED_PM_ENABLE`{.interpreted-text role="ref"} would be selected by default, while it would be selected forcefully in Menuconfig at coexistence mode.

### Connectionless Modules Power-saving {#connectionless-module-power-save}

Connectionless modules are those Wi-Fi modules not relying on Wi-Fi connection, e.g ESP-NOW, DPP, FTM. These modules start from `esp_wifi_start`{.interpreted-text role="cpp:func"}, working until `esp_wifi_stop`{.interpreted-text role="cpp:func"}.
...
Meanwhile, `esp_wifi_80211_tx`{.interpreted-text role="cpp:func"} is supported at sleep as well.

#### Connectionless Modules RX

For each connectionless module, two parameters shall be configured to RX at sleep, which are [Window]{.title-ref} and [Interval]{.title-ref}.

At the start of [Interval]{.title-ref} time, RF, PHY, BB would be turned on and kept for [Window]{.title-ref} time. Connectionless Module could RX in the duration.

**Interval**

> - There is only one [Interval]{.title-ref}. Its configured by `esp_wifi_set_connectionless_interval`{.interpreted-text role="cpp:func"}. The unit is milliseconds.
> - The default value of [Interval]{.title-ref} is [ESP_WIFI_CONNECTIONLESS_INTERVAL_DEFAULT_MODE]{.title-ref}.
> - Event `wifi-event-connectionless-module-wake-interval-start`{.interpreted-text role="ref"} would be posted at the start of [Interval]{.title-ref}. Since [Window]{.title-ref} also starts at that moment, its recommended to TX in that event.
> - At connected state, the start of [Interval]{.title-ref} would be aligned with TBTT. To improve the packet reception success rate in connectionless modules, the sender and receiver can be connected to the same AP, and packets can be transmitted within the event `wifi-event-connectionless-module-wake-interval-start`{.interpreted-text role="ref"}. This synchronization helps align the connectionless modules transmission window.
>
> ::: only
> esp32
>
...
> - Each connectionless module has its own [Window]{.title-ref} after start. Connectionless Modules Power-saving would work with the max one among them.
> - [Window]{.title-ref} is configured by `module_name_set_wake_window`{.interpreted-text role="cpp:func"}. The unit is milliseconds.
> - The default value of [Window]{.title-ref} is the maximum.
...
If [Interval]{.title-ref} is `ESP_WIFI_CONNECTIONLESS_INTERVAL_DEFAULT_MODE` with non-zero [Window]{.title-ref}, Connectionless Modules Power-saving would work in default mode.
