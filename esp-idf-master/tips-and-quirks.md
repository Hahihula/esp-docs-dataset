---
original_file_path: api-guides/jtag-debugging/tips-and-quirks.rst
---
...
## Breakpoints and Watchpoints Available {#jtag-debugging-tip-breakpoints}

{IDF_TARGET_NAME} debugger supports {IDF_TARGET_SOC_CPU_BREAKPOINTS_NUM} hardware implemented breakpoints and 64 software ones. Hardware breakpoints are implemented by {IDF_TARGET_NAME} chip\'s logic and can be set anywhere in the code: either in flash or IRAM program\'s regions. Additionally there are 2 types of software breakpoints implemented by OpenOCD: flash (up to 32) and IRAM (up to 32) breakpoints. Currently GDB can not set software breakpoints in flash. So until this limitation is removed those breakpoints have to be emulated by OpenOCD as hardware ones (see `below <jtag-debugging-tip-where-breakpoints>`{.interpreted-text role="ref"} for details). {IDF_TARGET_NAME} also supports {IDF_TARGET_SOC_CPU_WATCHPOINTS_NUM} watchpoints, so {IDF_TARGET_SOC_CPU_WATCHPOINTS_NUM} variables can be watched for change or read by the GDB command `watch myVariable`. Note that menuconfig option `CONFIG_FREERTOS_WATCHPOINT_END_OF_STACK`{.interpreted-text role="ref"} uses the last watchpoint and will not provide expected results, if you also try to use it within OpenOCD/GDB. See menuconfig\'s help for detailed description.

## What Else Should I Know About Breakpoints? {#jtag-debugging-tip-where-breakpoints}
...
## Flash Mappings vs SW Flash Breakpoints {#jtag-debugging-tip-flash-mappings}
...
## Why Stepping with \"next\" Does Not Bypass Subroutine Calls? {#jtag-debugging-tip-why-next-works-as-step}
...
## Support Options for OpenOCD at Compile Time {#jtag-debugging-tip-code-options}

ESP-IDF has some support options for OpenOCD debugging which can be set at compile time:

- `CONFIG_ESP_DEBUG_OCDAWARE`{.interpreted-text role="ref"} is enabled by default. If a panic or unhandled exception is thrown and a JTAG debugger is connected (ie OpenOCD is running), ESP-IDF will break into the debugger.
- `CONFIG_FREERTOS_WATCHPOINT_END_OF_STACK`{.interpreted-text role="ref"} (disabled by default) sets watchpoint index 1 (the second of two) at the end of any task stack. This is the most accurate way to debug task stack overflows. Click the link for more details.

Please see the `project configuration menu for Windows <get-started-configure>`{.interpreted-text role="ref"}, `Linux, or macOS <get-started-configure-linux-macos>`{.interpreted-text role="ref"} menu for more details on setting compile-time options.

## FreeRTOS Support {#jtag-debugging-tip-freertos-support}

OpenOCD has explicit support for the ESP-IDF FreeRTOS. GDB can see FreeRTOS tasks as threads. Viewing them all can be done using the GDB `i threads` command, changing to a certain task is done with `thread n`, with `n` being the number of the thread. FreeRTOS detection can be disabled in target\'s configuration. For more details see `jtag-debugging-tip-openocd-configure-target`{.interpreted-text role="ref"}.

GDB has a Python extension for FreeRTOS support. ESP-IDF automatically loads this module into GDB with the `idf.py gdb` command when the system requirements are met. See more details in `jtag-debugging-examples-command-line-08`{.interpreted-text role="ref"}.

::: only
esp32

## Why to Set SPI Flash Voltage in OpenOCD Configuration? {#jtag-debugging-tip-code-flash-voltage}
...
## Configuration of OpenOCD for Specific Target {#jtag-debugging-tip-openocd-configure-target}
...
### OpenOCD Configuration Variables {#jtag-debugging-tip-openocd-config-vars}
...
## How Debugger Resets {IDF_TARGET_NAME}? {#jtag-debugging-tip-reset-by-debugger}

The board can be reset by entering `mon reset` or `mon reset halt` into GDB.

## Can JTAG Pins Be Used for Other Purposes? {#jtag-debugging-tip-jtag-pins-reconfigured}

::: only
SOC_USB_SERIAL_JTAG_SUPPORTED

{IDF_TARGET_NAME} contains a USB Serial/JTAG Controller which can be used for debugging. By default, {IDF_TARGET_NAME} JTAG interface is connected to the built-in USB SERIAL/JTAG peripheral. For details, please refer to `Configure {IDF_TARGET_NAME} built-in JTAG Interface <../jtag-debugging/configure-builtin-jtag>`{.interpreted-text role="doc"}.

When you use USB Serial/JTAG Controller for debugging, [\|jtag-gpio-list\|](##SUBST##|jtag-gpio-list|) can be used for other purposes.

However, if you switch the USB JTAG interface to the GPIOs by burning eFuses, [\|jtag-gpio-list\|](##SUBST##|jtag-gpio-list|) can be used for JTAG debugging. When they perform this function, they cannot be used for other purposes.
:::
...
## JTAG with Flash Encryption or Secure Boot {#jtag-debugging-security-features}
...
Please note that once JTAG is permanently disabled, it cannot be re-enabled for JTAG access. However, we do have the option of disabling JTAG softly. For more details on soft disabling and re-enabling soft-disabled JTAG, please refer to the `hmac_for_enabling_jtag`{.interpreted-text role="ref"}.
:::

The project configuration option `CONFIG_SECURE_BOOT_ALLOW_JTAG`{.interpreted-text role="ref"} will keep JTAG enabled at this time, removing all physical security but allowing debugging. (Although the name suggests Secure Boot, this option can be applied even when only Flash Encryption is enabled).

However, OpenOCD may attempt to automatically read and write the flash in order to set `software breakpoints <jtag-debugging-tip-where-breakpoints>`{.interpreted-text role="ref"}. This has two problems:
...
To disable software breakpoints while using JTAG, add an extra argument `-c 'set ESP_FLASH_SIZE 0'` to the start of the OpenOCD command line, see `jtag-debugging-tip-openocd-config-vars`{.interpreted-text role="ref"}.
...
To make JTAG available, build new firmware that is not using pins GPIO12 to GPIO15 dedicated to JTAG communication. After that, flash the firmware onto your module. See also `jtag-debugging-tip-jtag-pins-reconfigured`{.interpreted-text role="ref"}.
:::

## Reporting Issues with OpenOCD/GDB {#jtag-debugging-tip-reporting-issues}
