---
original_file_path: api-reference/system/freertos_additions.rst
---
...
The following example demonstrates the usage of `xRingbufferCreate`{.interpreted-text role="cpp:func"} and `xRingbufferSend`{.interpreted-text role="cpp:func"} to create a ring buffer and then send an item to it:
...
The following example demonstrates the usage of `xRingbufferSendAcquire`{.interpreted-text role="cpp:func"} and `xRingbufferSendComplete`{.interpreted-text role="cpp:func"} instead of `xRingbufferSend`{.interpreted-text role="cpp:func"} to acquire memory on the ring buffer (of type `RINGBUF_TYPE_NOSPLIT`{.interpreted-text role="cpp:enumerator"}) and then send an item to it. This adds one more step, but allows getting the address of the memory to write to, and writing to the memory yourself.
...
The following example demonstrates retrieving and returning an item from a **No-Split ring buffer** using `xRingbufferReceive`{.interpreted-text role="cpp:func"} and `vRingbufferReturnItem`{.interpreted-text role="cpp:func"}
...
The following example demonstrates retrieving and returning an item from an **Allow-Split ring buffer** using `xRingbufferReceiveSplit`{.interpreted-text role="cpp:func"} and `vRingbufferReturnItem`{.interpreted-text role="cpp:func"}
...
The following example demonstrates retrieving and returning an item from a **byte buffer** using `xRingbufferReceiveUpTo`{.interpreted-text role="cpp:func"} and `vRingbufferReturnItem`{.interpreted-text role="cpp:func"}
...
For ISR safe versions of the functions used above, call `xRingbufferSendFromISR`{.interpreted-text role="cpp:func"}, `xRingbufferReceiveFromISR`{.interpreted-text role="cpp:func"}, `xRingbufferReceiveSplitFromISR`{.interpreted-text role="cpp:func"}, `xRingbufferReceiveUpToFromISR`{.interpreted-text role="cpp:func"}, and `vRingbufferReturnItemFromISR`{.interpreted-text role="cpp:func"}.
...
::: {.packetdiag caption="Sending items to No-Split or Allow-Split ring buffers" align="center"}
...
::: {.packetdiag caption="Sending items to byte buffers" align="center"}
...
::: {.packetdiag caption="SendAcquire/SendComplete items in No-Split ring buffers" align="center"}
...
::: {.packetdiag caption="Wrap around in No-Split buffers" align="center"}
...
::: {.packetdiag caption="Wrap around in Allow-Split buffers" align="center"}
...
Allow-Split buffers treat both parts of the split item as two separate items, therefore call `xRingbufferReceiveSplit`{.interpreted-text role="cpp:func"} instead of `xRingbufferReceive`{.interpreted-text role="cpp:func"} to receive both parts of a split item in a thread safe manner.
::::

::: {.packetdiag caption="Wrap around in byte buffers" align="center"}
...
::: {.packetdiag caption="Retrieving/Returning items in No-Split and Allow-Split ring buffers" align="center"}
...
::: {.packetdiag caption="Retrieving/Returning data in byte buffers" align="center"}
../../../\_static/diagrams/ring-buffer/ring_buffer_read_ret_byte_buf.diag
:::

Byte buffers **do not allow multiple retrievals before returning** (every retrieval must be followed by a return before another retrieval is permitted). When using `xRingbufferReceive`{.interpreted-text role="cpp:func"} or `xRingbufferReceiveFromISR`{.interpreted-text role="cpp:func"}, all continuous stored data will be retrieved. `xRingbufferReceiveUpTo`{.interpreted-text role="cpp:func"} or `xRingbufferReceiveUpToFromISR`{.interpreted-text role="cpp:func"} can be used to restrict the maximum number of bytes retrieved. Since every retrieval must be followed by a return, the space is freed as soon as the data is returned.

Referring to the diagram above, the 38 bytes of continuous stored data at the tail of the buffer is retrieved, returned, and freed. The next call to `xRingbufferReceive`{.interpreted-text role="cpp:func"} or `xRingbufferReceiveFromISR`{.interpreted-text role="cpp:func"} then wraps around and does the same to the 30 bytes of continuous stored data at the head of the buffer.
...
Retrieving items from Allow-Split buffers must be done via `xRingbufferReceiveSplit`{.interpreted-text role="cpp:func"} or `xRingbufferReceiveSplitFromISR`{.interpreted-text role="cpp:func"} instead of `xRingbufferReceive`{.interpreted-text role="cpp:func"} or `xRingbufferReceiveFromISR`{.interpreted-text role="cpp:func"}.
::::

### Ring Buffers with Queue Sets

Ring buffers can be added to FreeRTOS queue sets using `xRingbufferAddToQueueSetRead`{.interpreted-text role="cpp:func"} such that every time a ring buffer receives an item or data, the queue set is notified. Once added to a queue set, every attempt to retrieve an item from a ring buffer should be preceded by a call to `xQueueSelectFromSet`{.interpreted-text role="cpp:func"}. To check whether the selected queue set member is the ring buffer, call `xRingbufferCanRead`{.interpreted-text role="cpp:func"}.
...
The `xRingbufferCreateStatic`{.interpreted-text role="cpp:func"} can be used to create ring buffers with specific memory requirements (such as a ring buffer being allocated in external RAM). All blocks of memory used by a ring buffer must be manually allocated beforehand, then passed to the `xRingbufferCreateStatic`{.interpreted-text role="cpp:func"} to be initialized as a ring buffer. These blocks include the following:

- The ring buffer\'s data structure of type `StaticRingbuffer_t`{.interpreted-text role="cpp:type"}.
...
When deleting a ring buffer created via `xRingbufferCreateStatic`{.interpreted-text role="cpp:func"}, the function `vRingbufferDelete`{.interpreted-text role="cpp:func"} will not free any of the memory blocks. This must be done manually by the user after `vRingbufferDelete`{.interpreted-text role="cpp:func"} is called.
::::
...
- FreeRTOS tick hook can be enabled via the `CONFIG_FREERTOS_USE_TICK_HOOK`{.interpreted-text role="ref"} option. The application must provide the `void vApplicationTickHook( void )` callback.
- FreeRTOS idle hook can be enabled via the `CONFIG_FREERTOS_USE_IDLE_HOOK`{.interpreted-text role="ref"} option. The application must provide the `void vApplicationIdleHook( void )` callback.
...
- For tick hooks:
  ...
  > - Register using `esp_register_freertos_tick_hook`{.interpreted-text role="cpp:func"} or `esp_register_freertos_tick_hook_for_cpu`{.interpreted-text role="cpp:func"}
  > - Deregister using `esp_deregister_freertos_tick_hook`{.interpreted-text role="cpp:func"} or `esp_deregister_freertos_tick_hook_for_cpu`{.interpreted-text role="cpp:func"}

- For idle hooks:

  > - Register using `esp_register_freertos_idle_hook`{.interpreted-text role="cpp:func"} or `esp_register_freertos_idle_hook_for_cpu`{.interpreted-text role="cpp:func"}
  > - Deregister using `esp_deregister_freertos_idle_hook`{.interpreted-text role="cpp:func"} or `esp_deregister_freertos_idle_hook_for_cpu`{.interpreted-text role="cpp:func"}
...
The tick interrupt stays active while the cache is disabled, therefore any tick hook (FreeRTOS or ESP-IDF) functions must be placed in internal RAM. Please refer to the `SPI flash API documentation <iram-safe-interrupt-handlers>`{.interpreted-text role="ref"} for more details.
::::
...
- set a task\'s TLSPs by calling `vTaskSetThreadLocalStoragePointer`{.interpreted-text role="cpp:func"} after the task has been created.
- get a task\'s TLSPs by calling `pvTaskGetThreadLocalStoragePointer`{.interpreted-text role="cpp:func"} during the task\'s lifetime.
...
- `vTaskSetThreadLocalStoragePointerAndDelCallback`{.interpreted-text role="cpp:func"} sets both a particular TLSP and its associated callback.
- Calling the Vanilla FreeRTOS function `vTaskSetThreadLocalStoragePointer`{.interpreted-text role="cpp:func"} simply sets the TLSP\'s associated Deletion Callback to [NULL]{.title-ref}, meaning that no callback is called for that TLSP during task deletion.
...
- The callback is called shortly before a deleted task\'s memory is freed. Thus, the callback can either be called from `vTaskDelete`{.interpreted-text role="cpp:func"} itself, or from the idle task.

## IDF Additional API {#freertos-idf-additional-api}

The `freertos/esp_additions/include/freertos/idf_additions.h`{.interpreted-text role="component_file"} header contains FreeRTOS-related helper functions added by ESP-IDF. Users can include this header via `#include "freertos/idf_additions.h"`.
...
- [ORIG_INCLUDE_PATH]{.title-ref} - contains an absolute path to freertos root include folder. Thus instead of [#include \"freertos/FreeRTOS.h\"]{.title-ref} you can refer to headers directly: [#include \"FreeRTOS.h\"]{.title-ref}.
