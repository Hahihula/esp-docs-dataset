---
original_file_path: api-reference/peripherals/spi_slave.rst
---
...
The SPI slave driver allows using the SPI peripherals as full-duplex Devices. The driver can send/receive transactions up to {IDF_TARGET_MAX_DATA_BUF} bytes in length, or utilize DMA to send/receive longer transactions. However, there are some `known issues <spi_dma_known_issues>`{.interpreted-text role="ref"} related to DMA.

The SPI slave driver supports registering the SPI ISR to a certain CPU core. If multiple tasks try to access the same SPI Device simultaneously, it is recommended that your application be refactored so that each SPI peripheral is only accessed by a single task at a time. Please also use `spi_bus_config_t::isr_cpu_id`{.interpreted-text role="cpp:member"} to register the SPI ISR to the same core as SPI peripheral related tasks to ensure thread safety.
...
The attributes of a transaction are determined by the configuration structure for an SPI peripheral acting as a slave device `spi_slave_interface_config_t`{.interpreted-text role="cpp:type"}, and transaction configuration structure `spi_slave_transaction_t`{.interpreted-text role="cpp:type"}.

As not every transaction requires both writing and reading data, you can choose to configure the `spi_transaction_t`{.interpreted-text role="cpp:type"} structure for TX only, RX only, or TX and RX transactions. If `spi_slave_transaction_t::rx_buffer`{.interpreted-text role="cpp:member"} is set to `NULL`, the read phase will be skipped. Similarly, if `spi_slave_transaction_t::tx_buffer`{.interpreted-text role="cpp:member"} is set to `NULL`, the write phase will be skipped.
...
A Host should not start a transaction before its Device is ready for receiving data. It is recommended to use another GPIO pin for a handshake signal to sync the Devices. For more details, see `transaction_interval`{.interpreted-text role="ref"}.
::::

## Driver Usage

- Initialize an SPI peripheral as a Device by calling the function `spi_slave_initialize`{.interpreted-text role="cpp:func"}. Make sure to set the correct I/O pins in the struct [bus_config]{.title-ref}. Set the unused signals to `-1`.
...
- Before initiating transactions, fill one or more `spi_slave_transaction_t`{.interpreted-text role="cpp:type"} structs with the transaction parameters required. Either queue all transactions by calling the function `spi_slave_queue_trans`{.interpreted-text role="cpp:func"} and, at a later time, query the result by using the function `spi_slave_get_trans_result`{.interpreted-text role="cpp:func"}, or handle all requests individually by feeding them into `spi_slave_transmit`{.interpreted-text role="cpp:func"}. The latter two functions will be blocked until the Host has initiated and finished a transaction, causing the queued data to be sent and received.
- (Optional) Enable/Disable driver functions: Slave driver supports disabling / enabling driver after it is initialized by calling to `spi_slave_disable`{.interpreted-text role="cpp:func"} / `spi_slave_enable`{.interpreted-text role="cpp:func"}, to be able to change clock or power config or sleep to save power. By default, the driver state is [enabled]{.title-ref} after initialized.
- (Optional) To unload the SPI slave driver, call `spi_slave_free`{.interpreted-text role="cpp:func"}.

## Transaction Data and Master/Slave Length Mismatches

Normally, the data that needs to be transferred to or from a Device is read or written to a chunk of memory indicated by the `spi_slave_transaction_t::rx_buffer`{.interpreted-text role="cpp:member"} and `spi_slave_transaction_t::tx_buffer`{.interpreted-text role="cpp:member"}. The SPI driver can be configured to use DMA for transfers, in which case these buffers must be allocated in DMA-capable memory using `pvPortMallocCaps(size, MALLOC_CAP_DMA)`.

The amount of data that the driver can read or write to the buffers is limited by `spi_slave_transaction_t::length`{.interpreted-text role="cpp:member"}. However, this member does not define the actual length of an SPI transaction. A transaction\'s length is determined by the clock and CS lines driven by the Host. The actual length of the transmission can be read only after a transaction is finished from the member `spi_slave_transaction_t::trans_len`{.interpreted-text role="cpp:member"}.

If the length of the transmission is greater than the buffer length, only the initial number of bits specified in the `spi_slave_transaction_t::length`{.interpreted-text role="cpp:member"} member will be sent and received. In this case, `spi_slave_transaction_t::trans_len`{.interpreted-text role="cpp:member"} is set to `spi_slave_transaction_t::length`{.interpreted-text role="cpp:member"} instead of the actual transaction length. To meet the actual transaction length requirements, set `spi_slave_transaction_t::length`{.interpreted-text role="cpp:member"} to a value greater than the maximum `spi_slave_transaction_t::trans_len`{.interpreted-text role="cpp:member"} expected. If the transmission length is shorter than the buffer length, only the data equal to the length of the buffer will be transmitted.
...
As a solution, a Device\'s response rate can be doubled by using the functions `spi_slave_queue_trans`{.interpreted-text role="cpp:func"} and then `spi_slave_get_trans_result`{.interpreted-text role="cpp:func"} instead of using `spi_slave_transmit`{.interpreted-text role="cpp:func"}.

You can also configure a GPIO pin through which the Device will signal to the Host when it is ready for a new transaction. A code example of this can be found in `peripherals/spi_slave`{.interpreted-text role="example"}.
...
## Restrictions and Known Issues {#spi_dma_known_issues}
...
The code example for Device/Host communication can be found in the `peripherals/spi_slave`{.interpreted-text role="example"} directory of ESP-IDF examples.

- 

  example
  :   [peripherals/spi_slave/receiver]{.title-ref} demonstrates how to configure an SPI slave to receive data from an SPI master and implement handshaking to manage data transfer readiness.

- 

  example
  :   [peripherals/spi_slave/sender]{.title-ref} demonstrate how to configure an SPI master to send data to an SPI slave and use handshaking to ensure proper timing for data transmission.
