---
original_file_path: get-started/windows-setup.rst
---
...
This document describes the default and recommended way to install ESP-IDF v6.0 and newer versions. ESP-IDF also supports the `legacy method for updating ESP-IDF tools on Windows <windows-setup-update-legacy>`{.interpreted-text role="doc"}.
::::

## Step 1: Install the EIM

Install the EIM Graphical User Interface (GUI) or Command Line Interface (CLI) via [WinGet](https://learn.microsoft.com/en-us/windows/package-manager/winget/):
...
Alternatively, download the EIM installer for Windows from the [Espressif Download Page](https://dl.espressif.com/dl/eim/), which provides both online and offline installers available in both CLI and GUI versions.
::::
