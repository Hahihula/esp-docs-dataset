---
original_file_path: api-guides/lwip.rst
---
...
### Adapted APIs {#lwip-dns-limitation}
...
> :::
>
> When using any lwIP API other than the [BSD Sockets API](#bsd-sockets-api), please make sure that the API is thread-safe. To check if a given API call is thread-safe, enable the `CONFIG_LWIP_CHECK_THREAD_SAFETY`{.interpreted-text role="ref"} configuration option and run the application. This enables lwIP to assert the correct access of the TCP/IP core functionality. If the API is not accessed or locked properly from the appropriate [lwIP FreeRTOS Task](#lwip-freertos-task), the execution will be aborted. The general recommendation is to use the `/api-reference/network/esp_netif`{.interpreted-text role="doc"} component to interact with lwIP.
> ::::

Some common lwIP app APIs are supported indirectly by ESP-IDF:

- Dynamic Host Configuration Protocol (DHCP) Server & Client are supported indirectly via the `/api-reference/network/esp_netif`{.interpreted-text role="doc"} functionality.
- Domain Name System (DNS) is supported in lwIP; DNS servers could be assigned automatically when acquiring a DHCP address, or manually configured using the `/api-reference/network/esp_netif`{.interpreted-text role="doc"} API.
...
- Simple Network Time Protocol (SNTP) is also supported via the `/api-reference/network/esp_netif`{.interpreted-text role="doc"}, or directly via the `lwip/include/apps/esp_sntp.h`{.interpreted-text role="component_file"} functions, which also provide thread-safe API to `lwip/lwip/src/include/lwip/apps/sntp.h`{.interpreted-text role="component_file"} functions, see also `system-time-sntp-sync`{.interpreted-text role="ref"}. For implementation details, see `protocols/sntp`{.interpreted-text role="example"}. This example demonstrates how to use the LwIP SNTP module to obtain time from internet servers, configure the synchronization method and interval, and retrieve time using the SNTP-over-DHCP module.
- ICMP Ping is supported using a variation on the lwIP ping API, see `/api-reference/protocols/icmp_echo`{.interpreted-text role="doc"}.
- ICMPv6 Ping, supported by lwIP\'s ICMPv6 Echo API, is used to test IPv6 network connectivity. For more information, see `protocols/sockets/icmpv6_ping`{.interpreted-text role="example"}. This example demonstrates how to use the network interface to discover an IPv6 address, create a raw ICMPv6 socket, send an ICMPv6 Echo Request to a destination IPv6 address, and wait for an Echo Reply from the target.
- NetBIOS lookup is available using the standard lwIP API. `protocols/http_server/restful_server`{.interpreted-text role="example"} has the option to demonstrate using NetBIOS to look up a host on the LAN.
- mDNS uses a different implementation to the lwIP default mDNS, see `/api-reference/protocols/mdns`{.interpreted-text role="doc"}. But lwIP can look up mDNS hosts using standard APIs such as `gethostbyname()` and the convention `hostname.local`, provided the `CONFIG_LWIP_DNS_SUPPORT_MDNS_QUERIES`{.interpreted-text role="ref"} setting is enabled.
- The PPP implementation in lwIP can be used to create PPPoS (PPP over serial) interface in ESP-IDF. Please refer to the documentation of the `/api-reference/network/esp_netif`{.interpreted-text role="doc"} component to create and configure a PPP network interface, by means of the `ESP_NETIF_DEFAULT_PPP()` macro defined in `esp_netif/include/esp_netif_defaults.h`{.interpreted-text role="component_file"}. Additional runtime settings are provided via `esp_netif/include/esp_netif_ppp.h`{.interpreted-text role="component_file"}. PPPoS interfaces are typically used to interact with NBIoT/GSM/LTE modems. More application-level friendly API is supported by the [esp_modem](https://components.espressif.com/component/espressif/esp_modem) library, which uses this PPP lwIP module behind the scenes.
...
If you want to avoid any possible IP address conflicts and connectivity issues, the ESP-IDF DHCP client provides several options to validate the offered IPv4 address before binding it (see `CONFIG_LWIP_DHCP_CHECKS_OFFERED_ADDRESS`{.interpreted-text role="ref"}):
...
- `protocols/sockets/non_blocking`{.interpreted-text role="example"} demonstrates how to configure and run a non-blocking TCP client and server, supporting both IPv4 and IPv6 protocols.
- `protocols/sockets/tcp_server`{.interpreted-text role="example"} demonstrates how to create a TCP server that accepts client connection requests and receives data.
- `protocols/sockets/tcp_client`{.interpreted-text role="example"} demonstrates how to create a TCP client that connects to a server using a predefined IP address and port.
- `protocols/sockets/tcp_client_multi_net`{.interpreted-text role="example"} demonstrates how to use Ethernet and Wi-Fi interfaces together, connect to both simultaneously, create a TCP client for each interface, and send a basic HTTP request and response.
- `protocols/sockets/udp_server`{.interpreted-text role="example"} demonstrates how to create a UDP server that receives client connection requests and data.
- `protocols/sockets/udp_client`{.interpreted-text role="example"} demonstrates how to create a UDP client that connects to a server using a predefined IP address and port.
- `protocols/sockets/udp_multicast`{.interpreted-text role="example"} demonstrates how to use the IPV4 and IPV6 UDP multicast features via the BSD-style sockets interface.

### Supported Functions

The following BSD socket API functions are supported. For full details, see `lwip/lwip/src/include/lwip/sockets.h`{.interpreted-text role="component_file"}.
...
- `close()`: via `/api-reference/storage/vfs`{.interpreted-text role="doc"}
- `read()`, `readv()`, `write()`, `writev()`: via `/api-reference/storage/vfs`{.interpreted-text role="doc"}
- `recv()`, `recvmsg()`, `recvfrom()`
- `send()`, `sendmsg()`, `sendto()`
- `select()`: via `/api-reference/storage/vfs`{.interpreted-text role="doc"}
...
Below is a list of common error codes. For a more detailed list of standard POSIX/C error codes, please see [newlib errno.h](https://github.com/espressif/newlib-esp32/blob/master/newlib/libc/include/sys/errno.h) and the platform-specific extensions `esp_libc/platform_include/sys/errno.h`{.interpreted-text role="component_file"}.
...
- `SO_REUSEADDR`: available if `CONFIG_LWIP_SO_REUSE`{.interpreted-text role="ref"} is set, whose behavior can be customized by setting `CONFIG_LWIP_SO_REUSE_RXTOALL`{.interpreted-text role="ref"}
- `SO_KEEPALIVE`
- `SO_BROADCAST`
- `SO_ACCEPTCONN`
- `SO_RCVBUF`: available if `CONFIG_LWIP_SO_RCVBUF`{.interpreted-text role="ref"} is set
...
- `IP_TOS`
- `IP_TTL`
- `IP_PKTINFO`: available if `CONFIG_LWIP_NETBUF_RECVINFO`{.interpreted-text role="ref"} is set
...
The `fcntl()` function is a standard API for manipulating options related to a file descriptor. In ESP-IDF, the `/api-reference/storage/vfs`{.interpreted-text role="doc"} layer is used to implement this function.
...
The `ioctl()` function provides a semi-standard way to access some internal features of the TCP/IP stack. In ESP-IDF, the `/api-reference/storage/vfs`{.interpreted-text role="doc"} layer is used to implement this function.
...
- `CONFIG_LWIP_TCPIP_RECVMBOX_SIZE`{.interpreted-text role="ref"}
- `CONFIG_LWIP_TCPIP_TASK_STACK_SIZE`{.interpreted-text role="ref"}
- `CONFIG_LWIP_TCPIP_TASK_AFFINITY`{.interpreted-text role="ref"}

## IPv6 Support

Both IPv4 and IPv6 are supported in a dual-stack configuration and are enabled by default. Both IPv6 and IPv4 may be disabled separately if they are not needed, see `lwip-ram-usage`{.interpreted-text role="ref"}.
...
### Stateless Autoconfiguration Process {#lwip-ivp6-autoconfig}

To enable address autoconfiguration using the Router Advertisement protocol, please enable:

- `CONFIG_LWIP_IPV6_AUTOCONFIG`{.interpreted-text role="ref"}
...
### DHCPv6 {#lwip-ivp6-dhcp6}

DHCPv6 in lwIP is very simple and supports only stateless configuration. It could be enabled using:

- `CONFIG_LWIP_IPV6_DHCP6`{.interpreted-text role="ref"}

Since the DHCPv6 works only in its stateless configuration, the `lwip-ivp6-autoconfig`{.interpreted-text role="ref"} has to be enabled as well via `CONFIG_LWIP_IPV6_AUTOCONFIG`{.interpreted-text role="ref"}.
...
- Recursive Domain Name System (DNS): this belongs to the Neighbor Discovery Protocol (NDP) and uses `lwip-ivp6-autoconfig`{.interpreted-text role="ref"}.

  The number of servers must be set `CONFIG_LWIP_IPV6_RDNSS_MAX_DNS_SERVERS`{.interpreted-text role="ref"}, this option is disabled by default, i.e., set to 0.

- DHCPv6 stateless configuration, uses `lwip-ivp6-dhcp6`{.interpreted-text role="ref"} to configure DNS servers. Note that this configuration assumes IPv6 Router Advertisement Flags (RFC-5175) to be set to
...
To return to the default lwIP behavior, which is always-on timers, disable `CONFIG_LWIP_TIMERS_ONDEMAND`{.interpreted-text role="ref"}.

#### lwIP Timers API

When not using Wi-Fi, the lwIP timer can be turned off via the API to reduce power consumption.

The following API functions are supported. For full details, see `lwip/lwip/src/include/lwip/timeouts.h`{.interpreted-text role="component_file"}.
...
- Usage of NAPT is demonstrated in `network/vlan_support`{.interpreted-text role="example"}.

#### Default lwIP Hooks

ESP-IDF port layer provides a default hook file, which is included in the lwIP build process. This file is located at `lwip/port/include/lwip_default_hooks.h`{.interpreted-text role="component_file"} and defines several hooks that implement default ESP-IDF behavior of lwIP stack. These hooks could be further modified by choosing one of these options:
...
ESP-IDF allows applications to define a hook for handling extra DHCP options. This can be useful for implementing custom DHCP-based behaviors, such as retrieving specific vendor options. To enable this feature, configure `CONFIG_LWIP_HOOK_DHCP_EXTRA_OPTION`{.interpreted-text role="ref"} either to **Default** (`weak` implementation is provided which could be replaced by custom implementation) or **Custom** (you will have to implement the hook and define its link dependency to lwip)
...
- TCP ISN Hook (`CONFIG_LWIP_HOOK_TCP_ISN`{.interpreted-text role="ref"}): Allows custom randomization of TCP Initial Sequence Numbers (ESP-IDF provided implementation is the default option, set to *Custom* to provide custom implementation or *None* to use lwIP implementation).
- IPv6 Route Hook (`CONFIG_LWIP_HOOK_IP6_ROUTE`{.interpreted-text role="ref"}): Enables custom route selection for IPv6 packets (No hook by default, use *Default* or *Custom* to override).
- IPv6 Get Gateway Hook (`CONFIG_LWIP_HOOK_ND6_GET_GW`{.interpreted-text role="ref"}): Enables defining a custom gateway selection logic (No hook by default, use *Default* or *Custom* to override).
- IPv6 Source Address Selection Hook (`CONFIG_LWIP_HOOK_IP6_SELECT_SRC_ADDR`{.interpreted-text role="ref"}): Allows customization of source address selection (No hook by default, use *Default* or *Custom* to override).
- Netconn External Resolve Hook (`CONFIG_LWIP_HOOK_NETCONN_EXTERNAL_RESOLVE`{.interpreted-text role="ref"}): Allows overriding the DNS resolution logic for network connections (No hook by default, use *Default* or *Custom* to override).
- DNS External Resolve Hook (`CONFIG_LWIP_HOOK_DNS_EXTERNAL_RESOLVE`{.interpreted-text role="ref"}): Provides a hook for custom DNS resolution logic with callbacks (No hook by default, but could be selected by an external component to prefer the custom option; use *Default* or *Custom* to override).
- IPv6 Packet Input Hook (`CONFIG_LWIP_HOOK_IP6_INPUT`{.interpreted-text role="ref"}): Provides filtering or modification of incoming IPv6 packets (ESP-IDF provided `weak` implementation is the default option; use *Custom* or provide a strong definition to override the *Default* option; choose *None* to disable IPv6 packet input filtering)

Each of these hooks can be configured in menuconfig, allowing selection of default, custom, or no implementation.

#### Customized lwIP Hooks {#lwip-custom-hooks}
...
Alternatively, you can define your function-like macro in a header file which will be pre-included as an lwIP hook file, see `lwip-custom-hooks`{.interpreted-text role="ref"}.

## Network Interface Callbacks

- Status Callback (`CONFIG_LWIP_NETIF_STATUS_CALLBACK`{.interpreted-text role="ref"}): Enables [netif_set_status_callback()]{.title-ref} to notify when an interface comes up/down and when IPv4/IPv6 addresses change.
- Link Callback (`CONFIG_LWIP_NETIF_LINK_CALLBACK`{.interpreted-text role="ref"}): Enables [netif_set_link_callback()]{.title-ref} to notify when the physical link goes up/down. The callback is triggered by [netif_set_link_up()]{.title-ref} / [netif_set_link_down()]{.title-ref} calls in drivers or virtual interfaces. Can be used alongside [LWIP_NETIF_EXT_STATUS_CALLBACK]{.title-ref} for richer eventing.

### Limitations {#lwip-limitations}
...
Increasing the number of TX buffers in the `Wi-Fi <CONFIG_ESP_WIFI_TX_BUFFER>`{.interpreted-text role="ref"} or `Ethernet <CONFIG_ETH_DMA_TX_BUFFER_NUM>`{.interpreted-text role="ref"} project configuration as applicable may also help.
:::

::: only
not esp32 and SOC_WIFI_SUPPORTED

Increasing the number of TX buffers in the `Wi-Fi <CONFIG_ESP_WIFI_TX_BUFFER>`{.interpreted-text role="ref"} project configuration may also help.
:::

## Performance Optimization {#lwip-performance}
...
Espressif tests ESP-IDF TCP/IP throughput using the iperf test application: <https://iperf.fr/>, please refer to `improve-network-speed`{.interpreted-text role="ref"} for more details about the actual testing and using the optimized configuration.
...
- If a lot of tasks are competing for CPU time on the system, consider that the lwIP task has configurable CPU affinity (`CONFIG_LWIP_TCPIP_TASK_AFFINITY`{.interpreted-text role="ref"}) and runs at fixed priority (18, `ESP_TASK_TCPIP_PRIO`). To optimize CPU utilization, consider assigning competing tasks to different cores or adjusting their priorities to lower values. For additional details on built-in task priorities, please refer to `built-in-task-priorities`{.interpreted-text role="ref"}.
- If using `select()` function with socket arguments only, disabling `CONFIG_VFS_SUPPORT_SELECT`{.interpreted-text role="ref"} will make `select()` calls faster.
- If there is enough free IRAM, select `CONFIG_LWIP_IRAM_OPTIMIZATION`{.interpreted-text role="ref"} and `CONFIG_LWIP_EXTRA_IRAM_OPTIMIZATION`{.interpreted-text role="ref"} to improve TX/RX throughput.

::: only
SOC_WIFI_SUPPORTED

If using a Wi-Fi network interface, please also refer to `wifi-buffer-usage`{.interpreted-text role="ref"}.
:::
...
### Minimum RAM Usage {#lwip-ram-usage}
...
- Reducing `CONFIG_LWIP_MAX_SOCKETS`{.interpreted-text role="ref"} reduces the maximum number of sockets in the system. This also causes TCP sockets in the `WAIT_CLOSE` state to be closed and recycled more rapidly when needed to open a new socket, further reducing peak RAM usage.
- Reducing `CONFIG_LWIP_TCPIP_RECVMBOX_SIZE`{.interpreted-text role="ref"}, `CONFIG_LWIP_TCP_RECVMBOX_SIZE`{.interpreted-text role="ref"} and `CONFIG_LWIP_UDP_RECVMBOX_SIZE`{.interpreted-text role="ref"} reduce RAM usage at the expense of throughput, depending on usage.
- Reducing `CONFIG_LWIP_TCP_ACCEPTMBOX_SIZE`{.interpreted-text role="ref"} reduce RAM usage by limiting concurrent accepted connections.
- Reducing `CONFIG_LWIP_TCP_MSL`{.interpreted-text role="ref"} and `CONFIG_LWIP_TCP_FIN_WAIT_TIMEOUT`{.interpreted-text role="ref"} reduces the maximum segment lifetime in the system. This also causes TCP sockets in the `TIME_WAIT` and `FIN_WAIT_2` states to be closed and recycled more rapidly.
- Disabling `CONFIG_LWIP_IPV6`{.interpreted-text role="ref"} can save about 39 KB for firmware size and 2 KB RAM when the system is powered up and 7 KB RAM when the TCP/IP stack is running. If there is no requirement for supporting IPV6, it can be disabled to save flash and RAM footprint.
- Disabling `CONFIG_LWIP_IPV4`{.interpreted-text role="ref"} can save about 26 KB of firmware size and 600 B RAM on power up and 6 KB RAM when the TCP/IP stack is running. If the local network supports IPv6-only configuration, IPv4 can be disabled to save flash and RAM footprint.

::: only
SOC_WIFI_SUPPORTED

If using Wi-Fi, please also refer to `wifi-buffer-usage`{.interpreted-text role="ref"}.
:::
