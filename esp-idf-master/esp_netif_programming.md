---
original_file_path: api-reference/network/esp_netif_programming.rst
---

# ESP-NETIF Programmers Manual

## Configure IP, Gateway, and DNS {#esp_netif_set_ip}

Typically, IP addresses \-- including the gateway, network mask, and DNS servers \-- are automatically obtained through a DHCP server or Router Advertisement services. Notifications regarding IP address assignments are received via the `IP_EVENT`, which provides the relevant IP address information. You can also retrieve the current address details using the function `esp_netif_get_ip_info()`{.interpreted-text role="cpp:func"}. This function returns a structure, `esp_netif_ip_info_t`{.interpreted-text role="cpp:type"}, that contains the IPv4 address of the network interface, along with its network mask and gateway address. Similarly, the function `esp_netif_get_all_ip6()`{.interpreted-text role="cpp:func"} can be used to obtain all IPv6 addresses associated with the interface.

In order to configure static IP addresses and DNS servers, it\'s necessary to disable or stop DHCP client (which is enabled by default on some network interfaces, such as the default Ethernet, or the default WiFi station). Please refer to the example `/protocols/static_ip`{.interpreted-text role="example"} for more details.

To set IPv4 address, you can use `esp_netif_set_ip_info()`{.interpreted-text role="cpp:func"}. For IPv6, these two functions can be used for adding or removing addresses: `esp_netif_add_ip6_address()`{.interpreted-text role="cpp:func"}, `esp_netif_remove_ip6_address()`{.interpreted-text role="cpp:func"}. To configure DNS servers, please use `esp_netif_set_dns_info()`{.interpreted-text role="cpp:func"} API.
...
Configure these by setting `esp_netif_inherent_config_t::get_ip_event`{.interpreted-text role="cpp:member"} and `esp_netif_inherent_config_t::lost_ip_event`{.interpreted-text role="cpp:member"} when creating the interface:
...
Event data for these events is the same as the standard ones: `ip_event_got_ip_t`{.interpreted-text role="cpp:type"} is posted for "got IP", and `ip_event_got_ip_t`{.interpreted-text role="cpp:type"} with `ip_event_got_ip_t::ip_changed`{.interpreted-text role="cpp:member"} set accordingly for updates. For lost IP, an empty `ip_event_got_ip_t`{.interpreted-text role="cpp:type"} with only `ip_event_got_ip_t::esp_netif`{.interpreted-text role="cpp:member"} set is posted.

## Configure DHCP options {#esp_netif_set_dhcp}

Some network interfaces are pre-configured to use either a DHCP client (commonly for Ethernet interfaces) or a DHCP server (typically for Wi-Fi software access points). When manually creating a custom network interface, the configuration flags `esp_netif_flags_t`{.interpreted-text role="cpp:type"} are used to specify the behavior of the interface. Adding `ESP_NETIF_DHCP_CLIENT`{.interpreted-text role="cpp:enumerator"} or `ESP_NETIF_DHCP_SERVER`{.interpreted-text role="cpp:enumerator"} will enable the DHCP client or server, respectively.
...
To set or get a specific DHCP option, the common type `esp_netif_dhcp_option_id_t`{.interpreted-text role="cpp:type"} is used for both the DHCP server and client. However, not all options are supported for both. For details on the available options for the DHCP client, refer to the API documentation for `esp_netif_dhcpc_option()`{.interpreted-text role="cpp:func"}. Similarly, for the options available for the DHCP server, consult the API documentation for `esp_netif_dhcps_option()`{.interpreted-text role="cpp:func"}.

## SNTP Service {#esp_netif-sntp-api}

A brief introduction to SNTP, its initialization code, and basic modes can be found in Section `system-time-sntp-sync`{.interpreted-text role="ref"} in `System Time </api-reference/system/system_time>`{.interpreted-text role="doc"}.
...
1.  Initialize and configure the service using `esp_netif_sntp_init()`{.interpreted-text role="cpp:func"}. This function can only be called once unless the SNTP service has been destroyed using `esp_netif_sntp_deinit()`{.interpreted-text role="cpp:func"}.
2.  Start the service with `esp_netif_sntp_start()`{.interpreted-text role="cpp:func"}. This step is not necessary if the service was auto-started in the previous step (default behavior). However, it can be useful to start the service explicitly after connecting if DHCP-provided NTP servers are being used. Note that this option needs to be enabled before connecting, but the SNTP service should only be started afterward.
3.  Wait for the system time to synchronize using `esp_netif_sntp_sync_wait()`{.interpreted-text role="cpp:func"} (if required).
4.  Stop and destroy the service using `esp_netif_sntp_deinit()`{.interpreted-text role="cpp:func"}.
...
- Event data: pointer to `esp_netif_sntp_time_sync_t`{.interpreted-text role="cpp:type"} with the synchronized `timeval`
...
If you want to configure multiple SNTP servers, update the lwIP configuration option `CONFIG_LWIP_SNTP_MAX_SERVERS`{.interpreted-text role="ref"}.
::::

### Use DHCP-Obtained SNTP Server(s)

First, you need to enable the lwIP configuration option `CONFIG_LWIP_DHCP_GET_NTP_SRV`{.interpreted-text role="ref"}. Then, initialize the SNTP module with the DHCP option, without specifying an NTP server.
...
Then you start the service normally with `esp_netif_sntp_start()`{.interpreted-text role="cpp:func"}.

## L2 TAP Interface Usage {#esp_netif_l2tap}

The ESP-NETIF L2 TAP interface is used to access Data Link Layer, please refer to the `esp-netif structure`{.interpreted-text role="ref"} diagram to see how L2 TAP interacts with ESP-NETIF and application.

From a user perspective, the ESP-NETIF L2 TAP interface is accessed using file descriptors of VFS, which provides file-like interfacing (using functions like `open()`, `read()`, `write()`, etc). To learn more, refer to `/api-reference/storage/vfs`{.interpreted-text role="doc"}.
...
To be able to use the ESP-NETIF L2 TAP interface, it needs to be enabled in Kconfig by `CONFIG_ESP_NETIF_L2_TAP`{.interpreted-text role="ref"} first and then registered by `esp_vfs_l2tap_intf_register()`{.interpreted-text role="cpp:func"} prior usage of any VFS function.

### `open()`

Once the ESP-NETIF L2 TAP is registered, it can be opened at path name \"/dev/net/tap\". The same path name can be opened multiple times up to `CONFIG_ESP_NETIF_L2_TAP_MAX_FDS`{.interpreted-text role="ref"} and multiple file descriptors with a different configuration may access the Data Link Layer frames.
...
> - `L2TAP_S_DEVICE_DRV_HNDL` - is another way to bound the file descriptor to a specific Network Interface. In this case, the Network interface is identified directly by IO Driver handle (e.g., `esp_eth_handle_t`{.interpreted-text role="cpp:type"} in case of Ethernet). The IO Driver handle is passed to `ioctl()` as the third parameter.
...
ESP-NETIF L2 TAP `read()` implementation extends the standard and offers Extended Buffer mechanism to retrieve additional information about received frame. See `Extended Buffer <esp_netif_l2tap_ext_buff>`{.interpreted-text role="ref"} section for more information.
::::
...
Select is used in a standard way, just `CONFIG_VFS_SUPPORT_SELECT`{.interpreted-text role="ref"} needs to be enabled to make the `select()` function available.

### Extended Buffer {#esp_netif_l2tap_ext_buff}
...
Currently implement and used IREC data types are defined in `l2tap_irec_type_t`{.interpreted-text role="cpp:type"}.
...
- `L2TAP_IREC_FIRST()` - retrieves the first IREC from the `l2tap_extended_buff_t::info_recs_buff`{.interpreted-text role="cpp:member"} pool of Extended Buffer. If the `l2tap_extended_buff_t::info_recs_len`{.interpreted-text role="cpp:member"} is smaller than the size of a record header, it returns NULL.
...
Prior any Extended Buffer IO operation (either `write()` or `read()`), you first need to fully populate the Extended Buffer and its IREC fields. For example, when you want to retrieve Time Stamp, you need to set type of the IREC to `L2TAP_IREC_TIME_STAMP`{.interpreted-text role="cpp:enumerator"} and configure appropriate length. If you don\'t set the type correctly, the frame is still received or transmitted but information to be retrieved is lost. Similarly, when the IREC length is less than expected length, the frame is still received or transmitted but the type of affected IREC is marked to `L2TAP_IREC_INVALID`{.interpreted-text role="cpp:enumerator"} by the ESP-NETIF L2 TAP and information to be retrieved is lost.
...
## IP Event: Transmit/Receive Packet {#esp_netif_other_events}
...
The feature can be completely disabled during compilation time using the flag `CONFIG_ESP_NETIF_REPORT_DATA_TRAFFIC`{.interpreted-text role="ref"} in the kconfig.

**Run Time:**

At runtime, you can enable or disable this event using the functions `esp_netif_tx_rx_event_enable()`{.interpreted-text role="cpp:func"} and `esp_netif_tx_rx_event_disable()`{.interpreted-text role="cpp:func"}.
...
The event data structure, `ip_event_tx_rx_t`{.interpreted-text role="cpp:class"}, contains the following fields:

- `ip_event_tx_rx_t::dir`{.interpreted-text role="cpp:member"}: Indicates whether the packet was transmitted (`ESP_NETIF_TX`) or received (`ESP_NETIF_RX`).
- `ip_event_tx_rx_t::len`{.interpreted-text role="cpp:member"}: Length of the data frame.
- `ip_event_tx_rx_t::esp_netif`{.interpreted-text role="cpp:member"}: The network interface on which the packet was sent or received.
...
The event data structure is `ip_event_netif_status_t`{.interpreted-text role="cpp:type"}, which contains the `esp_netif` handle of the interface that changed state.

## API Reference {#esp_netif_api_reference}
