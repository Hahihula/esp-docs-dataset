---
original_file_path: api-reference/storage/nvs_flash.rst
---
...
Currently, NVS uses a portion of main flash memory through the `esp_partition <flash-partition-apis>`{.interpreted-text role="ref"} API. The library uses all the partitions with `data` type and `nvs` subtype. The application can choose to use the partition with the label `nvs` through the `nvs_open`{.interpreted-text role="cpp:func"} API function or any other partition by specifying its name using the `nvs_open_from_partition`{.interpreted-text role="cpp:func"} API function.
...
Duration of NVS initialization using `nvs_flash_init`{.interpreted-text role="cpp:func"} is proportional to the number of existing keys. Initialization of NVS requires approximately 0.5 seconds per 1000 keys.
::::

::: only
SOC_SPIRAM_SUPPORTED

By default, internal NVS allocates a heap in internal RAM. With a large NVS partition or big number of keys, the application can exhaust the internal RAM heap just on NVS overhead. Applications using modules with SPI-connected PSRAM can overcome this limitation by enabling the Kconfig option `CONFIG_NVS_ALLOCATE_CACHE_IN_SPIRAM`{.interpreted-text role="ref"} which redirects RAM allocation to the SPI-connected PSRAM. This option is available in the nvs_flash component of the menuconfig menu when SPIRAM is enabled and and `CONFIG_SPIRAM_USE`{.interpreted-text role="ref"} is set to `CONFIG_SPIRAM_USE_CAPS_ALLOC`. .. note:: Using SPI-connected PSRAM slows down NVS API for integer operations by an approximate factor of 2.5.
:::

## Use of NVS in Bootloader code {#nvs_bootloader}

The standard NVS API described in this guide is available to the running application. It is also possible to read data from NVS in the custom bootloader code. More information can be found in the `nvs_bootloader`{.interpreted-text role="doc"} guide.
...
- `nvs_entry_find`{.interpreted-text role="cpp:func"} creates an opaque handle, which is used in subsequent calls to the `nvs_entry_next`{.interpreted-text role="cpp:func"} and `nvs_entry_info`{.interpreted-text role="cpp:func"} functions.
- `nvs_entry_next`{.interpreted-text role="cpp:func"} advances an iterator to the next key-value pair.
- `nvs_entry_info`{.interpreted-text role="cpp:func"} returns information about each key-value pair

In general, all iterators obtained via `nvs_entry_find`{.interpreted-text role="cpp:func"} have to be released using `nvs_release_iterator`{.interpreted-text role="cpp:func"}, which also tolerates `NULL` iterators.

`nvs_entry_find`{.interpreted-text role="cpp:func"} and `nvs_entry_next`{.interpreted-text role="cpp:func"} set the given iterator to `NULL` or a valid iterator in all cases except a parameter error occurred (i.e., return `ESP_ERR_NVS_NOT_FOUND`). In case of a parameter error, the given iterator will not be modified. Hence, it is best practice to initialize the iterator to `NULL` before calling `nvs_entry_find`{.interpreted-text role="cpp:func"} to avoid complicated error checking before releasing the iterator.
...
NVS is not directly compatible with the {IDF_TARGET_NAME} flash encryption system. However, data can still be stored in encrypted form if NVS encryption is used together with {IDF_TARGET_NAME} flash encryption. Please refer to `nvs_encryption`{.interpreted-text role="doc"} for more details.
:::

::: only
SOC_HMAC_SUPPORTED

NVS is not directly compatible with the {IDF_TARGET_NAME} flash encryption system. However, data can still be stored in encrypted form if NVS encryption is used together with {IDF_TARGET_NAME} flash encryption or with the help of the HMAC peripheral. Please refer to `nvs_encryption`{.interpreted-text role="doc"} for more details.
:::
...
To address this issue, the Kconfig option `CONFIG_NVS_FLASH_VERIFY_ERASE`{.interpreted-text role="ref"} enables verification of flash erase operations by reading back the affected page. If the page is not fully erased to `0xFF` after a `flash_erase` operation, the erase is retried until the page is correctly cleared. The total number of erase attempts, including the initial attempt, is controlled by the Kconfig option `CONFIG_NVS_FLASH_ERASE_ATTEMPTS`{.interpreted-text role="ref"}.

## NVS Encryption {#nvs_encryption}

Please refer to the `nvs_encryption`{.interpreted-text role="doc"} guide for more details.

## NVS Partition Generator Utility

This utility helps generate NVS partition binary files which can be flashed separately on a dedicated partition via a flashing utility. Key-value pairs to be flashed onto the partition can be provided via a CSV file. For more details, please refer to `nvs_partition_gen`{.interpreted-text role="doc"}.
...
You can find code examples in the `storage/nvs`{.interpreted-text role="example"} directory of ESP-IDF examples:

`storage/nvs/nvs_rw_value`{.interpreted-text role="example"}
...
`storage/nvs/nvs_rw_blob`{.interpreted-text role="example"}
...
`storage/nvs/nvs_rw_value_cxx`{.interpreted-text role="example"}

> This example does exactly the same as `storage/nvs/nvs_rw_value`{.interpreted-text role="example"}, except that it uses the C++ NVS handle class.

`storage/nvs/nvs_statistics`{.interpreted-text role="example"}
...
`storage/nvs/nvs_iteration`{.interpreted-text role="example"}
...
### Structure of Entry {#structure_of_entry}
...
:   One byte indicating the value data type. See the `ItemType`{.interpreted-text role="cpp:type"} enumeration in `nvs_flash/include/nvs_handle.hpp`{.interpreted-text role="component_file"} for possible values.
