---
original_file_path: api-reference/system/freertos_idf.rst
---
...
IDF FreeRTOS source code is based on Vanilla FreeRTOS v10.5.1 but contains significant modifications to both kernel behavior and API in order to support dual-core SMP. However, IDF FreeRTOS can also be configured for single-core by enabling the `CONFIG_FREERTOS_UNICORE`{.interpreted-text role="ref"} option (see `freertos-idf-single-core`{.interpreted-text role="ref"} for more details).
...
This document assumes that the reader has a requisite understanding of Vanilla FreeRTOS, i.e., its features, behavior, and API usage. Refer to the [Vanilla FreeRTOS documentation](https://www.freertos.org/index.html) for more details.
::::
...
- `xTaskCreate`{.interpreted-text role="cpp:func"} creates a task. The task\'s memory is dynamically allocated.
- `xTaskCreateStatic`{.interpreted-text role="cpp:func"} creates a task. The task\'s memory is statically allocated, i.e., provided by the user.
...
- `xTaskCreatePinnedToCore`{.interpreted-text role="cpp:func"} creates a task with a particular core affinity. The task\'s memory is dynamically allocated.
- `xTaskCreateStaticPinnedToCore`{.interpreted-text role="cpp:func"} creates a task with a particular core affinity. The task\'s memory is statically allocated, i.e., provided by the user.
...
Task deletion in Vanilla FreeRTOS is called via `vTaskDelete`{.interpreted-text role="cpp:func"}. The function allows deletion of another task or the currently running task if the provided task handle is `NULL`. The actual freeing of the task\'s memory is sometimes delegated to the idle task if the task being deleted is the currently running task.

IDF FreeRTOS provides the same `vTaskDelete`{.interpreted-text role="cpp:func"} function. However, due to the dual-core nature, there are some behavioral differences when calling `vTaskDelete`{.interpreted-text role="cpp:func"} in IDF FreeRTOS:
...
Where possible, please design your own application so that when calling `vTaskDelete`{.interpreted-text role="cpp:func"}, the deleted task is in a known state. For example:
...
- Tasks placing themselves in the suspend state via `vTaskSuspend`{.interpreted-text role="cpp:func"} before being deleted by another task.
...
Vanilla FreeRTOS allows the scheduler to be suspended/resumed by calling `vTaskSuspendAll`{.interpreted-text role="cpp:func"} and `xTaskResumeAll`{.interpreted-text role="cpp:func"} respectively. While the scheduler is suspended:
...
On scheduler resumption, `xTaskResumeAll`{.interpreted-text role="cpp:func"} catches up all of the lost ticks and unblock any timed-out tasks.

In IDF FreeRTOS, suspending the scheduler across multiple cores is not possible. Therefore when `vTaskSuspendAll`{.interpreted-text role="cpp:func"} is called on a particular core (e.g., core A):
...
When `xTaskResumeAll`{.interpreted-text role="cpp:func"} is called on a particular core (e.g., core A):
...
Vanilla FreeRTOS allows interrupts to be disabled and enabled by calling `taskDISABLE_INTERRUPTS`{.interpreted-text role="c:macro"} and `taskENABLE_INTERRUPTS`{.interpreted-text role="c:macro"} respectively. IDF FreeRTOS provides the same API. However, interrupts are only disabled or enabled on the current core.
...
Users that require the use of the `float` type in an ISR routine should refer to the `CONFIG_FREERTOS_FPU_IN_ISR`{.interpreted-text role="ref"} configuration option.
::::
:::::
...
## Single-Core Mode {#freertos-idf-single-core}

Although IDF FreeRTOS is modified for dual-core SMP, IDF FreeRTOS can also be built for single-core by enabling the `CONFIG_FREERTOS_UNICORE`{.interpreted-text role="ref"} option.

For single-core targets (such as ESP32-S2 and ESP32-C3), the `CONFIG_FREERTOS_UNICORE`{.interpreted-text role="ref"} option is always enabled. For multi-core targets (such as ESP32 and ESP32-S3), `CONFIG_FREERTOS_UNICORE`{.interpreted-text role="ref"} can also be set, but will result in the application only running Core 0.
