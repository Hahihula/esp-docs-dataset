---
original_file_path: api-reference/system/random.rst
---
...
{IDF_TARGET_NAME} contains a hardware random number generator (RNG). You can use the APIs `esp_random`{.interpreted-text role="cpp:func"} and `esp_fill_random`{.interpreted-text role="cpp:func"} to obtained random values from it.
...
- RC_FAST_CLK is an asynchronous clock source, and it increases the RNG entropy by introducing circuit metastability. See the `secondary entropy`{.interpreted-text role="ref"} section for more details.
:::
...
![image](/../_static/esp_rng_noise_source_rf_available.svg){.align-center}
:::

::: only
not SOC_WIFI_SUPPORTED and not SOC_IEEE802154_SUPPORTED and not SOC_BT_SUPPORTED

![image](/../_static/esp_rng_noise_source_rf_unavailable.svg){.align-center}
:::
...
:   - RF subsystem is enabled, i.e., {IDF_TARGET_RF_NAME} {IDF_TARGET_RF_IS} enabled. When enabled, the RF subsystem internally enables the High Speed ADC that can be used as the entropy source. The High Speed ADC may only be available when the respective RF subsystem is active (e.g., not in sleep mode). See the `enabling RF subsystem`{.interpreted-text role="ref"} section for more details.

- The internal entropy source SAR ADC has been enabled by calling `bootloader_random_enable`{.interpreted-text role="cpp:func"} and not yet disabled by calling `bootloader_random_disable`{.interpreted-text role="cpp:func"}.
- While the ESP-IDF `second-stage-bootloader`{.interpreted-text role="ref"} is running. This is because the default ESP-IDF bootloader implementation calls `bootloader_random_enable`{.interpreted-text role="cpp:func"} when the bootloader starts, and `bootloader_random_disable`{.interpreted-text role="cpp:func"} before executing the application.
:::

When any of these conditions are true, samples of physical noise are continuously mixed into the internal hardware RNG state to provide entropy. Consult the **{IDF_TARGET_NAME} Technical Reference Manual** \> **Random Number Generator (RNG)** \[[PDF]({IDF_TARGET_TRM_EN_URL}#rng)\] chapter for more details.
...
## Enabling RF subsystem {#enabling RF subsystem}
...
:   - Wi-Fi: `esp_wifi_start`{.interpreted-text role="cpp:func"}

SOC_BT_SUPPORTED

:   - Bluetooth (NimBLE): `nimble_port_init()`{.interpreted-text role="cpp:func"} which internally calls `esp_bt_controller_enable()`{.interpreted-text role="cpp:func"}

SOC_BT_SUPPORTED

:   - Bluetooth (Bluedroid): `esp_bt_controller_enable()`{.interpreted-text role="cpp:func"}

SOC_IEEE802154_SUPPORTED

:   - Thread/Zigbee: `esp_openthread_init`{.interpreted-text role="cpp:func"}
:::
::::
...
To re-enable the entropy source temporarily during application startup, or for an application that does not use {IDF_TARGET_RF_NAME}, call the function `bootloader_random_enable`{.interpreted-text role="cpp:func"} to re-enable the internal entropy source. The function `bootloader_random_disable`{.interpreted-text role="cpp:func"} must be called to disable the entropy source again before using any of the following features:
...
## Secondary Entropy {#secondary entropy}
...
This function is implemented by calling `esp_fill_random`{.interpreted-text role="cpp:func"} internally.
...
This function is implemented by calling `getrandom`{.interpreted-text role="cpp:func"} internally.
