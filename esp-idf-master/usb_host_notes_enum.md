---
original_file_path: api-reference/peripherals/usb_host/usb_host_notes_enum.rst
---
...
For more detailed information about Bus enumeration, please refer to [USB 2.0 Specification](https://www.usb.org/document-library/usb-20-specification) \> Chapter 9.1.2 **Bus Enumeration**.
::::

### Host Stack Requirements

In addition to the USB 2.0 specification requirements, the Enum Driver also takes into consideration the requirements set for the overall Host Stack (see `./usb_host_notes_design`{.interpreted-text role="doc"}):
...
The USB device enumeration process implemented in the Enum Driver is mostly based same process in the [Windows USB stack](https://techcommunity.microsoft.com/t5/microsoft-usb-blog/how-does-usb-stack-enumerate-a-device/ba-p/270685). The Enum Driver\'s enumeration process involves the following steps:

1.  First device descriptor request (to obtain the MPS of EP0)
2.  Second device reset (to workaround some non-compliant devices)
3.  Set device address
4.  Second device descriptor request (to obtain and store the full device descriptor)
5.  Configuration descriptor request (to obtain the full configuration descriptor of selected configuration)
6.  Language ID Table request (checks to see if en-US is supported)
7.  Manufacturer string descriptor request
8.  Product string descriptor request
9.  Serial number string descriptor request
10. Set configuration request (sets the device to target configuration number)
