---
original_file_path: migration-guides/release-6.x/6.0/build-system.rst
---
...
2.  Explicitly place the orphan section using a `linker fragment file <ldgen-linker-fragment-files>`{.interpreted-text role="ref"}.
3.  Suppress errors by setting `CONFIG_COMPILER_ORPHAN_SECTIONS`{.interpreted-text role="ref"} to `warning` or `place`.
...
Initially, global constructors were executed using the internal `do_global_ctors()`[ function. This approach was used to support Xtensa targets, which emit ]{.title-ref}[.ctors.\*]{.title-ref}\` sections ordered in **descending** order.
...
The default compiler warnings will be considered as errors. The configuration option `CONFIG_COMPILER_DISABLE_DEFAULT_ERRORS`{.interpreted-text role="ref"} has been changed to N.
