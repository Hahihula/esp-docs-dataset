---
original_file_path: api-reference/system/ipc.rst
---
...
- The IPC tasks are created at the highest possible priority (i.e., `configMAX_PRIORITIES - 1`).
  ...
  > - If `CONFIG_ESP_IPC_USES_CALLERS_PRIORITY`{.interpreted-text role="ref"} is enabled, the target core\'s IPC task will be lowered to the current priority of the target core before executing the callback.
  > - If `CONFIG_ESP_IPC_USES_CALLERS_PRIORITY`{.interpreted-text role="ref"} is disabled, the target core will always execute the callback at the highest possible priority.

- Depending on the complexity of the callback, users may need to configure the stack size of the IPC task via `CONFIG_ESP_IPC_TASK_STACK_SIZE`{.interpreted-text role="ref"}.
...
- The callback must be of the `esp_ipc_func_t`{.interpreted-text role="cpp:type"} type.
...
- `esp_ipc_call`{.interpreted-text role="cpp:func"} triggers an IPC call on the target core. This function will block until the target core\'s IPC task **begins** execution of the callback.
- `esp_ipc_call_blocking`{.interpreted-text role="cpp:func"} triggers an IPC on the target core. This function will block until the target core\'s IPC task **completes** execution of the callback.
...
For such scenarios, the IPC ISR feature supports execution of callbacks in a `High Priority Interrupt </api-guides/hlinterrupts>`{.interpreted-text role="doc"} context.
:::
...
- The priority of the reserved High Priority Interrupt is dependent on the `CONFIG_ESP_SYSTEM_CHECK_INT_LEVEL`{.interpreted-text role="ref"} option.
:::

When the callback executes, users need to consider the following:

::: list
\- The calling core will disable interrupts of priority level 3 and lower. :CONFIG_IDF_TARGET_ARCH_XTENSA: - Although the priority of the reserved interrupt depends on `CONFIG_ESP_SYSTEM_CHECK_INT_LEVEL`{.interpreted-text role="ref"}, during the execution of IPC ISR callback, the target core will disable interrupts of priority level 5 and lower regardless of what `CONFIG_ESP_SYSTEM_CHECK_INT_LEVEL`{.interpreted-text role="ref"} is set to. :CONFIG_IDF_TARGET_ARCH_RISCV: - Although the priority of the reserved interrupt depends on `CONFIG_ESP_SYSTEM_CHECK_INT_LEVEL`{.interpreted-text role="ref"}, during the execution of IPC ISR callback, the target core will disable all interrupts.
:::
...
- The callback must be of type `esp_ipc_isr_func_t`{.interpreted-text role="cpp:type"} but implemented entirely in assembly.
...
High Priority Interrupt IPC callbacks must be of type `esp_ipc_isr_func_t`{.interpreted-text role="cpp:type"} and have the same restrictions as for regular interrupt handlers. The callback function can be written in C.
:::

The IPC feature offers the API listed below to execute a callback in a High Priority Interrupt context:

- `esp_ipc_isr_call`{.interpreted-text role="cpp:func"} triggers an IPC call on the target core. This function will busy-wait until the target core **begins** execution of the callback.
- `esp_ipc_isr_call_blocking`{.interpreted-text role="cpp:func"} triggers an IPC call on the target core. This function will busy-wait until the target core **completes** execution of the callback.
...
The number of scratch registers available for use is sufficient for most simple use cases. But if your callback requires more scratch registers, `void *arg` can point to a buffer that is used as a register save area. The callback can then save and restore more registers. See the `system/ipc/ipc_isr`{.interpreted-text role="example"}.
::::
...
For more examples of High Priority Interrupt IPC callbacks, you can refer to `components/esp_system/port/arch/xtensa/esp_ipc_isr_routines.S`{.interpreted-text role="idf_file"} and `components/esp_system/test_apps/esp_system_unity_tests/main/port/arch/xtensa/test_ipc_isr.S`{.interpreted-text role="idf_file"}.
::::
:::::::
...
See `examples/system/ipc/ipc_isr/riscv/main/main.c`{.interpreted-text role="idf_file"} for an example of its use.
:::

::: only
CONFIG_IDF_TARGET_ARCH_XTENSA

See `examples/system/ipc/ipc_isr/xtensa/main/main.c`{.interpreted-text role="idf_file"} for an example of its use.
:::
...
:   - `esp_ipc_isr_stall_other_cpu`{.interpreted-text role="cpp:func"} stalls the target core. The calling core disables interrupts of level 3 and lower, while the target core will busy-wait with all interrupts disabled. The target core will busy-wait until `esp_ipc_isr_release_other_cpu`{.interpreted-text role="cpp:func"} is called.

CONFIG_IDF_TARGET_ARCH_XTENSA

:   - `esp_ipc_isr_stall_other_cpu`{.interpreted-text role="cpp:func"} stalls the target core. The calling core disables interrupts of level 3 and lower while the target core will busy-wait with interrupts of level 5 and lower disabled. The target core will busy-wait until `esp_ipc_isr_release_other_cpu`{.interpreted-text role="cpp:func"} is called.

- `esp_ipc_isr_release_other_cpu`{.interpreted-text role="cpp:func"} resumes the target core.
:::
...
- `system/ipc/ipc_isr/xtensa`{.interpreted-text role="example"} demonstrates how to use the IPC ISR feature to run an IPC in the context of a High Priority Interrupt, allowing users to quickly get the state of the other CPU, with two assembly callbacks that return the PS register of the other core and save/restore registers while performing tasks based on input arguments.
:::

::: only
CONFIG_IDF_TARGET_ARCH_RISCV

- `system/ipc/ipc_isr/riscv`{.interpreted-text role="example"} demonstrates how to use the IPC ISR feature on {IDF_TARGET_NAME} to run an IPC in the context of a High Priority Interrupt, including how to quickly get the state of the other CPU and how to return multiple values from the callback function.
:::
