---
original_file_path: migration-guides/release-6.x/6.0/system.rst
---
...
`CONFIG_LIBC_PICOLIBC_NEWLIB_COMPATIBILITY`{.interpreted-text role="ref"}, which is enabled by default, provides limited compatibility with Newlib by providing thread-local copies of `global stdin`, `stdout`, `stderr`, and the `getreent()` implementation. If a library built with Newlib headers operates with \"internal\" fields of \"struct reent\", there may be task stack corruption. Note that manipulating `struct reent` fields is expected only by the Newlib library itself.

If you are not linking against external libraries built against Newlib headers, you may disable `CONFIG_LIBC_PICOLIBC_NEWLIB_COMPATIBILITY`{.interpreted-text role="ref"} to save a small amount of memory.

Newlib is still maintained in ESP-IDF toolchains. To switch to using it, select Newlib in menuconfig via the option LIBC_NEWLIB in `CONFIG_LIBC`{.interpreted-text role="ref"}.
...
Even when `CONFIG_LIBC_NEWLIB_NANO_FORMAT`{.interpreted-text role="ref"} is enabled, which disables float formatting, applications with Picolibc are still smaller by 6% (224,592 vs 239,888 bytes).
::::
...
In previous versions of ESP-IDF, the API `esp_sleep_get_wakeup_cause`{.interpreted-text role="cpp:func"} was used to retrieve the wakeup reason after the chip exited sleep. However, this function only returns one wakeup source, even if multiple sources were triggered simultaneously, which may cause users to miss other active wakeup events.

Since ESP-IDF v6.0, a new API `esp_sleep_get_wakeup_causes`{.interpreted-text role="cpp:func"} has been introduced. This function returns a bitmap representing all wakeup sources that caused the chip to exit sleep. Each bit corresponds to a value in the `esp_sleep_wakeup_cause_t`{.interpreted-text role="cpp:type"} enum (e.g., ESP_SLEEP_WAKEUP_TIMER, ESP_SLEEP_WAKEUP_EXT1, etc.). Users can check each wakeup source using bitwise operations.

The original `esp_sleep_get_wakeup_cause()`{.interpreted-text role="cpp:func"} function has been marked as deprecated, and it is recommended to migrate to the new interface. This legacy API may be removed in future versions. Migration can be performed as shown in the example below:
...
For runtime configuration override, a new callback system is available. See the `Application Tracing documentation <../../../api-guides/app_trace>`{.interpreted-text role="doc"} for details on `esp_apptrace_get_user_params()` and `esp_trace_get_user_params()`.
...
- `xTaskGetAffinity`{.interpreted-text role="cpp:func"} -- Use `xTaskGetCoreID`{.interpreted-text role="cpp:func"} instead.
- `xTaskGetIdleTaskHandleForCPU`{.interpreted-text role="cpp:func"} -- Use `xTaskGetIdleTaskHandleForCore`{.interpreted-text role="cpp:func"} instead.
- `xTaskGetCurrentTaskHandleForCPU`{.interpreted-text role="cpp:func"} -- Use `xTaskGetCurrentTaskHandleForCore`{.interpreted-text role="cpp:func"} instead.
...
- `xQueueGenericReceive`{.interpreted-text role="cpp:func"} - Use `xQueueReceive`{.interpreted-text role="cpp:func"}, `xQueuePeek`{.interpreted-text role="cpp:func"}, or `xQueueSemaphoreTake`{.interpreted-text role="cpp:func"} instead, depending on your use case.
- `vTaskDelayUntil`{.interpreted-text role="cpp:func"} - Use `xTaskDelayUntil`{.interpreted-text role="cpp:func"} instead.
- `ulTaskNotifyTake`{.interpreted-text role="cpp:func"} - Use the macro `ulTaskNotifyTake` instead.
- `xTaskNotifyWait`{.interpreted-text role="cpp:func"} - Use the macro `xTaskNotifyWait` instead.

### Deprecated Functions

The function `pxTaskGetStackStart`{.interpreted-text role="cpp:func"} has been deprecated. Use `xTaskGetStackStart`{.interpreted-text role="cpp:func"} instead for improved type safety.
...
To reduce IRAM usage, the default placement for [esp_ringbuf]{.title-ref} functions has been changed from IRAM to Flash. Consequently, the `CONFIG_RINGBUF_PLACE_FUNCTIONS_INTO_FLASH` option has been removed. This change saves a significant amount of IRAM but may have a slight performance impact. For performance-critical applications, the previous behavior can be restored by enabling the new `CONFIG_RINGBUF_IN_IRAM`{.interpreted-text role="ref"} option.
...
- `esp_log_buffer_hex`{.interpreted-text role="cpp:func"} - Use `ESP_LOG_BUFFER_HEX`{.interpreted-text role="cpp:func"} instead.
- `esp_log_buffer_char`{.interpreted-text role="cpp:func"} - Use `ESP_LOG_BUFFER_CHAR`{.interpreted-text role="cpp:func"} instead.
...
Binary data format has been dropped. [CONFIG_ESP_COREDUMP_DATA_FORMAT_BIN]{.title-ref} is no longer supported. ELF is now the default data format.

CRC data integrity check has been dropped. [ESP_COREDUMP_CHECKSUM_CRC32]{.title-ref} is no longer supported. SHA256 is now the default checksum algorithm.

The function `esp_core_dump_partition_and_size_get()`{.interpreted-text role="cpp:func"} now returns [ESP_ERR_NOT_FOUND]{.title-ref} for blank (erased) partitions instead of [ESP_ERR_INVALID_SIZE]{.title-ref}.
...
To use partial download features in your OTA applications, you need to enable the component-level configuration `CONFIG_ESP_HTTPS_OTA_ENABLE_PARTIAL_DOWNLOAD`{.interpreted-text role="ref"} in menuconfig (`Component config` \> `ESP HTTPS OTA` \> `Enable partial HTTP download for OTA`).

### Removed Deprecated APIs

The following deprecated functions have been removed from the `app_update` component:

- `esp_ota_get_app_description`{.interpreted-text role="cpp:func"} -- Use `esp_app_get_description`{.interpreted-text role="cpp:func"} instead.
- `esp_ota_get_app_elf_sha256`{.interpreted-text role="cpp:func"} -- Use `esp_app_get_elf_sha256`{.interpreted-text role="cpp:func"} instead.
...
`CONFIG_COMPILER_ASSERT_NDEBUG_EVALUATE`{.interpreted-text role="ref"} default value is changed to [n]{.title-ref}. This means asserts will no longer evaluate the expression inside the assert when `NDEBUG` is set. This reverts the default behavior to be in line with the C standard.

## ULP

The LP-Core will now wake up the main CPU when it encounters an exception during deep sleep. This feature is enabled by default but can be disabled via the `CONFIG_ULP_TRAP_WAKEUP`{.interpreted-text role="ref"} Kconfig option is this behavior is not desired.

## Heap

In previous versions of ESP-IDF, the capability `MALLOC_CAP_EXEC` would be available regardless of the memory protection configuration state. This implied that a call to e.g., `heap_caps_malloc`{.interpreted-text role="cpp:func"} with `MALLOC_CAP_EXEC` would return NULL when `CONFIG_ESP_SYSTEM_MEMPROT_FEATURE` or `CONFIG_ESP_SYSTEM_PMP_IDRAM_SPLIT` are enabled.
