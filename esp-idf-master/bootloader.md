---
original_file_path: api-guides/bootloader.rst
---
...
2.  Initialize `/security/flash-encryption`{.interpreted-text role="doc"} and/or `Secure Boot </security/secure-boot-v2>`{.interpreted-text role="doc"} features, if configured;
...
For a full description of the startup process including the ESP-IDF second stage bootloader, see `startup`{.interpreted-text role="doc"}.

## Bootloader Compatibility

It is recommended to update to newer `versions of ESP-IDF </versions>`{.interpreted-text role="doc"}: when they are released. The OTA (over the air) update process can flash new apps in the field but cannot flash a new bootloader. For this reason, the bootloader supports booting apps built from newer versions of ESP-IDF.
...
Bootloaders built from very old versions of ESP-IDF (before ESP-IDF V2.1) perform less hardware configuration than newer versions. When using a bootloader from these early ESP-IDF versions and building a new app, enable the config option `CONFIG_APP_COMPATIBLE_PRE_V2_1_BOOTLOADERS`{.interpreted-text role="ref"}.

### Before ESP-IDF V3.1

Bootloaders built from versions of ESP-IDF before V3.1 do not support MD5 checksums in the partition table binary. When using a bootloader from these ESP-IDF versions and building a new app, enable the config option `CONFIG_APP_COMPATIBLE_PRE_V3_1_BOOTLOADERS`{.interpreted-text role="ref"}.

### Before ESP-IDF V5.1

Bootloaders built from versions of ESP-IDF prior to V5.1 do not support `CONFIG_ESP_SYSTEM_ESP32_SRAM1_REGION_AS_IRAM`{.interpreted-text role="ref"}. When using a bootloader from these ESP-IDF versions and building a new app, you should not use this option.
:::

### SPI Flash Configuration

Each ESP-IDF application or bootloader .bin file contains a header with `CONFIG_ESPTOOLPY_FLASHMODE`{.interpreted-text role="ref"}, `CONFIG_ESPTOOLPY_FLASHFREQ`{.interpreted-text role="ref"}, `CONFIG_ESPTOOLPY_FLASHSIZE`{.interpreted-text role="ref"} embedded in it. These are used to configure the SPI flash during boot.

The `first-stage-bootloader`{.interpreted-text role="ref"} reads the `second-stage-bootloader`{.interpreted-text role="ref"} header information from flash and uses this information to load the rest of the `second-stage-bootloader`{.interpreted-text role="ref"} from flash. However, at this time the system clock speed is lower than configured and not all flash modes are supported. When the `second-stage-bootloader`{.interpreted-text role="ref"} then runs, it will reconfigure the flash using values read from the currently selected app binary\'s header (and NOT from the `second-stage-bootloader`{.interpreted-text role="ref"} header). This allows an OTA update to change the SPI flash settings in use.
...
The default bootloader log level is \"Info\". By setting the `CONFIG_BOOTLOADER_LOG_LEVEL`{.interpreted-text role="ref"} option, it is possible to increase or decrease this level. This log level is separate from the log level used in the app (see `/api-reference/system/log`{.interpreted-text role="doc"}).
...
To roll back to the original \"factory\" device configuration and clear any user settings, configure the config item `CONFIG_BOOTLOADER_FACTORY_RESET`{.interpreted-text role="ref"} in the bootloader.

The factory reset mechanism allows the device to be factory reset in two ways:

- Clear one or more data partitions. The `CONFIG_BOOTLOADER_DATA_FACTORY_RESET`{.interpreted-text role="ref"} option allows users to specify which data partitions will be erased when the factory reset is executed.
...
- Boot from \"factory\" app partition. Enabling the `CONFIG_BOOTLOADER_OTA_DATA_ERASE`{.interpreted-text role="ref"} option will cause the device to boot from the default \"factory\" app partition after a factory reset (or if there is no factory app partition in the partition table then the default ota app partition is selected instead). This reset process involves erasing the OTA data partition which holds the currently selected OTA partition slot. The \"factory\" app partition slot (if it exists) is never updated via OTA, so resetting to this allows reverting to a \"known good\" firmware application.
...
- `CONFIG_BOOTLOADER_NUM_PIN_FACTORY_RESET`{.interpreted-text role="ref"}- The input GPIO number used to trigger a factory reset. This GPIO must be pulled low or high (configurable) on reset to trigger this.
- `CONFIG_BOOTLOADER_HOLD_TIME_GPIO`{.interpreted-text role="ref"}- this is hold time of GPIO for reset/test mode (by default 5 seconds). The GPIO must be held continuously for this period of time after reset before a factory reset or test partition boot (as applicable) is performed.
- `CONFIG_BOOTLOADER_FACTORY_RESET_PIN_LEVEL`{.interpreted-text role="ref"} - configure whether a factory reset should trigger on a high or low level of the GPIO. If the GPIO has an internal pullup then this is enabled before the pin is sampled, consult the {IDF_TARGET_NAME} datasheet for details on pin internal pullups.

::: only
SOC_RTC_FAST_MEM_SUPPORTED

If an application needs to know if the factory reset has occurred, users can call the function `bootloader_common_get_rtc_retain_mem_factory_reset_state`{.interpreted-text role="cpp:func"}.
...
Note that this feature reserves some RTC FAST memory (the same size as the `CONFIG_BOOTLOADER_SKIP_VALIDATE_IN_DEEP_SLEEP`{.interpreted-text role="ref"} feature).
:::

::: only
not SOC_RTC_FAST_MEM_SUPPORTED

Sometimes an application needs to know if the factory reset has occurred. The {IDF_TARGET_NAME} chip does not have RTC FAST memory, so there is no API to detect it. Instead, there is a workaround: you need an NVS partition that will be erased by the bootloader if factory reset occurs (add this partition to `CONFIG_BOOTLOADER_DATA_FACTORY_RESET`{.interpreted-text role="ref"}). In this NVS partition, create a \"factory_reset_state\" token that will be increased in the application. If the \"factory_reset_state\" is 0 then the factory reset has occurred.
:::

## Boot from Test Firmware {#bootloader_boot_from_test_firmware}

It is possible to write a special firmware app for testing in production, and boot this firmware when needed. The project partition table will need a dedicated app partition entry for this testing app, type `app` and subtype `test` (see `/api-guides/partition-tables`{.interpreted-text role="doc"}).
...
To support this functionality in the main project\'s bootloader, set the configuration item `CONFIG_BOOTLOADER_APP_TEST`{.interpreted-text role="ref"} and configure the following three items:

- `CONFIG_BOOTLOADER_NUM_PIN_APP_TEST`{.interpreted-text role="ref"} - GPIO number to boot test partition. The selected GPIO will be configured as an input with internal pull-up enabled. This GPIO must be pulled low or high (configurable) on reset to trigger this.
...
- `CONFIG_BOOTLOADER_HOLD_TIME_GPIO`{.interpreted-text role="ref"} - this is the hold time of GPIO for reset/test mode (by default 5 seconds). The GPIO must be held continuously for this period of time after reset before a factory reset or test partition boot (as applicable) is performed.

- `CONFIG_BOOTLOADER_APP_TEST_PIN_LEVEL`{.interpreted-text role="ref"} - configure whether a test partition boot should trigger on a high or low level of the GPIO. If the GPIO has an internal pull-up, then this is enabled before the pin is sampled. Consult the {IDF_TARGET_NAME} datasheet for details on pin internal pull-ups.

## Rollback

Rollback and anti-rollback features must be configured in the bootloader as well.

Consult the `app_rollback`{.interpreted-text role="ref"} and `anti-rollback`{.interpreted-text role="ref"} sections in the `OTA API reference document </api-reference/system/ota>`{.interpreted-text role="doc"}.

## Watchdog {#bootloader-watchdog}

The chips come equipped with two groups of watchdog timers: Main System Watchdog Timer (MWDT_WDT) and RTC Watchdog Timer (RTC_WDT). Both watchdog timer groups are enabled when the chip is powered up. However, in the bootloader, they will both be disabled. If `CONFIG_BOOTLOADER_WDT_ENABLE`{.interpreted-text role="ref"} is set (which is the default behavior), RTC_WDT is re-enabled. It tracks the time from the bootloader is enabled until the user\'s main function is called. In this scenario, RTC_WDT remains operational and will automatically reset the chip if no application successfully starts within 9 seconds. This functionality is particularly useful in preventing lockups caused by an unstable power source during startup.

- The timeout period can be adjusted by setting `CONFIG_BOOTLOADER_WDT_TIME_MS`{.interpreted-text role="ref"} and recompiling the bootloader.
- The RTC Watchdog can be disabled in the bootloader by disabling the `CONFIG_BOOTLOADER_WDT_ENABLE`{.interpreted-text role="ref"} setting and recompiling the bootloader. This is not recommended.
- See `app-hardware-watchdog-timers`{.interpreted-text role="ref"} to learn how RTC_WDT is used in the application.
...
When enabling additional bootloader functions, including `/security/flash-encryption`{.interpreted-text role="doc"} or Secure Boot, and especially if setting a high `CONFIG_BOOTLOADER_LOG_LEVEL`{.interpreted-text role="ref"} level, then it is important to monitor the bootloader .bin file\'s size.

When using the default `CONFIG_PARTITION_TABLE_OFFSET`{.interpreted-text role="ref"} value 0x8000, the size limit is {IDF_TARGET_CONFIG_PARTITION_TABLE_OFFSET} bytes.
...
- Set `bootloader compiler optimization <CONFIG_BOOTLOADER_COMPILER_OPTIMIZATION>`{.interpreted-text role="ref"} back to \"Size\" if it has been changed from this default value.
- Reduce `bootloader log level <CONFIG_BOOTLOADER_LOG_LEVEL>`{.interpreted-text role="ref"}. Setting log level to Warning, Error or None all significantly reduce the final binary size (but may make it harder to debug).
- Set `CONFIG_PARTITION_TABLE_OFFSET`{.interpreted-text role="ref"} to a higher value than 0x8000, to place the partition table later in the flash. This increases the space available for the bootloader. If the `partition table </api-guides/partition-tables>`{.interpreted-text role="doc"} CSV file contains explicit partition offsets, they will need changing so no partition has an offset lower than `CONFIG_PARTITION_TABLE_OFFSET + 0x1000`. (This includes the default partition CSV files supplied with ESP-IDF.)
...
The bootloader has the `CONFIG_BOOTLOADER_SKIP_VALIDATE_IN_DEEP_SLEEP`{.interpreted-text role="ref"} option which allows the wake-up time from Deep-sleep to be reduced (useful for reducing power consumption). This option is available when the `CONFIG_SECURE_BOOT`{.interpreted-text role="ref"} option is disabled or `CONFIG_SECURE_BOOT_INSECURE`{.interpreted-text role="ref"} is enabled along with Secure Boot. The reduction in time is achieved by ignoring image verification.
...
The current bootloader implementation allows a project to extend it or modify it. There are two ways of doing it: by implementing hooks or by overriding it. Both ways are presented in `custom_bootloader`{.interpreted-text role="example"} folder in ESP-IDF examples:

- `custom_bootloader/bootloader_hooks`{.interpreted-text role="example"} presents how to connect some hooks to the bootloader initialization
- `custom_bootloader/bootloader_override`{.interpreted-text role="example"} presents how to override the bootloader implementation

In the bootloader space, you cannot use the drivers and functions from other components unless they explicitly support run in bootloader. If necessary, then the required functionality should be placed in the project\'s [bootloader_components]{.title-ref} directory (note that this will increase its size). Examples of components that can be used in the bootloader are:

- `storage/nvs/nvs_bootloader`{.interpreted-text role="example"}

If the bootloader grows too large then it can collide with the partition table, which is flashed at offset 0x8000 by default. Increase the `partition table offset <CONFIG_PARTITION_TABLE_OFFSET>`{.interpreted-text role="ref"} value to place the partition table later in the flash. This increases the space available for the bootloader.
...
- The eFuse can be set using `espefuse` or by calling `esp_efuse_set_recovery_bootloader_offset()`{.interpreted-text role="cpp:func"} in the user application.
...
- If the secure version in eFuse is not updated in the ROM bootloader, then the application can update it using the `esp_efuse_write_field_blob`{.interpreted-text role="cpp:func"} function.
