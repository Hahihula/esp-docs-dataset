---
original_file_path: api-guides/startup.rst
---
...
::: list
1.  `first-stage-bootloader`{.interpreted-text role="ref"} loads the second stage bootloader image to RAM (IRAM & DRAM) from flash offset {IDF_TARGET_CONFIG_BOOTLOADER_OFFSET_IN_FLASH}.
2.  `second-stage-bootloader`{.interpreted-text role="ref"} loads partition table and main app image from flash. Main app incorporates both RAM segments and read-only segments mapped via flash cache.

SOC_HP_CPU_HAS_MULTIPLE_CORES

:   3.  `application-startup`{.interpreted-text role="ref"} executes. At this point, the second CPU and RTOS scheduler are started, which then run the `main_task`, leading to the execution of `app_main`.

not SOC_HP_CPU_HAS_MULTIPLE_CORES

:   3.  `application-startup`{.interpreted-text role="ref"} executes. At this point, the RTOS scheduler is started, which then runs the `main_task`, leading to the execution of `app_main`.
:::

This process is explained in detail in the following sections.

## First stage (ROM) bootloader {#first-stage-bootloader}
...
Second stage bootloader binary image is loaded from flash starting at address {IDF_TARGET_CONFIG_BOOTLOADER_OFFSET_IN_FLASH}. If `/security/secure-boot-v1`{.interpreted-text role="doc"} is in use then the first 4 kB sector of flash is used to store secure boot IV and digest of the bootloader image. Otherwise, this sector is unused.
:::
...
Second stage bootloader reads the partition table found by default at offset {IDF_TARGET_CONFIG_PARTITION_TABLE_OFFSET} (`configurable value <CONFIG_PARTITION_TABLE_OFFSET>`{.interpreted-text role="ref"}). See `partition tables <partition-tables>`{.interpreted-text role="doc"} documentation for more information. The bootloader finds factory and OTA app partitions. If OTA app partitions are found in the partition table, the bootloader consults the `otadata` partition to determine which one should be booted. See `/api-reference/system/ota`{.interpreted-text role="doc"} for more information.

For a full description of the configuration options available for the ESP-IDF bootloader, see `bootloader`{.interpreted-text role="doc"}.

For the selected partition, second stage bootloader reads the binary image from flash one segment at a time:

- For segments with load addresses in internal `iram`{.interpreted-text role="ref"} or `dram`{.interpreted-text role="ref"}, the contents are copied from flash to the load address.
- For segments which have load addresses in `drom`{.interpreted-text role="ref"} or `irom`{.interpreted-text role="ref"} regions, the flash MMU is configured to provide the correct mapping from the flash to the load address.
...
Understanding all stages of ESP-IDF app initialization is often not necessary. To understand initialization from the application developer\'s perspective only, skip forward to `app-main-task`{.interpreted-text role="ref"}.
::::

### Port Initialization

ESP-IDF application entry point is `call_start_cpu0` function found in `components/esp_system/port/cpu_start.c`{.interpreted-text role="idf_file"}. This function is executed by the second stage bootloader, and never returns.
...
::: list
- Reconfigure CPU exceptions for the app (allowing app interrupt handlers to run, and causing `fatal-errors`{.interpreted-text role="doc"} to be handled using the options configured for the app rather than the simpler error handler provided by ROM).
- If the option `CONFIG_BOOTLOADER_WDT_ENABLE`{.interpreted-text role="ref"} is not set then the RTC watchdog timer is disabled.
- Initialize internal memory (data & bss).

\- Finish configuring the MMU cache. :SOC_SPIRAM_SUPPORTED: - Enable PSRAM if configured. - Set the CPU clocks to the frequencies configured for the project. :SOC_MEMPROT_SUPPORTED: - Initialize memory protection if configured. :esp32: - Reconfigure the main SPI flash based on the app header settings (necessary for compatibility with bootloader versions before ESP-IDF V4.0, see `bootloader-compatibility`{.interpreted-text role="ref"}). :SOC_HP_CPU_HAS_MULTIPLE_CORES: - If the app is configured to run on multiple cores, start the other core and wait for it to initialize as well (inside the similar \"port layer\" initialization function `call_start_cpu1`).
:::

::: only
SOC_HP_CPU_HAS_MULTIPLE_CORES

Once `call_start_cpu0` completes running, it calls the \"system layer\" initialization function `start_cpu0` found in `components/esp_system/startup.c`{.interpreted-text role="idf_file"}. Other cores will also complete port-layer initialization and call `start_other_cores` found in the same file.
:::

::: only
not SOC_HP_CPU_HAS_MULTIPLE_CORES

Once `call_start_cpu0` completes running, it calls the \"system layer\" initialization function `start_cpu0` found in `components/esp_system/startup.c`{.interpreted-text role="idf_file"}.
:::
...
::: list
- Log information about this application (project name, `app-version`{.interpreted-text role="ref"}, etc.) if default log level enables this.
...
\- Setup libc stdin, stdout, and stderr according to the `serial console configuration <CONFIG_ESP_CONSOLE_UART>`{.interpreted-text role="ref"}. :esp32: - Perform any security-related checks, including burning efuses that should be burned for this configuration (including `disabling ROM download mode on ESP32 V3 <CONFIG_SECURE_UART_ROM_DL_MODE>`{.interpreted-text role="ref"}, `CONFIG_ESP32_DISABLE_BASIC_ROM_CONSOLE`{.interpreted-text role="ref"}). :not esp32: - Perform any security-related checks, including burning efuses that should be burned for this configuration (including `permanently limiting ROM download modes <CONFIG_SECURE_UART_ROM_DL_MODE>`{.interpreted-text role="ref"}). - Initialize SPI flash API support. - Call global C++ constructors and any C functions marked with `__attribute__((constructor))`.
:::

Secondary system initialization allows individual components to be initialized. If a component has an initialization function annotated with the `ESP_SYSTEM_INIT_FN` macro, it will be called as part of secondary initialization. Component initialization functions have priorities assigned to them to ensure the desired initialization order. The priorities are documented in `esp_system/system_init_fn.txt`{.interpreted-text role="component_file"} and `ESP_SYSTEM_INIT_FN` definition in source code are checked against this file.

### Running the Main Task {#app-main-task}
...
The main task that runs `app_main` has a fixed RTOS priority (one higher than the minimum) and a `configurable stack size <CONFIG_ESP_MAIN_TASK_STACK_SIZE>`{.interpreted-text role="ref"}.

::: only
SOC_HP_CPU_HAS_MULTIPLE_CORES

The main task core affinity is also configurable: `CONFIG_ESP_MAIN_TASK_AFFINITY`{.interpreted-text role="ref"}.
:::
...
When running system initialization, the code on PRO CPU sets the entry point for APP CPU, de-asserts APP CPU reset, and waits for a global flag to be set by the code running on APP CPU, indicating that it has started. Once this is done, APP CPU jumps to `call_start_cpu1` function in `components/esp_system/port/cpu_start.c`{.interpreted-text role="idf_file"}.
