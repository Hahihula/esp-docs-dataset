---
original_file_path: api-guides/linker-script-generation.rst
---
...
There are several `memory regions <memory-layout>`{.interpreted-text role="ref"} where code and data can be placed. Code and read-only data are placed by default in flash, writable data in RAM, etc. However, it is sometimes necessary to change these default placements.
...
#### Placing Object Files {#ldgen-placing-object-files }
...
What happens to `my_src3.o`? Since it is not specified, default placements are used for `my_src3.o`. More on default placements `here <ldgen-default-placements>`{.interpreted-text role="ref"}.
...
There are `limitations<ldgen-symbol-granularity-placements>`{.interpreted-text role="ref"} in placing code/data at symbol granularity. In order to ensure proper placements, an alternative would be to group relevant code and data into source files, and `use object-granularity placements<ldgen-placing-object-files>`{.interpreted-text role="ref"}.
::::
...
#### Configuration-Dependent Placements {#ldgen-conditional-placements}
...
### The \'default\' Placements {#ldgen-default-placements}

Up until this point, the term \'default placements\' has been mentioned as fallback placements when the placement rules `rtc` and `noflash` are not specified. It is important to note that the tokens `noflash` or `rtc` are not merely keywords, but are actually entities called fragments, specifically `schemes <ldgen-scheme-fragment>`{.interpreted-text role="ref"}.

In the same manner as `rtc` and `noflash` are schemes, there exists a `default` scheme which defines what the default placement rules should be. As the name suggests, it is where code and data are usually placed, i.e., code/constants is placed in flash, variables placed in RAM, etc. More on the default scheme `here <ldgen-default-scheme>`{.interpreted-text role="ref"}.
...
For an example of an ESP-IDF component using the linker script generation mechanism, see `freertos/CMakeLists.txt`{.interpreted-text role="component_file"}. `freertos` uses this to place its object files to the instruction RAM for performance reasons.
::::
...
Linking is the last step in the process of turning C/C++ source files into an executable. It is performed by the toolchain\'s linker, and accepts linker scripts which specify code/data placements, among other things. With the linker script generation mechanism, this process is no different, except that the linker script passed to the linker is dynamically generated from: (1) the collected `linker fragment files <ldgen-linker-fragment-files>`{.interpreted-text role="ref"} and (2) `linker script template <ldgen-linker-script-template>`{.interpreted-text role="ref"}.
...
The tool that implements the linker script generation mechanism lives under `tools/ldgen`{.interpreted-text role="idf"}.
::::

### Linker Fragment Files {#ldgen-linker-fragment-files }

As mentioned in the quick start guide, fragment files are simple text files with the `.lf` extension containing the desired placements. This is a simplified description of what fragment files contain, however. What fragment files actually contain are \'fragments\'. Fragments are entities which contain pieces of information which, when put together, form placement rules that tell where to place sections of object files in the output binary. There are three types of fragments: `sections <ldgen-sections-fragment>`{.interpreted-text role="ref"}, `scheme <ldgen-scheme-fragment>`{.interpreted-text role="ref"} and `mapping <ldgen-mapping-fragment>`{.interpreted-text role="ref"}.
...
- key, value: Contents of the fragment; each fragment type may support different keys and different grammars for the key values.
  ...
  > - For `sections <ldgen-sections-fragment>`{.interpreted-text role="ref"} and `scheme <ldgen-scheme-fragment>`{.interpreted-text role="ref"}, the only supported key is `entries`
  > - For `mappings <ldgen-mapping-fragment>`{.interpreted-text role="ref"}, both `archive` and `entries` are supported.
...
The `default scheme` is defined in `esp_system/app.lf`{.interpreted-text role="component_file"}. The `noflash` and `rtc` scheme fragments which are built-in schemes referenced in the quick start guide are also defined in this file.

::: {#ldgen-mapping-fragment }
**Mapping**
:::
...
#### On Symbol-Granularity Placements {#ldgen-symbol-granularity-placements }
...
### Linker Script Template {#ldgen-linker-script-template }
...
>
> The linker script template currently used is `esp_system/ld/{IDF_TARGET_PATH_NAME}/sections.ld.in`{.interpreted-text role="component_file"}; the generated output script `sections.ld` is put under its build directory.
