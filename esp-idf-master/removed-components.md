---
original_file_path: migration-guides/release-5.x/5.0/removed-components.rst
---
...
Please note that http parser functionality which was previously part of `nghttp` component is now part of `http_parser <http_parser>`{.interpreted-text role="component"} component.
::::
...
- `tcpip_adapter`. Please use the `ESP-NETIF </api-reference/network/esp_netif>`{.interpreted-text role="doc"} component instead; you can follow the `tcpip-adapter`{.interpreted-text role="ref"}.
...
OpenSSL-API component is no longer supported. It is not available in the ESP Component Registry, either. Please use `ESP-TLS </api-reference/protocols/esp_tls>`{.interpreted-text role="doc"} or `mbedtls`{.interpreted-text role="component"} API directly.
::::
