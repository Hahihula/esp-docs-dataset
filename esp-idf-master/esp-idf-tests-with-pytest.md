---
original_file_path: contribute/esp-idf-tests-with-pytest.rst
---
...
- [pytest-embedded](https://github.com/espressif/pytest-embedded) with default services `esp,idf`
- [pytest-rerunfailures](https://github.com/pytest-dev/pytest-rerunfailures)
- [pytest-ignore-test-results](https://github.com/espressif/pytest-ignore-test-results)
...
This is a simple test script that could run with the ESP-IDF getting-started example `get-started/hello_world`{.interpreted-text role="example"}.
...
For the detailed explanation of the environment markers, please refer to `env_markers definition <pytest.ini>`{.interpreted-text role="idf_file"}
::::
...
For detailed multi-dut parametrization documentation, please refer to [pytest-embedded Multi-DUT documentation](https://docs.espressif.com/projects/pytest-embedded/en/latest/key_concepts.html#multi-duts).
::::
...
::: {.blockdiag caption="Target Test Child Pipeline Workflow" align="center"}
...
All build jobs and target test jobs are generated automatically by our CI script `tools/ci/dynamic_pipelines`{.interpreted-text role="project"}.
...
We also have a set of common components (defined as `common_components` in `.idf_build_apps.toml`{.interpreted-text role="idf_file"}). `common_components` is a list of baseline (core) components that are used by many apps. In general, if one of these components changes, you usually want to rebuild and retest the apps that depend on it.
...
Deprecated (prefer using `depends_components` / `common_components` instead): `deactivate_dependency_driven_build_by_components` disables the dependency-driven checks if certain components change.
...
## Running Tests Locally {#run_the_tests_locally}
...
This is the simplest use case. Let\'s take `examples/get-started/hello_world`{.interpreted-text role="project"} as an example. Assume we\'re testing with a ESP32 board.
...
Some test cases may need to run with different sdkconfig files. Let\'s take `examples/system/console/basic`{.interpreted-text role="project"} as an example. Assume we\'re testing with a ESP32 board, and test with `sdkconfig.ci.history`.
...
Some test cases may need to run with multiple DUTs. Let\'s take `examples/openthread`{.interpreted-text role="project"} as an example. The test case function looks like this:
...
This code example is taken from `panic/conftest.py <tools/test_apps/system/panic/conftest.py>`{.interpreted-text role="idf_file"}.
...
This code example is taken from `pytest_esp_eth.py <components/esp_eth/test_apps/pytest_esp_eth.py>`{.interpreted-text role="idf_file"}.
...
This code example is taken from `pytest_panic.py <tools/test_apps/system/panic/pytest_panic.py>`{.interpreted-text role="idf_file"}
...
You can add new markers by adding one line under the `pytest.ini`{.interpreted-text role="idf_file"}. If it is a marker that specifies a type of test environment, it should be added into `env_markers` section. Otherwise, it should be added into `markers` section. The syntax should be: `<marker_name>: <marker_description>`.
