---
original_file_path: api-reference/system/himem.rst
---
...
In order to use the Himem API, you have to enable it in the menuconfig using `CONFIG_SPIRAM_BANKSWITCH_ENABLE`{.interpreted-text role="ref"}, as well as set the amount of banks reserved for this in `CONFIG_SPIRAM_BANKSWITCH_RESERVE`{.interpreted-text role="ref"}. This decreases the amount of external memory allocated by functions like `malloc()`, but it allows you to use the Himem API to map any of the remaining memory into the reserved banks.
...
- `system/himem`{.interpreted-text role="example"} demonstrates how to use the Himem API on {IDF_TARGET_NAME} to run a memory test of the upper 4 MiB of an 8 MiB PSRAM chip, illustrating how to allocate address space, allocate the physical memory, and switch it in or out of the allocated address space.
