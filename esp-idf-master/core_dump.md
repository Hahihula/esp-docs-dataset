---
original_file_path: api-guides/core_dump.rst
---
...
Core dump data is saved to a core dump file according to a particular format, see `Core dump internals <core_dump_internals>`{.interpreted-text role="doc"} for more details. However, ESP-IDF\'s `idf.py` command provides special subcommands to decode and analyze the core dump file.
...
The `CONFIG_ESP_COREDUMP_TO_FLASH_OR_UART`{.interpreted-text role="ref"} option enables or disables core dump, and selects the core dump destination if enabled. When a crash occurs, the generated core dump file can either be saved to flash, or output to a connected host over UART.
...
The `CONFIG_ESP_COREDUMP_MAX_TASKS_NUM`{.interpreted-text role="ref"} option configures the number of task snapshots saved by the core dump. Crashed task registers and the stack are always saved, regardless of this configuration option. Other tasks are included in order of their priority (starting with the highest-priority ready task).
...
Core dump routines run from a separate stack due to core dump itself needing to parse and save all other task stacks. The `CONFIG_ESP_COREDUMP_STACK_SIZE`{.interpreted-text role="ref"} option controls the size of the core dump\'s stack in number of bytes.
...
By default, core dumps typically save CPU registers, tasks data and summary of the panic reason. When the `CONFIG_ESP_COREDUMP_CAPTURE_DRAM`{.interpreted-text role="ref"} option is selected, `.bss` and `.data` sections and `heap` data will also be part of the dump.
...
The `idf.py coredump-info` and `idf.py coredump-debug` commands are wrappers around the [esp-coredump]{.title-ref} tool for easier use in the ESP-IDF environment. For more information see `core_dump_commands`{.interpreted-text role="ref"} section.
::::

## Core Dump to UART

When the core dump file is output to UART, the output file is Base64-encoded. The `CONFIG_ESP_COREDUMP_DECODE`{.interpreted-text role="ref"} option allows for selecting whether the output file is automatically decoded by the ESP-IDF monitor or kept encoded for manual decoding.

### Automatic Decoding

If `CONFIG_ESP_COREDUMP_DECODE`{.interpreted-text role="ref"} is set to automatically decode the UART core dump, ESP-IDF monitor will automatically decode the data, translate any function addresses to source code lines, and display it in the monitor. The output to ESP-IDF monitor would resemble the following output:
...
The `CONFIG_ESP_COREDUMP_UART_DELAY`{.interpreted-text role="ref"} allows for an optional delay to be added before the core dump file is output to UART.

### Manual Decoding

If you set `CONFIG_ESP_COREDUMP_DECODE`{.interpreted-text role="ref"} to no decoding, then the raw Base64-encoded body of core dump is output to UART between the following header and footer of the UART output:
...
## Core Dump Commands {#core_dump_commands}
...
1.  In `project-configuration-menu`{.interpreted-text role="ref"}, enable `COREDUMP TO FLASH <CONFIG_ESP_COREDUMP_TO_FLASH_OR_UART>`{.interpreted-text role="ref"}, then save and exit.
