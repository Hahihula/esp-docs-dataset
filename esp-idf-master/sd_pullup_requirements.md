---
original_file_path: api-reference/peripherals/sd_pullup_requirements.rst
---
...
By default, the MTDI bootstrapping pin is incompatible with the DAT2 line pull-up if the flash voltage is 3.3 V. For more information, see `mtdi_strapping_pin`{.interpreted-text role="ref"} below.
:::
...
- `compatibility_overview_espressif_hw_sdio`{.interpreted-text role="ref"} between the default pull states on pins of Espressif\'s products and the states required by the SD bus
- `sdio_solutions`{.interpreted-text role="ref"} - ideas on how to resolve compatibility issues
- `related_info_sdio`{.interpreted-text role="ref"} - other relevant information

## Overview of Compatibility {#compatibility_overview_espressif_hw_sdio}
...
- ESP32 (except for D2WD versions, see [ESP32 datasheet](https://www.espressif.com/sites/default/files/documentation/esp32_datasheet_en.pdf)):
  ...
  > - `sd_pull-up_no_pull-ups`{.interpreted-text role="ref"}
  > - `strapping_conflicts_dat2`{.interpreted-text role="ref"} for models with 3.3 V flash chip

- ESP32-D2WD:

  > - `sd_pull-up_no_pull-ups`{.interpreted-text role="ref"}
  > - `no_pull-up_on_gpio12`{.interpreted-text role="ref"}
:::
...
- ESP32-PICO-D4:
  ...
  > - `sd_pull-up_no_pull-ups`{.interpreted-text role="ref"}
  > - `strapping_conflicts_dat2`{.interpreted-text role="ref"}
...
### Development Boards {#sdio_dev_kits}

- ESP32-PICO-KIT, including PICO-KIT v4.1, v4.0, and v3

  > - `sd_pull-up_no_pull-ups`{.interpreted-text role="ref"}
  > - `strapping_conflicts_dat2`{.interpreted-text role="ref"}
  > - `gpio2_strapping_pin`{.interpreted-text role="ref"}
...
- ESP-WROVER-KIT
  ...
  > - `pull-up_conflicts_on_gpio13`{.interpreted-text role="ref"} (v4.1, v3, v2, and v1)
  > - `strapping_conflicts_dat2`{.interpreted-text role="ref"} (v4.1, v2, and v1)
  > - `gpio2_strapping_pin`{.interpreted-text role="ref"} (v2, v1)
  >
...
- ESP32-LyraT
  ...
  > - Required pull-ups are provided
  > - `pull-up_conflicts_on_gpio13`{.interpreted-text role="ref"}
:::
...
## Solutions {#sdio_solutions}

### No Pull-ups {#sd_pull-up_no_pull-ups}
...
- If your host and slave device are on separate boards, replace one of them with a board that has pull-ups. For the list of Espressif\'s development boards with pull-ups, go to `sdio_dev_kits`{.interpreted-text role="ref"}.
- Attach external pull-ups by connecting each pin which requires a pull-up to VDD via a 10 kΩ resistor.
:::

::: only
not esp32 and not esp32s3
...
### Pull-up Conflicts on GPIO13 {#pull-up_conflicts_on_gpio13}
...
### Conflicts Between Bootstrap and SDIO on DAT2 {#strapping_conflicts_dat2}

There is a conflict between the boot strapping requirements of the ESP32 and the SDIO protocol. For details, see `mtdi_strapping_pin`{.interpreted-text role="ref"}.
...
For more details, see **{IDF_TARGET_NAME} Technical Reference Manual** \[[PDF]({IDF_TARGET_TRM_EN_URL}#efuse)\].
...
### No Pull-up on GPIO12 {#no_pull-up_on_gpio12}

Your module is compatible with the SDIO protocol. Just connect GPIO12 to VDD via a 10 kΩ resistor.

### Download Mode Not Working (minor issue) {#gpio2_strapping_pin}
...
## Related Information {#related_info_sdio}

### MTDI Strapping Pin {#mtdi_strapping_pin}
...
- On boards using the internal regulator and a 3.3 V flash chip, GPIO12 must be pulled low at reset. This is incompatible with the SD card operation. For reference information on compatibility of Espressif\'s boards with the SD card operation, see `compatibility_overview_espressif_hw_sdio`{.interpreted-text role="ref"}.
