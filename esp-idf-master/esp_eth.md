---
original_file_path: api-reference/network/esp_eth.rst
---
...
> 1.  `basic-ethernet-concepts`{.interpreted-text role="ref"}
> 2.  `driver-configuration-and-installation`{.interpreted-text role="ref"}
> 3.  `connect-driver-to-stack`{.interpreted-text role="ref"}
> 4.  `misc-operation-of-driver`{.interpreted-text role="ref"}
...
::: {.rackdiag caption="Ethernet Data Frame Format" align="center"}
...
## Configure MAC and PHY {#driver-configuration-and-installation}
...
<img src="../../../_static/rmii-interface.png" alt="Ethernet RMII Interface" />
<figcaption aria-hidden="true">Ethernet RMII Interface</figcaption>
</figure>
...
ESP-IDF only supports the RMII interface. Therefore, always set `eth_esp32_emac_config_t::interface`{.interpreted-text role="cpp:member"} to `eth_data_interface_t::EMAC_DATA_INTERFACE_RMII`{.interpreted-text role="cpp:enumerator"}.
::::
...
- Some PHY chips can derive the `REF_CLK` from its externally connected 25 MHz crystal oscillator (as seen the option **a** in the picture). In this case, you should configure `eth_mac_clock_config_t::clock_mode`{.interpreted-text role="cpp:member"} of `eth_esp32_emac_config_t::clock_config`{.interpreted-text role="cpp:member"} to `emac_rmii_clock_mode_t::EMAC_CLK_EXT_IN`{.interpreted-text role="cpp:enumerator"}.
- Some PHY chip uses an externally connected 50 MHz crystal oscillator or other clock sources, which can also be used as the `REF_CLK` for the MAC side (as seen the option **b** in the picture). In this case, you still need to configure `eth_mac_clock_config_t::clock_mode`{.interpreted-text role="cpp:member"} of `eth_esp32_emac_config_t::clock_config`{.interpreted-text role="cpp:member"} to `emac_rmii_clock_mode_t::EMAC_CLK_EXT_IN`{.interpreted-text role="cpp:enumerator"}.
- Some EMAC controllers can generate the `REF_CLK` using an internal high-precision PLL (as seen the option **c** in the picture). In this case, you should configure `eth_mac_clock_config_t::clock_mode`{.interpreted-text role="cpp:member"} of `eth_esp32_emac_config_t::clock_config`{.interpreted-text role="cpp:member"} to `emac_rmii_clock_mode_t::EMAC_CLK_OUT`{.interpreted-text role="cpp:enumerator"}.
...
If the RMII clock mode is configured to `emac_rmii_clock_mode_t::EMAC_CLK_OUT`{.interpreted-text role="cpp:enumerator"}, internal Audio PLL clock is used as a source of 50 MHz clock. Hence be sure it is not in collision with I2S bus configuration.
...
If the RMII clock mode is configured to `emac_rmii_clock_mode_t::EMAC_CLK_OUT`{.interpreted-text role="cpp:enumerator"}, {IDF_TARGET_SOC_REF_CLK_OUT_GPIO} can be selected as output pin of the `REF_CLK` signal via IO_MUX.

If the RMII clock mode is configured to `emac_rmii_clock_mode_t::EMAC_CLK_EXT_IN`{.interpreted-text role="cpp:enumerator"}, {IDF_TARGET_SOC_REF_CLK_IN_GPIO} can be selected as input pin for the `REF_CLK` signal via IO_MUX.
::::
:::::
...
If the RMII clock mode is configured to `emac_rmii_clock_mode_t::EMAC_CLK_OUT`{.interpreted-text role="cpp:enumerator"}, the `REF_CLK` output signal must be looped back to the EMAC externally. You have to configure `eth_mac_clock_config_t::clock_mode`{.interpreted-text role="cpp:member"} of `eth_esp32_emac_config_t::clock_config_out_in`{.interpreted-text role="cpp:member"} to `emac_rmii_clock_mode_t::EMAC_CLK_EXT_IN`{.interpreted-text role="cpp:enumerator"} and select GPIO number associated with `REF_CLK` input GPIO\'s ({IDF_TARGET_SOC_REF_CLK_IN_GPIO}).

::: only
esp32p4

<figure class="align-center">
<img src="../../../_static/rmii_ref_clk_esp32p4.png" alt="RMII REF_CKL Output Loopback" />
<figcaption aria-hidden="true">RMII REF_CKL Output Loopback</figcaption>
</figure>
:::
:::::
::::::
...
Signals used in the data plane can be configured to predefined set of GPIOs via IO_MUX for the RMII, see below table. The data plane GPIO configuration is performed by the driver based on content of `eth_esp32_emac_config_t::emac_dataif_gpio`{.interpreted-text role="cpp:member"}. Signals used in the control plane can be routed to any free GPIOs via GPIO Matrix.

  -----------------------------------------------------------------------
  Pin Name                            GPIO Number
  ----------------------------------- -----------------------------------
  TX_EN                               {IDF_TARGET_SOC_RMII_TX_EN}

  TXD0                                {IDF_TARGET_SOC_RMII_TXD0}

  TXD1                                {IDF_TARGET_SOC_RMII_TXD1}

  CRS_DV                              {IDF_TARGET_SOC_RMII_CRS_DV}

  RXD0                                {IDF_TARGET_SOC_RMII_RXD0}

  RXD1                                {IDF_TARGET_SOC_RMII_RXD1}
  -----------------------------------------------------------------------

  : {IDF_TARGET_NAME} RMII Data Plane GPIO
::::
:::::
:::::::::::::::::::::::
...
Basic common configuration for MAC layer is described in `eth_mac_config_t`{.interpreted-text role="cpp:class"}, including:

::: list
- `eth_mac_config_t::sw_reset_timeout_ms`{.interpreted-text role="cpp:member"}: software reset timeout value, in milliseconds. Typically, MAC reset should be finished within 100 ms.
- `eth_mac_config_t::rx_task_stack_size`{.interpreted-text role="cpp:member"} and `eth_mac_config_t::rx_task_prio`{.interpreted-text role="cpp:member"}: the MAC driver creates a dedicated task to process incoming packets. These two parameters are used to set the stack size and priority of the task.
- `eth_mac_config_t::flags`{.interpreted-text role="cpp:member"}: specifying extra features that the MAC driver should have, it could be useful in some special situations. The value of this field can be OR\'d with macros prefixed with `ETH_MAC_FLAG_`. For example, if the MAC driver should work when the cache is disabled, then you should configure this field with `ETH_MAC_FLAG_WORK_WITH_CACHE_DISABLE`{.interpreted-text role="c:macro"}.
:::

::::: only
SOC_EMAC_SUPPORTED

Specific configuration for **internal MAC module** is described in `eth_esp32_emac_config_t`{.interpreted-text role="cpp:class"}, including:

::: list
- `eth_esp32_emac_config_t::smi_mdc_gpio_num`{.interpreted-text role="cpp:member"} and `eth_esp32_emac_config_t::smi_mdio_gpio_num`{.interpreted-text role="cpp:member"}: the GPIO number used to connect the SMI signals.
- `eth_esp32_emac_config_t::interface`{.interpreted-text role="cpp:member"}: configuration of MAC Data interface to PHY (MII/RMII).
- `eth_esp32_emac_config_t::clock_config`{.interpreted-text role="cpp:member"}: configuration of EMAC Interface clock (`REF_CLK` mode and GPIO number in case of RMII).
- `eth_esp32_emac_config_t::intr_priority`{.interpreted-text role="cpp:member"}: sets the priority of the MAC interrupt. If it is set to `0` or a negative value, the driver will allocate an interrupt with a default priority. Otherwise, the driver will use the given priority. Note that *Low* and *Medium* interrupt priorities (1 to 3) can be set since these can be handled in C.

SOC_EMAC_USE_MULTI_IO_MUX

:   - `eth_esp32_emac_config_t::emac_dataif_gpio`{.interpreted-text role="cpp:member"}: configuration of EMAC MII/RMII data plane GPIO numbers.

not SOC_EMAC_RMII_CLK_OUT_INTERNAL_LOOPBACK

:   - `eth_esp32_emac_config_t::clock_config_out_in`{.interpreted-text role="cpp:member"}: configuration of EMAC input interface clock when `REF_CLK` signal is generated internally and is looped back to the EMAC externally. The mode must be always configured to `emac_rmii_clock_mode_t::EMAC_CLK_EXT_IN`{.interpreted-text role="cpp:enumerator"}. This option is valid only when configuration of `eth_esp32_emac_config_t::clock_config`{.interpreted-text role="cpp:member"} is set to `emac_rmii_clock_mode_t::EMAC_CLK_OUT`{.interpreted-text role="cpp:enumerator"}.
:::
...
::: list
- **Short and frequent frames dominate network traffic**: If your network traffic primarily consists of very short and frequently transmitted/received frames, you may observe issues such as lower-than-expected throughput (despite the rated 100 Mbps) and missed frames during reception. On transmission, the socket send API may return `errno` equal to `ENOMEM`, accompanied by the [insufficient TX buffer size]{.title-ref} message (if debug log level is enabled). This is because the default memory configuration is optimized for larger frames; `CONFIG_ETH_DMA_BUFFER_SIZE`{.interpreted-text role="ref"} is set to 512 bytes by default to ensure a better *data buffer* to *descriptor* size overhead ratio. The solution is to increase `CONFIG_ETH_DMA_RX_BUFFER_NUM`{.interpreted-text role="ref"} or `CONFIG_ETH_DMA_TX_BUFFER_NUM`{.interpreted-text role="ref"}. Additionally, consider decreasing `CONFIG_ETH_DMA_BUFFER_SIZE`{.interpreted-text role="ref"} to match the typical frame size in your network to maintain a reasonable memory footprint of the Ethernet driver.
- **High throughput leads to buffer exhaustion**: If the socket send API intermittently returns `errno` equal to `ENOMEM`, accompanied by the [insufficient TX buffer size]{.title-ref} message (if debug log level is enabled), and the throughput is close to the rated 100 Mbps, this likely indicates nearing hardware limitations. In such case, the hardware cannot keep up with the transmission requests. The solution is to increase `CONFIG_ETH_DMA_TX_BUFFER_NUM`{.interpreted-text role="ref"} to buffer more frames and mitigate temporary peaks in transmission requests. However, this will not help if the requested traffic consistently exceeds the rated throughput. In such situations, the only solution is to limit the bandwidth by software means at the application level.
:::
:::::

Configuration for PHY is described in `eth_phy_config_t`{.interpreted-text role="cpp:class"}, including:

::: list
- `eth_phy_config_t::phy_addr`{.interpreted-text role="cpp:member"}: multiple PHY devices can share the same SMI bus, so each PHY needs a unique address. Usually, this address is configured during hardware design by pulling up/down some PHY strapping pins. You can set the value from `0` to `15` based on your Ethernet board. Especially, if the SMI bus is shared by only one PHY device, setting this value to `-1` can enable the driver to detect the PHY address automatically.
- `eth_phy_config_t::reset_timeout_ms`{.interpreted-text role="cpp:member"}: reset timeout value, in milliseconds. Typically, PHY reset should be finished within 100 ms.
- `eth_phy_config_t::autonego_timeout_ms`{.interpreted-text role="cpp:member"}: auto-negotiation timeout value, in milliseconds. The Ethernet driver starts negotiation with the peer Ethernet node automatically, to determine to duplex and speed mode. This value usually depends on the ability of the PHY device on your board.
- `eth_phy_config_t::reset_gpio_num`{.interpreted-text role="cpp:member"}: if your board also connects the PHY reset pin to one of the GPIO, then set it here. Otherwise, set this field to `-1`.
- `eth_phy_config_t::hw_reset_assert_time_us`{.interpreted-text role="cpp:member"}: Time the PHY reset pin is asserted in usec. Set this field to `0` to use chip specific default timing.
- `eth_phy_config_t::post_hw_reset_delay_ms`{.interpreted-text role="cpp:member"}: Time to wait after the PHY hardware reset is done in msec. Set this field to `0` to use chip specific default timing. Set this field to `-1` to not wait after the PHY hardware reset.
:::

ESP-IDF provides a default configuration for MAC and PHY in macro `ETH_MAC_DEFAULT_CONFIG`{.interpreted-text role="c:macro"} and `ETH_PHY_DEFAULT_CONFIG`{.interpreted-text role="c:macro"}.
...
Any Ethernet PHY chip compliant with IEEE 802.3 can be used when creating new PHY instance with `esp_eth_phy_new_generic`{.interpreted-text role="cpp:func"}. However, while basic functionality should always work, some specific features might be limited, even if the PHY meets IEEE 802.3 standard. A typical example is loopback functionality, where certain PHYs may require setting a specific speed mode to operate correctly. If this is the concern and you need PHY driver specifically tailored to your chip needs, use drivers for PHY chips the ESP-IDF already officially supports or consult with `Custom PHY Driver <custom-phy-driver>`{.interpreted-text role="ref"} section to create a new custom driver.
::::
...
Espressif provides drivers for several specific Ethernet PHY chips in the [esp-eth-drivers](https://github.com/espressif/esp-eth-drivers) repository. Drivers are distributed as components and are available in the [ESP Component Registry](https://components.espressif.com/).
::::
...
\* When creating MAC and PHY instances for SPI-Ethernet modules (e.g., DM9051), the constructor function must have the same suffix (e.g., [esp_eth_mac_new_dm9051]{.title-ref} and [esp_eth_phy_new_dm9051]{.title-ref}). This is because we don not have other choices but the integrated PHY.

- The SPI device configuration (i.e., [spi_device_interface_config_t]{.title-ref}) may slightly differ for other Ethernet modules or to meet SPI timing on specific PCB. Please check out your module\'s specs and the examples in ESP-IDF.
::::
...
Espressif provides drivers for various SPI-Ethernet modules in the [esp-eth-drivers](https://github.com/espressif/esp-eth-drivers) repository. Drivers are distributed as components and are available in the [ESP Component Registry](https://components.espressif.com/).
::::

## Install Driver

To install the Ethernet driver, we need to combine the instance of MAC and PHY and set some additional high-level configurations (i.e., not specific to either MAC or PHY) in `esp_eth_config_t`{.interpreted-text role="cpp:class"}:

- `esp_eth_config_t::mac`{.interpreted-text role="cpp:member"}: instance that created from MAC generator (e.g., `esp_eth_mac_new_esp32`{.interpreted-text role="cpp:func"}).
- `esp_eth_config_t::phy`{.interpreted-text role="cpp:member"}: instance that created from PHY generator (e.g., `esp_eth_phy_new_generic`{.interpreted-text role="cpp:func"}).
- `esp_eth_config_t::check_link_period_ms`{.interpreted-text role="cpp:member"}: Ethernet driver starts an OS timer to check the link status periodically, this field is used to set the interval, in milliseconds.
- `esp_eth_config_t::stack_input`{.interpreted-text role="cpp:member"} or `esp_eth_config_t::stack_input_info`{.interpreted-text role="cpp:member"}: In most Ethernet IoT applications, any Ethernet frame received by a driver should be passed to the upper layer (e.g., TCP/IP stack). This field is set to a function that is responsible to deal with the incoming frames. You can even update this field at runtime via function `esp_eth_update_input_path`{.interpreted-text role="cpp:func"} after driver installation.
- `esp_eth_config_t::on_lowlevel_init_done`{.interpreted-text role="cpp:member"} and `esp_eth_config_t::on_lowlevel_deinit_done`{.interpreted-text role="cpp:member"}: These two fields are used to specify the hooks which get invoked when low-level hardware has been initialized or de-initialized.

ESP-IDF provides a default configuration for driver installation in macro `ETH_DEFAULT_CONFIG`{.interpreted-text role="c:macro"}.
...
For more information about the network interface, please refer to `Network Interface <esp_netif>`{.interpreted-text role="doc"}.
...
## Misc Control of Ethernet Driver {#misc-operation-of-driver}

The following functions should only be invoked after the Ethernet driver has been installed.

- Stop Ethernet driver: `esp_eth_stop`{.interpreted-text role="cpp:func"}
- Update Ethernet data input path: `esp_eth_update_input_path`{.interpreted-text role="cpp:func"}
- Misc get/set of Ethernet driver attributes: `esp_eth_ioctl`{.interpreted-text role="cpp:func"}
...
::: list
\* `eth_mac_ptp_config_t::clk_src`{.interpreted-text role="cpp:member"}: Clock source for PTP. Select one of the clock sources offered by the `soc_periph_emac_ptp_clk_src_t`{.interpreted-text role="cpp:type"} enumeration.

- `eth_mac_ptp_config_t::clk_src_period_ns`{.interpreted-text role="cpp:member"}: Period of the clock source for PTP in nanoseconds. For example, if the clock source is 40MHz, the period is 25ns.
- `eth_mac_ptp_config_t::required_accuracy_ns`{.interpreted-text role="cpp:member"}: Required accuracy for PTP in nanoseconds. The required accuracy must be worse than clock source for PTP. For example, if the clock source is 40MHz (25ns period), the required accuracy is 40ns.
- `eth_mac_ptp_config_t::roll_type`{.interpreted-text role="cpp:member"}: Rollover mode (digital or binary) for subseconds register. The binary rollover mode is recommended as it provides a more precise time synchronization.
:::

Time stamps for transmitted and received frames can be accessed via the last argument of the registered `esp_eth_config_t::stack_input_info`{.interpreted-text role="cpp:member"} function for the receive path, and via the `ctrl` argument of the `esp_eth_transmit_ctrl_vargs`{.interpreted-text role="cpp:func"} function for the transmit path. However, a more user-friendly approach to retrieve time stamp information in user space is by utilizing the L2 TAP `Extended Buffer <esp_netif_l2tap_ext_buff>`{.interpreted-text role="ref"} mechanism.
...
> - `ethernet/basic`{.interpreted-text role="example"} demonstrates how to use the Ethernet driver, covering driver installation, attaching it to [esp_netif]{.title-ref}, sending DHCP requests, and obtaining a pingable IP address.
> - `ethernet/iperf`{.interpreted-text role="example"} demonstrates how to use the Ethernet capabilities to measure the throughput/bandwidth using iPerf.
> - `ethernet/ptp`{.interpreted-text role="example"} demonstrates the use of Precision Time Protocol (PTP) for time synchronization over Ethernet.
> - `network/vlan_support`{.interpreted-text role="example"} demonstrates how to create virtual network interfaces over Ethernet, including VLAN and non-VLAN interfaces.
> - `network/sta2eth`{.interpreted-text role="example"} demonstrates how to create a 1-to-1 bridge using a Wi-Fi station and a wired interface such as Ethernet or USB.
> - `network/simple_sniffer`{.interpreted-text role="example"} demonstrates how to use Wi-Fi and Ethernet in sniffer mode to capture packets and save them in PCAP format.
> - `network/eth2ap`{.interpreted-text role="example"} demonstrates how to implement a bridge that forwards packets between an Ethernet port and a Wi-Fi AP interface. It uses {IDF_TARGET_NAME} to create a 1-to-many connection between Ethernet and Wi-Fi without initializing the TCP/IP stack.
> - `network/bridge`{.interpreted-text role="example"} demonstrates how to use the LwIP IEEE 802.1D bridge to forward Ethernet frames between multiple network segments based on MAC addresses.
> - Most protocol examples should also work for Ethernet: `protocols`{.interpreted-text role="example"}.
...
The majority of PHY management functionality required by the ESP-IDF Ethernet driver is covered by the `esp_eth/src/phy/esp_eth_phy_802_3.c`{.interpreted-text role="component_file"}. However, the following may require developing chip-specific management functions:
...
2.  Prepare derived PHY management object info structure which:
    ...
    > - must contain at least parent IEEE 802.3 `phy_802_3_t`{.interpreted-text role="cpp:class"} object
