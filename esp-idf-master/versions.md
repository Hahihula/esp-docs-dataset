---
original_file_path: versions.rst
---
...
See `updating`{.interpreted-text role="ref"} if you already have a local copy of ESP-IDF and wish to update it.
...
If writing code that needs to support multiple ESP-IDF versions, the version can be checked at compile time using `compile-time macros <idf-version-h>`{.interpreted-text role="ref"}.
...
+----------------------------+----------------------------------------------------------------------------------------------------------------+
| `v3.1-beta1-75-g346d6b0ea` | | Beta version in development (on a `release branch <updating-release-branch>`{.interpreted-text role="ref"}). |
...
## Updating ESP-IDF {#updating}

Updating ESP-IDF depends on which version(s) you wish to follow:

- `updating-stable-releases`{.interpreted-text role="ref"} is recommended for production use.
- `updating-master`{.interpreted-text role="ref"} is recommended for the latest features, development use, and testing.
- `updating-release-branch`{.interpreted-text role="ref"} is a compromise between the first two.
...
These guides assume that you already have a local copy of ESP-IDF cloned. To get one, check Step 2 in the `Getting Started </get-started/index>`{.interpreted-text role="doc"} guide for any ESP-IDF version.
::::

### Updating to Stable Release {#updating-stable-releases}
...
### Updating to a Pre-Release Version {#updating-pre-release}

It is also possible to `git checkout` a tag corresponding to a pre-release version or release candidate, the process is the same as `updating-stable-releases`{.interpreted-text role="ref"}.

Pre-release tags are not always found on the [Releases page](https://github.com/espressif/esp-idf/releases). Consult the [list of tags](https://github.com/espressif/esp-idf/tags) on GitHub for a full list. Caveats for using a pre-release are similar to `updating-release-branch`{.interpreted-text role="ref"}.

### Updating to Master Branch {#updating-master}
