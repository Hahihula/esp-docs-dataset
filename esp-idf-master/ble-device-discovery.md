---
original_file_path: api-guides/ble/get-started/ble-device-discovery.rst
---
...
Extended advertising packets are composed of [ADV_EXT_IND]{.title-ref} and [AUX_ADV_IND]{.title-ref}, transmitted on the primary and secondary advertising channels, respectively. The primary advertising channels correspond to channels 37-39, while the secondary advertising channels correspond to channels 0-36. Since the receiver always receives advertising data on the primary advertising channels, the advertiser must send [ADV_EXT_IND]{.title-ref} on the primary advertising channels and [AUX_ADV_IND]{.title-ref} on the secondary advertising channels. [ADV_EXT_IND]{.title-ref} will indicate the secondary advertising channels where [AUX_ADV_IND]{.title-ref} is transmitted. This mechanism allows the receiver to obtain the complete extended advertising packet by first receiving [ADV_EXT_IND]{.title-ref} on the primary advertising channels and then going to the specified secondary advertising channels to receive [AUX_ADV_IND]{.title-ref}.

  --------------------------------------------------------------------------------------------------------------------------------------------
  Type                            Channels                     Purpose
  ------------------------------- ---------------------------- -------------------------------------------------------------------------------
  Primary Advertising Channel     37-39                        Used to transmit [ADV_EXT_IND]{.title-ref} of the extended advertising packet

  Secondary Advertising Channel   0-36                         Used to transmit [AUX_ADV_IND]{.title-ref} of the extended advertising packet
  --------------------------------------------------------------------------------------------------------------------------------------------
...
::: {#adv_packet_structure}
<figure class="align-center">
<img src="../../../../_static/ble/ble-4.2-adv-packet-structure.png" alt="Advertising Packet Structure" />
...
These advertising behaviors can be combined into four common types of advertising, corresponding to four different PDU types:
  ...
  --------------- ------------- -------------- ------------------------------- -----------------------------------------------------------------------------------------------------
  Y               Y             Y              [ADV_IND]{.title-ref}           The most common advertising type

  Y               N             N              [ADV_DIRECT_IND]{.title-ref}    Commonly used for reconnecting with known devices

  N               N             Y              [ADV_NONCONN_IND]{.title-ref}   Used by beacon devices to advertising data without connection

  N               Y             Y              [ADV_SCAN_IND]{.title-ref}      Used by beacons to advertise additional data via a scan response when packet length is insufficient
  ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
::: {.seqdiag caption="Advertising With a Public Address" align="center" scale="200%"}
...
::: {.seqdiag caption="Advertising With a Resolvable Random Private Address" align="center" scale="200%"}
...
::: {.seqdiag caption="Advertising With a Random Static Address" align="center" scale="200%"}
...
For Bluetooth LE 4.2 devices, the advertiser only sends data on the advertising channels, which are channels 37-39. For Bluetooth LE 5.0 devices, if the advertiser has enabled extended advertising, it sends [ADV_EXT_IND]{.title-ref} on the primary advertising channels and [AUX_ADV_IND]{.title-ref} on the secondary advertising channels. Thus, for Bluetooth LE 4.2 devices, scanners only need to receive advertising data on advertising channels. For Bluetooth LE 5.0 devices, scanners must first receive the [ADV_EXT_IND]{.title-ref} on the primary advertising channels and, if it indicates a secondary channel, move to the corresponding secondary channel to receive the [AUX_ADV_IND]{.title-ref}.
...
<figure class="align-center">
<img src="../../../../_static/ble/ble-advertise-and-scan-sequence.png" alt="Advertising and Scanning Timing Diagram" />
<figcaption aria-hidden="true">Advertising and Scanning Timing Diagram</figcaption>
</figure>

#### Scan Request and Scan Response {#scan_request_and_scan_response}
...
<figure class="align-center">
<img src="../../../../_static/ble/ble-advertiser-rx-scan-request.png" alt="Scan Request Reception and Scan Response Transmission" />
<figcaption aria-hidden="true">Scan Request Reception and Scan Response Transmission</figcaption>
</figure>

## Hands-On Practice

After learning the relevant concepts of advertising and scanning, let\'s apply this knowledge in practice using the `NimBLE_Beacon <bluetooth/ble_get_started/nimble/NimBLE_Beacon>`{.interpreted-text role="example"} example to create a simple beacon device.
...
If you haven\'t set up the ESP-IDF development environment yet, please refer to `IDF Get Started <../../../get-started/index>`{.interpreted-text role="doc"}.

### Try It Out

#### Building and Flashing

The reference example for this tutorial is `NimBLE_Beacon <bluetooth/ble_get_started/nimble/NimBLE_Beacon>`{.interpreted-text role="example"}.
...
Please replace [\<ESP-IDF Path\>]{.title-ref} with your local ESP-IDF folder path. Then, you can open the NimBLE_Beacon project using VSCode or another IDE you prefer. For example, after navigating to the example directory via the command line, you can open the project in VSCode using the following command:
...
<figure class="align-center">
<img src="../../../../_static/ble/ble-scan-list-nimble-beacon.jpg" alt="NimBLE Beacon" />
...
<figure class="align-center">
<img src="../../../../_static/ble/ble-adv-packet-raw-data.jpg" alt="ADV Packet Raw Data" />
...
**Details** table summarizes all advertising data structures in the advertising data packet and the scan response data packet:
  ...
  ----------------------------- ---------- -------------------- -------------------------------------------------- --------------------------------------------
  Flags                         2 Bytes    [0x01]{.title-ref}   [0x06]{.title-ref}                                 General Discoverable, BR/EDR Not Supported

  Complete Local Device Name    14 Bytes   [0x09]{.title-ref}   [0x4E696D424C455F426561636F6E]{.title-ref}         NimBLE_Beacon

  TX Power Level                2 Bytes    [0x0A]{.title-ref}   [0x09]{.title-ref}                                 9 dBm

  Appearance                    3 Bytes    [0x19]{.title-ref}   [0x0002]{.title-ref}                               Generic Tag (Generic category)

  LE Role                       2 Bytes    [0x1C]{.title-ref}   [0x00]{.title-ref}                                 Only Peripheral Role supported

  LE Bluetooth Device Address   8 Bytes    [0x1B]{.title-ref}   [0x46F506BDF5F000]{.title-ref}                     [F0:F5:BD:06:F5:46]{.title-ref}

  URI                           17 Bytes   [0x24]{.title-ref}   [0x172F2F6573707265737369662E636F6D]{.title-ref}   [https://espressif.com]{.title-ref}
  -------------------------------------------------------------------------------------------------------------------------------------------------------------
...
You may also notice that the Raw Data for the Device Appearance is [0x0002]{.title-ref}, while in the code, the definition for Generic Tag is [0x0200]{.title-ref}. Additionally, the Raw Data for the Device Address appears to be completely reversed, except for the last byte ([0x00]{.title-ref}). This is because Bluetooth LE air packets follow a little-endian transmission order, meaning the lower bytes are placed at the front.
...
::: {#nimble_beacon_project_structure}
The root directory of `NimBLE_Beacon <bluetooth/ble_get_started/nimble/NimBLE_Beacon>`{.interpreted-text role="example"} is roughly divided into the following parts:
:::
...
::: {#nimble_beacon_program_behavior}
...
::: {#nimble_beacon_entry_point}
As with other projects, the entry function of the application is the [app_main]{.title-ref} function in the [main/main.c]{.title-ref} file, where we typically initialize the modules. In this example, we mainly do the following:
:::
...
The {IDF_TARGET_NAME} Bluetooth stack uses NVS Flash to store related configurations, so before initializing the Bluetooth stack, we must call the [nvs_flash_init]{.title-ref} API to initialize NVS Flash. In some cases, we may need to call the [nvs_flash_erase]{.title-ref} API to erase NVS Flash before initialization.
...
Next, you can call [nimble_port_init]{.title-ref} API to initialize NimBLE host stack.
...
Then, we call the [gap_init]{.title-ref} function defined in the [gap.c]{.title-ref} file to initialize the GAP service and set the device name and appearance.
...
::: {#nimble_beacon_start_advertising}
When developing applications using the NimBLE host stack, the programming model is event-driven.
:::

For example, after the NimBLE host stack synchronizes with the Bluetooth controller, a synchronization completion event will be triggered, invoking the [ble_hs_cfg.sync_cb]{.title-ref} function. When setting up the callback function, we point the function pointer to the [on_stack_sync]{.title-ref} function, which is the actual function called upon synchronization completion.

In the [on_stack_sync]{.title-ref} function, we call the [adv_init]{.title-ref} function to initialize advertising operations. In [adv_init]{.title-ref}, we first call the [ble_hs_util_ensure_addr]{.title-ref} API to confirm that a usable Bluetooth address is available. Then, we call the [ble_hs_id_infer_auto]{.title-ref} API to obtain the optimal Bluetooth address type.
...
Next, we copy the Bluetooth address data from the NimBLE stack\'s memory space into the local [addr_val]{.title-ref} array, preparing it for subsequent use.
...
The [ble_hs_adv_fields]{.title-ref} structure predefines some commonly used advertising data types. After completing the data setup, we can enable the corresponding advertising data structures by setting the relevant is_present field to 1 or by assigning a non-zero value to the corresponding length field (len). For example, in the code above, we configure the device\'s transmission power with [adv_fields.tx_pwr_lvl = BLE_HS_ADV_TX_PWR_LVL_AUTO;]{.title-ref}, and then enable that advertising data structure by setting [adv_fields.tx_pwr_lvl_is_present = 1;]{.title-ref}. If we only configure the transmission power without setting the corresponding is_present field, the advertising data structure becomes invalid. Similarly, we configure the device name with [adv_fields.name = (uint8_t \*)name;]{.title-ref} and set the name\'s length with [adv_fields.name_len = strlen(name);]{.title-ref} to add the device name as an advertising data structure to the advertising packet. If we only configure the device name without specifying its length, the advertising data structure will also be invalid.

Finally, we call the [ble_gap_adv_set_fields]{.title-ref} API to finalize the setup of the advertising data structures in the advertising packet.
...
Finally, we set the advertising parameters and initiate the advertising by calling the [ble_gap_adv_start]{.title-ref} API.
...
Through this tutorial, you have learned the basic concepts of advertising and scanning, and you mastered the method of building a Bluetooth LE Beacon device using the NimBLE host stack through the `NimBLE_Beacon <bluetooth/ble_get_started/nimble/NimBLE_Beacon>`{.interpreted-text role="example"} example.

You can try to modify the data in the example and observe the changes in the **nRF Connect for Mobile** app. For instance, you might modify the [adv_fields]{.title-ref} or [rsp_fields]{.title-ref} structures to change the populated advertising data structures, or swap the advertising data structures between the advertising packet and the scan response packet. However, keep in mind that the maximum size for the advertising data in both the advertising packet and the scan response packet is 31 bytes; if the size of the advertising data structure exceeds this limit, calling the [ble_gap_adv_start]{.title-ref} API will fail.
