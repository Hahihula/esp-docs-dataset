---
original_file_path: api-guides/unit-tests.rst
---
...
- Linux-host based unit tests in which part of the hardware can be abstracted via mocks. Currently, Linux-host based tests are still under development and only a small fraction of IDF components support them. More information on running IDF applications on the host can be found here: `Running Applications on the Host Machine <host-apps>`{.interpreted-text role="doc"}.
...
## Timing Code with Cache Compensated Timer {#cache-compensated-timer}
...
Refer to `cmock/CMock/docs/CMock_Summary.md`{.interpreted-text role="component_file"} for more details on how CMock works and how to create and use mocks.
...
If a mocked component, called a *component mock*, is already available in ESP-IDF, then it can be used right away as long as it satisfies the required functionality. Refer to `component-linux-mock-support`{.interpreted-text role="ref"} to see which components are mocked already. Then refer to `adjustments_for_mocks`{.interpreted-text role="ref"} in order to use the component mock.
...
For more details about the CMock configuration yaml file, have a look at `cmock/CMock/docs/CMock_Summary.md`{.interpreted-text role="component_file"}.
...
Examples of component mocks can be found under `tools/mocks`{.interpreted-text role="idf"} in the IDF directory. General information on how to *override an IDF component* can be found in `Multiple components with the same name <cmake-components-same-name>`{.interpreted-text role="ref"}. There are several examples for testing code while mocking dependencies with CMock (non-exhaustive list):

- `unit test for the NVS Page class <nvs_flash/host_test/nvs_page_test/README.md>`{.interpreted-text role="component_file"}.
- `unit test for esp_event <esp_event/host_test/esp_event_unit_test/main/esp_event_test.cpp>`{.interpreted-text role="component_file"}.

### Adjustments in Unit Test {#adjustments_for_mocks}
...
Users can refer to the `esp_event` host-based unit test and its `esp_event/host_test/esp_event_unit_test/CMakeLists.txt`{.interpreted-text role="component_file"} as an example of a component mock.

## Application Examples

`system/unit_test`{.interpreted-text role="example"} demonstrates how to use the Unity library to add unit tests to custom components in an {IDF_TARGET_NAME} development environment, showcasing features such as assertions, test registration, and the use of UNITY_BEGIN() and UNITY_END() macros.
