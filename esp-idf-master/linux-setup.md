---
original_file_path: get-started/linux-setup.rst
---
...
This document describes the default and recommended way to install ESP-IDF v6.0 and newer versions. ESP-IDF also supports the `legacy installation method on Linux <linux-macos-setup-legacy>`{.interpreted-text role="doc"}, which was the default before ESP-IDF v6.0.
::::

## Step 1: Install the Prerequisites (Optional)

Skip this step if you plan to install EIM using `APT <install-eim-linux-apt>`{.interpreted-text role="ref"}.
...
- [Download the EIM Installer](#download-the-eim-installer)

Installing via APT or DNF allows you to easily keep EIM up to date.

### Debian-Based Linux Installation via APT {#install-eim-linux-apt}
...
Alternatively, download the EIM installer for Linux from the [Espressif Download Page](https://dl.espressif.com/dl/eim/), which provides both online and offline installers available in both CLI and GUI versions.
