---
original_file_path: api-guides/wifi-security.rst
---
...
- Additional cipher suites supported by mbedtls when the [USE_MBEDTLS_CRYPTO]{.title-ref} flag is enabled
...
- WPA3-Enterprise 192-bit mode is supported only on modules that have `SOC_WIFI_GCMP_SUPPORT`{.interpreted-text role="c:macro"}.
- To enable WPA3-Enterprise 192-bit mode, configure `CONFIG_ESP_WIFI_SUITE_B_192`{.interpreted-text role="ref"}.
...
- **EAP-FAST**
  ...
  An authentication method based on Protected Access Credentials (PAC), requiring an identity and password. To use EAP-FAST, `CONFIG_ESP_WIFI_MBEDTLS_TLS_CLIENT`{.interpreted-text role="ref"} must be **disabled**.

### Examples

- `wifi/wifi_eap_fast`{.interpreted-text role="example"}
...
- `wifi/wifi_enterprise`{.interpreted-text role="example"}

  Demonstrates connecting {IDF_TARGET_NAME} using EAP-TLS, EAP-PEAP, and EAP-TTLS. For details on generating certificates with OpenSSL and running the example, refer to `wifi/wifi_enterprise/README.md`{.interpreted-text role="example_file"}.
...
A configuration option `CONFIG_ESP_WIFI_ENABLE_WPA3_SAE`{.interpreted-text role="ref"} is provided to enable/disable WPA3 for the station. By default, it is kept enabled. If disabled, {IDF_TARGET_NAME} will not be able to establish a WPA3 connection. Also under the Wi-Fi component, a configuration option `CONFIG_ESP_WIFI_SOFTAP_SAE_SUPPORT`{.interpreted-text role="ref"} is provided to enable/disable WPA3 for SoftAP. Additionally, since PMF is mandated by WPA3 protocol, PMF Optional is set by default for station and SoftAP. PMF Required can be configured using Wi-Fi configuration. For WPA3 SoftAP, PMF Required is mandatory and will be configured and stored in NVS implicitly if not specified by the user.
...
After configuring all required setting for WPA3-Personal SoftAP, application developers have to set `WIFI_AUTH_WPA3_PSK` for [authmode]{.title-ref} in `wifi_ap_config_t`{.interpreted-text role="cpp:type"} to start AP in WPA3 security. SoftAP can be also configured to use `WIFI_AUTH_WPA2_WPA3_PSK` mixed mode.

Note that binary size will be increased by about 6.5 kilobytes after enabling `CONFIG_ESP_WIFI_SOFTAP_SAE_SUPPORT`{.interpreted-text role="ref"}.
...
A configuration option `CONFIG_ESP_WIFI_ENABLE_WPA3_OWE_STA`{.interpreted-text role="ref"} and configuration parameter `owe_enabled`{.interpreted-text role="cpp:type"} in `wifi_sta_config_t`{.interpreted-text role="cpp:type"} is provided to enable OWE support for the station. To use OWE transition mode, along with the configuration provided above, [authmode]{.title-ref} from `wifi_scan_threshold_t`{.interpreted-text role="cpp:type"} should be set to `WIFI_AUTH_OPEN`.
