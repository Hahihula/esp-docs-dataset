...
4. Configure register: Write IV block to register DS_IV_m_REG (m: 0 ~ 3). For more information on the IV block, please refer to Chapter 18 AES Accelerator (AES).

5. Write X to memory block DS_X_MEM: Write Xi (i ∈ {0, 1, ..., n−1}), where n = N/32, to memory block DS_X_MEM whose capacity is 96 words. Each word can store one base-b digit. The memory block uses the little endian format for storage, i.e., the least significant digit of the operand is in the lowest address. Words in DS_X_MEM block after the configured length of X (N bits, as described in Section 22.3.2), are ignored.
