**Chapter Title:**
Chapter 1 Processor Instruction Extensions (PIE)

**Section Heading:**
1.8.164 EE.VMULAS.U16.QACC.LD.IP

**Instruction Word Table:**
- imm16[5:4] : qu[2:1]
- qy[0]    : 000
- qu[0]    : 101
- qx[1:0]  : 111
- qx[2:1]  : 
- imm16[3:0]: as[3:0]

**Subheading - Assembler Syntax:**
EE.VMULAS.U16.QACC.LD.IP qu, as, imm16, qx, qy

**Description Section:**
This instruction divides registers qx and qy into 8 data segments by 16 bits. The unsigned multiplication result of the 8 sets of segments is added to the corresponding 40-bit data segment in special registers QACC_H and QACC_L respectively. The calculated result is saturated to a 40-bit unsigned number and then stored to the corresponding 40-bit data segment in QACC_H and QACC_L.

During the operation, the lower 4 bits of the access address in register as are forced to be O, and then the 16-byte data is loaded from the memory to register qu. After the access is completed, the value in register as is incremented by 6-bit sign-extended constant in the instruction code segment left-shifted by 4.

**Operation Section:**
```
QACC_L[39:0] = min(QACC_L[39:0] + qx[15:0] * qy[15:0], 2^{40}-1)
QACC_L[79:40] = min(QACC_L[79:40] + qx[31:16] * qy[31:16], 2^{40}-1)
...
QACC_L[159:120] = min(QACC_L[159:120] + qx[63:48] * qy[63:48], 2^{40}-1)
QACC_H[39:0] = min(QACC_H[39:0] + qx[79:64] * qy[79:64], 2^{40}-1)
QACC_H[79:40] = min(QACC_H[79:40] + qx[95:80] * qy[95:80], 2^{40}-1)
QACC_H[159:120] = min(QACC_H[159:120] + qx[127:112] * qy[127:112], 2^{40}-1)

qu[127:0] = load128({as[31:4],4{0}})
as[31:0] = as[31:0] + {22imm16[5]},imm16[5:0],4{0}}
```

**Footer Information:**
Espressif Systems
Page 246 ESP32-S3 TRM (Version 1.7)
Submit Documentation Feedback