...
The MCPWM peripheral on ESP32-P4 supports the Event Task Matrix (ETM) function, which allows MCPWM’s ETM tasks to be triggered by any peripherals’ ETM events, or MCPWM’s ETM events to trigger any peripherals’ ETM tasks. The capture module, the fault detection module, three timers, and three operators can generate events and respond to tasks independently. This section introduces the ETM tasks and events related to MCPWM. For more information, please refer to Chapter 13 Event Task Matrix (ETM).

### 56.3.5.2 MCPWM-Related ETM Events

When setting enable field to 1, after the generation condition is met, the corresponding event would be generated. For details, please refer to Table 56.3-7 below:

**Table 56.3-7. MCPWM-Related ETM Events**

| Enable Field                  | Generation Condition                                                                 | Event Generated                     |
|-------------------------------|--------------------------------------------------------------------------------------|-------------------------------------|
| `MCPWM_EVT_CAPn_EN`           | CAPn capture event occurs                                                           | `MCPWMm_EVT_CAPn`                   |
| `MCPWM_EVT_TZn_OST_EN`        | PWM operator n performs a One-Shot trip (OST) operation                             | `MCPWMm_EVT_TZn_OST`                |
| `MCPWM_EVT_TZn_CBC_EN`        | PWM operator n performs a cycle-by-cycle trip (CBC) operation                       | `MCPWMm_EVT_TZn_CBC`                |
| `MCPWM_EVT_Fn_CLR_EN`         | Fault event fault_eventn is cleared                                                 | `MCPWMm_EVT_Fn_CLR`                 |
