...
When Duty Cycle Fading finishes in all ranges (the number of ranges is specified by `LEDC_CHn_GAMMA_ENTRY_NUM`), the PWM signal stops fading and keeps the duty cycle of the last fade. Given that the duty cycle fades differently and linearly in each range, several linear fading ranges would be fitted to a gamma curve.

Figure 55.4-5 illustrates a gamma curve fading PWM signal.

![Figure 55.4-5 Output Signal of Gamma Curve Fading](image)

Figure 55.4-5. Output Signal of Gamma Curve Fading

55.4.3.3 Suspend and Resume Duty Cycle Fading
