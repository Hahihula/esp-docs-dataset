...
- If the TRACE_MATCH_MODE is 0, and TRACE_P_NOTIFY is set, then all instructions will be traced, and the matched primary comparator will be notified.
...
## 2.8.3 Enable Encoder

* Configure the address space for the trace memory via `TRACE_MEM_START_ADDR_REG` and `TRACE_MEM_END_ADDR_REG`. The encoder can access L2 MEM and PSRAM:
    - L2MEM: 0x4FF00000 ~ 0x4FBFFFFFF
