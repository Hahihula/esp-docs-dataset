...
configured to a maximum value according to your application, otherwise a valid received level may be mistaken as a level in idle state. If the RAM space of this RX channel is used up by the received data, the receiver stops receiving data, and an RMT_CHm_ERR_INT interrupt is triggered by RAM FULL event.

57.3.5.2 Wrap RX Mode
...
57.3.6 Configuration Update

To update RMT registers configuration, please set RMT_CONF_UPDATE_CHn/m for each channel first. All the bits/fields listed in the second column of Table 57.3-1 should follow this rule.
```