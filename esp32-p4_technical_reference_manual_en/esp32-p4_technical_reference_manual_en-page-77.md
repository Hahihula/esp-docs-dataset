...
This CSR is designed to be accessed using CSRRSI/CSRRRC instructions, where the value read is a pointer to an entry in the trap handler table and the write back updates the interrupt-enable status. In addition, writes to the mnxti have side-effects that update the interrupt context state.
