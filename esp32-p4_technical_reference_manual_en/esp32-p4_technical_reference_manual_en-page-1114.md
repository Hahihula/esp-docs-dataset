...
Under the SPI boot mode, MWDTO as well as RWDT, are automatically enabled. Stage 0 for the enabled MWDTO is automatically configured as HP core reset action upon timeout, known as HP core reset. Likewise, stage 0 for RWDT is configured to system reset, which resets the main system and RTC when it times out.

After SPI booting, TIMG_WDT_FLASHBOOT_MOD_EN and RTC_WDT_FLASHBOOT_MOD_EN should be cleared to stop the flash boot protection procedure for both MWDTO and RWDT respectively. After this, MWDTO and RWDT can be configured by software.
