...
## 42.5 Programming Procedures

### 42.5.1 Register Type
...
To distinguish between these two groups of registers easily, all registers that implement the clock domain crossing design have the `_SYNC` suffix, and are put together in Section 42.6. Those without the `_SYNC` suffix in Section 42.6 are configuration registers that require no clock domain crossing.
```