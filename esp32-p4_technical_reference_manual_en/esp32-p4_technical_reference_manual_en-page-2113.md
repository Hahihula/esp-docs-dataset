...
42.4.3 Baud Rate Generation and Detection

42.4.3.1 Baud Rate Generation
...
When `UART_CLKDIV_FRAG` is 0, the baud rate generator is an integer clock divider where an output pulse is generated every `UART_CLKDIV` input clock pulses.

When `UART_CLKDIV_FRAG` is not 0, the divider is fractional and the output baud rate clock pulses are not strictly uniform. As shown in Figure 42.4-1, for every 16 output pulses, the generator divides either
```