...
ESP32-P4's LEDC can generate the `LEDC_INT` interrupt signal, which will be sent to the *Interrupt Matrix*. Interrupt signal `LEDC_INT` can be generated by the following internal interrupt sources:
...
## 55.6 Programming Procedures

The configuration process of the LED PWM controller to generate PWM signals is as follows:

1. Configure the `Timerx` according to section 55.4.1. After that, the clock source, divider, and counter are ready.
2. Configure the `PWMn` generator according to section 55.4.2. After that, `PWMn` selects one of the four timers and the phase and duty cycle of the `PWMn` output is set.
