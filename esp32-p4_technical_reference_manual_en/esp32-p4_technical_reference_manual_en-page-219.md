...
The hardware trigger module provides three CSRs. See Section 3.6 for details. Among them, `tdata1` and `tdata2` are abstract CSRs, which means they are shadow registers for accessing internal registers in the trigger units, one at a time.
```