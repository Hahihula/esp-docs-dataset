...
## 15.7.2 Configure a One-Time Alarm in Target Mode

1. Set `SYSTIMER_TARGETx_TIMER_UNIT_SEL` to select the counter (UNIT0 or UNIT1) used for comparison with COMPx.
2. Read the current count value with reference to Section 15.7.1. This value will be used to calculate the alarm value (t) in Step 4.
...
## 15.7.3 Configure Periodic Alarms in Period Mode
...
## 15.7.4 Update After Light-sleep

1. Configure RTC timer before the chip goes to Light-sleep mode, to record the exact sleep time. For more information, see Chapter 14 Low-Power Management.
2. Read the sleep time from the RTC timer when the chip wakes up from Light-sleep mode.
3. Read the current count value of system timer, see Section 15.7.1.
```