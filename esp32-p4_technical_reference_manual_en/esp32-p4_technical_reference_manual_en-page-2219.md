...
```markdown
As shown in Table 43.5-10, the registers in each cell should be configured to set the SPI2/SPI3 bus to corresponding bit mode, i.e., the mode shown in the table header, at a specific state (corresponding to the first column).
...
2.  Configure ADDR state related registers.
    ...
    *   Clear `SPI_FADDR_QUAD` and `SPI_FCMD_OCT`.
...
4.  Configure DIN state related registers.
    ...
    *   Clear `SPI_FREAD_DUAL` and `SPI_FCMD_OCT`.
