...
|----------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------|
| TRACE_MEM_FULL_INTR       | Triggered when the packet size exceeds the capacity of the trace memory, namely when `TRACE_MEM_CURRENT_ADDR_REG` reaches the value of `TRACE_MEM_END_ADDR_REG`. If necessary, this interrupt can be enabled to notify the HP CPU for processing, such as applying for a new memory space again. | COREn_TRACE_INTR |
| TRACE_FIFO_OVERFLOW_INTR  | Triggered when the internal FIFO overflows and one or more packets have been lost.                                                                                                                                                       | COREn_TRACE_INTR |
