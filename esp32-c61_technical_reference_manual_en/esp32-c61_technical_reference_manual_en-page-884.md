...
```markdown
## 26.5.4 Unaligned Byte Transfer
...
## 26.5.5 Transfer Types

The transfer types supported by GP-SPI2 working as a master or a slave are shown in Table 26.5-5.

Table 26.5-5. Supported Transfer Types as Master or Slave
...
## 26.5.6 CPU-Controlled Data Transfer

GP-SPI2 provides 16 x 32-bit data buffers: `SPI_WO_REG~SPI_W15_REG`. Figure 26.5-1 shows the buffer's structure. CPU-controlled transfer indicates the transfer in which the data to send is from GP-SPI2 data buffer and the received data is stored to GP-SPI2 data2 buffer. In such transfer, every single transaction needs to be triggered by the CPU, after its related registers are configured. For such reason, the CPU-controlled transfer is
```