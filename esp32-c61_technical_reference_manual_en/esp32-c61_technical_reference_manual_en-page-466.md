

```markdown
Register 10.19. SOC_ETM_EVT_ST1_CLR_REG (0x01B4)

Continued from the previous page...

SOC_ETM_LEDC_EVT_TIMERO_CMP_ST_CLR Configures whether or not to clear LEDC_EVT_TIMERO_CMP trigger status.
O: Invalid. No effect
1: Clear
(WT)

SOC_ETM_LEDC_EVT_TIMER1_CMP_ST_CLR Configures whether or not to clear LEDC_EVT_TIMER1_CMP trigger status.
O: Invalid. No effect
1: Clear
(WT)

SOC_ETM_LEDC_EVT_TIMER2_CMP_ST_CLR Configures whether or not to clear LEDC_EVT_TIMER2_CMP trigger status.
O: Invalid. No effect
1: Clear
(WT)

SOC_ETM_LEDC_EVT_TIMER3_CMP_ST_CLR Configures whether or not to clear LEDC_EVT_TIMER3_CMP trigger status.
O: Invalid. No effect
1: Clear
(WT)

SOC_ETM_TGO_EVT_CNT_CMP_TIMERO_ST_CLR Configures whether or not to clear TGO_EVT_CNT_CMP_TIMERO trigger status.
O: Invalid. No effect
1: Clear
(WT)

SOC_ETM_TGO_EVT_CNT_CMP_TIMER1_ST_CLR Configures whether or not to clear TGO_EVT_CNT_CMP_TIMER1 trigger status.
O: Invalid. No effect
1: Clear
(WT)

SOC_ETM_TG1_EVT_CNT_CMP_TIMERO_ST_CLR Configures whether or not to clear TG1_EVT_CNT_CMP_TIMERO trigger status.
O: Invalid. No effect
1: Clear
(WT)

Continued on the next page...
```