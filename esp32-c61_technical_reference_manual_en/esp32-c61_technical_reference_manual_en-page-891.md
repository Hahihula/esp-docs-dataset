...
```markdown
## 26.5.8.2 Data Flow Control When GP-SPI2 Works as Master

Figure 26.5-3 shows the data flow when GP-SPI2 works as master. Its control logic is as follows:
...
The data in `buf_tx_afifo` is sent out to Timing Module in 1/2/4-bit modes, controlled by GP-SPI2 state machine. The Timing Module can be used for timing compensation. For more information, see Section 26.8.

## 26.5.8.3 Data Flow Control When GP-SPI2 Works as Slave

Figure 26.5-4 shows the data flow when GP-SPI2 works as slave. Its control logic is as follows:
```