...
*   `Block_A`: the block whose key purpose is EFUSE_KEY_PURPOSE_XTS_AES_128_KEY (please refer to Table 5.3-2 Secure Key Purpose Values) in Chapter 5 eFuse Controller (EFUSE). If `Block_A` exists, a 256-bit `Key_A` is stored in it.

There are two possibilities of how the `Key` is generated depending on whether `Block_A` exists or not, as shown in Table 23.4-1. In each case, the `Key` can be uniquely determined.

Table 23.4-1. Key Generated Based on `Key_A`
