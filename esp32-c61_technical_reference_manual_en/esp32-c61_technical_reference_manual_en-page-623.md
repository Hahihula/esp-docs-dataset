...
### 13.4.2 54-bit Time-base Counter
...
### 13.4.3 Alarm Generation

A timer can be configured to trigger an alarm when the timer’s current value matches the alarm value. An alarm will cause an interrupt to occur and (optionally) an automatic reload of the timer’s current value (see Section 13.4.4).

The 54-bit alarm value is configured using TIMG_TOALARMLO_REG and TIMG_TOALARMIHI_REG, which represent the lower 32-bits and higher 22-bits of the alarm value, respectively. However, the configured alarm value is ineffective until the alarm is enabled by setting the TIMG_TO_ALARM_EN field. To avoid alarm being enabled “too late” (i.e., the timer value has already passed the alarm value when the alarm is enabled), the hardware will trigger the alarm immediately if the current timer value is:
...
Table 13.4-1 and Table 13.4-2 show the relationship between the current value of the timer, the alarm value, and when an alarm is triggered. The current time value and the alarm value are defined as follows:

*   TIMG_VALUE = {TIMG_TOHI_REG, TIMG_TOLO_REG}
*   ALARM_VALUE = {TIMG_TOALARMIHI_REG, TIMG_TOALARMLO_REG}
```