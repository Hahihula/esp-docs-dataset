...
When incoming data changes near the rising edge of the clock, the slave samples on the falling edge, and vice versa, as shown in Figure 30.5-8.

![Figure 30.5-8. Sampling Timing Diagram](image-placeholder)
...
The slave can also select which edge to drive the output lines, to compensate for any latency caused by the physical signal path. The output timing is shown in Figure 30.5-9.
```