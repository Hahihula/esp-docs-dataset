...
| I2C_COMMAND2 (master)                    | READ    | 0         | 0       | N            |          |
...
10. I2C<sub>slave</sub> compares the slave address sent by I2C<sub>master</sub> with its own address in I2C_SLAVE_ADDR (slave).
    - When ack_check_en (master) in I2C<sub>master</sub>'s WRITE command is 1, I2C<sub>master</sub> checks ACK value each time it sends a byte. When ack_check_en (master) is 0, I2C<sub>master</sub> does not check the ACK value and takes I2C<sub>slave</sub> as a matching slave by default.
...
11. After I2C_SLAVE_STRETCH_INT (slave) is generated, the I2C_STRETCH_CAUSE bit is 0. The address of I2C<sub>slave</sub> matches the address sent over SDA, and I2C<sub>slave</sub> needs to send data.
