...
**Note:**
For detailed information about the functions and configuration procedures of CPU interrupts, see Chapter 1 ESP-RISC-V CPU > Section 1.8 Interrupt Controller. The configuration registers of CPU interrupts are listed in Section 9.6.2 Software Interrupt Register Summary.

## 9.5.3 Assign Peripheral Interrupt Source to HP CPU Peripheral Interrupt
...
### 9.5.3.1 Assign One Peripheral Interrupt Source to HP CPU Peripheral Interrupt
...
### 9.5.3.2 Assign Multiple Peripheral Interrupt Sources to HP CPU Peripheral Interrupt

Setting the corresponding source mapping register INTMTX_COREO_SOURCE_INTR_MAP_REG of each SOURCE to the same Num_P assigns multiple sources to the same Interrupt_P. Any of these sources can trigger CPU Interrupt_P. When an interrupt signal is generated, the CPU should check the interrupt status registers to figure out which peripheral generated the interrupt. For more information, see Chapter 1 ESP-RISC-V CPU > Section 1.8 Interrupt Controller.
