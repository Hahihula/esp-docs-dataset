

```markdown
Chapter 18 Debug Assistant

Register 18.1. MEM_MONITOR_LOG_SETTING_REG (0x0000)

Continued from the previous page...

MEM_MONITOR_LOG_DMA_O_ENA Configures whether to enable DMA_0 bus access logging.
O: Disable
1: Enable
Other values: Invalid
(R/W)

MEM_MONITOR_LOG_DMA_1_ENA Configures whether to enable DMA_1 bus access logging.
O: Disable
1: Enable
Other values: Invalid
(R/W)

Register 18.2. MEM_MONITOR_LOG_SETTING1_REG (0x0004)


MEM_MONITOR_LOG_DMA_2_ENA Configures whether to enable DMA_2 bus access logging.
O: Disable
1: Enable
Other values: Invalid
(R/W)

MEM_MONITOR_LOG_DMA_3_ENA Configures whether to enable DMA_3 bus access logging.
O: Disable
1: Enable
Other values: Invalid
(R/W)
```