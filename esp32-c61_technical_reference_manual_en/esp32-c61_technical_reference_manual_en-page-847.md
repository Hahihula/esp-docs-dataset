...
ESP32-C61's UARTn can generate the following interrupt signals that will be sent to the Interrupt Matrix.
* UARTn_INTR

There are several internal interrupt sources from UARTn that can generate the above interrupt signals. UARTn interrupt sources are listed as follows:
```