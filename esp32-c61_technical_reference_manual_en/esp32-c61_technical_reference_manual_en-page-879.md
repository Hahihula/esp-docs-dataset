...
Figure 26.4-1 shows an overview of this SPI module. GP-SPI2 exchanges data with SPI devices in the following ways:

• CPU-controlled transfer: CPU <> GP-SPI2 <> SPI devices
• DMA-controlled transfer: GDMA <> GP-SPI2 <> SPI devices

The signals for GP-SPI2 are prefixed with "FSPI" (Fast SPI). FSPI bus signals are routed to GPIO pins via either the GPIO matrix or the IO MUX. For more information, see Chapter 6 GPIO Matrix and IO MUX.
```