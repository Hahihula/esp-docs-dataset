...
```markdown
Figure 31.4-1. Frequency Division When LEDC_CLK_DIV is a Non-Integer Value
...
31.4.1.3 20-Bit Counter

Each timer contains a 20-bit timebase counter that uses `ref_pulsex` as its reference clock (see Figure 31.3-2). The `LEDC_TIMERx_DUTY_RES` field configures the actual used bit width of this 20-bit counter. Hence, the maximum resolution of the PWM signal is 20 bits. The counter counts up to `2^LEDC_TIMERx_DUTY_RES - 1`, overflows and begins counting from 0 again. The counter's value can be read, reset, and suspended by software. Figure 31.4-2 shows the relationship between the counter and PWM resolution.

Figure 31.4-2. Relationship Between Counter And Resolution
