...
Table 15.3-1. Configure RTC Timer to Log the Occurrence Time of Specific Events
...
- Generate counting interrupts at target time
  - The RTC Timer supports configuring the target time, referred to as target time O. When the timer reaches target time 0, it will trigger the `RTC_TIMER_CMPO_INT` interrupt.
  - Configure the target time through the registers below:

Table 15.3-2. Target Time Configuration

| Configuration Register | Description |
|-------------------------|-------------|
| `RTC_TIMER_MAIN_TIMER_TARGET_ENO` | Enable target time configuration. |
| `RTC_TIMER_MAIN_TIMER_TARGET_LOW0` | Configure the lower 32 bits of the target time. |
| `RTC_TIMER_MAIN_TIMER_TARGET_HIGH0` | Configure the higher 16 bits of the target time. |
