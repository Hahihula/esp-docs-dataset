...
DPA (Differential Power Analysis) is a side-channel attack method in cryptography, through which an attacker can statistically analyze data collected from multiple encryption operations to calculate intermediate values in the encryption computation. ESP32-C61 XTS_AES supports two Anti-DPA methods to defend against external DPA attacks.
...
ESP32-C61 allows users to enable anti-DPA function separately during the above-mentioned two steps to enhance security. See details in the following sections.

### 23.6.1 Clock Anti-DPA Function
