...
The debug module of the HP CPU has a trigger unit. This defines a match control register (`mcontrol`) containing a 4-bit `action` field, and reserves codes 2 ~ 4 of this field for trace use. ESP32-C61 has implemented this feature. The values of the `action` field are listed in Table 2.8-1.

Table 2.8-1. Debug module trigger support (the `action` field of the `mcontrol` register)
...
For details about the debug module trigger unit, please refer to Chapter 1 ESP-RISC-V CPU.

## 2.8.2 Filter Configuration

The filter unit described in Section 2.5.4 can be configured as follows:
