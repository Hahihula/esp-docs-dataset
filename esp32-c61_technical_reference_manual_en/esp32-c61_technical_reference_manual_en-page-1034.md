...
At this point, the data format control is completed. The data after format control will be sent sequentially from high to low. Figure 28.9-1 shows the complete process of TX data format control.
...
ESP32-C61 I2S supports both TDM TX mode and PDM TX mode. Set `I2S_TX_TDM_EN` to enable TDM TX mode, or set `I2S_TX_PDM_EN` to enable PDM TX mode.
```