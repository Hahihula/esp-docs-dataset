...
```markdown
9. Set `SPI_DMA_SEG_TRANS_DONE_INT_ENA` in register `SPI_DMA_INT_ENA_REG` and wait for the interrupt `SPI_DMA_SEG_TRANS_DONE_INT`, which means that the segmented transfer has finished and data has been put into the related memory. Other interrupts described in Section 26.9 are optional.
...
### 26.5.10.6 Configuration of Slave Segmented Transfer in Full-Duplex

GDMA must be used in this mode. In such transfer, the data is transferred from and to the GDMA buffer. The interrupt `GDMA_IN_SUC_EOF_CHn_INT` is triggered when the transfer ends.
...
5. Set `SPI_DMA_TX_ENA` and `SPI_DMA_RX_ENA`. Configure GDMA TX/RX link and start GDMA TX/RX engine, as shown in Section 26.5.7 and Section 26.5.8.
