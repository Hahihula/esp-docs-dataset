...
```markdown
## 26.5.10.4 Configuration of Slave Single Transfer
...
8. Set `SPI_TRANS_DONE_INT_ENA` in register `SPI_DMA_INT_ENA_REG` and wait for the interrupt `SPI_TRANS_DONE_INT`. In DMA-controlled mode, it is recommended to wait for the interrupt `GDMA_IN_SUC_EOF_CHn_INT` when DMA RX buffer is used, which means that data has been stored in the related memory. Other interrupts described in Section 26.9 are optional.

## 26.5.10.5 Configuration of Slave Segmented Transfer in Half-Duplex
...
6. Set `SPI_DMA_AFIFO_RST`, `SPI_BUF_AFIFO_RST`, and `SPI_RX_AFIFO_RST` to reset these buffers.
7. Set bits `SPI_DMA_RX_ENA` and `SPI_DMA_TX_ENA`. Clear the bit `SPI_RX_EOF_EN`. Configure GDMA TX/RX link and start GDMA TX/RX engine, as shown in Section 26.5.7 and Section 26.5.8.
