...
*   Mode 0:
    ...
    *   The brown-out detector triggers an interrupt when the counter counts to the thresholds predefined in Int Comparator (`LP_ANA_BOD_MODEO_INTR_WAIT`). If `LP_ANA_BOD_MODEO_CLOSE_FLASH_ENA` is set, flash suspend will be triggered; if `LP_ANA_BOD_MODEO_PD_RF_ENA` is set, the RF module will be powered down.
    *   The brown-out detector resets the chip based on the configuration of `bod_mode0_rst_sel` (`LP_ANA_BOD_MODEO_RESET_SEL`) when the brown-out counter counts to the thresholds predefined in Rst Comparator (`LP_ANA_BOD_MODEO_RESET_WAIT`). The reset is enabled by `bod_mode0_rst_en` (`LP_ANA_BOD_MODEO_RESET_ENA`).
