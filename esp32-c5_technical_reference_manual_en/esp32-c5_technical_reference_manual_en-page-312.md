...
3. Configure HP IO MUX register to enable output via HP GPIO matrix. Set `IO_MUX_GPIOx_REG` corresponding to GPIO pin `X` as follows:
    ...
    - If using open drain mode, set or clear the `IO_MUX_GPIOx_FUN_WPU` and `IO_MUX_GPIOx_FUN_LWPD` bits to enable or disable the internal pull-up and pull-down resistors.
