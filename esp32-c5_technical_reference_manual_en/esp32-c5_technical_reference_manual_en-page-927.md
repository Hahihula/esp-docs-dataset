...
¹ The message digest is stored in registers from most significant bits to the least significant bits, with the first word stored in register `SHA_H_O_REG` and the second word stored in register `SHA_H_1_REG`. For details, please see subsection 26.4.1.2.
```
