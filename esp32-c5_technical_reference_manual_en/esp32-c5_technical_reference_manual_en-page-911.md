...
$$
\frac{N}{32} \quad Z = (Z_{n-1}Z_{n-2}\cdots Z_0)_b \\
...
$Z_{n-1}$, $X_{n-1}$, $Y_{n-1}$, $M_{n-1}$ and $\overline{r}_{n-1}$ are the most significant bits of $Z$, $X$, $Y$, $M$, and $\overline{r}$, while $Z_0$, $X_0$, $Y_0$, $M_0$ and $\overline{r}_0$ are the least significant bits.
...
3. Write $X_i$, $Y_i$, $M_i$ and $\overline{r}_i$ for $i \in \{0, 1, \dots, n-1\}$ to memory blocks `RSA_X_MEM`, `RSA_Y_MEM`, `RSA_M_MEM` and `RSA_Z_MEM`. The capacity of each memory block is 96 words. Each word of each
```