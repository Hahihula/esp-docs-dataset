...
ESP32-C5’s LEDC can generate the `LEDC_INT` interrupt signal, which will be sent to the **Interrupt Matrix**. Interrupt signal `LEDC_INT` can be generated by the following internal interrupt sources:
...
1. Configure the `Timerx` according to section **40.4.1**. After that, the clock source, divider, and counter are ready.
2. Configure the `PWMn` generator according to section **40.4.2**. After that, `PWMn` selects one of the four timers and the phase and duty cycle of the `PWMn` output is set.
