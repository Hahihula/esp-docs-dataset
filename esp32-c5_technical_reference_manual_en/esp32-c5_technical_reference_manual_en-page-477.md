...
After enabling a SOURCE, you can query its current interrupt status by reading the corresponding bit value in `INTMTX_COREO_INT_STATUS_n_REG` (read only). For the mapping between `INTMTX_COREO_INT_STATUS_n_REG` and the SOURCE, please refer to Table 11.5-1.
```